...
def train(
    ...
    change_perm=True,
    n_perm=1,
    unlabeled_category="UNK",  ### Added Antoine Collin
    **kwds,
):
    ...
    if learning_rate is None:
        ...
        optimizer = opt.__dict__[optimizer](
            lr=learning_rate, clipvalue=clip_grad
        )

    print(model, loss, optimizer)
    print(type(network))
    if (
        type(network) == BatchRemovalAutoencoder
    ):  # We compile the model differently with the combined loss
        model.compile(
            loss={
                "batch_removal": network.batch_removal_loss,
                "reconstruction": network.reconstruction_loss,
            },
            loss_weights={
                "batch_removal": -network.batch_removal_weight,  # we take the opposite because we need to maximize this loss
    ...
    if reduce_lr:
        lr_cb = ReduceLROnPlateau(
            monitor="loss", patience=reduce_lr, verbose=verbose
        )
        callbacks.append(lr_cb)
    if early_stop:
        es_cb = EarlyStopping(
            monitor="loss", patience=early_stop, verbose=verbose
        )
        callbacks.append(es_cb)
    if tensorboard:
        tb_log_dir = os.path.join(output_dir, "tb")
        tb_cb = TensorBoard(
            log_dir=tb_log_dir, histogram_freq=1, write_grads=True
        )
    ...
    if permute:
        ...
        loss = model.fit(
            batch_generator_training_permuted(
                adata=adata,
                class_key=class_key,
    ...
    ##### End Addded Antoine Collin ######
        ...
        if output_subset:
            gene_idx = [
                np.where(adata.raw.var_names == x)[0][0] for x in output_subset
            ]
            output = (
                adata.raw.X[:, gene_idx]
