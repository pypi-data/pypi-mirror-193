...
from .tfidfi_idfi import TFIDFi_IDFi
from .backend._utils import select_backend

from .misc import _save_pkl
from .misc import _read_pkl
import time


class Graph2Topic:

    def __init__(self, top_n_words=10, nr_topics=10, embedding_model=None, embedding=None,dim_size=-1, word_select_method=None,graph_method=None,seed=42):
    ...
    def _weighting_words(self, documents_per_topic, all_documents):
        ...
        words = self.vectorizer_model.get_feature_names()#list 
        ...
        elif self.word_select_method == 'tfidfi_idfi':
            socres = TFIDFi_IDFi(X_per_cluster).socre()#k*vocab
    ...
    def _detect_graph_topic(self, embeddings, documents):
        from .graphs import create_networkx_graph

        print("---detect graph topic---")
        # state_time=time.process_time()
        graph, graph_creation_time = create_networkx_graph(documents["ID"], embeddings,percentile_cutoff=95)
        # print('graph_creation_time:'+str(graph_creation_time))
        # print(nx.info(graph))
        # print('graph density:'+str(nx.density(graph)))
