...
class LookupProjection:
    ...
    async def _view_single(
        ...
        if isinstance(items, list) and len(items) > 0:
            item = items.pop()
            try:
                final = self.render(item=item, query=query, context=context)
                return orjson.dumps(final.dict())  # noqa
    ...
    async def _view_listing(
        ...
        if items and isinstance(items, list) and len(items) > 0:
            try:
                ...
                return orjson.dumps(final)  # noqa
            except Exception as _list_result_exc:
    ...
    async def _view_paginated(
        ...
        if isinstance(items, list) and len(items) > 0:
            ...
            try:
                ...
                return orjson.dumps({'data': fallback})  # noqa
            return orjson.dumps({'data': _response})  # noqa
        return orjson.dumps({'data': fallback})  # noqa
