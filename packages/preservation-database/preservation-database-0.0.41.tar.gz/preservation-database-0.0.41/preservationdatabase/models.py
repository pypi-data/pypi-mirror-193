...
class LastFill(models.Model):
    ...
    def get_last_fill(archive_name):
        try:
            return \
                LastFill.objects.get(archive_name=archive_name).last_fill_date
...
class LockssPreservation(models.Model):
    ...
    def __str__(self):
        return '{} (issn: {}) (eissn: {}) (v{}({})) ' \
               '(in progress: {} ({})) '.format(self.title, self.issn,
                                                self.eissn,
...
class PKPPreservation(models.Model):
    ...
    def __str__(self):
        return '{} (issn: {}) (v{}({})) '.format(self.title, self.issn,
                                                 self.preserved_volumes,
                                                 self.preserved_no,
                                                 )
...
class PorticoPreservation(models.Model):
    ...
    def __str__(self):
        return '{} (issn: {}) (eissn: {}) (v{}) '.format(self.title,
                                                         self.issn, self.eissn,
                                                         self.preserved_volumes)
...
class OculScholarsPortalPreservation(models.Model):
    ...
    def preservation(container_title, issn, volume, no=None):
        ...
        if len(pis) > 0:
            ...
            if len(pis) > 0:
                for pi in pis:
                    matches = re.findall(basic_volume_regex,
                                         pi.preserved_volumes)
            ...
            if len(pis) > 0:
                for pi in pis:
                    matches = re.findall(unpack_volume_regex,
                                         pi.preserved_volumes)
        ...
        # OCUL gives volume formats as follows:
        # v24(1)
        # 1, 2, 3-4, 3.4
        # 1 to 4
        volume_regex = r'v(\d+)'

        for pis in preserved_item:
            print(pis.preserved_volumes)
            matches = re.findall(volume_regex, pis.preserved_volumes)

            volume = str(volume)

            if volume in matches:
                return pis, pis.preserved
