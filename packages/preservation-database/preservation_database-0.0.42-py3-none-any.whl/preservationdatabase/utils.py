...
def show_preservation(
    ...
    for issn_item in issn:
        ...
        for issn_obj in issn_objs:
            issnl_obj = ISSNL.objects.filter(identifier=issn_obj.issnl)

            issn_reversed = ISSN.objects.filter(issnl=issn_obj.issnl)

            for reversed in issn_reversed:
                final_issn_list.append(reversed.identifier)
    ...
    for system in preservation_systems:
        preserved, done = system.preservation(
            container_title, final_issn_list, volume, no
        )
...
def show_preservation_for_doi(
    doi_input: str, archive: str = None
) -> (dict | None, str):
    """
    ...
    preserves, doi_echo = show_preservation(
        container_title, issns, volume, no, doi, archive
    )
...
@transaction.atomic
def random_db_entries(model, count) -> list:
    """
    ...
    for i in range(0, count - 1):
        ...
        output.append(model.objects.all()[random_index])
