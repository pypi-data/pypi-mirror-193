...
class TTNMixin:
    ...
    @property
    def stim_params(self) -> dict[Literal["main", "mapping", "opto", "system"], dict[str, Any]]:
    ...
    def stim_scripts(self) -> dict[Literal["main", "mapping", "opto"], str]:
        "Relative path to script on Stim computer from current location."
        return {label: np_config.local_to_unc(
            self.rig.sync, 
            pathlib.Path(
                f"camstim_scripts/ttn_{label}_camstim_script.py"
            ).resolve()
