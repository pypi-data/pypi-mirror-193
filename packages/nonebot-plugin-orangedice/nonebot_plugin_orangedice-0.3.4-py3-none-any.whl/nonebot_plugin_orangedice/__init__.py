...
# @nickname.handle()
# async def set_nickname(event: GroupMessageEvent, matcher: Matcher, bot: Bot):
#     user_id = event.user_id
#     attrs = Attribute(data.get_card(event.user_id).skills)
#     san, hp = attrs.get("san"), attrs.get("hp")
#     # await bot.set_group_card(group_id=event.group_id, user_id=user_id, card=f"{get_name(event).split()[0]} {san} {hp}")
#     await matcher.finish(f"已将你的昵称设置为：{msg}")