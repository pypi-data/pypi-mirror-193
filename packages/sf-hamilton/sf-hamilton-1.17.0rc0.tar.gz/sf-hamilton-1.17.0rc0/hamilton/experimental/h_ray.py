...
def convert_pandas_to_numpy(pandas_object: Union[pd.DataFrame, pd.Series, Any]) -> Union[Any, Dict[str, np.ndarray]]:
    if not isinstance(pandas_object, (pd.DataFrame, pd.Series)):
        return pandas_object
    if isinstance(pandas_object, pd.DataFrame):
        df_dict = {col: pandas_object[col].to_numpy() for col in pandas_object.columns}
        df_dict["__type__"] = "pandas_df"
        df_dict["__index__"] = pandas_object.index.to_numpy()
    else:
        df_dict = {pandas_object.name: pandas_object.to_numpy()}
        df_dict["__type__"] = "pandas_series"
        df_dict["__index__"] = pandas_object.index.to_numpy()
    return df_dict

def convert_numpy_to_pandas(numpy_dict: Any) -> Union[pd.DataFrame, pd.Series, Any]:
    if not isinstance(numpy_dict, dict) or "__type__" not in numpy_dict:
        return numpy_dict
    if numpy_dict["__type__"] == "pandas_df":
        numpy_dict.pop("__type__")
        index = numpy_dict.pop("__index__")
        return pd.DataFrame(numpy_dict, index=index)
    elif numpy_dict["__type__"] == "pandas_series":
        numpy_dict.pop("__type__")
        index = numpy_dict.pop("__index__")
        series_name, series_values = numpy_dict.popitem()
        return pd.Series(series_values, name=series_name, index=index)
    else:
        raise ValueError(f"Unknown serialization type {numpy_dict['__type__']}")
...
class RayGraphAdapter(base.HamiltonGraphAdapter, base.ResultMixin):
    ...
    def execute_node(self, node: node.Node, kwargs: typing.Dict[str, typing.Any]) -> typing.Any:
        ...
        """
        r_fn = raify(node.callable)
        def serde_function(*args, **kwargs) -> typing.Any:
            """If it's pandas data, convert to dict of numpy arrays"""
            kwarg_values = {k: convert_numpy_to_pandas(v) for k, v in kwargs.items()}
            result = r_fn(**kwarg_values)
            return convert_pandas_to_numpy(result)
        #a: object ref
        # ray.put()  # look at pyarrow & ray dataset to see if that speeds things up.
        return ray.remote(serde_function).remote(**kwargs)
        # return ray.remote(r_fn).remote(**kwargs)
    ...
    def build_result(self, **outputs: typing.Dict[str, typing.Any]) -> typing.Any:
        ...
        def serde_function(*args, **kwargs) -> typing.Any:
            """If it's pandas data, convert to dict of numpy arrays"""
            kwarg_values = {k: convert_numpy_to_pandas(v) for k, v in kwargs.items()}
            result = self.result_builder.build_result(**kwarg_values)
            return convert_pandas_to_numpy(result)
        remote_combine = ray.remote(serde_function).remote(**outputs)
        ...
        return convert_numpy_to_pandas(result)
