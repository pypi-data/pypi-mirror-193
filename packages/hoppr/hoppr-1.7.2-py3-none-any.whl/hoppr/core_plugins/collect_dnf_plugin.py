...
def _is_rpm_repo(repo_url: str, creds: Optional[CredObject]):
...
class CollectDnfPlugin(BatchCollectorPlugin):
    ...
    def __init__(
        ...
        config_file: Union[str, PathLike] = Path.cwd().joinpath(".hoppr-dnf", "dnf.conf"),
    ) -> None:
        super().__init__(
            context=context,
            config=config,
    ...
    def pre_stage_process(self) -> Result:
        ...
        for idx, repo in enumerate(self.context.manifest.consolidated_repositories[PurlType.RPM]):
            temp_repo = f"hoppr-tmp-{idx}"
            creds = Credentials.find_credentials(repo.url)

            if not _is_rpm_repo(repo.url, creds):
    ...
    def collect(self, comp: Component) -> Result:
        ...
        artifact = _artifact_string(purl)  # type: ignore
