...
import bf_nlu.nlu.utils.bilou_utils
from bf_nlu.shared.constants import NEXT_MAJOR_VERSION_FOR_DEPRECATIONS
from bf_nlu.nlu.constants import NUMBER_OF_SUB_TOKENS
...
from bf_nlu.utils.tensorflow.callback import bf_nluTrainingLogger, bf_nluModelCheckpoint
from bf_nlu.utils.tensorflow.data_generator import bf_nluBatchDataGenerator
from bf_nlu.utils.tensorflow.model_data import bf_nluModelData
from bf_nlu.shared.nlu.constants import SPLIT_ENTITIES_BY_COMMA
    ...
    for tag_spec in entity_tag_specs:
        ...
        if bilou_flag:
            (
                tags,
                confidences,
            ) = bf_nlu.nlu.utils.bilou_utils.ensure_consistent_bilou_tagging(
                tags, confidences
            )
...
def create_data_generators(
    ...
    shuffle: bool = True,
) -> Tuple[bf_nluBatchDataGenerator, Optional[bf_nluBatchDataGenerator]]:
    ...
    if eval_num_examples > 0:
        ...
        validation_data_generator = bf_nluBatchDataGenerator(
    ...
    data_generator = bf_nluBatchDataGenerator(
        model_data,
        batch_size=batch_sizes,
        epochs=epochs,
        batch_strategy=batch_strategy,
...
def create_common_callbacks(
    ...
    - bf_nluTrainingLogger callback
    - Optional TensorBoard callback
    - Optional bf_nluModelCheckpoint callback
    ...
    callbacks = [bf_nluTrainingLogger(epochs, silent=False)]
    ...
    if checkpoint_dir:
        callbacks.append(bf_nluModelCheckpoint(checkpoint_dir))
...
def _check_evaluation_setting(component_config: Dict[Text, Any]) -> None:
    if (
        ...
        bf_nlu.shared.utils.io.raise_warning(warning)
...
def _check_confidence_setting(component_config: Dict[Text, Any]) -> None:
    if component_config[MODEL_CONFIDENCE] == COSINE:
        raise InvalidConfigException(
            f"{MODEL_CONFIDENCE}={COSINE} was introduced in bf_nlu Open Source 2.3.0 "
