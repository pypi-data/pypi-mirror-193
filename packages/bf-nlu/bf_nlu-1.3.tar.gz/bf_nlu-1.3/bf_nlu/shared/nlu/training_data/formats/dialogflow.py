...
DOCS_URL_MIGRATE_GOOGLE = DOCS_BASE_URL + "/migrate-from/google-dialogflow-to-bf_nlu/"
...
class DialogflowReader(TrainingDataReader):
    ...
    def read(self, filename: Union[Text, Path], **kwargs: Any) -> "TrainingData":
        ...
        root_js = bf_nlu.shared.utils.io.read_json_file(filename)
    ...
    def _extract_entity(
        ...
        if "meta" in chunk or "alias" in chunk:
            ...
            if entity_type != "@sys.ignore":
                entity = bf_nlu.shared.nlu.training_data.util.build_entity(
    ...
    def _read_examples(
        ...
        if os.path.isfile(examples_fn):
            return bf_nlu.shared.utils.io.read_json_file(examples_fn)
        else:
            return None
