...
from bf_nlu.shared.constants import DOCS_URL_RULES
from bf_nlu.shared.exceptions import bf_nluException
import bf_nlu.shared.utils.io
from bf_nlu.shared.core.events import LoopInterrupted, UserUttered, ActionExecuted
from bf_nlu.core.featurizers.tracker_featurizers import TrackerFeaturizer
from bf_nlu.core.policies.memoization import MemoizationPolicy
from bf_nlu.core.policies.policy import SupportedData, PolicyPrediction
from bf_nlu.shared.core.trackers import (
...
from bf_nlu.shared.core.domain import InvalidDomain, State, Domain
from bf_nlu.shared.nlu.constants import ACTION_NAME, INTENT_NAME_KEY
import bf_nlu.core.test
from bf_nlu.core.training.training import create_action_fingerprints, ActionFingerprint

logger = logging.getLogger(__name__)


# These are bf_nlu Open Source default actions and overrule everything at any time.
...
class InvalidRule(bf_nluException):
...
class RulePolicy(MemoizationPolicy):
    ...
    def _predicted_action_name(
        ...
        if (
            ...
            bf_nlu.core.test.emulate_loop_rejection(tracker)
            predicted_action_name, prediction_source = self._predict_next_action(
                tracker, domain
            )
    ...
    def _run_prediction_on_trackers(
        ...
        pbar = tqdm(
            ...
            disable=bf_nlu.shared.utils.io.is_logging_disabled(),
        )
        for tracker in pbar:
    ...
    def _predict(
        ...
        # bf_nlu Open Source default actions overrule anything. If users want to achieve
