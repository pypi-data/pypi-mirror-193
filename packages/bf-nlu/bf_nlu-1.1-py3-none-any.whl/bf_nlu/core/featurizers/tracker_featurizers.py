...
from bf_nlu.core.exceptions import InvalidTrackerFeaturizerUsageError
import bf_nlu.shared.core.trackers
import bf_nlu.shared.utils.io
from bf_nlu.shared.nlu.constants import TEXT, INTENT, ENTITIES, ACTION_NAME
...
from bf_nlu.shared.core.events import Event, ActionExecuted, UserUttered
...
from bf_nlu.utils.tensorflow.constants import LABEL_PAD_ID
...
class InvalidStory(bf_nluException):
...
class TrackerFeaturizer:
    ...
    def _choose_last_user_input(
        ...
        for states in trackers_as_states:
            ...
            if not bf_nlu.shared.core.trackers.is_prev_action_listen_in_state(last_state):
                continue
    ...
    def persist(self, path: Union[Text, Path]) -> None:
        ...
        bf_nlu.shared.utils.io.create_directory_for_file(featurizer_file)
    ...
    def load(path: Union[Text, Path]) -> Optional[TrackerFeaturizer]:
        ...
        if featurizer_file.is_file():
            return jsonpickle.decode(bf_nlu.shared.utils.io.read_file(featurizer_file))
    ...
    def _cleanup_last_user_state_with_action_listen(
        ...
        for states in trackers_as_states:
            if not states:
                continue
            last_state = states[-1]
            if bf_nlu.shared.core.trackers.is_prev_action_listen_in_state(last_state):
                del states[-1]
