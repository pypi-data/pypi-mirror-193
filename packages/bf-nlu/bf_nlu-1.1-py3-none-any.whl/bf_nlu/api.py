...
#   These functions are imported in `bf_nlu.__init__` and any top level import
#   added here will get executed as soon as someone runs `import bf_nlu`.
#   Some imports are very slow (e.g. `tensorflow`) and we want them to get
#   imported when running `import bf_nlu`. If you add more imports here,
#   please check that in the chain you are importing, no slow packages
#   are getting imported.

if TYPE_CHECKING:
    from bf_nlu.model_training import TrainingResult
...
def run(
    ...
    **kwargs: "Dict[Text, Any]",
    ...
    from bf_nlu.shared.utils.cli import print_warning
    import bf_nlu.shared.utils.common
    from bf_nlu.shared.constants import DOCS_BASE_URL
    ...
    kwargs = bf_nlu.shared.utils.common.minimal_kwargs(
        kwargs, bf_nlu.core.run.serve_application
    )
    bf_nlu.core.run.serve_application(
        model,
        channel=connector,
...
def train(
    ...
    """Runs bf_nlu Core and NLU training in `async` loop.
...
def test(
    ...
    stories: "Text",
    nlu_data: "Text",
    output: "Text" = bf_nlu.shared.constants.DEFAULT_RESULTS_PATH,
    additional_arguments: "Optional[Dict]" = None,
) -> None:
    """Test a bf_nlu model against a set of test data.
    ...
    """
    from bf_nlu.model_testing import test_core
    from bf_nlu.model_testing import test_nlu
