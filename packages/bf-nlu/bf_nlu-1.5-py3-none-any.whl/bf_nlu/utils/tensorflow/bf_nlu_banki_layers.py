...
from bf_nlu.shared.nlu.constants import TEXT
from bf_nlu.utils.tensorflow.model_data import FeatureSignature
...
from bf_nlu.utils.tensorflow.exceptions import TFLayerConfigException
from bf_nlu.utils.tensorflow.transformer import TransformerEncoder
from bf_nlu.nlu.constants import DEFAULT_TRANSFORMER_SIZE


class bf_nluCustomLayer(tf.keras.layers.Layer):
    """Parent class for all classes in `bf_nlu_layers.py`.
    ...
    `ConcatenateSparseDenseFeatures`, `bf_nluSequenceLayer` and
    `bf_nluFeatureCombiningLayer` all inherit from `bf_nluCustomLayer` and thus can
    ...
    def adjust_sparse_layers_for_incremental_training(
        ...
        for name, layer in self._tf_layers.items():
            if isinstance(layer, bf_nluCustomLayer):
...
class ConcatenateSparseDenseFeatures(bf_nluCustomLayer):
...
class bf_nluFeatureCombiningLayer(bf_nluCustomLayer):
    ...
    def __init__(
        ...
        """Creates a new `bf_nluFeatureCombiningLayer` object."""
        ...
        super().__init__(name=f"bf_nlu_feature_combining_layer_{attribute}")
...
class bf_nluSequenceLayer(bf_nluCustomLayer):
    ...
    1. Combine features using `bf_nluFeatureCombiningLayer`.
    ...
    Output shape:
        outputs: `(batch_size, seq_length, units)` where `units` matches the underlying
            ...
            size of the underlying `bf_nluFeatureCombiningLayer` if the `Ffnn` block has 0
            ...
            shape of `bf_nluFeatureCombiningLayer`). If both feature types are present,
        ...
        seq_sent_features: `(batch_size, seq_length, hidden_dim)`, where `hidden_dim` is
            ...
            underlying `bf_nluFeatureCombiningLayer` if the `Ffnn` block has 0 layers.
    ...
    def __init__(
        ...
        """Creates a new `bf_nluSequenceLayer` object."""
        ...
        super().__init__(name=f"bf_nlu_sequence_layer_{attribute}")

        self._tf_layers: Dict[Text, Any] = {
            self.FEATURE_COMBINING: bf_nluFeatureCombiningLayer(
    ...
    def _calculate_output_units(
        ...
        pipeline that is `bf_nluFeatureCombiningLayer` -> `Ffnn` -> `Transformer`, since
        ...
        # only the bf_nluFeatureCombiningLayer is present
