...
from bf_nlu.utils import common as bf_nlu_utils
...
if typing.TYPE_CHECKING:
    ...
    from bf_nlu.core.channels.channel import InputChannel
...
TELEMETRY_ENABLED_ENVIRONMENT_VARIABLE = "bf_nlu_TELEMETRY_ENABLED"
TELEMETRY_DEBUG_ENVIRONMENT_VARIABLE = "bf_nlu_TELEMETRY_DEBUG"

# the environment variable can be used for local development to set a test key
# e.g. `bf_nlu_TELEMETRY_WRITE_KEY=12354 bf_nlu train`
TELEMETRY_WRITE_KEY_ENVIRONMENT_VARIABLE = "bf_nlu_TELEMETRY_WRITE_KEY"
EXCEPTION_WRITE_KEY_ENVIRONMENT_VARIABLE = "bf_nlu_EXCEPTION_WRITE_KEY"
...
# https://bf_nlu.com/docs/bf_nlu/telemetry
...
TELEMETRY_bf_nlu_X_LOCAL_STARTED_EVENT = "bf_nlu X Local Started"
...
def print_telemetry_reporting_info() -> None:
    ...
    message = textwrap.dedent(
        f"""
        bf_nlu Open Source reports anonymous usage telemetry to help improve the product
        for all its users.

        If you'd like to opt-out, you can use `bf_nlu telemetry disable`.
...
def _write_default_telemetry_configuration(
    ...
    success = bf_nlu_utils.write_global_config_value(
...
def _is_telemetry_enabled_in_configuration() -> bool:
    """Read telemetry configuration from the user's bf_nlu config file in $HOME.
    ...
    """
    try:
        stored_config = bf_nlu_utils.read_global_config_value(
...
def is_telemetry_enabled() -> bool:
    ...
    try:
        return bf_nlu_utils.read_global_config_value(
            CONFIG_FILE_TELEMETRY_KEY, unavailable_ok=False
        )[CONFIG_TELEMETRY_ENABLED]
    except ValueError:
        return False
...
def _fetch_write_key(tool: Text, environment_variable: Text) -> Optional[Text]:
    ...
    from bf_nlu import __name__ as name
...
def telemetry_write_key() -> Optional[Text]:
    ...
    versions of bf_nlu Open Source. This avoids running telemetry locally when
    developing on bf_nlu or when running CI builds.
...
def _is_docker() -> bool:
    ...
    try:
        return "docker" in bf_nlu.shared.utils.io.read_file("/proc/self/cgroup", "utf8")
    except Exception:  # skipcq:PYL-W0703
        return False
...
def _default_context_fields() -> Dict[Text, Any]:
    ...
    if not TELEMETRY_CONTEXT:
        ...
        TELEMETRY_CONTEXT = {
            ...
            "bf_nlu_open_source": bf_nlu.__version__,
...
def _track(
    ...
    events, or events that are generated from many parts of the bf_nlu Open Source code.
...
def get_telemetry_id() -> Optional[Text]:
    """Return the unique telemetry identifier for this bf_nlu Open Source install.
    ...
    try:
        telemetry_config = (
            bf_nlu_utils.read_global_config_value(CONFIG_FILE_TELEMETRY_KEY) or {}
        )
...
def toggle_telemetry_reporting(is_enabled: bool) -> None:
    ...
    Args:
        is_enabled: `True` if the telemetry reporting should be enabled,
            `False` otherwise.
    """
    configuration = bf_nlu_utils.read_global_config_value(CONFIG_FILE_TELEMETRY_KEY)
    ...
    bf_nlu_utils.write_global_config_value(CONFIG_FILE_TELEMETRY_KEY, configuration)
...
def initialize_error_reporting() -> None:
    ...
    # https://sentry.io/settings/bf_nluhq/projects/bf_nlu-open-source/install/python/
    # for local testing, set the key using `bf_nlu_EXCEPTION_WRITE_KEY=key bf_nlu <command>`
    ...
    sentry_sdk.init(
        ...
        ignore_errors=[
            ...
            # expected bf_nlu errors
            bf_nluException,
            OSError,
        ],
        in_app_include=["bf_nlu"],  # only submit errors in this package
        with_locals=False,  # don't submit local variables
        release=f"bf_nlu-{bf_nlu.__version__}",
...
def track_model_training(
    ...
    Args:
        ...
        model_type: Specifies the type of training, should be either "bf_nlu", "core"
            or "nlu".
...
def track_server_start(
    ...
    """Tracks when a user starts a bf_nlu server.
    ...
    Args:
        ...
        is_api_enabled: whether the bf_nlu API server is enabled
    """
    from bf_nlu.core.utils import AvailableEndpoints
...
def track_project_init(path: Text) -> None:
    """Track when a user creates a project using bf_nlu init.
...
def track_shell_started(model_type: Text) -> None:
    """Track when a user starts a bot using bf_nlu shell.

    Args:
        model_type: Type of the model, core / nlu or bf_nlu.
    """
    _track(TELEMETRY_SHELL_STARTED_EVENT, {"type": model_type})


@ensure_telemetry_enabled
def track_bf_nlu_x_local() -> None:
    """Track when a user runs bf_nlu X in local mode."""
    _track(TELEMETRY_bf_nlu_X_LOCAL_STARTED_EVENT)
