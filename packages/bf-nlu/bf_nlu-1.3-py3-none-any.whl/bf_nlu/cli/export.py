...
from bf_nlu.cli.arguments import export as arguments
from bf_nlu.shared.constants import DOCS_URL_EVENT_BROKERS, DOCS_URL_TRACKER_STORES
from bf_nlu.exceptions import PublishingError
from bf_nlu.shared.exceptions import bf_nluException
from bf_nlu.core.brokers.pika import PikaEventBroker

if typing.TYPE_CHECKING:
    from bf_nlu.core.brokers.broker import EventBroker
    from bf_nlu.core.tracker_store import TrackerStore
    from bf_nlu.core.exporter import Exporter
...
def add_subparser(
    ...
    """Add subparser for `bf_nlu export`.
...
async def _export_trackers(args: argparse.Namespace) -> None:
    ...
    endpoints = bf_nlu.core.utils.read_endpoints_from_path(args.endpoints)
    ...
    try:
        ...
        bf_nlu.shared.utils.cli.print_error_and_exit(str(e))


def _get_continuation_command(exporter: "Exporter", timestamp: float) -> Text:
    """Build CLI command to continue 'bf_nlu export' where it was interrupted.
