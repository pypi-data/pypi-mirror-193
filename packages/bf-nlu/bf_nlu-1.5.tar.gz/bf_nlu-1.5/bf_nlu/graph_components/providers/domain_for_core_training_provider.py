...
from bf_nlu.shared.constants import REQUIRED_SLOTS_KEY
from bf_nlu.shared.core.domain import KEY_RESPONSES, Domain, SESSION_CONFIG_KEY, KEY_FORMS
...
class DomainForCoreTrainingProvider(GraphComponent):
    ...
    References:
        - `bf_nlu.core.featurizer.tracker_featurizer.py` (used by all policies)
        - `bf_nlu.core.featurizer.single_state_featurizer.py` (used by ML policies)
        - `bf_nlu.shared.core.domain.get_active_state` (used by above references)
        - `bf_nlu.shared.core.slots.as_features` (used by above references)
        - `bf_nlu.shared.core.training_data.structures.StoryStep.explicit_events`
