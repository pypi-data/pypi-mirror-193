...
from bf_nlu.core.featurizers.single_state_featurizer import SingleStateFeaturizer
from bf_nlu.core.featurizers.tracker_featurizers import FEATURIZER_FILE
import bf_nlu.utils.common
import bf_nlu.shared.utils.io
from bf_nlu.shared.exceptions import bf_nluException, FileIOException
from bf_nlu.shared.nlu.constants import ENTITIES, INTENT, TEXT, ACTION_TEXT, ACTION_NAME
from bf_nlu.shared.core.domain import Domain, State
...
from bf_nlu.shared.core.constants import USER, SLOTS, PREVIOUS_ACTION, ACTIVE_LOOP
...
class Policy(GraphComponent):
    ...
    def _create_featurizer(self) -> TrackerFeaturizer:
        ...
        featurizer_func = _get_featurizer_from_config(
            ...
            lookup_path="bf_nlu.core.featurizers.tracker_featurizers",
        )
        ...
        if state_featurizer_configs:
            state_featurizer_func = _get_featurizer_from_config(
                ...
                self.__class__.__name__,
                lookup_path="bf_nlu.core.featurizers.single_state_featurizer",
            )
    ...
    def _get_valid_params(func: Callable, **kwargs: Any) -> Dict:
        ...
        """
        valid_keys = bf_nlu.shared.utils.common.arguments_of(func)
    ...
    def _featurize_for_training(
        ...
        **kwargs: Any,
        ...
        Args:
            training_trackers:
                the list of the :class:`bf_nlu.core.trackers.DialogueStateTracker`
            domain: the :class:`bf_nlu.shared.core.domain.Domain`
    ...
    def __init__(
        ...
        Args:
            ...
            diagnostic_data: Intermediate results or other information that is not
                necessary for bf_nlu to function, but intended for debugging and
...
class InvalidPolicyConfig(bf_nluException):
...
def _get_featurizer_from_config(
    ...
    featurizer_func = bf_nlu.shared.utils.common.class_from_module_path(
