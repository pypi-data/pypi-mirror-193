...
class Exporter:
    ...
    def _get_message_headers(self) -> Optional[Dict[Text, Text]]:
        ...
        Returns:
            ...
            `bf_nlu_EXPORT_PROCESS_ID_HEADER_NAME` key if `self.event_broker` is a
            `PikaEventBroker`, else `None`.

        """
        if isinstance(self.event_broker, PikaEventBroker):
            return {bf_nlu_EXPORT_PROCESS_ID_HEADER_NAME: uuid.uuid4().hex}
