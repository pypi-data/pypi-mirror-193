...
from bf_nlu.nlu.utils import write_to_file
...
def convert_training_data(
    ...
    Args:
        data_file (Union[list, Text]): Path to the file or directory
            containing bf_nlu data.
        out_file (Text): File or existing path where to save
            training data in bf_nlu format.
    ...
    td = bf_nlu.shared.nlu.training_data.loading.load_data(data_file, language)
    if output_format == "json":
        output = td.nlu_as_json(indent=2)
    else:
        output = bf_nluYAMLWriter().dumps(td)
