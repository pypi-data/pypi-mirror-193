...
from bf_nlu.cli.arguments.default_arguments import add_model_param, add_endpoint_param
from bf_nlu.core import constants


def set_run_arguments(parser: argparse.ArgumentParser) -> None:
    """Arguments for running bf_nlu directly using `bf_nlu run`."""
    add_model_param(parser)
    add_server_arguments(parser)


def set_run_action_arguments(parser: argparse.ArgumentParser) -> None:
    """Set arguments for running bf_nlu SDK."""
...
def add_interface_argument(
    ...
    """Binds the bf_nlu process to a network interface."""
    parser.add_argument(
        "-i",
        "--interface",
...
def add_server_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--log-file",
        type=str,
        # bf_nlu should not log to a file by default, otherwise there will be problems
    ...
    server_arguments.add_argument(
        "--remote-storage",
        help="Set the remote location where your bf_nlu model is stored, e.g. on AWS.",
    )
    server_arguments.add_argument(
        "--ssl-certificate",
