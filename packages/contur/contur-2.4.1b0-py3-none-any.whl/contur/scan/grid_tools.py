...
def grid_loop(scan_path=None,patterns=None,extract=False,clean=True,unmerge=False,archive=False,check=False,check_all=False,resub=False,queue=""):
    ...
    :arg scan_path: the top directory to start from

    :arg patterns: if extract, then extract the analyses that match this pattern

    :arg extract: extract analyses matching pattern into  new directory

    :arg clean: remove unnecessary files

    :arg unmerge: remove the merged yoda files and decompress the raw yoda files

    :arg archive: if true, remove unneccessary files and compress the rest
    ...
    if clean:
        cfg.contur_log.info("Removing unnecessary files from grid")

    if extract:
        new_dir = scan_path+"_new"
        contur.scan.mkoutdir(new_dir)
    ...
    for root, dirs, files in sorted(os.walk(scan_path)):
        ...
        merged_yoda_file = os.path.join(root, cfg.tag + run_point_num + '.yoda')
        ...
        if unmerge or archive or check or check_all:
            ...
            for name in files:
                ...
                if name.startswith(cfg.tag+run_point_num+".sh.e"):
                    got_batch_logs = True
                if name==cfg.tag+run_point_num+".sh":
                    batch_script = name
...
def merge_yodas(root,files,has_subdir):
    ...
    run_point_num = os.path.basename(root)
    out_file = os.path.join(root, cfg.tag + run_point_num + '.yoda')
...
def find_param_point(fileList,tag,paramList,verbose=False):
    """
    ...
    cfg.contur_log.debug('Looking in {}'.format(fileList))
