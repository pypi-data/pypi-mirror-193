...
def load_ref_aos(f):
    ...
    for path,ao in aos.items():
        if not rivet.isRefPath(path):
            continue

        load_ref_ao(path,ao,aos)

def load_ref_ao(path, ao, aos):
    """
...
def load_sm_ao(path,ao,sm):
    ...
    if cbuilder.hasBreakdown:
        if cfg.useTheoryCorr:
            sm.corr[path] = cbuilder.buildCovFromBreakdown(ignore_corrs=cfg.diag)
        # always fill the unCorr case in case we need it later
        sm.uncorr[path] = cbuilder.buildCovFromBreakdown(ignore_corrs=True)

    else:
        sm.uncorr[path] = cbuilder.buildCovFromErrorBar()
        sm.corr[path] = sm.uncorr[path].copy() 
...
class YodaFactory(object):
    ...
    It will also (optional) write yoda dat files for each plot.
    The path to output plot objects is detemined by cfg.plot_dir
        ...
        for path, ao in cutil.progress_bar(mc_histos.items(), total=len(mc_histos)):
            ...
            if cdb.validHisto(path):
                ...
                if (observable._ref is not None) and not (observable._theoryComp and observable._thy is None):
                    ...
                    if not cfg.silenceWriter:
                        cutil.write_yoda_dat(observable, nostack=cfg.nostack)
