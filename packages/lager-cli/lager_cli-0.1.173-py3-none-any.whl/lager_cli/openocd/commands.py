...
def run_openocd_tunnel(ctx, host, port, gateway, rtt, rtt_port):
    ...
    rtt_params = None
    if rtt:
        rtt_params = ctx.obj.websocket_connection_params(socktype='rtt', gateway_id=gateway)
    try:
        trio.run(serve_tunnel, host, port, connection_params, 'telnet', True, rtt_params, rtt_port)
