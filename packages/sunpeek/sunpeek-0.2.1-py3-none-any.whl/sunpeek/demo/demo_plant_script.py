...
def get_fluid(fluid_str='FHW'):
    ...
    elif fluid_str == 'water':
        return CoolPropFluid(get_definition(fluid_str))
    elif fluid_str == 'ASHRAE, Propylene Glycol':
        return CoolPropFluid(get_definition(fluid_str), concentration=Q(40, 'percent'))
    else:
...
# STEP 3: Run PC Method & create plots
pc_output = pc.PCMethod.create_improved(plant=plant, equation_id=2).run()
# pc_output = pc.PCMethod.create_iso(plant=plant, equation_id=1).run()
