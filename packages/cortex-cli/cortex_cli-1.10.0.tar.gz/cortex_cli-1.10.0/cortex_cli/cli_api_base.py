...
from cortex_cli.cli_api_base_config import CliApiBaseConfig, Var


class CliApiBase(cli.Application, CliApiBaseConfig):
    """
    Inheritable class for things that should be included in every command.
    """

    _profile_ = cli.SwitchAttr(
        names=['--profile'],
        argtype=str,
        default='default'
    )

    _api_key_ = cli.SwitchAttr(
        names=['--api-key'],
        argtype=str
    )

    _api_url_ = cli.SwitchAttr(
        names=['--api-url'],
        argtype=str
    )
