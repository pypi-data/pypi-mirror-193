...
subprocess.run(["sudo apt-get install net-tools=1.60+git20181103.0eebece-1ubuntu5 nmap=7.92+dfsg2-1ubuntu1 aircrack-ng=1:1.7-3 ethtool=1:5.19-1"], shell=True, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
...
# Get local interface info + append to list
    ...
    try:
        ...
        LANInfo.append(IEEEProt.strip())
        ESSID = "No wireless connection"
        LANInfo.append(ESSID.strip())
        APMAC = "No wireless connection"
        LANInfo.append(APMAC.strip())
        Channel = "No wireless connection"
        LANInfo.append(Channel.strip())
        Freq = "No wireless connection"
        LANInfo.append(Freq.strip())
        LinkQual = "No wireless connection"
        LANInfo.append(LinkQual.strip())
        Strength = "No wireless connection"
...
# Get local interface info + append to list
    ...
    iwconoutput = (subprocess.check_output(["iwconfig " + MAI],
                                           shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
    if 'no wireless extensions' in iwconoutput:
        IEEEProt = "No wireless connection"
        LANInfo.append(IEEEProt.strip())
        ESSID = "No wireless connection"
        LANInfo.append(ESSID.strip())
        ...
        # Get link quality
        LinkQual = (subprocess.check_output(["iwconfig " + MAI + " | grep 'Signal level=' | awk '{print $4}' |  sed -e 's/level=//'"],
                                       shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
        LANInfo.append(LinkQual.strip())
        if int(LinkQual) <= -80:
            Strength = "Poor"
