...
class Snaplib__222:
    ...
    def encode_dataframe(df : pd.DataFrame, 
        ...
        if not isinstance(inplace, bool):
            raise TypeError('The df must be a bool type instance.')


        if inplace is False:
            df_copied = df.copy()

            return encoder.Encoder().encode_dataframe(df_copied)
        elif inplace is True:
            return encoder.Encoder().encode_dataframe(df)


        

    @staticmethod
    def decode_dataframe(df : pd.DataFrame, 
                        inplace : bool = False, 
                        ) -> pd.DataFrame: 
        '''
        encode a dataframe
        ...
        if inplace is False:
            df_copied = df.copy()
            return encoder.Encoder().decode_dataframe(df_copied)
        elif inplace is True:
            return encoder.Encoder().decode_dataframe(df)
