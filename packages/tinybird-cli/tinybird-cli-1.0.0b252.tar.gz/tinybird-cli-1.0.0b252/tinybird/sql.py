...
def get_format_group(q):
    ...
    FORMAT_RE = r'\s+format\s+(\w+)\s*$'
    q = q.strip()
    format = re.search(FORMAT_RE, q, re.I)
    return format.group() if format else ''


def wrap_finalize_aggregation(sql, describe_result, fm_group=None):
    if not fm_group:
        fm_group = get_format_group(sql)
        sql = sql[0:-len(fm_group)] if fm_group else sql

    if describe_result:
        columns = [f"finalizeAggregation({c['name']}) as {c['name']}" if 'Aggregate' in c['type'] and 'SimpleAggregate' not in c['type'] else f"{c['name']}" for c in describe_result['data']]
        columns = ",\n\t".join(columns)
        sql = sql.replace('\n', '\n\t')
        qq = f'SELECT \n\t{columns} \nFROM ({sql} \n) {fm_group}'
    else:
