...
from tinybird.tornado_template import ParseError
...
class TestCLICheck(TestCLI):
    ...
    @tornado.testing.gen_test
    async def test_check_pipe_with_closed_if(self):
        await self._auth()
...
SQL >
    ...
    SELECT
        ...
        with self.assertRaises(ParseError):
            await self._tb(["check", pipe_file.name])

    @tornado.testing.gen_test
    async def test_check_all_files_in_directory(self):
        await self._auth()

        output = await self._tb(["check", self._get_resource_path('')])
        expected = [
            FeedbackManager.error_parsing_file(filename=self._get_resource_path('test_ds_a_bad_jsonpath2.datasource'), lineno=5, error='')
