...
class TestPartitions(unittest.TestCase):
    ...
    def test_ch_attach_string_partitions(self):
        ...
        partitions = [p.partition for p in partitions if p.table == 'test_ch_partitions_substring']
    ...
    def test_ch_attach_compound_partitions(self):
        ...
        partitions = async_to_sync(ch_query_table_partitions)(self.workspace.database_server, [self.workspace.database])
        partitions = [p.partition for p in partitions if p.table == 'test_ch_partitions_compound']
...
class TestTruncateTable(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_ch_create_empty_copy(self):
        ...
        # Regular expresion replace needed in order to remove the difference produced by different naming inside the
        # parenthesis of the ReplicatedMergeTree, so we can compare the rest of parameters of the field 'engine_full'
        table_meta['engine_full'] = re.sub(r"ReplicatedMergeTree(?s)(.*)',", "ReplicatedMergeTree()", table_meta['engine_full'])
        new_table_meta['engine_full'] = re.sub(r"ReplicatedMergeTree(?s)(.*)',", "ReplicatedMergeTree()", new_table_meta['engine_full'])
