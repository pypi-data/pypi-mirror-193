...
class Record(BaseModel, ABC):
    ...
    async def update_revision_hashes(self: T):
        ...
        """
        self.revision_hashes = [self.item_hash] + await SDK.fetch_revisions(type(self), ref=self.item_hash)
    ...
    async def fetch_revision(self: T, rev_no: int = None, rev_hash: str = None) -> T:
        ...
        self.__dict__.update((await SDK.fetch_records(type(self), item_hashes=[self.item_hash]))[0].content)
    ...
    async def upsert(self):
        ...
        """
        await SDK.post_or_amend_object(self)
    ...
    async def from_post(cls: Type[T], post: Dict[str, Any]) -> T:
        ...
        if post.get('ref') is None:
            ...
            obj.revision_hashes = await SDK.fetch_revisions(cls, ref=obj.item_hash)
    ...
    async def get(cls: Type[T], hashes: Union[str, List[str]]) -> List[T]:
        ...
        if not isinstance(hashes, List):
            hashes = [hashes]
        return await SDK.fetch_records(cls, list(hashes))
    ...
    async def fetch_all(cls: Type[T]) -> List[T]:
        ...
        """
        return await SDK.fetch_records(cls)
...
class Index(Record):
    ...
    async def fetch(self, keys: Union[OrderedDict[str], List[OrderedDict[str]]] = None) -> List[Record]:
        ...
        return await SDK.fetch_records(self.datatype, list(hashes))
...
class SDK:
    ...
    def init(cls,
        ...
        """
        cls.account = account if account else get_fallback_account()
        cls.channel = channel if channel else 'AARS_TEST'
        cls.api_url = api_url if api_url else settings.API_HOST
    ...
    async def forget_objects(cls, objs: List[T], account: Account = None, channel: str = None):
        ...
        await client.forget(account=account, hashes=hashes, reason=None, channel=channel, api_server=cls.api_url)
