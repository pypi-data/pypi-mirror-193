...
! Size related parameters.'''
    ...
    def new_fit_config(self, nm, config, na=None, nmt=None, nat=None, adi=None, di=None, seed=None):
...
! as well as all related work indices for that size data.'''
        ...
        # Setting up "di"
        di_present = ctypes.c_bool(True)
        if (di is None):
            di_present = ctypes.c_bool(False)
            di = ctypes.c_long()
        else:
            di = ctypes.c_long(di)
        if (type(di) is not ctypes.c_long): di = ctypes.c_long(di)
        ...
        # Call C-accessible Fortran wrapper.
        clib.c_new_fit_config(ctypes.byref(nm), ctypes.byref(na_present), ctypes.byref(na), ctypes.byref(nmt_present), ctypes.byref(nmt), ctypes.byref(nat_present), ctypes.byref(nat), ctypes.byref(adi_present), ctypes.byref(adi), ctypes.byref(di_present), ctypes.byref(di), ctypes.byref(seed_present), ctypes.byref(seed), ctypes.byref(config))
