...
#            y_pred=estimator.predict([X[0][self._feature_importance[:self.nb_features_to_keep]]])
    ...
    def _fit_local_model(self, X_train, y_train, estimator, get_feature_importance=False, important_features=None):
        ...
        preprocessor = ColumnTransformer(
            transformers=[
                ("num", StandardScaler(), [c for c in self.numerical_features if c in self.features[important_features]] ),
                ("cat", OneHotEncoder(handle_unknown="ignore"), [c for c in self.categorical_features if c in self.features[important_features]]),
            ]
        )
        classifier = Pipeline(
            steps=[("preprocessor", preprocessor), ("regressor", self.estimator)]
        )

        return classifier.fit(X_train_df, y_train)
...
if __name__ == '__main__':
    ...
    lr=LazyRegression(n_neighbors=20, linear_model=True, augment_samples_ratio=0.5,inddependent_var_name="QteFact")
    target=data["PoidsKG"]
