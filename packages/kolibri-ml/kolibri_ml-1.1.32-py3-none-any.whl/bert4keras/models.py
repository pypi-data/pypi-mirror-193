...
class BERT(Transformer):
    ...
    def variable_mapping(self):
        ...
        mapping = {
            ...
            'Embedding-Mapping': [
                'bert/encoders/embedding_hidden_mapping_in/kernel',
                'bert/encoders/embedding_hidden_mapping_in/bias',
            ],
            'Pooler-Dense': [
        ...
        for i in range(self.num_hidden_layers):
            prefix = 'bert/encoders/layer_%d/' % i
            mapping.update({
...
class ALBERT(BERT):
    ...
    def variable_mapping(self):
        ...
        prefix = 'bert/encoders/transformer/group_0/inner_group_0/'
        mapping.update({
...
class GPT(LM_Mask, BERT):
    ...
    def variable_mapping(self):
        ...
        mapping = {
            k: [
                i.replace('bert/', 'gpt/').replace('encoders', 'transformer')
                for i in v
            ]
            for k, v in mapping.items()
        }
...
class T5_Base(Transformer):
    ...
    def load_variable(self, checkpoint, name):
        ...
        if name == 'shared/embedding':
            return self.load_embeddings(variable)
        elif name == 'decoders/logits/kernel':
    ...
    def variable_mapping(self):
        ...
        mapping = {
            ...
            'Encoder-Embedding-Relative-Position': [
                'encoders/block_000/layer_000/SelfAttention/relative_attention_bias'
            ],
            'Encoder-Output-Norm': ['encoders/final_layer_norm/scale'],
            'Decoder-Embedding-Relative-Position': [
                'decoders/block_000/layer_000/SelfAttention/relative_attention_bias',
            ],
            'Decoder-Output-Norm': ['decoders/final_layer_norm/scale'],
        }

        for i in range(self.num_hidden_layers):
            # Encoder主体
            prefix = 'encoders/block_%03d/' % i
            mapping.update({
        ...
        if self.version.endswith('t5.1.1'):
            mapping['Decoder-Output-LM'] = ['decoders/logits/kernel']
            for i in range(self.num_hidden_layers):
                for layer in [
                    'Encoder-Transformer-%d-FeedForward' % i,
                    'Decoder-Transformer-%d-FeedForward' % i
            ...
            if self.version == 'mt5.1.1':
                mapping['Encoder-Output-Norm'] = ['encoders/rms_norm/scale']
                mapping['Decoder-Output-Norm'] = ['decoders/rms_norm/scale']
                mapping = {
