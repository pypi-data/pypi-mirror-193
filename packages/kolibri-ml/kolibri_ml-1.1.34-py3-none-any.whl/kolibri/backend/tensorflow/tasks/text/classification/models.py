...
class FastText(BaseTextClassificationModel):
    ...
    @classmethod
    def default_hyper_parameters(cls):
        return {
            'dense_layer': {
                'units': 128,
                'activation': 'relu'
...
def get_model(model_type, embedding=None, hyper_parameters=None):
    ...
    if model_type == 'avrnn':
        ...
        raise Exception(f'Model "{model_type}" does not exist in the library of text classification models')


if __name__ == "__main__":
#    print(BiLSTM_Model.default_hyper_parameters())
    ...
    corpus =pd.read_excel("/Users/mohamedmentis/Dropbox/Mac (2)/Documents/Mentis/Development/Python/kolibri-ml/examples/Emails_parsed.xlsx")
    corpus=corpus.dropna(subset=['f_body'])
    #corpus = corpus.sample(n=5000)
    min_samples=200
    corpus=corpus[corpus.language=='fr']
    bytag = corpus.groupby('target').aggregate(np.count_nonzero)
    tags = bytag[bytag.id >= min_samples].index
#
    corpus=corpus[corpus['target'].isin(tags)]
