...
#   * vec_tim, len, [(vec_val, id), ...] = tsvals(typelt, [id, ...], attr, [interval], valid_only=True)
...
#  05/02/2023 (2.1.6); added tsvals for efficiently returning multiple ts
#****
...
def tsvals(typelt: ElemType, id_seq: Sequence[str], sattr: str,
    ...
    # Return TS, length (None, 0 for nonexistent items) or (TS, ID) in valid_only mode
    if valid_only:
        ret_vals = [(all_ts[k, :], sid) for k, (idx, sid) in enumerate(zip(idx_item, id_seq)) if idx]
    else:
