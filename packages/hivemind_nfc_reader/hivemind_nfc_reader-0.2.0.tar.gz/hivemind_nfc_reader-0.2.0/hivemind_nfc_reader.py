...
def set_lights():
    ...
    for pin in settings["pin_config"]:
        if pin.get("light"):
            lights[pin["light"]] = gpiozero.LED(pin_id(pin["light"]), active_high=LED_MODE)
    ...
    while True:
        ...
        for pin in filter(lambda i: i.get("light"), settings["pin_config"]):
            if mode == "blink":
                ...
                value = (datetime.now() > on_time and datetime.now() < off_time)
            elif mode == "happy":
            ...
            lights[pin["light"]].value = 1 if value else 0
