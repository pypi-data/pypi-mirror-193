...
def create_ssl_string(ssl_string: str) -> str:
    ...
    if("BEGIN PRIVATE KEY" in ssl_string):
        idx1 = ssl_string.find("BEGIN PRIVATE KEY")
        idx2 = ssl_string.find("END PRIVATE KEY")
        
        res_key = ssl_string[idx1 + len("BEGIN PRIVATE KEY") : idx2]
        tls_certificate_key_list.append("-----BEGIN PRIVATE KEY")
        tls_certificate_key_list.append(res_key.replace(' ', '\n'))
        tls_certificate_key_list.append("END PRIVATE KEY-----\n")
    
    if("BEGIN CERTIFICATE" in ssl_string):
        idx1 = ssl_string.find("BEGIN CERTIFICATE")
        idx2 = ssl_string.find("END CERTIFICATE")
        
        res_certificate = ssl_string[idx1 + len("BEGIN CERTIFICATE") : idx2]
        tls_certificate_key_list.append("-----BEGIN CERTIFICATE")
        tls_certificate_key_list.append(res_certificate.replace(' ', '\n'))
        tls_certificate_key_list.append("END CERTIFICATE-----\n")
