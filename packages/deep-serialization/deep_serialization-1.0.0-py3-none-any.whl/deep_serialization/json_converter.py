...
class DumpLoader:
    ...
    def add_imports(self, imports: Iterable[str]):
        for module in imports:
            submodules = module.split('.')[:-1]
            for i, _ in enumerate(submodules):
                fullpath = '.'.join(submodules[:i+1])
                if fullpath not in self.imports:
                    globals()[fullpath] = importlib.import_module(fullpath)
                    self.imports.add(fullpath)
    ...
    def load_object(self, python_id: PythonId) -> object:
        ...
        self.memory[PythonId(id(real_object))] = real_object
        return real_object
