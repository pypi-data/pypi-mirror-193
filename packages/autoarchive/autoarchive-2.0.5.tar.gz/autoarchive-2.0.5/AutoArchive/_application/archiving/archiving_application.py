...
class ArchivingApplication:
    ...
    def executeCreateAction(self, selectedArchiveSpecs):
        ...
        archiveSpecsForProcessing = self.__getArchiveSpecsForProcessing(selectedArchiveSpecs)
        ...
        return self.__computeReturnValue()
    ...
    def executeListAction(self, selectedArchiveSpecs):
        ...
        orphanedArchives = frozenset(self.__getOrphanedArchives(archiveSpecsForProcessing))
    ...
    def executePurgeAction(self, selectedArchiveSpecs):
        ...
        orphanedArchives = frozenset(self.__getOrphanedArchives(archiveSpecsForProcessing))
        knownValidArchiveNames = frozenset(self.__getValidArchiveNames(archiveSpecsForProcessing))
        try:
    ...
    def __computeReturnValue(self):
        return self.__actionResult == self.__ActionResults.Successful



    def __getOrphanedArchives(self, archiveSpecs):
        return (name
                for name in self.__archiving.getStoredArchiveNames()
                if name not in frozenset(self.__getValidArchiveNames(archiveSpecs)))



    @Utils.uniq
    def __getValidArchiveNames(self, archiveSpecs):
        return self.__archiving.filterValidSpecFiles((archSpecInf.path for archSpecInf in archiveSpecs))



    def __getArchiveSpecsForProcessing(self, selectedArchiveSpecs):
        ...
        if len(selectedArchiveSpecs) == 0 or self.__configuration[Options.ALL]:
            try:
                allUniqueArchiveSpecs = tuple(getAllUniqueArchiveSpecs())
