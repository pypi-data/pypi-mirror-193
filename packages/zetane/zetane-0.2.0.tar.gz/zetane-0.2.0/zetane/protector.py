...
class Connection:
    ...
    def config(self, org=None,  project=None):
        if org and check_org_in_user(self, org.lower()):
            self.org = org.lower()
            print('Organization configuration successful')
        if self.org and project and check_project_in_org(self, self.org, project.lower()):
...
class Protector(Connection):
    def __init__(self, api_key=None, org=None, project=None):
        ...
        self.model = None
        #self.reportname = None This attribute is not used
        self.dataset = None
        self.config(org=org, project=project)
