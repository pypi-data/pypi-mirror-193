...
class PyTime(object):
    ...
    def _grouping(obj_list, min, max, time_type, differential_times_=None):
        ...
        while True:
            ...
            # 모든 요소값이 같은 경우( 1로, 혹은 실제 있던 시간 차이값이 의미없는 0 혹은 1 의 값들일때를 고려)
            if len(set(np.setdiff1d(differential_times, 0))) <= 1:
    ...
    def get_timeFromFilename(filename,exclude_strings=['screenshot']):
        ...
        # 180501
        p5 = re.compile(r'(\d{4})(0[1-9]|1[0-2]|[1-9])(0[1-9]|[1-9]|[1-2][0-9]|3[0-1])')
        ...
        regexs = [
            ...
            r'(\d{4})(0[1-9]|1[0-2]|[1-9])(0[1-9]|[1-9]|[1-2][0-9]|3[0-1])',
        ]
        try:
