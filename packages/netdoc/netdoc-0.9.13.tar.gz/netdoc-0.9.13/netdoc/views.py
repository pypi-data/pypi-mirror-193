...
class DiagramView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        if instance.sites.all():
            interface_qs = interface_qs.filter(device__site__in=instance.sites.all())
        if instance.device_roles.all():
            interface_qs = interface_qs.filter(
                device__device_role__in=instance.device_roles.all()
            )
        if instance.mode == "l2":
            # Filter out Interface without cable
            interface_qs = interface_qs.exclude(cable__isnull=True)
            if instance.vrfs.all():
                interface_qs = interface_qs.filter(
                    ip_addresses__vrf__in=instance.vrfs.all()
                )
        if instance.mode == "l3":
...
class DiscoveryLogExportView(generic.ObjectDeleteView):
    """
    ...
    * DiscoveryLogView clicking on the Discovery button on a specific Discoverable.
    ...
    queryset = models.DiscoveryLog.objects.all()
    template_name = "netdoc/discoverylog_export.html"
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Return only the rendered deletion form as modal content
        viewname = get_viewname(self.queryset.model, action="export")
        form_url = reverse(viewname, kwargs={"pk": obj.pk})
        return render(
            request,
            "netdoc/htmx/logexport_form.html",
            {
                "object": obj,
                "object_type": self.queryset.model._meta.verbose_name,  # pylint: disable=protected-access
                "form": form,
    ...
    def post(self, request, *args, **kwargs):
        """Download the log after the confirmation page."""
        logger = logging.getLogger("netbox.plugins.netdoc")
        obj = self.get_object(**kwargs)
        form = ConfirmationForm(request.POST)
        ...
        if form.is_valid():
            logger.debug("Form validation was successful")
            msg = f"Exporting discovery log {obj.id}"
            logger.info(msg)
            messages.success(request, msg)  # TODO: not working
            data = {
                "command": obj.command,
                "configuration": obj.configuration,
                "details": obj.details,
                "order": obj.order,
                "parsed_output": obj.parsed_output,
                "raw_output": obj.raw_output,
                "template": obj.template,
                "success": obj.success,
                "parsed": obj.parsed,
                "ingested": obj.ingested,
                "discoverable__mode": obj.discoverable.mode,
            }
            response = HttpResponse(json.dumps(data), content_type="application/json")
            response["Content-Disposition"] = f"attachment; filename={obj.id}.json"
            return response
