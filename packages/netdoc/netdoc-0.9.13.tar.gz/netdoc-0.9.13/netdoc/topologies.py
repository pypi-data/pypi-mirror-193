...
<table>
    <tbody>
        ...
        <tr>
            <th>VRF: </th>
            <td>{% if network.vrf %}{{ network.vrf.name }}{% else %}Global{% endif %}</td>
        </tr>
        {% endif %}
        {% if interface %}
        <tr>
...
def get_l3_topology_data(queryset, details):
    ...
    for interface_o in queryset:
        for address_o in interface_o.ip_addresses.all():
            ...
            # Create device
            vrf_name = address_o.vrf.name if address_o.vrf else None
            device_o = interface_o.device
            ...
            if network_id not in networks:
                networks[network_id] = {
                    ...
                    "title": Template(NETWORK_TEMPLATE).render(network=network_o),
                    "shape": "box",
                    "borderWidth": 2,
                    "color": {
                        "border": "#1b9bd0",
