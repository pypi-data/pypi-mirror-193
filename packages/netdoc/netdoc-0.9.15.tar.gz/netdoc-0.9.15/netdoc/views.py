...
class DiagramView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        if instance.mode == "l3":
            ...
            if instance.include_global_vrf and not instance.vrfs.all():
                # Include Global VRF only
                interface_qs = interface_qs.filter(
                    ip_addresses__vrf__isnull=True
                )
            elif instance.include_global_vrf and instance.vrfs.all():
                # Include selected VRFs and Global VRF
                interface_qs = interface_qs.filter(
                    Q(ip_addresses__vrf__isnull=True) | Q(ip_addresses__vrf__in=instance.vrfs.all())
                )

            elif not instance.include_global_vrf and instance.vrfs.all():
