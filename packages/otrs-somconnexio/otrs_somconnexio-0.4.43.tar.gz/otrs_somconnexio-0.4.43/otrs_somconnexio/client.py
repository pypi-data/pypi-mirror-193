...
class OTRSClient():
    ...
    def update_ticket_if_not_set(self, ticket_id, article, dynamic_fields=[]):
        ticket = self.client.ticket_get_by_id(ticket_id, dynamic_fields=True)
        if all(not ticket.dynamic_field_get(df.name).value for df in dynamic_fields):
            self.update_ticket(ticket_id, article, dynamic_fields)

    def link_tickets(self, src_ticket_id, dst_ticket_id,
                     link_type="Normal"):
        """
        Sets a LINK between to OTRS tickets to interconnect them.
        """
        self.client.link_add(
            src_ticket_id,
            dst_ticket_id,
            link_type=link_type
        )
