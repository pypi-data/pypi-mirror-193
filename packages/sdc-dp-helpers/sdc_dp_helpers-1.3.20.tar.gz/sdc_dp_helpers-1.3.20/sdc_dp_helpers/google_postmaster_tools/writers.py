...
class GooglePostmasterWriter(BaseWriter):
    ...
    def verify_data(
        ...
        if not {"date", "vertical", "brand", "data"}.issubset(set(payload.keys())):
            raise KeyError("invalid payload expecting: date, vertical, brand and data.")
        ...
        dimension: str = payload["brand"]
        vertical: str = payload["vertical"]
        write_path: str = f"{self.folder_path}/{vertical}/{dimension}/{date}"
