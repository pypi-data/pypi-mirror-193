...
class _Info:
    # driver card bus_info version capabilities device_capabilities crop_capabilities buffers formats frame_sizes inputs control
    ...
    def _caps(self):
        if self._caps_cache is None:
            with self.device:
                self._caps_cache = read_capabilities(self.device)
        return self._caps_cache

    @property
    def driver(self):
        return self._caps.driver.decode()

    @property
    def card(self):
        return self._caps.card.decode()

    @property
    def bus_info(self):
        return self._caps.bus_info.decode()
    ...
    def capabilities(self) -> Capability:
        return Capability(self._caps.capabilities)

    @property
    def device_capabilities(self) -> Capability:
        return Capability(self._caps.device_caps)
