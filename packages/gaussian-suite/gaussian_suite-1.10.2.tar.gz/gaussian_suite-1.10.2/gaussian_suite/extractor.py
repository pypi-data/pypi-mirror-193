...
class FchkExtractor(PlainTextExtractor):
        ...
        with open(self.txt_file, 'r') as f:
            for line in f:
                ...
                if m:
                    ...
                    yield (self.format_label(), self.format_data(data))
    ...
    def format_data(self, data):
        ...
        def vib_quadratic_solve(n_ele):
            N = (18 + np.sqrt(324 + (36*n_ele)))/(18)
            return (3*int(N), 3*int(N)-6)
        ...
        func = {
            ...
            'Vib-Modes':  # (3N * 3N-6)
                lambda x: np.reshape(
                    x, vib_quadratic_solve(np.size(x)), order='F'
                ).astype(float)
        }
