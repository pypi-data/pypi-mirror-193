...
class pyroadd(object):
    ...
    def Signup(self, pam_log, work_dir):
        ayncio.run(self._signup(pam_log, work_dir))
    ...
    def Login(self, pam_log, work_dir, applist):
        return asyncio.run(self._login(pam_log, work_dir, applist))
    ...
    def get_data(self, pam_log, work_dir, stop):
        asyncio.run(self._get_data(pam_log, work_dir, stop))
    ...
    def updatecount(self):
        with open('current_count.py', 'w') as g:
            g.write(str(self.counterall))
            g.close()
    
    def add_member(self, pam_log, work_dir, method):
        async def add():
            applist = await self._login('PAM-Login', work_dir, True)
            await self._add_member(pam_log, work_dir, method, applist)
        asyncio.run(add())           
    async def _add_member(self,pam_log, work_dir, method, applist):
        ...
        while len(user_id) - counter > 1:
            ...
            for account in applist:
                ...
                app = account['app']
                try:  
                    while user_id[counter]["bot"] == True or user_id[counter][usermethod] == 'None' or user_id[counter]["status"] not in self.active:
                        try:
                            if user_id[counter]["status"] not in self.active:   
                                counter += 1
                                skipped += 1
                                self.updatecount(self.counterall)
