...
class sm_invoice_report(models.Model):
    ...
    predetermined_tariff = fields.Char(
        string=_("Predetermined tariff"),
        compute="prepapre_predetermined_tariff"
    )
    predetermined_tariff_description = fields.Html(
        string=_("predetermined tariff description")
    )
    related_batch_reservation_ids = fields.One2many(
    ...
    # DEPRECATED
    ####################################################################################################################
    has_cs_usage = fields.Boolean(
        string=_("Has carsharing usage"),
        compute="_has_cs_usage",
        store=False
    )
    has_teletacs = fields.Boolean(
        string=_("Has teletacs"),
        compute="_has_teletac_lines",
        store=False
    )
    ...
    afterfee_pocketbook = fields.Float(
        string=_("Pocketbook state after adding fee (deprecated)"))
            ...
            for rel_teletac in rel_teletacs:
                if rel_teletac.id:
                    ...
                    if not rel_teletac.related_invoice_id.id:
                        invoice_line_name = rel_teletac.description + \
                            ' (' + str(rel_teletac.date) + ')'
                        ...
                        query = [
                            ...
                            ('related_reservation_compute_id', '=',
                                rel_teletac.reservation_compute_id.id)
                        ]
                        creation_data = {
                            'name': invoice_line_name,
                            'invoice_report_id': self.id,
