...
class request:
    def __init__(self, framework):
        ...
        vf_api_secrets = framework.get_vf_secrets()
        self.vf_api = voxelfarmclient.rest(vf_api_url)
        if vf_api_secrets:
            self.vf_api.set_file_credentials(vf_api_secrets)
...
class workflow_lambda_host:
    ...
    def __init__(self, framework = None):  
        ...
        vf_api_secrets = self.lambda_framework.get_vf_secrets()
        self.vf_api = voxelfarmclient.rest(vf_api_url)
        if vf_api_secrets:
            self.vf_api.set_file_credentials(vf_api_secrets)
    ...
    def get_callback_update_type(self):
        caller_id = self.lambda_framework.get_callback_entity_id()
        if caller_id == '0':
            return None
    ...
    def _scan_workflow(self, workflow, list):
        if 'tracks' in workflow:
            for track in workflow['tracks']:
                self._scan_workflow(track, list)
            else:
            ...
            if existing_version:
                for prop in existing_version:
                    if prop.startswith('workflow_singleton_'):
                        existing_singleton_ids[prop] = 1
            singleton_ids = {}
            for singleton in workflow['singletons']:
                print(f'Creating Product singleton {workflow["id"]} {parent_folder} {singleton}...')
                if not f'workflow_singleton_{singleton}' in existing_singleton_ids:
                    ...
                    singleton_id = result.id
                    singleton_ids[f'workflow_singleton_{singleton}'] = singleton_id
                    print(f'Created Raw Singleton {singleton_id}')
            vf.update_entity(
                project=project_id, 
                id=parent_folder, 
        ...
        if 'tracks' in workflow:
            for track in workflow['tracks']:
                ...
                if field_id in project_entity:
                    ...
                    if folder != None:
                        result = self._create_workflow_folders(vf, track, project_id, project_entity, folder_id, fields, crs)
                        if not result.success:
                            return result
                        continue
                track_name = track['name']
                print(f'Creating Product folder {product_id} {parent_folder} {track_name}...')
                result = vf.create_folder(
                    project=project_id, 
                ...
                if not result.success:
                    return result
                sub_folder_id = result.id

                fields[field_id] = sub_folder_id
                print('Created Product folder')
                result = self._create_workflow_folders(vf, track, project_id, project_entity, sub_folder_id, fields, crs)
                if not result.success:
                    return result
        result.success = True
        return result    
    ...
    def set_workflow_definition(self, workflow_definition):
        ...
        if workflow_request.state == state.initialize:
            project_entity = self.lambda_framework.get_entity(workflow_request.project_id)

            encoded_workflow = json.dumps(workflow_definition)
            ...
            api_result = self.vf_api.get_project_crs(workflow_request.project_id)
            if not api_result.success:
                self.lambda_framework.log('error: ' + api_result.error_info)
                return result

            crs = result.crs
            self.lambda_framework.log('Create Entity: Retrieved project CRS')

            self._create_workflow_folders(self.vf_api, workflow_definition, workflow_request.project_id, project_entity, '0', fields, crs)
        else:
