...
from crop_coach.envs.sample_soil_site import extract_site_params, extract_soil_params

# from crop_coach.envs.pcse.models import Wofost72_WLP_FD, Wofost72_PP
from pcse.models import Wofost72_WLP_FD, Wofost72_PP, Wofost80_NWLP_FD_beta
...
class Wofost:
    ...
    @staticmethod
    def init_wofost(self, latitude: float = 51.97, longitude: float = 5.67):
        ...
        # site_list = ["gharb-plain", "haouz-plain", "loukkos-plain", "saïs-plateau"]
        # soil_list = ["brown-soil", "calcimagnesic-soil", "hydromorphic-soil", "reg-soil"]
        ...
        data_["crop"] = read_cabo_file(os.path.join(os.path.dirname(__file__),"..","default_data", "crop", "crop.cab"))
        data_["site"] = read_cabo_file(os.path.join(os.path.dirname(__file__),"..","default_data", "site", "site.cab"))
        data_["soil"] = read_cabo_file(os.path.join(os.path.dirname(__file__),"..","default_data", "soil", "soil.cab"))
        # print(site)
        # print(f"------------------------------ site : {site} --------------------------\n ")
        # print(os.path.join(os.path.dirname(__file__),"..","default_data", "site", site+".cab"))
        # data_["site"] = read_cabo_file(os.path.join(os.path.dirname(__file__),"..","default_data", "site", site+".cab"))
        # data_["soil"] = read_cabo_file(os.path.join(os.path.dirname(__file__),"..","default_data", "soil", soil+".cab"))


        # site_soil_params = np.array(extract_site_params(os.path.join(os.path.dirname(__file__),"..","default_data", "site", site+".cab"))+extract_soil_params(os.path.join(os.path.dirname(__file__),"..","default_data", "soil", soil+".cab")))
        ...
        return params, wdp, config, site_soil_params
    ...
    def run_wofost(agromanagement, params, wdp, config) -> Tuple[np.array, float]:
        ...
        wofost = Wofost80_NWLP_FD_beta(params, wdp, agromanagement)
        # config
        ...
        r[0] = {k: 0 if v is None else v for k, v in r[0].items()}
