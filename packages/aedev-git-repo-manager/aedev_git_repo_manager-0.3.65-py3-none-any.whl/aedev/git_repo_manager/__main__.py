...
def _check_types_linting_tests(pdv: PdvType):
    ...
    with _in_prj_dir_venv(project_path):
        ...
        out: List[str] = []
        extra_args = [f"--max-line-length={mll}", "--output-format=text", "--recursive=y"] \
            + ["--ignore=" + _ for _ in excludes] + options + args  # with --recursive no --source-roots needed for ns
...
def _renew_project(ini_pdv: PdvType, project_type: str) -> PdvType:
    ...
    new_pdv = {
        'namespace_name': namespace_name, 'package_name': package_name, 'package_version': version,
        'project_desc': pdv_str(ini_pdv, 'project_desc'), 'project_path': project_path, 'project_type': project_type}
