...
class FindDifferences:
    ...
    def __init__(self, df1: object, df2: object, df_comments: object) -> None:
        """ blah """
    ...
    def find_differences(self, sheet: object) -> list:
        ...
        self.__log.debug(f"{columns1=}")
        self.__log.debug(f"{columns2=}")
        self.__log.debug(f"{columns_deleted=}")
        self.__log.debug(f"{columns_added=}")
        self.__log.debug(f"{columns_common=}")
        ...
        if not df1_sheet.equals(df2_sheet):
            ...
            for column in columns_common:
                ...
                if not df1_sheet[column].equals(df2_sheet[column]):
                    self.__log.debug(f"df1_sheet[{column}] != df2_sheet[{column}]")
                    rows = set(pd.concat([df1_sheet[column],
                                          df2_sheet[column]]).drop_duplicates(keep=False).index.to_list())
                    self.__log.debug(f"{rows=}")
                    if rows:
                        self.__log.debug("in if.")
        ...
        self.__log.info(f"Return [df_style, df2_sheet, self.dfComments].")
