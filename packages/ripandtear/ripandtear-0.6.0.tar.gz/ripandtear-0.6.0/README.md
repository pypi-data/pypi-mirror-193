...
Bunkr is a special use case with Ripandtear. 
They use a lot of javascript and are constantly changing the layout/backend information. 
Ripandtear's goal is to focus on accuracy and making sure every availible url is found, instead of trying to guess what the download url is, like other downloaders do (not intended as a diss). 
Ripandtear uses Playwright to find all the content located from Bunkr urls. 
Ripandtear also uses Playwright to download files and videos from the appropriate pages. 
Because of this Ripandtear is unable to show a progress bar like it does with other websites.
If you are downloading a video/file from bunkr you will first see the filename that is being downloaded printed in white.
When the same filename is printed again, but in green it means the file was successfully downloaded.
Bunkr images and direct links to video files (that beginning with 'media-files') will be downloaded with a progress bar.

If downloading extreamly large videos you might see a warning pop up about not enough event listeners. 
This is a warning from Playwright. 
In my experience I have yet to have a problem with the memory leak it warns you about so it is most likely a not issue (again in my experience)

Also to be kinder on Bunkr's servers, Ripandtear's Bunkr extractor incorporates random delays.
...
Bunkr videos will probably download slower than expected, but should be more consistent.
Currently if Ripandtear is going to have a problem with a site, it is going to be Bunkr.
With that being said it is still very useable
...
`Tiktits-<direct_url_name>.<extension>`
