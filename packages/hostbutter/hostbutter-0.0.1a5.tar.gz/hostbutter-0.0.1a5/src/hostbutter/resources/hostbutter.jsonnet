...
local build_env_steps(domain, domainTrigger={}) = [
...
] + [
  {
    ...
    volumes: dockerConfig,
    commands: [
      '[ ! -f "Dockerfile" ] || (butter --cluster DinD stack-build --tag-version "${DRONE_TAG}" --registry "$HB_IMAGE_REGISTRY")',
    ],
    when: domainTrigger,
  },
  {
    ...
    commands: [
      ...
      '! ls docker-compose*.y*ml 1>/dev/null 2>&1 || butter --cluster $TARGET_DOCKER_CLUSTER stack-up',
    ],
    when: domainTrigger,
  },
] + [
...
local publishDockerHubSteps = if dockerHubUser == null || std.length(dockerHubUser) == 0 || std.length(domainList) == 0 then [] else [
  {
    ...
    volumes: dockerConfig,
    commands: [
      'butter --cluster DinD registry-import "$HB_IMAGE_REGISTRY/${DRONE_REPO}:${DRONE_TAG}" --skopeo-registry "' + ciImageRegistry + '" --registry "docker.io" --owner "' + dockerHubUser + '"',
    ],
    when: { event: 'tag' },
  },
];
