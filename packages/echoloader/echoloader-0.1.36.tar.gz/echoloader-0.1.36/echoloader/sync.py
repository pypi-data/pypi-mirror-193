...
class Sync(threading.Thread):
    ...
    def sr(self, sid):
        return requests.get(f"{self.api_url}/study/sr/{sid}", headers=self.auth.get_headers(),
    ...
    def load_mods(self, sid):
        params = {**self.params, 'page': 1, 'page_size': 10_000}
        result = []
        count = 1
        while len(result) < count:
            mods = unpack(requests.get(
                f"{self.api_url}/sync/modification/{sid}", params=params, headers=self.auth.get_headers()))
    ...
    def sync(self):
        ...
        res = unpack(requests.get(f'{self.api_url}/study/search',
                                  params={**self.params, 'uploader': self.uploader, 'reportCompleted': True},
                                  headers=self.auth.get_headers()), {})
