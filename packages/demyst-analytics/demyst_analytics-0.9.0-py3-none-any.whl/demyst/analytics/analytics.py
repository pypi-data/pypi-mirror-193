...
TYPES_URL = "https://demyst.com/console/documentation/integration#demyst_types"
            ...
            with tag("tr"):
                with tag("th"):
                    if self.col in type_specs:
                        with tag("a", href=TYPES_URL + "_" + self.col, target="_blank"):
                            text(self.col)
                    else:
                        text(self.col)
                with tag("td", style="background-color: lightpink"):
                        ...
                        for input in variant:
                            with tag("tr"):
                                ...
                                with tag("td", style=style):
                                    with tag("a", href=TYPES_URL + "_" + name, target="_blank"):
                                        text(name)
...
class Analytics(object):
    ...
    def product_stats(self, provider_names=[], all_products=False):
        ...
        df_res.rename(columns={'consistency_rate':'attribute_consistency_rate',
                                ...
                                'field_is_populated_rate':'attribute_fill_rate',
                                'flattened_name':'attribute_name',
                                'generic_flattened_name':'attribute_generic_name',
                                'hit_rate':'product_match_rate',
                                'num_distinct_values':'unique_values',
                                'full_field_name':'attribute_full_name',
                                'last_updated_at': 'stats_updated_on',
                                'max_val': 'max_value',
                                'min_val': 'min_value',
                                'standard_deviation': 'std'}, inplace = True)
        ...
        df_res["attribute_consistency_rate"] = df_res["attribute_consistency_rate"].apply(rnd)
        # Reorder columns
        return(df_res[[
                    'input_entity',
                    'stats_updated_on',
    ...
    def __add_extra_stats_columns(self, df, provider_names):
        public_providers_list = self.__config.new_all_providers()
        public_providers = {}
        ...
        def extract_attribute_use_case(row):
            use_cases = filter(lambda tag: tag["category"] == "Use-Case", self.__config.lookup_provider(row.at["product_name"])["tags"])
            return ", ".join(map(lambda tag: tag["name"], use_cases))
    ...
    def enrich(self, providers_or_channel_id, inputs=None, hosted_input=None, validate=True, all_updates=False, fixed_list_size=1):
        ...
        if providers:
            ...
            self.__check_provider_names(provider_names, new_cache=False)
    ...
    def __run_batch(self, input_id, region_id, all_updates, fixed_list_size, providers=None, channel_id=None):
        ...
        if (self.__config.get_credits_or_caps() == "one_credit_to_rule_them_all"):    
            self.__print_mordor_dollars(draft_res)
        else:
            self.__print_credits(draft_res)

        org_credits = draft_res["organization"]["credit_balance"]
        run_credits = draft_res["estimated_credit_price"]
        run_credits = float(run_credits) if isinstance(run_credits, str) and run_credits.replace(".","").isdigit() else 0
    ...
    def __print_mordor_dollars(self, batch_run_resp):
        if self.__sample_mode:
            print("This enrichment will cost 0 USD dollars because you are using sample data.",
                  file=sys.stderr)
            print("To switch to live mode, please set sample_mode=False in the Analytics class.",
                  file=sys.stderr)
        else:
            run_credits = batch_run_resp["estimated_credit_price"]
            run_credits = float(run_credits) if isinstance(run_credits, str) and run_credits.replace(".","").isdigit() else 0
            run_dollars = "{:.2f}".format(run_credits / 100)
            billing_page = "https://demyst.com/app/billing"
            print("And in the data bind them.", file=sys.stderr)
            print("This enrichment will cost " + run_dollars + " USD.", file=sys.stderr)
            print("Check your billing page at " + billing_page + ", for more details.", file=sys.stderr)
    ...
    def __print_credits(self, batch_run_resp):
        if self.__sample_mode:
            ...
            run_credits = batch_run_resp["estimated_credit_price"]
            run_credits = float(run_credits) if isinstance(run_credits, str) and run_credits.replace(".","").isdigit() else 0
            run_credits = "{:.2f}".format(run_credits)
    ...
    def enrich_credits(self, providers_or_channel_id, inputs, validate=True):
        """
        ...
        if isinstance(providers_or_channel_id, list):
            ...
            providers = self.__channel_id_to_providers(providers_or_channel_id)
            channel = True

        self.__check_provider_names(providers, new_cache=True)
        inputs = self.__stringify_dframe(inputs)
        if (validate):
        ...
        num_rows = len(inputs)
        if channel is True:
            payload = {"credit_price": {"rows": num_rows, "channel_id": providers_or_channel_id}}
            price = self.__config.auth_post(self.__config.get("MANTA_URL") + "aegean_batch_runs/credit_prices", json=payload).json()
            if (self.__config.get_credits_or_caps() == "one_credit_to_rule_them_all"):
                mordor_price = "{:.2f}".format(price / 100)
                print("This enrichment costs " + mordor_price + " USD.")
                print("Actual price is dependent upon channel configuration and match status.")
                return mordor_price
            else:
                print("This enrichment costs {:n} credits.".format(price))
                print("Actual price is dependent upon channel configuration and match status.")
                return price
        else:
            payload = {"credit_price": {"rows": num_rows, "provider_names": providers}}
            price = self.__config.auth_post(self.__config.get("MANTA_URL") + "aegean_batch_runs/credit_prices", json=payload).json()
            if (self.__config.get_credits_or_caps() == "one_credit_to_rule_them_all"):
                mordor_price = "{:.2f}".format(price / 100)
                print("This enrichment costs " + mordor_price + " USD.")
    ...
    def product_details(self, inputs=None, tags=None, provider_names=None, view="html"):
        """
        View full connector details as they are on the platform
        to make more informed decisions on enrichments, etc by being
        able to see diligence, related connectors, and more.
        """  
        ...
        if provider_names and isinstance(provider_names, str):
            provider_names = provider_names.split(',')
        if  provider_names and isinstance(provider_names, list):
            connectors = []
            self.__check_provider_names(provider_names, new_cache=True)
            for provider_name in provider_names:
                connector = self.__config.lookup_provider(provider_name)
                if connector:
                    connectors.append(self.__transform_provider_to_detailed_connector(connector))
                else:
                    print("Unknown connector: " + provider_name, file=sys.stderr)
        else:
            providers = self.__config.all_providers()
            connectors = [self.__transform_provider_to_detailed_connector(p) for p in providers]
        if (view == "html"):
            return self.__render_connectors(connectors, inputs, details=True)
        elif (view == "json"):
            return connectors
        elif (view == "dataframe"):
            return pd.DataFrame(connectors).T
        else:
            raise RuntimeError("View must be html, json, or dataframe.")


    def __render_connectors(self, connectors, inputs, details=False):
        doc, tag, text, line = Doc().ttl()
        table = {}
        if connectors == []:
                                                ...
                                                with tag('td'):
                                                    with tag('span', style="font-size: 20px;"):
                                                        if connector_inputs_opt[i] in opt_inputs:
                                                            ...
                                                            text(" ")
                        # Add info for product_details
                        if details:
                            with tag('div',style='padding:10px'):
                                with tag('table', style='text-align: left', cellpadding="5"):
                                    with tag('span', style="text-align: left"):
                                        with tag('tr'):
                                            line('th', 'Cost')
                                            line('th', ":")
                                            if connector.get('credit_price') != '':
                                                line('td', str(float(connector.get('credit_price'))/100) +' USD per transaction + 0.05 USD Platform Fee')
                                            elif connector.get('credit_price') == '':
                                                line('td', 'Unknown' + ' USD per transaction + 0.05 USD Platform Fee')
                                        with tag('tr'):    
                                            line('th', 'Diligence')
                                            line('th', ":")
                                            line('td', connector.get('diligence'))
                                        with tag('tr'):
                                            line('th', 'Diligence Conditions')
                                            line('th', ":")
                                            line('td', connector.get('diligence_notes'))
                                        if str(connector.get('org_id')) == '2':
                                            with tag('tr'):
                                                line('th', 'Internal Notes')
                                                line('th', ":")
                                                line('td', connector.get('internal_notes'))
                                        with tag('tr'):    
                                            line('th', 'Data Use Cases')
                                            line('th', ":")
                                            line('td', ', '.join(str(x) for x in connector.get('data_use_case')))
                                        with tag('tr'):    
                                            line('th', 'Data Entity')
                                            line('th', ":")
                                            line('td', connector.get('data_entity'))
                                        with tag('tr'):    
                                            line('th', 'Data Categories')
                                            line('th', ":")
                                            line('td', ', '.join(str(x) for x in connector.get('data_categories')))        
        return HTML(doc.getvalue())
    ...
    def __check_provider_names(self, providers, new_cache=False):
        if new_cache:
            print("Verifying providers...")
            for p in providers:
                if not self.__config.new_lookup_provider(p):
    ...
    def __channel_id_to_providers(self, channel_id):
        params = { "for_nemo": "true"}
        channel_page = self.__config.auth_get(self.__config.get("MANTA_URL") + "channels/" + str(channel_id), params=params).json()
        providers = [provider.get('name') for provider in channel_page.get('table_providers')]
        return providers

    def validate(self, inputs=None, providers=None, channel_id=None, notebook=True):
        """
        ...
        if inputs is None:
            inputs = pd.DataFrame({})
        if not (providers or channel_id):
            providers = []
        if channel_id:
            providers = self.__channel_id_to_providers(channel_id)
    ...
    # Get org ID
    def get_org_id(self):
        return self.__config.get_organization()
    ...
    def __transform_provider_to_detailed_connector(self, provider):
        # Start with required and optional input groups
        required_input_name_sets = []
        optional_input_name_sets = []
        if type(provider["latest_version"]) is dict:
            version = provider["latest_version"]
            for data in version["schema"]:
                for variant in data["input"]["required"]:
                    req_name_set = [input["name"] for input in variant]
                    required_input_name_sets.append(req_name_set)
        ...
        provider_name = provider.get("name") or ""
        if provider.get('alias') == None or provider.get('alias')=='':
        ...
        if data_source:
            ...
            data_source_name = provider_name
        # Provider regions of coverage
        regions = [region["name"] for region in provider["data_regions"]]
        # Provider tags include:
        # use cases, entity, and sub-categories
        use_cases = []
        categories = []
        for tag in provider["tags"]:
            if tag["category"] == "Use-Case":
                use_cases.append(tag["name"])
            elif tag["category"] == "SubEntity(Data-Category)":
                categories.append(tag["name"])
        # Provider entity (main category)
        entity = provider.get("data_category")
        if entity:
            data_entity = entity.get("name") or ""
        else: 
            data_entity = ""

        # Org ID for internal notes
        org_id = self.get_org_id()

        return {
            "name": provider_name,
            "alias": provider_alias,
            "product_type": provider.get("product_type") or "",
            "data_source_name": data_source_name,
            "data_source_logo_url": provider.get("logo") or "",
            "required_input_name_sets": required_input_name_sets,
            "optional_input_name_sets": optional_input_name_sets,
            "description": provider.get("description") or "",
            "credit_price": provider.get("credit_price") or "",
            "diligence": provider.get("diligence_tag") or "",
            "diligence_notes": provider.get("text_conditionally_approved") or "N/A",
            "internal_notes": provider.get("notes") or "N/A",
            "data_use_case": use_cases,
    ...
    def sample_data(self, provider_name, row_limit=None, filters=None):
        ...
        def trying_dtypes(rows):
            df = pd.read_csv(resp.json().get("url"), na_filter=False, nrows=rows)
            types = df.dtypes.to_dict()
            try:
                df = pd.read_csv(resp.json().get("url"), na_filter=False, dtype=types)
                return df
            except (pd.errors.DtypeWarning, ValueError):
                rows += 100
                df = trying_dtypes(rows)
                return df
        df = trying_dtypes(100)
