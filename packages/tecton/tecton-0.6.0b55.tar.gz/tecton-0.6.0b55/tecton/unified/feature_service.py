...
logger = logger.get_logger("FeatureService")
...
class FeatureService(unified_common.BaseTectonObject, internal_fco.Fco):
    ...
    _feature_references: Tuple[unified_feature_view.FeatureReference, ...] = attrs.field(
    ...
    def __init__(
        ...
        *,
        ...
        features: List[Union[unified_feature_view.FeatureReference, unified_feature_view.FeatureView]] = None,
        ...
        for feature in features:
            if isinstance(feature, unified_feature_view.FeatureReference):
                feature_references.append(feature)
            elif isinstance(feature, unified_feature_view.FeatureView):
                feature_references.append(unified_feature_view.FeatureReference(feature_definition=feature))
            else:
                raise TypeError(
    ...
    def _from_spec(cls, spec: specs.FeatureServiceSpec, fco_container: fco_container.FcoContainer) -> "FeatureService":
        ...
        for feature_set_item in spec.feature_set_items:
            ...
            ref = unified_feature_view.FeatureReference(
    ...
    @property
    @sdk_decorators.sdk_public_method
    def features(self) -> List[unified_feature_view.FeatureReference]:
    ...
    def _check_can_query_from_source(self, from_source: Optional[bool]) -> unified_feature_view.QuerySources:
        all_query_sources = unified_feature_view.QuerySources()
    ...
    def get_historical_features(
        ...
        print(f"Computing historical feature values for {len(self._feature_definitions)} {feature_view_text}.")
        ...
        if isinstance(spine, str):
            raise TypeError(
                "When using spark compute, `spine` must be one of (pyspark.sql.dataframe.DataFrame, pandas.core.frame.DataFrame, tecton.unified.data_frame.TectonDataFrame); got str instead."
            )
    ...
    def _request_context(self) -> request_context.RequestContext:
        ...
        for fv in self._feature_definitions:
            if isinstance(fv, unified_feature_view.OnDemandFeatureView):
...
def _feature_reference_to_feature_service_feature_package(
    feature_reference: unified_feature_view.FeatureReference,
