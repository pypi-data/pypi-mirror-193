...
from tecton.unified import athena_api
from tecton.unified import common as unified_common
from tecton.unified import compute_mode
from tecton.unified import configs
from tecton.unified import data_frame as tecton_dataframe
from tecton.unified import data_source as unified_data_source
from tecton.unified import delete_keys_api
from tecton.unified import entity as unified_entity
from tecton.unified import filtered_source
from tecton.unified import materialization_api
from tecton.unified import query_helper
from tecton.unified import run_api
from tecton.unified import snowflake_api
from tecton.unified import spark_api
...
from tecton.unified.common import TectonObjectInfo  # nopycln: import - Needed for SDK docs generation.
...
class QuerySources:
    ...
    def _print_from_source_message(self, count: int, single: str, message: str):
        if count > 0:
            ...
            print(f"{count} {feature_view_text} {message}")
...
class FeatureView(unified_common.BaseTectonObject, internal_fco.Fco):
    ...
    def _get_dependent_objects(self, include_indirect_dependencies: bool) -> List[unified_common.BaseTectonObject]:
        raise NotImplementedError
...
class MaterializedFeatureView(FeatureView):
    ...
    def get_historical_features(
        ...
        """
        print(f"Computing feature values for Feature View {self._feature_definition.name}.")
    ...
    @sdk_decorators.sdk_public_method
    @utils.requires_local_object(error_message=errors.CANNOT_USE_LOCAL_RUN_ON_REMOTE_OBJECT)
    def local_run(
        self,
        start_time: datetime.datetime,
        end_time: datetime.datetime,
    ...
    def feature_start_time(self) -> Optional[datetime.datetime]:
        if self._args is not None:
            return (
                self._args.materialized_feature_view_args.feature_start_time.ToDatetime()
...
def stream_feature_view(
    *,
    ...
    :param name: Unique, human friendly name that identifies the FeatureView. Defaults to the function name.
    :param max_batch_aggregation_interval: (Advanced) Configure the time range materialized by each backfill jobs. This can help optimize large backfill jobs.
    :param output_stream: Configuration for a stream to write feature outputs to, specified as a :class:`tecton.unified.conifgs.KinesisOutputStream` or :class:`tecton.unified.configs.KafkaOutputStream`.
    :return: An object of type :class:`tecton.declarative.feature_view.MaterializedFeatureView`.
...
class FeatureTable(FeatureView):
    ...
    def get_historical_features(
        ...
        print(f"Computing feature values for {self._feature_definition.name} from offline Feature Tables.")
...
class OnDemandFeatureView(FeatureView):
    ...
    def get_historical_features(
        ...
        print(f"Computing feature values for On-Demand Feature View {self._feature_definition.name}.")
        sources = self._check_can_query_from_source(from_source)
        sources.display()
...
def _build_pipeline_for_materialized_feature_view(
    fv_name: str,
    feature_view_function: Callable,
    pipeline_function: Callable[..., unified_transformation.PipelineNodeWrapper],
    sources: Sequence[Union[unified_data_source.DataSource, filtered_source.FilteredSource]],
) -> unified_transformation.PipelineNodeWrapper:
...
def _source_to_pipeline_node(
    source: Union[
        unified_data_source.DataSource,
    ...
    if isinstance(source, unified_data_source.DataSource):
    ...
    return unified_transformation.PipelineNodeWrapper(node_proto=pipeline_node)
...
) -> Dict[str, unified_transformation.PipelineNodeWrapper]:
    kwargs = {}
