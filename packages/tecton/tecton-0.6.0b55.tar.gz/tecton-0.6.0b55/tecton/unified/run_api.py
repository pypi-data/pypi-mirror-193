...
) -> "tecton.unified.data_frame.TectonDataFrame":
...
def _querytree_run_batch(
    ...
    framework_version: FrameworkVersion,
    aggregation_level: Optional[str],
) -> "tecton.unified.data_frame.DataFrame":
    ...
    return tecton.unified.data_frame.TectonDataFrame._create(qt)
...
def run_ondemand(
...
) -> Union[Dict[str, Any], "tecton.unified.data_frame.TectonDataFrame"]:
    ...
    if isinstance(output, pandas.DataFrame):
        output = tecton.unified.data_frame.TectonDataFrame._create(output)
