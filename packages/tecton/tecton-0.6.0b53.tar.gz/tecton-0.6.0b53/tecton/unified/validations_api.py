...
def format_server_errors(
    ...
    if response.error:
        return f"{tecton_object.__class__.__name__} '{tecton_object.info.name}' failed validation with the following error: {response.error}"
    ...
    if len(validation_errors) == 1:
        ...
        return f"{tecton_object.__class__.__name__} '{tecton_object.info.name}' failed validation with the following error: {error.message}"
    else:
        error_strings = [
