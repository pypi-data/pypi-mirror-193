...
from typing import Optional, Any, Generator, Callable, Mapping, Type, Tuple, List, Dict, Union
...
class S3Manager(MicroserviceDatabaseManager):
    ...
    def __create_awss3folder(awss3folder: AWSS3Folder):
        client = boto3.client('s3')
        client.put_object(Bucket=awss3folder.bucket_id, Key=awss3folder.display_name + '/', StorageClass=awss3folder.storage_class.name)

    @staticmethod
    def __create_awss3file(awss3file: AWSS3FileObject):
        client = boto3.client('s3')
        client.put_object(Bucket=awss3file.bucket_id, Key=awss3file.display_name, StorageClass=awss3file.storage_class.name)
