...
@api.command()
@click.argument('processing_ids', required=True, nargs=-1, type=click.INT)
@click.option('-q', '--image-quality', type=int, default=70,
              help="Jpeg image quality between 0 and 100. Default is 70.")
@click.option('-s', '--segment-size', type=int, default=20,
              help="Number of images in job. Set 0 to not split task into jobs.")
@pass_rebotics_context
def export_to_cvat(ctx, processing_ids, image_quality, segment_size):
    if ctx.verbose:
    ...
    try:
        if ctx.verbose:
            click.echo('Sending request...')
        response = ctx.provider.export_to_cvat(processing_ids, image_quality, segment_size)
