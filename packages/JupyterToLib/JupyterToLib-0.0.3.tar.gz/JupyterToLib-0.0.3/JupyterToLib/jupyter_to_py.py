...
class Transform:
  ...
  def convert_file_indentation(self, file_path):
    with open(file_path, 'r') as f:
      file_str = f.read()
    ...
    self.extract_libraries(importaciones_str,req_path)
    with open(file_path, 'w') as f:
      f.write(new_file_str)
  
  def ProcessLib(self, nb_file, nb_file2, verbose = True):
    if verbose:
    ...
    self.convert_file_indentation(nb_file2)
