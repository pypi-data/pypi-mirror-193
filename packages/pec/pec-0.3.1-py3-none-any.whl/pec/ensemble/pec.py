...
class _ProgressiveEnsembleClustering:
    ...
    def __init__(self, data, n_clusters=2, n_runs=4, alg='k-means', decision='inertia', random_state=None,
        ...
        self.observer = self.__setupObserver(observer, {})
    ...
    def __setupObserver(self, observer, observer_args):
        if observer is None:
            return None

        if not inspect.isclass(observer):
            raise RuntimeError(f'observer must be a subclass (not instantiated) of ProgressionObserver')
        else:
            observerInstance = observer(self, self.data, observer_args)
            if not isinstance(observerInstance, ProgressionObserver):
                raise RuntimeError(f'observer must be instance of ProgressionObserver')
            else:
    ...
    def __exec(self):
        ...
        while self.__active:
            ...
            if self.observer is not None:
                currentResult.info.is_last_et = self.observer._isEarlyTermination(previousResult, currentResult)
            ...
            if self.observer is not None:
                self.observer._notifyPartialResult(previousResult, currentResult)
        ###
        ### process completed
        ###
        self.__on_end()
