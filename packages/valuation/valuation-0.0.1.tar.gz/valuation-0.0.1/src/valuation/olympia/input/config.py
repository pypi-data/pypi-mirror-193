...
from valuations.olympia import secrets_manager
from valuations.olympia.input import defaults
from valuations.olympia.input.exception import OlympiaImportError
from valuations.olympia.input.market_data_connection.base_parser import get_parsers
from valuations.olympia.input.market_data_connection.source import CLOUD_SOURCES, CloudSource, LOCAL_SOURCES, LocalSource, OLYMPIA, \
...
class Configuration:
    ...
    def _read_default_params(self) -> None:
        ...
        if VALUATION_DEFAULTS not in self._config_parser.sections():
            return
        section = self._config_parser[VALUATION_DEFAULTS]
        for key, value in section.items():
            new_key, new_value = get_section_item_pair(key, value)
            if new_value is not None:
                new_path = get_source_path(new_value, self._defaults_base_path)
                if new_key == DEFAULT_PARAMETERS:
                    self._default_parameters = new_path
                elif new_key == VALUATIONS:
                    self._valuations = new_path
                elif new_key == RESULT_TYPES:
                    self._result_types = new_path
                else:
                    raise OlympiaConfigurationError(
                        f'Unknown key {new_key} in section {VALUATION_DEFAULTS}, possible keys are {(DEFAULT_PARAMETERS, VALUATIONS)}')
