...
from valuations.olympia.input.exception import OlympiaImportError
from valuations.olympia.input.market_data_connection import get_parsers
from valuations.olympia.input.storage_conversion import InstrumentFinalization, OlympiaStorage, get_generators
...
from valuations.universal_transfer import Reference, Storage, StorageDataBase

if __type_checking__:
    # pylint: disable=ungrouped-imports
    from valuations.universal_transfer import Signature
    from valuations.olympia.input.config import Configuration
    from valuations.olympia.input.market_data_connection.source import SourceDescriptor, Source
    from valuations.olympia.input.market_data_connection.base_parser import Parser
    from valuations.olympia.input.storage_conversion.storage_generation import StorageGeneration
    from valuations.olympia.input.request import Request
...
class RequestDataBase(ConfiguredStorageDataBase):
    ...
    def _create_default_valuation(self, instrument: Storage) -> None:
        ...
        if not valuations_types or not result_types:
            Log.info(f'Instrument Type {instrument_type} has no default result types and/or valuation type.'
                     f'No valuation object was created.')
            return
