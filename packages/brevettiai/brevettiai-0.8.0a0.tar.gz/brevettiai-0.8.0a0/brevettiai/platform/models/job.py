...
from brevettiai.io import IoTools, io_tools, Credentials, AnyPath, get_default_s3_client_manager
...
from brevettiai.datamodel import Dataset, Tag
...
class Job(BaseModel):
    ...
    _job_dir: Optional[str] = PrivateAttr(default=None)
    _backend: PlatformBackend = PrivateAttr(default=None)
    _io: IoTools = PrivateAttr(default=None)
    _credentials: Credentials = PrivateAttr(default_factory=DefaultJobCredentialsChain)
    _temp_dir_reference: Optional[tempfile.TemporaryDirectory] = PrivateAttr(default=None)
    _temp_dir: Optional[str] = PrivateAttr(default=None)
    ...
    def __init__(self, io=io_tools, backend=default_backend, cache_path=None, job_dir=None, **data) -> None:
        ...
        self._temp_dir = AnyPath(cache_path) if cache_path else cache_path
        self._job_dir = AnyPath(job_dir) if job_dir else job_dir
    ...
    def prepare_start(self, resolve_access_rights: bool = True, cache_remote_files: bool = True, set_credentials: bool = True):
        ...
        self._credentials.set_credentials(type="JobCredentials", user=self.id, secret=self.api_key)
        #if cache_remote_files:
        #    self.io.set_cache_root(self.temp_path)
    ...
    def resolve_access_rights(self) -> None:
        ...
        manager.resolve_access_rights(
            partial_path=self.job_dir,
            credentials_getter=lambda: self._credentials.get_credentials(resource_id=self.id, resource_type="job")
        )
        for ds in self.datasets:
            ds.resolve_access_rights(self._credentials)

        for model in self.models:
            model["bucket"] = AnyPath(model['bucket'])
            manager.resolve_access_rights(
                partial_path=model["bucket"],
                credentials_getter=lambda: self._credentials.get_credentials(resource_id=model['id'],
                                                                             resource_type="job")
            )

    def disable_access_rights(self):
        for dataset in self.datasets:
            dataset.disable_access_rights()
        for model in self.models:
            try:
                model["bucket"].client.disable()
            except AttributeError:
                pass
        try:
            self.job_dir.client.disable()
        except AttributeError:
            pass
    ...
    def prepare_temp_dir(self):
        ...
        tempdir = os.getenv(env_variables.BREVETTI_AI_CACHE, self._temp_dir)
        if tempdir is None:
            self._temp_dir_reference = tempfile.TemporaryDirectory(prefix=f"brevettiai-job-{self.id}-")
            self._temp_dir = AnyPath(self._temp_dir_reference.name)
        else:
            self._temp_dir = AnyPath(tempdir)

    @property
    def job_dir(self) -> str:
        if self._job_dir is None:
            self._job_dir = AnyPath(self.backend.resource_path(self.id))
    ...
    def upload_job_output(self, name="output.json"):
        """
        Upload / update the output.json artifact containing parsed settings, etc.
        :param name:
        :return:
        """
        target = self.artifacts_path / name
        target.write_text(self.job_output.json())

    def get_output_segmentation_dir(self, dataset_id):
        """
        ...
        """
        return self.artifacts_path / self.backend.output_segmentation_dir / dataset_id
