...
def _align_phonems():
    ...
    cpu_count = cpu_count - 1 if cpu_count > 1 else cpi_count

    command = f'{align_exe} "{folder}" "{dict_path}" "{trained_model}" "{out_dir}"  -s {str(speakers)} {quiet} --single_speaker --use_mp -j {cpu_count}'
