...
import numpy as np
import pandas as pd
from datagen_protocol.schema.humans.human import FacialHair, Eyebrows
...
from datagen.api.catalog.attributes import AllOf, AnyOf, Exactly, Generator
...
class CatalogInstancesCache(Generic[Asset]):
    ...
    def _load(self, asset_id) -> Asset:
        try:
            asset = self._cache[asset_id].copy()
        except KeyError:
...
class AttributesDataFrame:
    ...
    def _create_attr_series(
        self, attr_name: str, attr_query_val: Union[str, Enum, AllOf, AnyOf, Exactly]
    ) -> pd.Series:
