...
from engine.model import CChessModel
from engine.player import CChessPlayer, VisitState
from cchess import BLACK, Move
from engine.model_helper import load_best_model_weight
from engine.env import CChessEnv
    ...
    def move(self):
        ...
        for nominate, action_state in self.ai.search_results.items():
            try:
                ...
                log.error(e)
        # end_time = time.time()
        # log.info(f"ai move time: {(end_time - start_time):.1f}s")
        return True, action, self.mcts_moves
