...
from cczero.lookup_tables import ActionLabelsRed
from cczero.api import CChessModelAPI
...
class CChessModel:
    ...
    def load(self, config_path, weight_path):
        if os.path.exists(config_path) and os.path.exists(weight_path):
            ...
            self.graph = tf.get_default_graph()
            log.info(f"loaded model digest = {self.digest}")
            return True
        else:
            log.info(f"model files does not exist at {config_path} and {weight_path}")
            return False

    def save(self, config_path, weight_path):
        log.info(f"save model to {config_path}")
        with open(config_path, "wt") as f:
            json.dump(self.model.get_config(), f)
            self.model.save_weights(weight_path)
        self.digest = self.fetch_digest(weight_path)
        log.info(f"saved model digest {self.digest}")
