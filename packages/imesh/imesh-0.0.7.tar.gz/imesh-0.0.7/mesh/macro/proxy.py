...
class Proxy(Generic[T]):
    ...
    def __new__(cls, kind: type, handler: InvocationHandler, *args, **kwargs):
        """
        ...
        """
        proxy_class = cls.create_proxy_class(kind, handler)
    ...
    @classmethod
    def create_proxy_class(cls, kind: type, handler: InvocationHandler) -> Type["Proxy"]:
        """
        Creates a proxy for the given class
        """
        qname = f"{str(kind)}{str(handler.__class__)}"
        ...
        methods = Proxy.get_abstract_methods(kind)
        namespace = {}
        ...
        for name in methods:
            namespace[name] = Proxy.make_delegate(methods[name], handler)
        cls.__proxy_classes__[qname] = type("%s(%s)" % (cls.__name__, kind.__name__), (kind, cls), namespace)
    ...
    @staticmethod
    def make_delegate(method, handler: InvocationHandler):
        def invoke(self, *args, **kwargs):
            return handler.invoke(self, method, *args, **kwargs)
