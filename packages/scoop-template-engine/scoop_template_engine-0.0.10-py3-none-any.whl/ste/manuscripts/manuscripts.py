...
% Insert the user-defined prepreamble.
{prePreamble:s}'''.format(
        customPrePreambleFileNameWithoutExtension = self.CustomPrePreambleFileNameWithoutExtension(),
...
{LaTeXChanges:s}'''.format(
        customPreambleFileNameWithoutExtension = self.CustomPreambleFileNameWithoutExtension(),
  ...
  # * running scoop-template-engine.py again on the specialized template-bibgenerator.tex with a random --outname
  # * to create the temporary file random-bibgenerator.bcf, from which
  # * scoop-prepare-bibtex-file.py then derives manuscript-template.bib.
  def CreateCustomBibTeXFile(self):
    ...
    # Invoke scoop-template-engine.py recursively with the same command line arguments plus '--outname random --template bibgenerator --quiet', minus '--dump'.
    commandString = (self.thisScriptAbsolutePathCallSummary + " --outname {outname:s} --template bibgenerator --quiet").format(outname = outname).replace(' --dump','')
    returnValue = subprocess.run(commandString, shell = True)
    
    # Run pdflatex once on the generated file, suppressing the output.
    texFileNameStub = "/".join(filter(None, [self.outDirectory, outname + '-bibgenerator']))
    ...
    # Invoke scoop-prepare-bibtex-file.py on the generated .bcf file, provided it is non-empty.
    bcfFileName = re.sub('.tex$', '.bcf', texFileName)
    bibFileName = "/".join(filter(None, [self.outDirectory, re.sub('.tex$', '.bib', self.outFileBaseName)]))
    commandString = re.sub('-template-engine.py$', '-prepare-bibtex-file.py', self.thisScriptAbsolutePathCallSummary.split(" ")[0]) + " " + bcfFileName + " " + self.customBibliographySwitches
