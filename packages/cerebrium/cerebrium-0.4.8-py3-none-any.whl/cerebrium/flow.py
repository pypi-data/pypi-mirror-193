...
def _check_valid_model(
    ...
    if model_type == ModelType.HUGGINGFACE_TRANSFORMER:
        if not isinstance(model_filepath, dict):
            raise TypeError(
                f"Model {pipeline_position}: For HuggingFace Transformers, model_filepath must be of type dict, but is {type(model_filepath)}."
            )
    else:
    ...
    if (
        model_type == ModelType.HUGGINGFACE_TRANSFORMER
        and not isinstance(model_filepath, dict)
        and "task" not in model_filepath
        and "hf_id" not in model_filepath
    ):
        raise TypeError(
            f"Model {pipeline_position}: For HUGGINGFACE_TRANSFORMER, model_filepath must be a dictionary with keys 'task' and 'hf_id', but is {model_filepath}"
        )
    elif (
        model_type == ModelType.HUGGINGFACE_TRANSFORMER
        and model_filepath["hf_id"] not in HUGGINGFACE_ALLOWED_IDS
    ):
        raise TypeError(
            f"Model {pipeline_position}: For HUGGINGFACE_TRANSFORMER, model_filepath['hf_id'] must be a supported HuggingFace model ID, but is {model_filepath['hf_id']}. Use one of {HUGGINGFACE_ALLOWED_IDS}. If you would like to use larger LLMs, consult the Cerebrium prebuilt model catalog."
        )
    elif (
        model_type != ModelType.PREBUILT
        and model_type != ModelType.HUGGINGFACE_TRANSFORMER
        ...
        if processor is not None:
            ...
            if len(processor_args) != 1:
                raise NotImplementedError(
                    f"Model {processor}: The {stage}-processing function must have exactly one argument, but has {len(processor_args)}"
                )
