...
class Conduit:
    ...
    def _determine_hardware(self):
        ...
        if any(
            [
                model_type
                in [ModelType.TORCH, ModelType.ONNX, ModelType.HUGGINGFACE_TRANSFORMER]
    ...
    def load(self, directory: str = "/cache/", direct_from_flow: bool = False):
        ...
        if self.flow == []:
            ...
            for model_type, model_path, _ in self.flow:
                ...
                elif model_type == ModelType.HUGGINGFACE_TRANSFORMER:
                    from transformers.pipelines import pipeline

                    model = pipeline(
                        model_path["task"], model_path["hf_id"]
                    )
    ...
    def run(self, data: list):
        ...
        if not self.ready:
            ...
            for (model_type, _, processors), (model) in zip(self.flow, self.graph):
                ...
                preprocessor, postprocessor = processors.get(
                    "pre", None
                ), processors.get("post", None)
                if preprocessor:
    ...
    def setup_loggers(self):
        for platform, config in self.logger_configs.items():
            if platform == "Censius":
                from cerebrium.loggers.censius import CensiusLogger
                ...
                from cerebrium.loggers.arize import ArizeLogger
            ...
            json.dump(
                {
                    ...
                    "logger_configs": str(self.logger_configs),
                    "processors": [
                        {
                            "pre": None
                                ...
                                for root, _, files in os.walk(true_path):
                                    for file in files:
                                        ...
                                        zip.write(
                                            ...
                                            os.path.join(
                                                f"{os.path.basename(true_path)}{directory}", file
                                            ),
                                        )
