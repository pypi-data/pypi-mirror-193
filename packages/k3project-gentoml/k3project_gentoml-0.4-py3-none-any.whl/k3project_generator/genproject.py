...
def create_reference_project(targetDirectory, projectName, selectedLicense=None, createDoc=True, createTest=True) -> None:
    ...
    _copyfile(join(RESOURCE_DIR, "dummy_setup.cfg"), join(projRoot, "setup.cfg"))
    _copyfile(join(RESOURCE_DIR, "dummy_pyproject.toml"), join(projRoot, "pyproject.toml"))
    _replace_in_file(join(projRoot, "setup.cfg"), "placeholderproj_root", rootPackage)
    _replace_in_file(join(projRoot, "setup.cfg"), "placeholderprojnm", projectName)
