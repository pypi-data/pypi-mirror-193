...
def main():
    ...
    custom_nodes = ["testnet.openhive.network"]
    hive = AppBase(nodes=custom_nodes, chain_id=TESTNET, timeout=5, retries=1, warning=True)
    ...
    data = hive.request(method, params, strict=False, mock=True)
    print("Accounts: " + json.dumps(data, indent=2))
    """

    counter += 1
    print(f"\nTEST #{counter}: Using the `broadcast` method - `condenser_api`.")
    hive.append_wif(wif=tests["wif1"], role="posting")
    method = "condenser_api.get_transaction_hex"
    transaction = {
        "ref_block_num": 27033,
        "ref_block_prefix": 1234340064,
        "expiration": "2022-11-11T6:15:00",
        "operations": [
            [
                "vote",
                {
                    "voter": tests["username1"],
                    "author": tests["username3"],
                    "permlink": tests["permlink3"],
                    "weight": 10000,
                },
            ]
        ],
    ...
    data = hive.broadcast(method, transaction, strict=False, mock=True)
    print(data)
