...
class _HandlerCore:
    ...
    def __init__(self, token):
        ...
        self._channels, cursor = self._read_channels(None)
        while cursor is not None:
            additional, cursor = self._read_channels(cursor)
            self._channels.update(additional)


    def _read_channels(self,cursor:Optional[str])->Tuple[Dict[str,str],Optional[str]]:
        """Read channels with pagination"""
        next_cursor = None
        response = self.client.conversations_list(types="public_channel,private_channel", cursor=cursor)
        if response['ok']:
            channelresp = response['channels']
            if (more := response.get('response_metadata', None)) != None:
                 ncstr = more.get('next_cursor', None)
                 next_cursor = ncstr if ncstr else None
            return {c['name']: c['id'] for c in channelresp}, next_cursor
    ...
    def channel_id(self, channelname: str) -> str:
        ...
        raise ValueError(f"Invalid channel name {channelname}, valid channels are {','.join(self._channels)}")
