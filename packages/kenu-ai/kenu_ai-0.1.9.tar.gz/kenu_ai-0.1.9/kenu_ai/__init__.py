...
def chat(value):
    if("sahibin kim" in value.lowercase()):
        print("kenan")
        ...
        if("zackybot" in translated.text.lowercase()):
            new_string = "Adım, kenu-ai. r4isy tarafından yazıldım."
            return new_string
        elif("acobot.ai" in translated.text.lowercase() or "acobot" in translated.text.lowercase()):
        else:
