...
def update_list_profit_loss(account):
    ...
    pnl = (order['price'][-1] - order['price'][-2]) * 200 - order['fee'][-1] - order['fee'][-2] - order['tax'][-1] - order['tax'][-2]
    ...
    if os.path.isfile('%s_profit_loss.csv'%account):
        df = pd.concat([pd.read_csv('%s_profit_loss.csv'%account, index_col='ts'), df], axis=0)
