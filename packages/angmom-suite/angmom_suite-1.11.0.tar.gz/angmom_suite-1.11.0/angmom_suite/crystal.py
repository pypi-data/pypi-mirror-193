...
from .basis import unitary_transform, cartesian_op_squared, \
    calc_ang_mom_ops, make_angmom_ops_from_mult, project_angm_basis, \
...
def rotate_angm(op, quax):
    return np.einsum('ji,imn', quax, op)
...
def evaluate_term_space(sf_mult, model_space, coupling=None, complete=False):
    ...
    def reorder_and_expand(cg_vec, term_space):
        ...
        if complete:
            ...
            # re-adjust phases
            vec = vec.at[:, :len(cg_mults)].multiply(jnp.diag(rmat))
        else:
...
def evaluate_term_operators(sf_angm, sf_ener, sf_amfi, sf_mult, term_space,
    ...
    if quax is not None:
        sf_angm = [rotate_angm(a, quax) for a in sf_angm]
        sf_amfi = [[rotate_angm(v, quax) for v in row] for row in sf_amfi]

    sf_vecs = [project_angm_basis(iter(terms), a, complete=complete)
               for terms, a in zip(sf_terms, sf_angm)]

    term_amfi_blks = [[u.conj().T @ v @ w for v, w in zip(row, sf_vecs)]
                      for row, u in zip(sf_amfi, sf_vecs)]
    relative_phases = amfi_phase(term_amfi_blks)
...
def read_params(file, **mapping):
    with h5py.File(file, 'r') as h:
        for term in iter(h):
            if term == 'diag':
                pass
            else:
                for ops in iter(h[term]):
                    path = h['/'.join([term, ops, 'parameters'])]
...
class MagneticSusceptibility(Store):
    ...
    def evaluate(self, **ops):
        ...
        data = {temp: (temp * chi) if self.chi_T else chi
...
def eprg_tensor(spin, angm):
    muB = 0.5  # atomic units
    magm = magmom(spin, angm)
    return 2 / muB * jnp.einsum('kij,lji->kl', magm, magm)
