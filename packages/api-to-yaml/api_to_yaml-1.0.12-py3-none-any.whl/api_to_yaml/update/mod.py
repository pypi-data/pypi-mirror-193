...
from api_to_yaml.operators.mod import add_items_quantity, drop_tags, list_to_tags, metadata_to_args, override, remove_fields, tags_to_list, to_args, transform, with_args, with_default, with_tags
...
def update_etag(line):
    with open(line, "r") as f:
        content = yaml.safe_load(f)

    def wrap(res):
        etag = res["ETag"]
        content["metadata"]["ETag"] = etag
        if 'pipelineConfig' not in content:
            content["pipelineConfig"] = {"no-apply": True,
