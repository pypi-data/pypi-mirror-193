...
def main(action, line):
    ...
    kind, info, content = preprocess_file(line, True, wait_for_ref)
    ...
    if action == "create":
        ...
        current_expanded = {}
        if current:
            current_crd = export_ref_(kind, current, info, line)
            _, _, current_expanded = preprocess(
                current_crd, info, folder, True, wait_for_ref)
        res = diff_wrap(kind, current_expanded, info, content,  line)
    if need_diff:
