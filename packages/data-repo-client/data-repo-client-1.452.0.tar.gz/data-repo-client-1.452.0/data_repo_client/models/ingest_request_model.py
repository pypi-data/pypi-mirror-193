...
class IngestRequestModel(object):
    ...
    def update_strategy(self):
        ...
        Approach for how to treat updates to the target table:  * `append` - Default: only ever insert rows, regardless of whether or not the primary key value exists  * `replace` - If a primary key is present on the table, treat rows with matching primary keys              as updates. If duplicate IDs are found in your ingest, the ingest job              will fail. Note: the full new record must be specified.  * `merge` - If a primary key is present on the table, treat rows with matching primary keys              as partial updates. Any fields specified will overwrite their current values              in the matching table row.  Each ingest row must match exactly one table row,              and no duplicate IDs should be found in your ingest, otherwise the ingest job              will fail.   # noqa: E501
