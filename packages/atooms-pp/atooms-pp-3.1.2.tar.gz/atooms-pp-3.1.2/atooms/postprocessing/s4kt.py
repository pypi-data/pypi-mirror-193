...
class S4ktOverlap(FourierSpaceCorrelation):
    ...
    def __init__(self, trajectory, tgrid, kgrid=None, norigins=-1,
        ...
        # Setup time grid
        assert len(tgrid), 'currently only a single time interval for S_4(k,t) is supported'
    ...
    def _compute(self):
        ...
        cnt = [defaultdict(int) for _ in self.kgrid]
        npart = self._pos_unf[0].shape[0]
        for off, i in progress(self._discrete_tgrid):
            # As for fkt
            W = self._tabulate_W(self.kgrid, off, i, self.skip)
            ...
            # Normalization
            self.grid[0].append(self.trajectory.timestep * dt)
            self.value.append([float(w2_av[k] - (w_av[k]*w_av[k].conjugate())) / (cnt[k][dt]*npart) for k in range(len(cnt))])
