...
class AcFun(CheckIn):
    ...
    def danmu(self, session):
        ...
        subChannel = re.findall(r'{subChannelId:(\d+),subChannelName:"([\u4e00-\u9fa5]+)"}', response.text)
        if videoId:
    ...
    def get_info(session):
        ...
        info = res.get("info")
        return [{"name": "当前等级", "value": info.get("level")}, {"name": "持有香蕉", "value": info.get("banana")}]
    ...
    def main(self):
        ...
        if flag is True:
            ...
            msg = [{"name": "帐号信息", "value": phone}, sign_msg, like_msg, danmu_msg, throwbanana_msg] + info_msg
        else:
