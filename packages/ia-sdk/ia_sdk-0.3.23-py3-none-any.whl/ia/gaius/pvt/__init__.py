...
class PerformanceValidationTest():
    ...
    def __init__(self, agent: AgentClient,ingress_nodes: list,query_nodes: list,num_of_tests: int, pct_of_ds: float,
                 ...
                 shuffle:bool=False, sio=None, task=None, user_id:str=None, mongo_db=None, dataset_info:dict=None, test_id=None, test_configuration:dict={}, socket_channel:str='pvt_status'):
    ...
    def test_agent(self):
        ...
        if test_seq_len == 0:
            print(f'length of testing sequences is 0... returning\n')
            return
    ...
    def compute_incidental_probabilities(self, test_step_info: dict):
        ...
        if self.test_type == 'classification':
            ...
            if 'running_accuracy' not in test_step_info:
                test_step_info['true_positive']       = collections.defaultdict(dict)  
                test_step_info['false_positive']      = collections.defaultdict(dict)  
                test_step_info['true_negative']       = collections.defaultdict(dict)  
                test_step_info['false_negative']      = collections.defaultdict(dict)  
                test_step_info['unknown_percentage']  = collections.defaultdict(dict)  
                test_step_info['response_percentage'] = collections.defaultdict(dict)  
                test_step_info['response_counts']     = collections.defaultdict(dict)  
                test_step_info['running_accuracy']    = collections.defaultdict(dict)    
                test_step_info['running_precisions']  = collections.defaultdict(dict)  
                ...
                for k in test_step_info['predicted'].keys():
                    ...
                    for emotive in test_step_info['actual'].keys():
                        ...
                        if emotive in test_step_info['predicted'][k].keys():
                            ...
                            try:
                                test_step_info['running_precisions'][k][emotive] = (test_step_info['true_positive'][k][emotive] + 
                                                                                    test_step_info['true_negative'][k][emotive]) / test_step_info['response_counts'][k][emotive]
                            except ZeroDivisionError:
                                test_step_info['running_precisions'][k][emotive] = 0.0
                            
                            test_step_info['running_accuracy'][k][emotive] = (test_step_info['true_positive'][k][emotive]
                                                                            + test_step_info['true_negative'][k][emotive])  / (idx+1)
            ...
            if 'running_accuracy' not in test_step_info:
                ...
                test_step_info['response_percentage'] = collections.defaultdict(dict)  
                test_step_info['response_counts']     = collections.defaultdict(dict)  
                test_step_info['residuals']           = collections.defaultdict(dict)  
                ...
                for k in test_step_info['predicted'].keys():
                    for emotive in test_step_info['actual'].keys():
                       ...
                       # Unknown answer
                            ...
                            test_step_info['residuals'][k] = collections.defaultdict(float)
                            test_step_info['response_counts'][k][emotive] = 0
                            test_step_info['response_percentage'][k][emotive] = 0.0
                            test_step_info['unknown_percentage'][k][emotive] = 1.0
    ...
    def update_test_results_w_hive_classification_metrics(self, pvt_test_result):
        ...
        for node_name, test_data in pvt_test_result.items():                     
            if node_name == 'hive':
                ...
                prec_predictions       = [p for p, a in zip(test_data["predictions"], test_data['actuals']) if p != 'UNKNOWN']
