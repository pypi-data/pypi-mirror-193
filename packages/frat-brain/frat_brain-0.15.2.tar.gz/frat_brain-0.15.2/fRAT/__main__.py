...
def run_tests(GUI):
    ...
    if General['full_comparison']['Current']:
...
def find_example_dataset():
    ...
    full_data_folders = glob(os.path.expanduser('~/Documents/fRAT/*full_example_data*'))
    ...
    if not example_data_folders:
        raise FileNotFoundError(f'No example data folder in fRAT directory.\n'
                                f'Download it from https://osf.io/pbm3d/ and place it into the fRAT directory '
                                f'("~/Documents/fRAT/").')

    if len(full_data_folders) > 1:
        raise FileExistsError('More than one full example dataset found in fRAT directory, only keep the version that'
                              'matches your fRAT version.')
    elif General['full_comparison']['Current'] == 'true' and not full_data_folders:
        raise FileNotFoundError('Full comparison selected, but full example dataset is not present in fRAT directory.')

    if General['full_comparison']['Current'] == 'true':
        return full_data_folders[0]

    if not General['full_comparison']['Current'] == 'false' and not subject_data_folders:
        return full_data_folders[0]
    else:
