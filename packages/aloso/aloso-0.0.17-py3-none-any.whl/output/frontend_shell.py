...
class FrontendShell(Frontend):
    ...
    def install():
        ...
        if not exists(conn, ".nvm/versions/node/*"):
            conn.local(f"wget -qO- {config.nvm_wget_url} | bash")
        ...
        conn.local(f"cd {config.frontend_project_dir}/src && npm install")
        conn.local(f"cd {config.frontend_project_dir}/src && npm run dev")
