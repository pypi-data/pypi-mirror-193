...
from .constants import DEFAULT_CAPTURE_IMAGE_DIR_PATH, TIME_DIRECTORY_FORMAT, CAPTURE_DATA_DIR, \
    ...
    DEFAULT_CAPTURE_BLOB_DIR_PATH, DEFAULT_CAPTURE_LOG_FILE_PATH
...
class CaptureClient:
    ...
    def _init_base_logger(self):
        ...
        log_file_path = f"{self.path}/{DEFAULT_CAPTURE_LOG_FILE_PATH}"
    ...
    def _is_data_capture_enable(self):
        ...
        :return:
        """
        return self.is_enable_capture_input or \
            self.is_enable_capture_prediction or \
    ...
    def log(self,
        ...
        if input is not None and self.is_enable_capture_input == "True":
            message['input'] = input if isinstance(input, str) else json.dumps(input, cls=NpEncoder)

        if prediction is not None and self.is_enable_capture_prediction == "True":
            message['prediction'] = prediction if isinstance(prediction, str) else json.dumps(prediction,
                                                                                              cls=NpEncoder)
        if ground_truth is not None and self.is_enable_capture_ground_truth == "True":
        ...
        print(self._is_data_capture_enable())
        print(self.is_enable_capture_input)
        print(self.is_enable_capture_prediction)
        print(self.is_enable_capture_ground_truth)
        if self._is_data_capture_enable():
    ...
    def _load_env_setting_and_config(self, config: ModelCollectConfig):
        ...
        env_capture_path = DEFAULT_CAPTURE_DATA_PATH if len(os.environ.get(CAPTURE_DATA_DIR)) == 0 \
            else os.environ.get(CAPTURE_DATA_DIR).rstrip("/")
        config.path = config.path.rstrip("/") if len(config.path) > 0 else env_capture_path
        self.path = config.path if len(config.path) > 0 else DEFAULT_CAPTURE_DATA_PATH
    ...
    def _log_image(self,
        ...
        tmp_path = self._gen_image_save_path(correlation_id)
...
DefaultClient = CaptureClient(ModelCollectConfig())
