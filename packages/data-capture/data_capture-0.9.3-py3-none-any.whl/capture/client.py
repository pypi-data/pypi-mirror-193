...
class CaptureClient:
    ...
    def _init_base_logger(self):
        ...
        loguru_logger.add(handler, enqueue=True, level='INFO', encoding='utf-8')
    ...
    def log(self,
        ...
        if data is not None:
            ...
            for k, v in data.items():
                if isinstance(v, (list, dict)):
                    message[k] = json.dumps(v, cls=utils.NpEncoder)
        ...
        if input is not None:
            message['input'] = input if isinstance(input, str) else json.dumps(input, cls=utils.NpEncoder)
    ...
    def _record(self, message: Dict):
        ...
        :return:
        """
        json_encode_message = json.dumps(message, cls=utils.NpEncoder)
    ...
    def _load_env_setting_and_config(self, config: ModelCollectConfig):
        ...
        :return:
        """
        self.model_name = config.model_name if config.model_name is not None else utils.get_model_name()
