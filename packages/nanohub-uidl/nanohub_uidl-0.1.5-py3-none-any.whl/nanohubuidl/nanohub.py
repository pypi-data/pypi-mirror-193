...
class Auth:
    def Login(tp, Component, *args, **kwargs):
        AComponent = TeleportComponent(
            "AuthLogin", TeleportElement(MaterialContent(elementType="Paper"))
        )
        ...
        AComponent.addStateVariable(
            "showpassword", {"type": "string", "defaultValue": "visibility_off"}
        )
        AComponent.addStateVariable(
            "typepassword", {"type": "string", "defaultValue": "password"}
        )
        AComponent.addStateVariable(
      ...
      self.props.refreshToken(self);
        ...
        TypographyText = TeleportStatic(
            content=kwargs.get("title", "Sign in to nanoHUB.org")
        )
        ...
        Username.content.attrs["defaultValue"] = {
            "type": "dynamic",
            "content": {"referenceType": "state", "id": "username"},
        }
        Username.content.attrs["InputProps"] = {
            "type": "dynamic",
            "content": {"referenceType": "prop", "id": "username_icon()"},
        }
        Username.content.events["change"] = [
            {
                "type": "stateChange",
                "modifies": "username",
        ...
        Password.content.attrs["type"] = {
            "type": "dynamic",
            "content": {"referenceType": "state", "id": "typepassword"},
        }
        Password.content.attrs["placeholder"] = "Enter your Password"
        Password.content.attrs["defaultValue"] = {
            "type": "dynamic",
            "content": {"referenceType": "state", "id": "password"},
        }
        Password.content.attrs["InputProps"] = {
            "type": "dynamic",
            "content": {"referenceType": "prop", "id": "password_icon(this)"},
        }
        Password.content.events["change"] = [
            {
                "type": "stateChange",
                "modifies": "password",
                "newState": "$e.target.value",
        ...
        Message.content.attrs["value"] = {
            "type": "dynamic",
            "content": {"referenceType": "state", "id": "message"},
        }

        Conditional = TeleportConditional(MaterialContent(elementType="Paper"))
        Conditional.reference = {
            "type": "dynamic",
            "content": {"referenceType": "state", "id": "loading"},
        }
        ...
        Component.addStateVariable(
            kwargs.get("open_state", "login_open"),
    ...
    def validateCredentials(tp, Component, *args, **kwargs):
        ...
        js += (
            "          selfr.setState({'open' : true, 'loading' : false, 'message' : data.message});"
            + eol
        )
        js += "          selfr.props.onError(data.message);" + eol
        js += "        } else {" + eol
        js += (
            "          selfr.setState({'open' : true, 'loading' : false, 'message' : 'Error authenticating user'});"
            + eol
        )
        ...
        js += (
            "          "
            + store_name
            + ".setItem('nanohub_token', String(data.access_token));"
            + eol
        )
        js += (
            "          "
            + store_name
            + ".setItem('nanohub_expires', JSON.stringify(Date.now()+parseInt(data.expires_in)-200));"
            + eol
        )
        js += (
            "          "
            + store_name
            + ".setItem('nanohub_refresh_token', String(data.refresh_token));"
            + eol
        )
        js += (
            "          selfr.setState({'open' : false, 'loading' : false, 'message' : ''});"
            + eol
        )
        ...
        callbacklist = []
        callbacklist.append(
            {"type": "stateChange", "modifies": "open", "newState": True}
        )
        callbacklist.append(
            {"type": "stateChange", "modifies": "loading", "newState": True}
        )
        callbacklist.append(
            {"type": "stateChange", "modifies": "message", "newState": ""}
        )
        callbacklist.append({"type": "propCall2", "calls": "onClick", "args": ["self"]})
        return callbacklist

    def Session(tp, Component, *args, **kwargs):
        SComponent = TeleportComponent(
            "AuthSession", TeleportElement(MaterialContent(elementType="Paper"))
        )
        SComponent.node.content.style = {"width": "100%"}
        SComponent.addStateVariable("open", {"type": "boolean", "defaultValue": True})
        SComponent.addStateVariable(
            "message", {"type": "string", "defaultValue": "Validating Session"}
        )
    ...
    def validateSession(tp, Component, *args, **kwargs):
        ...
        js += "        if(data.message){" + eol
        js += (
            "          selfr.setState({'message' : '(' + data.code + ') ' +data.message});"
            + eol
        )
        js += (
            "          selfr.props.onError( '(' + data.code + ') ' +data.message );"
            + eol
        )
        js += "        } else {" + eol
        js += (
            "          selfr.setState({'message' : '(' + data.code + ') Error validating Session'});"
            + eol
        )
        js += (
            "          selfr.props.onError( '(' + data.code + ') Error validating Session' );"
            + eol
        )
        js += "        } " + eol
        js += "      } else {" + eol
        ...
        js += "          " + store_name + ".removeItem('nanohub_refresh_token');" + eol
        js += (
            "          selfr.setState({'open' : true, 'message' : 'Error validating Session, invalid token'});"
            + eol
        )
        ...
        js += "      " + store_name + ".removeItem('nanohub_refresh_token');" + eol
        js += (
            "      selfr.setState({'open' : true, 'message' : 'Error validating Session, unknown error'});"
            + eol
        )
        js += "      selfr.props.onError( 'Error validating Session' );" + eol
        js += "    })" + eol
    ...
    def refreshToken(tp, Component, *args, **kwargs):
        ...
        return [{"type": "propCall2", "calls": "refreshToken", "args": ["self", ""]}]
