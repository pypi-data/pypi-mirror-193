# Releasing the HEA Server Organizations Microservice
...
1. For numbered releases, remove `.dev` from the version number in setup.py, tag it in git to indicate a release,
   ...
   `x.x.x` with the actual version number being released. Name the tag `heaserver-organizations-x.x.x`, replacing
