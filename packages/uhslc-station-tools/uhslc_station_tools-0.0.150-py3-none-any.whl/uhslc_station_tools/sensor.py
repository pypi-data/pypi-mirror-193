    ...
    def save_fast_delivery(self, din_path: str, path: str, is_test_mode=False, callback: Callable = None):
        ...
        for month in self.month_collection:
            ...
            if month_end + 1 > 12:
                ...
                year_end = year + 1
            if is_hourly:
                data_hr = data_obj.copy()
                data_hr['hou']['time'] = np.array(data_obj['hou']['time'])
                data_hr['hou']['sealevel'] = np.array(data_obj['hou']['sealevel'])
            else:
            ...
            if is_hourly:
                hr_flat = data_hr[primary_sensor.lower()]['sealevel'].flatten()
            else:
