...
def autocreate(
    ...
    :param attach_events: Format to attach events in. If left unset, no events
            are attached.
    :param status_on_update: if set, status to the set the case to when events are added to an existing case.
    ...
    def _on_case_updated(case: dict):
        ...
        if case["status"] == "closed" and closed_case_update == "reopen":
            ...
            if not dry:
                update_case(case["id"], status=reopen_status)
            log.info(f"reopened case #{id_} (set to {reopen_status})")
        elif status_on_update and (case["status"] != status_on_update):
            if not dry:
                update_case(case["id"], status=status_on_update)
            log.info(f"set case #{id_} to {status_on_update}")
        if request_workflow:
            _request_workflow(case, request_workflow, workflow_comment)
        elif acknowledge_workflow:
            _acknowledge_workflow(case, acknowledge_workflow, workflow_comment)
