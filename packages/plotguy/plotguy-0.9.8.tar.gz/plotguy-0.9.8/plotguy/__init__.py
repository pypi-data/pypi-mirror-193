...
def resample_summary_to_daily(para_combination, folder=''):
    ...
    df_backtest = df_backtest.loc[df_backtest['action'].isnull() == False].copy() # All backtest force to treart as summary, ie filter action only
...
def cal_performance(para_combination):
    ...
    bah_annualized_return, bah_annualized_std, bah_annualized_sr = calculate_sharp_ratio(df, 'close', risk_free_rate)
