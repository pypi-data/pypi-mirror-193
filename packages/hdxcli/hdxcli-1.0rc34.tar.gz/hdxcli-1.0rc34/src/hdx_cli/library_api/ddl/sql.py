...
def _create_transform_output_column(col_def: ColumnDefinition,
    ...
    is_composed_datatype = isinstance(col_def.hdx_datatype, list)
...
#   the_transform['format_details']["delimiter"] = ddl.csv_delimiter
    ...
    output_columns = the_transform['settings']['output_columns']

    c: ColumnDefinition
    for c in ddl.columns:
        output_columns.append(
            _create_transform_output_column(c,
                                            from_input_index=ddl.csv_input_indexes.get(c.identifier),
                                            is_primary_key=(ddl.final_primary_key == c.identifier)))
