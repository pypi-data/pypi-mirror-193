...
def extract_ts(data_idx: int,
    ...
    try:
        ...
        with lock:
            ...
            result_labels_df[COL_DATA_PATH].loc[data_idx] = rel_path
        ...
        logger.warning(f"Radiomics extracting error: {e}, "
                       f"\ndata_path: {data_path}, "
                       f"\nmask_path: {mask_path}")
...
def extract(dataset_dir: str, result_dir: str, config: Union[str, dict],
    ...
    if data_type == DatasetType.NON_TS:
        ...
        for data_idxs, idx_list in zip(group_data_idxs, group_idxs):
            extract_non_ts(data_idxs,
                           ...
                           extractor,
                           lock)
        # TODO: run with multi-process
        #
        #     pool.apply_async(
        #         func=extract_non_ts,
        #         args=(group_data_idxs,
        #               [dataset.data_ids[_] for _ in idx_list]
        #               if dataset.data_ids else None,
        #               [dataset.data_paths[_] for _ in idx_list],
        #               [dataset.mask_paths[_] for _ in idx_list],
