...
def back_for_better(func):
    ...
    def wrapper(self,df=None,*args,**kwargs):
        if isinstance(df, list) or isinstance(df, tuple):
            for i in df:
                with qdbing.Sender('43.143.223.158',9009) as sender:
                    sender.dataframe(pd.DataFrame({'token':[self.token],'factor_name':[i],'operate_time':[datetime.datetime.now()]}),table_name='download_records')
        elif df is not None:
            with qdbing.Sender('43.143.223.158',9009) as sender:
                sender.dataframe(pd.DataFrame({'token':[self.token],'factor_name':[df],'operate_time':[datetime.datetime.now()]}),table_name='download_records')
        else:
            with qdbing.Sender('43.143.223.158',9009) as sender:
                sender.dataframe(pd.DataFrame({'token':[self.token],'factor_name':['info'],'operate_time':[datetime.datetime.now()]}),table_name='download_records')
        try:
            return func(self,df,*args,**kwargs)
        except Exception:
