...
import requests
import hashlib
token_safety_times = 1000 * 60 * 30  # 30min
...
class GeTui:
    ...
    # 透传消息(常用于用户在线时推送的事件)
    def push_transmission_msg(self, cid_list, content):
        msg = {
            'push_message': {
    ...
    def _push_msg(self, cid_list, msg_body):
        if len(cid_list) == 1:
            ...
            return self._pust_batch(cid_list, msg_body)
    ...
    def _pust_batch(self, cid_list, msg_body):
        headers = {'token': self._get_token()}
        # 创建消息
