...
class SQLConnector(Connector, ABC):
    ...
    @abstractmethod
    def _export(self, query, fmt, row_limit=None):
    ...
    def append(self, query, data_source, fmt):
        export = self._export(query, fmt)
    ...
    def replace(self, query, data_source, fmt, replace_condition=None, row_limit=None):
