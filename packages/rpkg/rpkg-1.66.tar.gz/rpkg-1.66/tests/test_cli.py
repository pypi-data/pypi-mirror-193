...
Initial version'''
    ...
    @patch('pyrpkg.Commands.commit')
    @patch('pyrpkg.specfile_uses_rpmautospec', return_value=True)
    def test_commit_with_clog_and_rpmautospec(self, specfile_uses_rpmautospec, commit):
        """
        specfile_uses_rpmautospec is patched always (it doesn't recognize its input arguments)
        """
        cli_cmd = ['rpkg', '--path', self.cloned_repo_path, 'commit', '--clog']

        with patch('sys.argv', new=cli_cmd):
            six.assertRaisesRegex(self, rpkgError,
                                  r'.+cannot generate the commit message from changelog while '
                                  r'using autochangelog.+',
                                  self.cli_commit)
        self.assertEqual(commit.call_args_list, [])

    @patch('pyrpkg.Commands.commit')
    @patch('pyrpkg.Commands.uses_autochangelog', return_value=True)
    def test_commit_with_clog_and_autochangelog(self, uses_autochangelog, commit):
        """
        uses_autochangelog should prevent command "commit" to finish
        """
        cli_cmd = ['rpkg', '--path', self.cloned_repo_path, 'commit', '--clog']
    ...
    @patch('pyrpkg.Commands.commit')
    @patch('pyrpkg.Commands.uses_autorelease', return_value=True)
    def test_commit_with_clog_and_autorelease(self, uses_autorelease, commit):
        """
        uses_autorelease should allow command "commit" to finish (unlike uses_autochangelog)
        """
        cli_cmd = ['rpkg', '--path', self.cloned_repo_path, 'commit', '--clog']
...
class TestPrep(CliTestCase):
    ...
    def test_prep_extra_args_with_space(self, _run_command):
        first_arg_with_space = 'first with space'
        second_arg_with_space = 'second with space'
        cli_cmd = ['rpkg', '--path', self.cloned_repo_path, '--release', 'rhel-8', 'prep', '--',
                   '--define', first_arg_with_space, second_arg_with_space]
        ...
        spec = os.path.join(cli.cmd.path, cli.cmd.spec)
        rpmbuild = ['rpmbuild'] + cli.cmd.rpmdefines + ['--define',
                                                        first_arg_with_space,
                                                        second_arg_with_space,
                                                        '--nodeps', '-bp', spec]
        _run_command.assert_called_once_with(rpmbuild)
...
class TestLint(CliTestCase):
    ...
    @patch('pyrpkg.Commands._run_command')
    def test_lint_warning_with_info_newer_lint_package(self, _run_command):
        self.checkout_branch(git.Repo(self.cloned_repo_path), 'eng-rhel-7')
...
class TestBuildPackage(FakeKojiCreds, CliTestCase):
    ...
    def assert_srpm_url(self, url, filename):
        re = '{0}/{1}$'.format(KOJI_UNIQUE_PATH_REGEX.rstrip('$'), filename)
        six.assertRegex(self, url, re)

    def assert_build(self, sub_command, cli_opts=[],
                     expected_chain_urls=None, expected_opts={},
                     expected_srpm=False, config_file=None):
        session = self.mock_ClientSession.return_value
        ...
        if sub_command == 'chain-build':
            ...
            self.assert_srpm_url(url, filename)
        elif expected_srpm:
            self.assert_srpm_url(url, '{0}.src.rpm'.format(cli.cmd.nvr))
        else:
    ...
    def assert_srpm_upload(self, cli, expected_srpm_file=None):
        session = self.mock_ClientSession.return_value
        ...
        if expected_srpm_file is None:
            nvr_path = os.path.join(cli.cmd.layout.srcrpmdir, cli.cmd.nvr)
            self.assertEqual('{0}.src.rpm'.format(nvr_path), srpm_file)
        else:
    ...
    def assert_option_srpm_use(self, expected_srpm_file=None):
        """Assert correct scratch-build invocation with explicit --srpm option.
        """
        ...
        self.assert_srpm_upload(cli, expected_srpm_file)

    def assert_default_srpm_use(self):
        """Assert correct scratch-build invocation in conditions where srpm is
        generated and uploaded by default.
        """
    ...
    @patch('pyrpkg.Commands.nvr', new_callable=PropertyMock)
    @patch('pyrpkg.Commands._run_command')
    def test_scratch_build_generates_srpm_if_repo_has_uncommitted_changed(self, _run_command, nvr):
        self.make_changes(filename='hello.py', content='print()')

        with patch('pyrpkg.layout.build', return_value=MockLayout(root_dir=self.cloned_repo_path)):
            nvr.return_value = 'docpkg-0.1-1.fc28'
            self.assert_default_srpm_use()

    @patch('pyrpkg.Commands.nvr', new_callable=PropertyMock)
    @patch('pyrpkg.Commands._run_command')
    def test_scratch_build_from_build_command_generates_srpm__if_repo_has_uncommitted_changes(
            self, _run_command, nvr):
        self.make_changes(filename='hello.py', content='print()')
