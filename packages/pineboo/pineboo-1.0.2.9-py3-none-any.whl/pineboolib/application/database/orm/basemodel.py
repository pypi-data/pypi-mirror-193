    ...
    def _check_integrity(self, ignore_foreignkey: bool = False) -> bool:
        ...
        if not table_meta.isQuery():
            ...
            for field in table_meta.fieldList():
                ...
                print("*", self._session.in_transaction())
                ...
                if relation_m1 is not None:
                    ...
                    print(
                        "**", self._session.in_transaction(), self._session.in_nested_transaction()
                    )
                    if foreign_class_ is not None:
                        ...
                        rel = relation_m1.foreignField()
                        print(
                            "****",
                            self._session.in_transaction(),
                            self._session.in_nested_transaction(),
                        )
                        foreign_field_obj = getattr(foreign_class_, rel, None)
                        print(
                            "*****",
                            self._session.in_transaction(),
                            foreign_field_obj,
