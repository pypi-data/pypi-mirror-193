...
class WebCrawler:
    ...
    def crawl_data(self, responses: List[Any], semaphore, url: Url, pipes: List[Observable] = [], headers: Dict[str, str] = None, allow_redirects: bool = True, timeout: int = 5, max_retries: int = 3) -> Any:
        with semaphore:
            try:
    ...
    def start_crawling(self, urls: List[Url], pipes: List[Observable] = [], headers: Dict[str, str] = None, allow_redirects: bool = True, timeout: int = 5, max_retries: int = 3) -> List[Any]:
        responses = []
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_threads) as executor:
            futures = []
            for url in urls:
                futures.append(executor.submit(self.crawl_data, responses, semaphore, url, pipes, headers, allow_redirects, timeout, max_retries))
