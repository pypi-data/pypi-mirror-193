...
def test_bad_parameters_datatype(test_input):
    ...
    with pytest.raises(ValidationError):
        Work(pipeline="test_pip", parameters=test_input)
...
def test_bad_results_datatype(test_input):
    ...
    with pytest.raises(ValidationError):
        Work(pipeline="test_pip", results=test_input)
...
def test_bad_priorities(test_input):
    ...
    with pytest.raises(ValidationError):
        Work(pipeline="test_priority_range", priority=test_input)


@pytest.mark.parametrize("test_input", [{}, 2, "123456"])
def test_precursors_bad_datatype(test_input):
    """Checks validation for precursors field."""
    with pytest.raises(ValidationError):
        Work(pipeline="test_precursors", precursors=test_input)
...
def test_products_bad_datatype(test_input):
    ...
    with pytest.raises(ValidationError):
        Work(pipeline="test_products", products=test_input)
...
def test_status_bad_datatype(test_input):
    ...
    with pytest.raises(ValidationError):
        Work(pipeline="test_status", status=test_input)
...
def test_site_bad_value(test_input):
    ...
    with pytest.raises(ValidationError):
        Work(pipeline="test_site", site=test_input)
