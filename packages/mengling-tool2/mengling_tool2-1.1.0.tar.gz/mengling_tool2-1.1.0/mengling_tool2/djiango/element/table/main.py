...
class Tabler:
    ...
    def getDatadts(self, table, lies='*', cellnum=None):
        return self.sqlt.select(lies, table, where=self.getWhere(), other=self.getOther(cellnum))

    def getMaxnum(self, table):
        return self.sqlt.getNum(table, self.getWhere())

    # 新增、修改或删除操作
    def adu_action(self, sqlt, table, and_where='1=1'):
        print(self.req.body)
    ...
    def getTable(self, table, cellnum: int, lies='*', rowdts: list = None, maxnum: int = 0) -> dict:
        if rowdts is None:
            rowdts = self.getDatadts(table, lies=lies, cellnum=cellnum)
        if type(lies) == str:
