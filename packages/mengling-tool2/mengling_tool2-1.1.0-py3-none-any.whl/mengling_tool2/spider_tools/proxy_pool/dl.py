...
from . import webfunc
from tools.goodlib import ThreadDictGoods
...
from ...spider_tools.functions.cookie import getHeaders_str
from ...spider_tools.functions.args_get import getFakeUserAgent
from ...spider_tools.spiders import Httpx
...
class DL:
    def __init__(self, dbindex, name, connect, checkurl=None, checkweek=60, maxerrornum=1, min_ip_num=20, ifcheck=True):
        self.name = name
        self.goodt = ThreadDictGoods({'spider': [Httpx, {'headers': ymx_headers}],  #
                                      'r': [RedisExecutor, {'dbindex': dbindex, **connect}]})
    ...
    def checkHaveIps(self):
        ...
        while True:
            ...
            if self.ifcheck:
                # 自检查
                threads_run(opition, [[ipdt] for ipdt in ipdts])
            else:
    ...
    def collect(self):
        ...
        def opition(ipdt):
            r = self.goodt.getThreadKeyGood('r')
            ip = ipdt['host']
            if r.hexists(self.name, ip):
                jdprint('[已存在]', ip)
            elif self.yz(ip) and self.ifcheck:
                r.hset(self.name, ip, json.dumps(ipdt))
                jdprint('[新增]', ipdt)
            elif self.ifcheck:
                jdprint('[失败]', ip)
