...
class TimeTool:
    def __init__(self, date: datetime or str or int or float, gs="%Y-%m-%d %H:%M:%S"):
        if type(date) is int or type(date) is float:
            timeArray = time.localtime(date)
            data = time.strftime(gs, timeArray)
        if type(date) is str:
            ts = re.findall('\d{2}', date)
            assert len(ts) > 3, f'{date} 格式有误'
            ...
            date = datetime.strptime(f'{year}-{month}-{day} {h}:{m}:{s}', "%Y-%m-%d %H:%M:%S")
        self.date = date
        self.gs = gs

    def getLastDay(self, if_return_str=False):
        return getLastDay(self.date.year, self.date.month, gs=self.gs, ifre_str=if_return_str)

    def next(self, dn, hn=0, mn=0, sn=0, if_replace=False, ifreturn_str=True):
