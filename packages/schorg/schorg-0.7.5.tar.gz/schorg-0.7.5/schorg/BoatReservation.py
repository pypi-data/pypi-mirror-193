...
class BoatReservationInheritedProperties(TypedDict):
    """A reservation for boat travel.Note: This type is for information about actual reservations, e.g. in confirmation emails or HTML pages with individual confirmations of reservations. For offers of tickets, use [[Offer]].
...
class BoatReservationProperties(TypedDict):
    """A reservation for boat travel.Note: This type is for information about actual reservations, e.g. in confirmation emails or HTML pages with individual confirmations of reservations. For offers of tickets, use [[Offer]].
...
class BoatReservationAllProperties(
    BoatReservationInheritedProperties, BoatReservationProperties, TypedDict
):
    pass


class BoatReservationBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="BoatReservation", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        BoatReservationProperties,
        BoatReservationInheritedProperties,
        BoatReservationAllProperties,
    ] = BoatReservationAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "BoatReservation"
    return model


BoatReservation = create_schema_org_model()


def create_boatreservation_model(
    model: Union[
        BoatReservationProperties,
        BoatReservationInheritedProperties,
        BoatReservationAllProperties,
    ...
    _type = deepcopy(BoatReservationAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of BoatReservation. Please see: https://schema.org/BoatReservation"
            )
    # delete_keys = []
...
def schema_json(model: BoatReservationAllProperties):
    pydantic_type = create_boatreservation_model(model=model)
