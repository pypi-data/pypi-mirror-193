...
class ChildCareInheritedProperties(TypedDict):
    """A Childcare center.
...
class ChildCareProperties(TypedDict):
    """A Childcare center.
...
class ChildCareAllProperties(
    ChildCareInheritedProperties, ChildCareProperties, TypedDict
):
    pass


class ChildCareBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="ChildCare", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ChildCareProperties, ChildCareInheritedProperties, ChildCareAllProperties
    ] = ChildCareAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "ChildCare"
    return model


ChildCare = create_schema_org_model()


def create_childcare_model(
    model: Union[
        ChildCareProperties, ChildCareInheritedProperties, ChildCareAllProperties
    ]
):
    _type = deepcopy(ChildCareAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of ChildCare. Please see: https://schema.org/ChildCare"
            )
    # delete_keys = []
...
def schema_json(model: ChildCareAllProperties):
    pydantic_type = create_childcare_model(model=model)
