...
class LendActionProperties(TypedDict):
    ...
    Note:
        Model Depth 4
    Attributes:
        borrower: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): A sub property of participant. The person that borrows the object being lent.
    """

    borrower: NotRequired[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]
...
def create_lendaction_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of LendAction. Please see: https://schema.org/LendAction"
            )
    # delete_keys = []
