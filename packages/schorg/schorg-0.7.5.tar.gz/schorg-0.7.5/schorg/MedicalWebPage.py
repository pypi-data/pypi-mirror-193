...
class MedicalWebPageProperties(TypedDict):
    ...
    Note:
        Model Depth 4
    Attributes:
        medicalAudience: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): Medical audience for page.
        aspect: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): An aspect of medical practice that is considered on the page, such as 'diagnosis', 'treatment', 'causes', 'prognosis', 'etiology', 'epidemiology', etc.
    """

    medicalAudience: NotRequired[
        Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]
    ]
    aspect: NotRequired[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]
...
def create_medicalwebpage_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of MedicalWebPage. Please see: https://schema.org/MedicalWebPage"
            )
    # delete_keys = []
