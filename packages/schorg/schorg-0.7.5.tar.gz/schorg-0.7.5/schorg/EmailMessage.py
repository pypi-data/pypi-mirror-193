...
class EmailMessageAllProperties(
    EmailMessageInheritedProperties, EmailMessageProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        ...
        EmailMessageInheritedProperties,
        EmailMessageAllProperties,
    ] = EmailMessageAllProperties
) -> Type[SchemaOrgBase]:
...
def create_emailmessage_model(
    ...
    _type = deepcopy(EmailMessageAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of EmailMessage. Please see: https://schema.org/EmailMessage"
            )
    # delete_keys = []
...
def schema_json(model: EmailMessageAllProperties):
