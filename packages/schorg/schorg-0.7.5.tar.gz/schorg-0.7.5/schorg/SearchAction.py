...
class SearchActionInheritedProperties(TypedDict):
    """The act of searching for an object.Related actions:* [[FindAction]]: SearchAction generally leads to a FindAction, but not necessarily.
...
class SearchActionProperties(TypedDict):
    """The act of searching for an object.Related actions:* [[FindAction]]: SearchAction generally leads to a FindAction, but not necessarily.
    ...
    Note:
        Model Depth 3
    Attributes:
        query: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): A sub property of instrument. The query used on this action.
    """

    query: NotRequired[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]


class SearchActionAllProperties(
    SearchActionInheritedProperties, SearchActionProperties, TypedDict
):
    pass


class SearchActionBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="SearchAction", alias="@id")
    ...
    class Config:
        ...
        fields = {"query": {"exclude": True}}


def create_schema_org_model(
    type_: Union[
        SearchActionProperties,
        SearchActionInheritedProperties,
        SearchActionAllProperties,
    ] = SearchActionAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "SearchAction"
    return model


SearchAction = create_schema_org_model()


def create_searchaction_model(
    model: Union[
        SearchActionProperties,
        SearchActionInheritedProperties,
        SearchActionAllProperties,
    ...
    _type = deepcopy(SearchActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of SearchAction. Please see: https://schema.org/SearchAction"
            )
    # delete_keys = []
...
def schema_json(model: SearchActionAllProperties):
    pydantic_type = create_searchaction_model(model=model)
