...
class BusReservationInheritedProperties(TypedDict):
    """A reservation for bus travel. Note: This type is for information about actual reservations, e.g. in confirmation emails or HTML pages with individual confirmations of reservations. For offers of tickets, use [[Offer]].
...
class BusReservationProperties(TypedDict):
    """A reservation for bus travel. Note: This type is for information about actual reservations, e.g. in confirmation emails or HTML pages with individual confirmations of reservations. For offers of tickets, use [[Offer]].
...
class BusReservationAllProperties(
    BusReservationInheritedProperties, BusReservationProperties, TypedDict
):
    pass


class BusReservationBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="BusReservation", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        BusReservationProperties,
        BusReservationInheritedProperties,
        BusReservationAllProperties,
    ] = BusReservationAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "BusReservation"
    return model


BusReservation = create_schema_org_model()


def create_busreservation_model(
    model: Union[
        BusReservationProperties,
        BusReservationInheritedProperties,
        BusReservationAllProperties,
    ...
    _type = deepcopy(BusReservationAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of BusReservation. Please see: https://schema.org/BusReservation"
            )
    # delete_keys = []
...
def schema_json(model: BusReservationAllProperties):
    pydantic_type = create_busreservation_model(model=model)
