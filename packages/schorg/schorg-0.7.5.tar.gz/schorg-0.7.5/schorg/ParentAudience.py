...
class ParentAudienceProperties(TypedDict):
    ...
    Note:
        Model Depth 5
    Attributes:
        childMinAge: (Optional[Union[List[Union[str, SchemaOrgObj, StrictInt, StrictFloat]], str, SchemaOrgObj, StrictInt, StrictFloat]]): Minimal age of the child.
        childMaxAge: (Optional[Union[List[Union[str, SchemaOrgObj, StrictInt, StrictFloat]], str, SchemaOrgObj, StrictInt, StrictFloat]]): Maximal age of the child.
    """
...
def create_parentaudience_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of ParentAudience. Please see: https://schema.org/ParentAudience"
            )
    # delete_keys = []
