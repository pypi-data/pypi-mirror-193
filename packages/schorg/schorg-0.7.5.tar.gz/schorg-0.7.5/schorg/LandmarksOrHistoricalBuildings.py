...
class LandmarksOrHistoricalBuildingsInheritedProperties(TypedDict):
    ...
    Attributes:
        ...
        longitude: (Optional[Union[List[Union[str, SchemaOrgObj, StrictInt, StrictFloat]], str, SchemaOrgObj, StrictInt, StrictFloat]]): The longitude of a location. For example ```-122.08585``` ([WGS 84](https://en.wikipedia.org/wiki/World_Geodetic_System)).
        smokingAllowed: (Optional[Union[List[Union[StrictBool, SchemaOrgObj, str]], StrictBool, SchemaOrgObj, str]]): Indicates whether it is allowed to smoke in the place, e.g. in the restaurant, hotel or hotel room.
        ...
        latitude: (Optional[Union[List[Union[str, SchemaOrgObj, StrictInt, StrictFloat]], str, SchemaOrgObj, StrictInt, StrictFloat]]): The latitude of a location. For example ```37.42242``` ([WGS 84](https://en.wikipedia.org/wiki/World_Geodetic_System)).
        ...
        publicAccess: (Optional[Union[List[Union[StrictBool, SchemaOrgObj, str]], StrictBool, SchemaOrgObj, str]]): A flag to signal that the [[Place]] is open to public visitors.  If this property is omitted there is no assumed default boolean value
        ...
        hasDriveThroughService: (Optional[Union[List[Union[StrictBool, SchemaOrgObj, str]], StrictBool, SchemaOrgObj, str]]): Indicates whether some facility (e.g. [[FoodEstablishment]], [[CovidTestingFacility]]) offers a service that can be used by driving through in a car. In the case of [[CovidTestingFacility]] such facilities could potentially help with social distancing from other potentially-infected users.
...
class LandmarksOrHistoricalBuildingsAllProperties(
...
def create_landmarksorhistoricalbuildings_model(
    model: Union[
        LandmarksOrHistoricalBuildingsProperties,
        LandmarksOrHistoricalBuildingsInheritedProperties,
        LandmarksOrHistoricalBuildingsAllProperties,
    ...
    _type = deepcopy(LandmarksOrHistoricalBuildingsAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of LandmarksOrHistoricalBuildings. Please see: https://schema.org/LandmarksOrHistoricalBuildings"
            )
    # delete_keys = []
...
def schema_json(model: LandmarksOrHistoricalBuildingsAllProperties):
