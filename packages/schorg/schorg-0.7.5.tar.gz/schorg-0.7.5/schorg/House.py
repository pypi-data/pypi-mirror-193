...
class HouseAllProperties(HouseInheritedProperties, HouseProperties, TypedDict):
    pass
...
def create_schema_org_model(
    type_: Union[
        HouseProperties, HouseInheritedProperties, HouseAllProperties
    ] = HouseAllProperties
) -> Type[SchemaOrgBase]:
...
def create_house_model(
    model: Union[HouseProperties, HouseInheritedProperties, HouseAllProperties]
):
    _type = deepcopy(HouseAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of House. Please see: https://schema.org/House"
            )
    # delete_keys = []
...
def schema_json(model: HouseAllProperties):
