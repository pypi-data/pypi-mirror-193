...
class VesselAllProperties(VesselInheritedProperties, VesselProperties, TypedDict):
    pass


class VesselBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Vessel", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        VesselProperties, VesselInheritedProperties, VesselAllProperties
    ] = VesselAllProperties
) -> Type[SchemaOrgBase]:
...
def create_vessel_model(
    model: Union[VesselProperties, VesselInheritedProperties, VesselAllProperties]
):
    _type = deepcopy(VesselAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Vessel. Please see: https://schema.org/Vessel"
            )
    # delete_keys = []
...
def schema_json(model: VesselAllProperties):
    pydantic_type = create_vessel_model(model=model)
