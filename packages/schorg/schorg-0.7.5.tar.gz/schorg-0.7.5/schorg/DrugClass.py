...
class DrugClassInheritedProperties(TypedDict):
    """A class of medical drugs, e.g., statins. Classes can represent general pharmacological class, common mechanisms of action, common physiological effects, etc.
...
class DrugClassProperties(TypedDict):
    """A class of medical drugs, e.g., statins. Classes can represent general pharmacological class, common mechanisms of action, common physiological effects, etc.
...
class DrugClassAllProperties(
    DrugClassInheritedProperties, DrugClassProperties, TypedDict
):
    pass


class DrugClassBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="DrugClass", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        DrugClassProperties, DrugClassInheritedProperties, DrugClassAllProperties
    ] = DrugClassAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "DrugClass"
    return model


DrugClass = create_schema_org_model()


def create_drugclass_model(
    model: Union[
        DrugClassProperties, DrugClassInheritedProperties, DrugClassAllProperties
    ]
):
    _type = deepcopy(DrugClassAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of DrugClass. Please see: https://schema.org/DrugClass"
            )
    # delete_keys = []
...
def schema_json(model: DrugClassAllProperties):
    pydantic_type = create_drugclass_model(model=model)
