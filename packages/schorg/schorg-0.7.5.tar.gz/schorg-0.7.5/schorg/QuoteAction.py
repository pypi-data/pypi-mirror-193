...
class QuoteActionInheritedProperties(TypedDict):
    """An agent quotes/estimates/appraises an object/product/service with a price at a location/store.
...
class QuoteActionProperties(TypedDict):
    """An agent quotes/estimates/appraises an object/product/service with a price at a location/store.
...
class QuoteActionAllProperties(
    QuoteActionInheritedProperties, QuoteActionProperties, TypedDict
):
    pass


class QuoteActionBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="QuoteAction", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        QuoteActionProperties, QuoteActionInheritedProperties, QuoteActionAllProperties
    ] = QuoteActionAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "QuoteAction"
    return model


QuoteAction = create_schema_org_model()


def create_quoteaction_model(
    model: Union[
        QuoteActionProperties, QuoteActionInheritedProperties, QuoteActionAllProperties
    ]
):
    _type = deepcopy(QuoteActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of QuoteAction. Please see: https://schema.org/QuoteAction"
            )
    # delete_keys = []
...
def schema_json(model: QuoteActionAllProperties):
    pydantic_type = create_quoteaction_model(model=model)
