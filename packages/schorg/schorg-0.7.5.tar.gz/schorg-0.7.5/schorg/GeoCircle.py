...
class GeoCircleProperties(TypedDict):
    ...
    Attributes:
        ...
        geoRadius: (Optional[Union[List[Union[str, SchemaOrgObj, StrictInt, StrictFloat]], str, SchemaOrgObj, StrictInt, StrictFloat]]): Indicates the approximate radius of a GeoCircle (metres unless indicated otherwise via Distance notation).
    """
...
def create_geocircle_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of GeoCircle. Please see: https://schema.org/GeoCircle"
            )
    # delete_keys = []
