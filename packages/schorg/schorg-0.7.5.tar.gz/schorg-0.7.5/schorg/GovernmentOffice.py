...
class GovernmentOfficeInheritedProperties(TypedDict):
    """A government office&#x2014;for example, an IRS or DMV office.
...
class GovernmentOfficeProperties(TypedDict):
    """A government office&#x2014;for example, an IRS or DMV office.
...
class GovernmentOfficeAllProperties(
    GovernmentOfficeInheritedProperties, GovernmentOfficeProperties, TypedDict
):
    pass


class GovernmentOfficeBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="GovernmentOffice", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        GovernmentOfficeProperties,
        GovernmentOfficeInheritedProperties,
        GovernmentOfficeAllProperties,
    ] = GovernmentOfficeAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "GovernmentOffice"
    return model


GovernmentOffice = create_schema_org_model()


def create_governmentoffice_model(
    model: Union[
        GovernmentOfficeProperties,
        GovernmentOfficeInheritedProperties,
        GovernmentOfficeAllProperties,
    ...
    _type = deepcopy(GovernmentOfficeAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of GovernmentOffice. Please see: https://schema.org/GovernmentOffice"
            )
    # delete_keys = []
...
def schema_json(model: GovernmentOfficeAllProperties):
    pydantic_type = create_governmentoffice_model(model=model)
