...
class WineryAllProperties(WineryInheritedProperties, WineryProperties, TypedDict):
    pass


class WineryBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Winery", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        WineryProperties, WineryInheritedProperties, WineryAllProperties
    ] = WineryAllProperties
) -> Type[SchemaOrgBase]:
...
def create_winery_model(
    model: Union[WineryProperties, WineryInheritedProperties, WineryAllProperties]
):
    _type = deepcopy(WineryAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Winery. Please see: https://schema.org/Winery"
            )
    # delete_keys = []
...
def schema_json(model: WineryAllProperties):
    pydantic_type = create_winery_model(model=model)
