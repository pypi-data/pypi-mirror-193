...
class CollectionProperties(TypedDict):
    ...
    Note:
        Model Depth 3
    Attributes:
        collectionSize: (Optional[Union[List[Union[str, SchemaOrgObj, int]], str, SchemaOrgObj, int]]): The number of items in the [[Collection]].
    """
...
def create_collection_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Collection. Please see: https://schema.org/Collection"
            )
    # delete_keys = []
