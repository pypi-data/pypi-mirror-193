...
class OrganizeActionInheritedProperties(TypedDict):
    """The act of manipulating/administering/supervising/controlling one or more objects.
...
class OrganizeActionProperties(TypedDict):
    """The act of manipulating/administering/supervising/controlling one or more objects.
...
class OrganizeActionAllProperties(
    OrganizeActionInheritedProperties, OrganizeActionProperties, TypedDict
):
    pass


class OrganizeActionBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="OrganizeAction", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ...
        OrganizeActionInheritedProperties,
        OrganizeActionAllProperties,
    ] = OrganizeActionAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "OrganizeAction"
    return model


OrganizeAction = create_schema_org_model()


def create_organizeaction_model(
    model: Union[
        OrganizeActionProperties,
        OrganizeActionInheritedProperties,
        OrganizeActionAllProperties,
    ...
    _type = deepcopy(OrganizeActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of OrganizeAction. Please see: https://schema.org/OrganizeAction"
            )
    # delete_keys = []
...
def schema_json(model: OrganizeActionAllProperties):
    pydantic_type = create_organizeaction_model(model=model)
