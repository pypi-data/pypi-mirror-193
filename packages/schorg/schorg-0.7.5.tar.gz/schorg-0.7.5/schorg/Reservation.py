...
class ReservationInheritedProperties(TypedDict):
    """Describes a reservation for travel, dining or an event. Some reservations require tickets. Note: This type is for information about actual reservations, e.g. in confirmation emails or HTML pages with individual confirmations of reservations. For offers of tickets, restaurant reservations, flights, or rental cars, use [[Offer]].
...
class ReservationProperties(TypedDict):
    """Describes a reservation for travel, dining or an event. Some reservations require tickets. Note: This type is for information about actual reservations, e.g. in confirmation emails or HTML pages with individual confirmations of reservations. For offers of tickets, restaurant reservations, flights, or rental cars, use [[Offer]].
...
class ReservationAllProperties(
    ReservationInheritedProperties, ReservationProperties, TypedDict
):
    pass


class ReservationBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Reservation", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ReservationProperties, ReservationInheritedProperties, ReservationAllProperties
    ] = ReservationAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Reservation"
    return model


Reservation = create_schema_org_model()


def create_reservation_model(
    model: Union[
        ReservationProperties, ReservationInheritedProperties, ReservationAllProperties
    ]
):
    _type = deepcopy(ReservationAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Reservation. Please see: https://schema.org/Reservation"
            )
    # delete_keys = []
...
def schema_json(model: ReservationAllProperties):
    pydantic_type = create_reservation_model(model=model)
