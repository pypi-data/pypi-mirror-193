...
class RecyclingCenterInheritedProperties(TypedDict):
    """A recycling center.
...
class RecyclingCenterProperties(TypedDict):
    """A recycling center.
...
class RecyclingCenterAllProperties(
    RecyclingCenterInheritedProperties, RecyclingCenterProperties, TypedDict
):
    pass


class RecyclingCenterBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="RecyclingCenter", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        RecyclingCenterProperties,
        RecyclingCenterInheritedProperties,
        RecyclingCenterAllProperties,
    ] = RecyclingCenterAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "RecyclingCenter"
    return model


RecyclingCenter = create_schema_org_model()


def create_recyclingcenter_model(
    model: Union[
        RecyclingCenterProperties,
        RecyclingCenterInheritedProperties,
        RecyclingCenterAllProperties,
    ...
    _type = deepcopy(RecyclingCenterAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of RecyclingCenter. Please see: https://schema.org/RecyclingCenter"
            )
    # delete_keys = []
...
def schema_json(model: RecyclingCenterAllProperties):
    pydantic_type = create_recyclingcenter_model(model=model)
