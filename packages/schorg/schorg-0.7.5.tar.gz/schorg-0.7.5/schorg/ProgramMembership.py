...
class ProgramMembershipProperties(TypedDict):
    ...
    Attributes:
        ...
        membershipPointsEarned: (Optional[Union[List[Union[str, SchemaOrgObj, StrictInt, StrictFloat]], str, SchemaOrgObj, StrictInt, StrictFloat]]): The number of membership points earned by the member. If necessary, the unitText can be used to express the units the points are issued in. (E.g. stars, miles, etc.)
...
def create_programmembership_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of ProgramMembership. Please see: https://schema.org/ProgramMembership"
            )
    # delete_keys = []
