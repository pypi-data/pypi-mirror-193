...
class WPSideBarAllProperties(
    WPSideBarInheritedProperties, WPSideBarProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        WPSideBarProperties, WPSideBarInheritedProperties, WPSideBarAllProperties
    ] = WPSideBarAllProperties
) -> Type[SchemaOrgBase]:
...
def create_wpsidebar_model(
    model: Union[
        WPSideBarProperties, WPSideBarInheritedProperties, WPSideBarAllProperties
    ]
):
    _type = deepcopy(WPSideBarAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of WPSideBar. Please see: https://schema.org/WPSideBar"
            )
    # delete_keys = []
...
def schema_json(model: WPSideBarAllProperties):
