...
class MotelInheritedProperties(TypedDict):
    """A motel.<br /><br />See also the <a href="/docs/hotels.html">dedicated document on the use of schema.org for marking up hotels and other forms of accommodations</a>.
...
class MotelProperties(TypedDict):
    """A motel.<br /><br />See also the <a href="/docs/hotels.html">dedicated document on the use of schema.org for marking up hotels and other forms of accommodations</a>.
...
class MotelAllProperties(MotelInheritedProperties, MotelProperties, TypedDict):
    pass


class MotelBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Motel", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        MotelProperties, MotelInheritedProperties, MotelAllProperties
    ] = MotelAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Motel"
    return model


Motel = create_schema_org_model()


def create_motel_model(
    model: Union[MotelProperties, MotelInheritedProperties, MotelAllProperties]
):
    _type = deepcopy(MotelAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Motel. Please see: https://schema.org/Motel"
            )
    # delete_keys = []
...
def schema_json(model: MotelAllProperties):
    pydantic_type = create_motel_model(model=model)
