...
class TouristTripAllProperties(
    TouristTripInheritedProperties, TouristTripProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        TouristTripProperties, TouristTripInheritedProperties, TouristTripAllProperties
    ] = TouristTripAllProperties
) -> Type[SchemaOrgBase]:
...
def create_touristtrip_model(
    model: Union[
        TouristTripProperties, TouristTripInheritedProperties, TouristTripAllProperties
    ]
):
    _type = deepcopy(TouristTripAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of TouristTrip. Please see: https://schema.org/TouristTrip"
            )
    # delete_keys = []
...
def schema_json(model: TouristTripAllProperties):
