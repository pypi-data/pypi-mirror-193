...
class UserReviewAllProperties(
    UserReviewInheritedProperties, UserReviewProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        UserReviewProperties, UserReviewInheritedProperties, UserReviewAllProperties
    ] = UserReviewAllProperties
) -> Type[SchemaOrgBase]:
...
def create_userreview_model(
    model: Union[
        UserReviewProperties, UserReviewInheritedProperties, UserReviewAllProperties
    ]
):
    _type = deepcopy(UserReviewAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of UserReview. Please see: https://schema.org/UserReview"
            )
    # delete_keys = []
...
def schema_json(model: UserReviewAllProperties):
