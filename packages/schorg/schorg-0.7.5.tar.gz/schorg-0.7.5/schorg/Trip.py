...
class TripAllProperties(TripInheritedProperties, TripProperties, TypedDict):
    pass
...
def create_schema_org_model(
    type_: Union[
        TripProperties, TripInheritedProperties, TripAllProperties
    ] = TripAllProperties
) -> Type[SchemaOrgBase]:
...
def create_trip_model(
    model: Union[TripProperties, TripInheritedProperties, TripAllProperties]
):
    _type = deepcopy(TripAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Trip. Please see: https://schema.org/Trip"
            )
    # delete_keys = []
...
def schema_json(model: TripAllProperties):
