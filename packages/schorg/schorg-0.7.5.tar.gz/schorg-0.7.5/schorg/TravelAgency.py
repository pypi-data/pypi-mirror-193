...
class TravelAgencyInheritedProperties(TypedDict):
    """A travel agency.
...
class TravelAgencyProperties(TypedDict):
    """A travel agency.
...
class TravelAgencyAllProperties(
    TravelAgencyInheritedProperties, TravelAgencyProperties, TypedDict
):
    pass


class TravelAgencyBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="TravelAgency", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        TravelAgencyProperties,
        TravelAgencyInheritedProperties,
        TravelAgencyAllProperties,
    ] = TravelAgencyAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "TravelAgency"
    return model


TravelAgency = create_schema_org_model()


def create_travelagency_model(
    model: Union[
        TravelAgencyProperties,
        TravelAgencyInheritedProperties,
        TravelAgencyAllProperties,
    ...
    _type = deepcopy(TravelAgencyAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of TravelAgency. Please see: https://schema.org/TravelAgency"
            )
    # delete_keys = []
...
def schema_json(model: TravelAgencyAllProperties):
    pydantic_type = create_travelagency_model(model=model)
