...
class AchieveActionInheritedProperties(TypedDict):
    """The act of accomplishing something via previous efforts. It is an instantaneous action rather than an ongoing process.
...
class AchieveActionProperties(TypedDict):
    """The act of accomplishing something via previous efforts. It is an instantaneous action rather than an ongoing process.
...
class AchieveActionAllProperties(
    AchieveActionInheritedProperties, AchieveActionProperties, TypedDict
):
    pass


class AchieveActionBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="AchieveAction", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        AchieveActionProperties,
        AchieveActionInheritedProperties,
        AchieveActionAllProperties,
    ] = AchieveActionAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "AchieveAction"
    return model


AchieveAction = create_schema_org_model()


def create_achieveaction_model(
    model: Union[
        AchieveActionProperties,
        AchieveActionInheritedProperties,
        AchieveActionAllProperties,
    ...
    _type = deepcopy(AchieveActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of AchieveAction. Please see: https://schema.org/AchieveAction"
            )
    # delete_keys = []
...
def schema_json(model: AchieveActionAllProperties):
    pydantic_type = create_achieveaction_model(model=model)
