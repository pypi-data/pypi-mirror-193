...
class ClassAllProperties(ClassInheritedProperties, ClassProperties, TypedDict):
    pass
...
def create_schema_org_model(
    type_: Union[
        ClassProperties, ClassInheritedProperties, ClassAllProperties
    ] = ClassAllProperties
) -> Type[SchemaOrgBase]:
...
def create_class_model(
    model: Union[ClassProperties, ClassInheritedProperties, ClassAllProperties]
):
    _type = deepcopy(ClassAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Class. Please see: https://schema.org/Class"
            )
    # delete_keys = []
...
def schema_json(model: ClassAllProperties):
