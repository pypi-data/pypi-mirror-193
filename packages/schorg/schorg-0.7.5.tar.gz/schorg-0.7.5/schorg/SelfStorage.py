...
class SelfStorageInheritedProperties(TypedDict):
    """A self-storage facility.
...
class SelfStorageProperties(TypedDict):
    """A self-storage facility.
...
class SelfStorageAllProperties(
    SelfStorageInheritedProperties, SelfStorageProperties, TypedDict
):
    pass


class SelfStorageBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="SelfStorage", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        SelfStorageProperties, SelfStorageInheritedProperties, SelfStorageAllProperties
    ] = SelfStorageAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "SelfStorage"
    return model


SelfStorage = create_schema_org_model()


def create_selfstorage_model(
    model: Union[
        SelfStorageProperties, SelfStorageInheritedProperties, SelfStorageAllProperties
    ]
):
    _type = deepcopy(SelfStorageAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of SelfStorage. Please see: https://schema.org/SelfStorage"
            )
    # delete_keys = []
...
def schema_json(model: SelfStorageAllProperties):
    pydantic_type = create_selfstorage_model(model=model)
