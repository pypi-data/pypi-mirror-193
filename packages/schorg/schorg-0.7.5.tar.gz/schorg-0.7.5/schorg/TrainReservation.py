...
class TrainReservationInheritedProperties(TypedDict):
    """A reservation for train travel.Note: This type is for information about actual reservations, e.g. in confirmation emails or HTML pages with individual confirmations of reservations. For offers of tickets, use [[Offer]].
...
class TrainReservationProperties(TypedDict):
    """A reservation for train travel.Note: This type is for information about actual reservations, e.g. in confirmation emails or HTML pages with individual confirmations of reservations. For offers of tickets, use [[Offer]].
...
class TrainReservationAllProperties(
    TrainReservationInheritedProperties, TrainReservationProperties, TypedDict
):
    pass


class TrainReservationBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="TrainReservation", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        TrainReservationProperties,
        TrainReservationInheritedProperties,
        TrainReservationAllProperties,
    ] = TrainReservationAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "TrainReservation"
    return model


TrainReservation = create_schema_org_model()


def create_trainreservation_model(
    model: Union[
        TrainReservationProperties,
        TrainReservationInheritedProperties,
        TrainReservationAllProperties,
    ...
    _type = deepcopy(TrainReservationAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of TrainReservation. Please see: https://schema.org/TrainReservation"
            )
    # delete_keys = []
...
def schema_json(model: TrainReservationAllProperties):
    pydantic_type = create_trainreservation_model(model=model)
