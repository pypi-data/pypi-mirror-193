...
class SteeringPositionValueInheritedProperties(TypedDict):
    """A value indicating a steering position.
...
class SteeringPositionValueProperties(TypedDict):
    """A value indicating a steering position.
...
class SteeringPositionValueAllProperties(
    SteeringPositionValueInheritedProperties, SteeringPositionValueProperties, TypedDict
):
    pass


class SteeringPositionValueBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="SteeringPositionValue", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        SteeringPositionValueProperties,
        SteeringPositionValueInheritedProperties,
        SteeringPositionValueAllProperties,
    ] = SteeringPositionValueAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "SteeringPositionValue"
    return model


SteeringPositionValue = create_schema_org_model()


def create_steeringpositionvalue_model(
    model: Union[
        SteeringPositionValueProperties,
        SteeringPositionValueInheritedProperties,
        SteeringPositionValueAllProperties,
    ...
    _type = deepcopy(SteeringPositionValueAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of SteeringPositionValue. Please see: https://schema.org/SteeringPositionValue"
            )
    # delete_keys = []
...
def schema_json(model: SteeringPositionValueAllProperties):
    pydantic_type = create_steeringpositionvalue_model(model=model)
