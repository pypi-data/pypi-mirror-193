...
class ControlActionInheritedProperties(TypedDict):
    """An agent controls a device or application.
...
class ControlActionProperties(TypedDict):
    """An agent controls a device or application.
...
class ControlActionAllProperties(
    ControlActionInheritedProperties, ControlActionProperties, TypedDict
):
    pass


class ControlActionBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="ControlAction", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ControlActionProperties,
        ControlActionInheritedProperties,
        ControlActionAllProperties,
    ] = ControlActionAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "ControlAction"
    return model


ControlAction = create_schema_org_model()


def create_controlaction_model(
    model: Union[
        ControlActionProperties,
        ControlActionInheritedProperties,
        ControlActionAllProperties,
    ...
    _type = deepcopy(ControlActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of ControlAction. Please see: https://schema.org/ControlAction"
            )
    # delete_keys = []
...
def schema_json(model: ControlActionAllProperties):
    pydantic_type = create_controlaction_model(model=model)
