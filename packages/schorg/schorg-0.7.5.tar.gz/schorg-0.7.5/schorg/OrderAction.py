...
class OrderActionAllProperties(
    OrderActionInheritedProperties, OrderActionProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        OrderActionProperties, OrderActionInheritedProperties, OrderActionAllProperties
    ] = OrderActionAllProperties
) -> Type[SchemaOrgBase]:
...
def create_orderaction_model(
    model: Union[
        OrderActionProperties, OrderActionInheritedProperties, OrderActionAllProperties
    ]
):
    _type = deepcopy(OrderActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of OrderAction. Please see: https://schema.org/OrderAction"
            )
    # delete_keys = []
...
def schema_json(model: OrderActionAllProperties):
