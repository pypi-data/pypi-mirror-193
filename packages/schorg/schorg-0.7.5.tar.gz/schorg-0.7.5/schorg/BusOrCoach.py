...
class BusOrCoachAllProperties(
    BusOrCoachInheritedProperties, BusOrCoachProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        BusOrCoachProperties, BusOrCoachInheritedProperties, BusOrCoachAllProperties
    ] = BusOrCoachAllProperties
) -> Type[SchemaOrgBase]:
...
def create_busorcoach_model(
    model: Union[
        BusOrCoachProperties, BusOrCoachInheritedProperties, BusOrCoachAllProperties
    ]
):
    _type = deepcopy(BusOrCoachAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of BusOrCoach. Please see: https://schema.org/BusOrCoach"
            )
    # delete_keys = []
...
def schema_json(model: BusOrCoachAllProperties):
