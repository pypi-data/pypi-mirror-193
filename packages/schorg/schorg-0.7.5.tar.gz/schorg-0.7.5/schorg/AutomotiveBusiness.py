...
class AutomotiveBusinessInheritedProperties(TypedDict):
    """Car repair, sales, or parts.
...
class AutomotiveBusinessProperties(TypedDict):
    """Car repair, sales, or parts.
...
class AutomotiveBusinessAllProperties(
    AutomotiveBusinessInheritedProperties, AutomotiveBusinessProperties, TypedDict
):
    pass


class AutomotiveBusinessBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="AutomotiveBusiness", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        AutomotiveBusinessProperties,
        AutomotiveBusinessInheritedProperties,
        AutomotiveBusinessAllProperties,
    ] = AutomotiveBusinessAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "AutomotiveBusiness"
    return model


AutomotiveBusiness = create_schema_org_model()


def create_automotivebusiness_model(
    model: Union[
        AutomotiveBusinessProperties,
        AutomotiveBusinessInheritedProperties,
        AutomotiveBusinessAllProperties,
    ...
    _type = deepcopy(AutomotiveBusinessAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of AutomotiveBusiness. Please see: https://schema.org/AutomotiveBusiness"
            )
    # delete_keys = []
...
def schema_json(model: AutomotiveBusinessAllProperties):
    pydantic_type = create_automotivebusiness_model(model=model)
