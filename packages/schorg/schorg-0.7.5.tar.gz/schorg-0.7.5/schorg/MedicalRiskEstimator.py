...
class MedicalRiskEstimatorAllProperties(
    MedicalRiskEstimatorInheritedProperties, MedicalRiskEstimatorProperties, TypedDict
):
    pass


class MedicalRiskEstimatorBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="MedicalRiskEstimator", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        MedicalRiskEstimatorProperties,
        MedicalRiskEstimatorInheritedProperties,
        MedicalRiskEstimatorAllProperties,
    ] = MedicalRiskEstimatorAllProperties
) -> Type[SchemaOrgBase]:
...
def create_medicalriskestimator_model(
    model: Union[
        MedicalRiskEstimatorProperties,
        MedicalRiskEstimatorInheritedProperties,
        MedicalRiskEstimatorAllProperties,
    ...
    _type = deepcopy(MedicalRiskEstimatorAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of MedicalRiskEstimator. Please see: https://schema.org/MedicalRiskEstimator"
            )
    # delete_keys = []
...
def schema_json(model: MedicalRiskEstimatorAllProperties):
    pydantic_type = create_medicalriskestimator_model(model=model)
