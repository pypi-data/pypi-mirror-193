...
class DeliveryEventProperties(TypedDict):
    ...
    Note:
        Model Depth 3
    Attributes:
        availableThrough: (Optional[Union[List[Union[str, SchemaOrgObj, datetime]], str, SchemaOrgObj, datetime]]): After this date, the item will no longer be available for pickup.
        ...
        availableFrom: (Optional[Union[List[Union[str, SchemaOrgObj, datetime]], str, SchemaOrgObj, datetime]]): When the item is available for pickup from the store, locker, etc.
    """
...
def create_deliveryevent_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of DeliveryEvent. Please see: https://schema.org/DeliveryEvent"
            )
    # delete_keys = []
