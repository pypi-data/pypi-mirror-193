...
class TaxonProperties(TypedDict):
    ...
    Note:
        Model Depth 2
    Attributes:
        parentTaxon: (Optional[Union[List[Union[AnyUrl, str, SchemaOrgObj]], AnyUrl, str, SchemaOrgObj]]): Closest parent taxon of the taxon in question.
        taxonRank: (Optional[Union[List[Union[AnyUrl, str, SchemaOrgObj]], AnyUrl, str, SchemaOrgObj]]): The taxonomic rank of this taxon given preferably as a URI from a controlled vocabulary – typically the ranks from TDWG TaxonRank ontology or equivalent Wikidata URIs.
        childTaxon: (Optional[Union[List[Union[AnyUrl, str, SchemaOrgObj]], AnyUrl, str, SchemaOrgObj]]): Closest child taxa of the taxon in question.
        hasDefinedTerm: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): A Defined Term contained in this term set.
    """

    parentTaxon: NotRequired[
        Union[List[Union[AnyUrl, str, SchemaOrgObj]], AnyUrl, str, SchemaOrgObj]
    ]
    taxonRank: NotRequired[
        Union[List[Union[AnyUrl, str, SchemaOrgObj]], AnyUrl, str, SchemaOrgObj]
    ]
    childTaxon: NotRequired[
        Union[List[Union[AnyUrl, str, SchemaOrgObj]], AnyUrl, str, SchemaOrgObj]
    ]
...
class TaxonAllProperties(TaxonInheritedProperties, TaxonProperties, TypedDict):
    pass


class TaxonBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Taxon", alias="@id")
    ...
    class Config:
        ...
        fields = {"parentTaxon": {"exclude": True}}
        fields = {"taxonRank": {"exclude": True}}
        fields = {"childTaxon": {"exclude": True}}
        fields = {"hasDefinedTerm": {"exclude": True}}


def create_schema_org_model(
    type_: Union[
        TaxonProperties, TaxonInheritedProperties, TaxonAllProperties
    ] = TaxonAllProperties
) -> Type[SchemaOrgBase]:
...
def create_taxon_model(
    model: Union[TaxonProperties, TaxonInheritedProperties, TaxonAllProperties]
):
    _type = deepcopy(TaxonAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Taxon. Please see: https://schema.org/Taxon"
            )
    # delete_keys = []
...
def schema_json(model: TaxonAllProperties):
    pydantic_type = create_taxon_model(model=model)
