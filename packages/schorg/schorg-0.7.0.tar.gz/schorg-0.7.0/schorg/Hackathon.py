...
class HackathonInheritedProperties(TypedDict):
    """A [hackathon](https://en.wikipedia.org/wiki/Hackathon) event.
...
class HackathonProperties(TypedDict):
    """A [hackathon](https://en.wikipedia.org/wiki/Hackathon) event.
...
class AllProperties(HackathonInheritedProperties , HackathonProperties, TypedDict):
    pass


class HackathonBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="Hackathon",alias='@id')
...
def create_schema_org_model(type_: Union[HackathonProperties, HackathonInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Hackathon"
    return model
    

Hackathon = create_schema_org_model()


def create_hackathon_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_hackathon_model(model=model)
