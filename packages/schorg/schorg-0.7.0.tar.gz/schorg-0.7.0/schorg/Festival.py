...
class FestivalInheritedProperties(TypedDict):
    """Event type: Festival.
...
class FestivalProperties(TypedDict):
    """Event type: Festival.
...
class AllProperties(FestivalInheritedProperties , FestivalProperties, TypedDict):
    pass


class FestivalBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="Festival",alias='@id')
...
def create_schema_org_model(type_: Union[FestivalProperties, FestivalInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Festival"
    return model
    

Festival = create_schema_org_model()


def create_festival_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_festival_model(model=model)
