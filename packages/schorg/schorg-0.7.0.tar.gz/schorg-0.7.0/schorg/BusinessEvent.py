...
class BusinessEventInheritedProperties(TypedDict):
    """Event type: Business event.
...
class BusinessEventProperties(TypedDict):
    """Event type: Business event.
...
class AllProperties(BusinessEventInheritedProperties , BusinessEventProperties, TypedDict):
    pass


class BusinessEventBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="BusinessEvent",alias='@id')
...
def create_schema_org_model(type_: Union[BusinessEventProperties, BusinessEventInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "BusinessEvent"
    return model
    

BusinessEvent = create_schema_org_model()


def create_businessevent_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_businessevent_model(model=model)
