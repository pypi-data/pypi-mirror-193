...
class MusicEventInheritedProperties(TypedDict):
    """Event type: Music event.
...
class MusicEventProperties(TypedDict):
    """Event type: Music event.
...
class AllProperties(MusicEventInheritedProperties , MusicEventProperties, TypedDict):
    pass


class MusicEventBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="MusicEvent",alias='@id')
...
def create_schema_org_model(type_: Union[MusicEventProperties, MusicEventInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "MusicEvent"
    return model
    

MusicEvent = create_schema_org_model()


def create_musicevent_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_musicevent_model(model=model)
