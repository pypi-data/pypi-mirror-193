...
class LandformInheritedProperties(TypedDict):
    """A landform or physical feature.  Landform elements include mountains, plains, lakes, rivers, seascape and oceanic waterbody interface features such as bays, peninsulas, seas and so forth, including sub-aqueous terrain features such as submersed mountain ranges, volcanoes, and the great ocean basins.
...
class LandformProperties(TypedDict):
    """A landform or physical feature.  Landform elements include mountains, plains, lakes, rivers, seascape and oceanic waterbody interface features such as bays, peninsulas, seas and so forth, including sub-aqueous terrain features such as submersed mountain ranges, volcanoes, and the great ocean basins.
...
class AllProperties(LandformInheritedProperties , LandformProperties, TypedDict):
    pass


class LandformBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="Landform",alias='@id')
...
def create_schema_org_model(type_: Union[LandformProperties, LandformInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Landform"
    return model
    

Landform = create_schema_org_model()


def create_landform_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_landform_model(model=model)
