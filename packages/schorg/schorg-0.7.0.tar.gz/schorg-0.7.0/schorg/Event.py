...
class EventInheritedProperties(TypedDict):
    """An event happening at a certain time and location, such as a concert, lecture, or festival. Ticketing information may be added via the [[offers]] property. Repeated events may be structured as separate Event objects.
...
class EventProperties(TypedDict):
    """An event happening at a certain time and location, such as a concert, lecture, or festival. Ticketing information may be added via the [[offers]] property. Repeated events may be structured as separate Event objects.
...
class AllProperties(EventInheritedProperties , EventProperties, TypedDict):
    pass


class EventBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="Event",alias='@id')
...
def create_schema_org_model(type_: Union[EventProperties, EventInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Event"
    return model
    

Event = create_schema_org_model()


def create_event_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_event_model(model=model)
