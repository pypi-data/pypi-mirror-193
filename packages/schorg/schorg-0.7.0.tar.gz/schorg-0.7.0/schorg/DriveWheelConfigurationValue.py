...
class DriveWheelConfigurationValueInheritedProperties(TypedDict):
    """A value indicating which roadwheels will receive torque.
...
class DriveWheelConfigurationValueProperties(TypedDict):
    """A value indicating which roadwheels will receive torque.
...
class AllProperties(DriveWheelConfigurationValueInheritedProperties , DriveWheelConfigurationValueProperties, TypedDict):
    pass


class DriveWheelConfigurationValueBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="DriveWheelConfigurationValue",alias='@id')
...
def create_schema_org_model(type_: Union[DriveWheelConfigurationValueProperties, DriveWheelConfigurationValueInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "DriveWheelConfigurationValue"
    return model
    

DriveWheelConfigurationValue = create_schema_org_model()


def create_drivewheelconfigurationvalue_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_drivewheelconfigurationvalue_model(model=model)
