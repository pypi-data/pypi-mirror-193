...
class EducationEventInheritedProperties(TypedDict):
    """Event type: Education event.
...
class EducationEventProperties(TypedDict):
    """Event type: Education event.
...
class AllProperties(EducationEventInheritedProperties , EducationEventProperties, TypedDict):
    pass


class EducationEventBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="EducationEvent",alias='@id')
...
def create_schema_org_model(type_: Union[EducationEventProperties, EducationEventInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "EducationEvent"
    return model
    

EducationEvent = create_schema_org_model()


def create_educationevent_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_educationevent_model(model=model)
