...
class TaxiServiceInheritedProperties(TypedDict):
    """A service for a vehicle for hire with a driver for local travel. Fares are usually calculated based on distance traveled.
...
class TaxiServiceProperties(TypedDict):
    """A service for a vehicle for hire with a driver for local travel. Fares are usually calculated based on distance traveled.
...
class AllProperties(TaxiServiceInheritedProperties , TaxiServiceProperties, TypedDict):
    pass


class TaxiServiceBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="TaxiService",alias='@id')
...
def create_schema_org_model(type_: Union[TaxiServiceProperties, TaxiServiceInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "TaxiService"
    return model
    

TaxiService = create_schema_org_model()


def create_taxiservice_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_taxiservice_model(model=model)
