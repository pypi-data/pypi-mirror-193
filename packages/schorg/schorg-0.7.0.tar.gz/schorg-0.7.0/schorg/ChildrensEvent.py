...
class ChildrensEventInheritedProperties(TypedDict):
    """Event type: Children's event.
...
class ChildrensEventProperties(TypedDict):
    """Event type: Children's event.
...
class AllProperties(ChildrensEventInheritedProperties , ChildrensEventProperties, TypedDict):
    pass


class ChildrensEventBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="ChildrensEvent",alias='@id')
...
def create_schema_org_model(type_: Union[ChildrensEventProperties, ChildrensEventInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "ChildrensEvent"
    return model
    

ChildrensEvent = create_schema_org_model()


def create_childrensevent_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_childrensevent_model(model=model)
