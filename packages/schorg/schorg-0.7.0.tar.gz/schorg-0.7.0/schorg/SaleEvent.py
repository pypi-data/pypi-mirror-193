...
class SaleEventInheritedProperties(TypedDict):
    """Event type: Sales event.
...
class SaleEventProperties(TypedDict):
    """Event type: Sales event.
...
class AllProperties(SaleEventInheritedProperties , SaleEventProperties, TypedDict):
    pass


class SaleEventBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="SaleEvent",alias='@id')
...
def create_schema_org_model(type_: Union[SaleEventProperties, SaleEventInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "SaleEvent"
    return model
    

SaleEvent = create_schema_org_model()


def create_saleevent_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_saleevent_model(model=model)
