...
class PerformingGroupInheritedProperties(TypedDict):
    """A performance group, such as a band, an orchestra, or a circus.
...
class PerformingGroupProperties(TypedDict):
    """A performance group, such as a band, an orchestra, or a circus.
...
class PerformingGroupAllProperties(
    PerformingGroupInheritedProperties, PerformingGroupProperties, TypedDict
):
    pass


class PerformingGroupBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="PerformingGroup", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ...
        PerformingGroupInheritedProperties,
        PerformingGroupAllProperties,
    ] = PerformingGroupAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "PerformingGroup"
    return model


PerformingGroup = create_schema_org_model()


def create_performinggroup_model(
    model: Union[
        PerformingGroupProperties,
        PerformingGroupInheritedProperties,
        PerformingGroupAllProperties,
    ...
    _type = deepcopy(PerformingGroupAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of PerformingGroupAllProperties")
    delete_keys = []
...
def schema_json(model: PerformingGroupAllProperties):
    pydantic_type = create_performinggroup_model(model=model)
