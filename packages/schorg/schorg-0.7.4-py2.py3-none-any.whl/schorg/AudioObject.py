...
class AudioObjectAllProperties(
    AudioObjectInheritedProperties, AudioObjectProperties, TypedDict
):
    pass


class AudioObjectBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="AudioObject", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        AudioObjectProperties, AudioObjectInheritedProperties, AudioObjectAllProperties
    ] = AudioObjectAllProperties
) -> Type[SchemaOrgBase]:
...
def create_audioobject_model(
    model: Union[
        AudioObjectProperties, AudioObjectInheritedProperties, AudioObjectAllProperties
    ]
):
    _type = deepcopy(AudioObjectAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of AudioObjectAllProperties")
    delete_keys = []
...
def schema_json(model: AudioObjectAllProperties):
    pydantic_type = create_audioobject_model(model=model)
