...
class _3DModelInheritedProperties(TypedDict):
    """A 3D model represents some kind of 3D content, which may have [[encoding]]s in one or more [[MediaObject]]s. Many 3D formats are available (e.g. see [Wikipedia](https://en.wikipedia.org/wiki/Category:3D_graphics_file_formats)); specific encoding formats can be represented using the [[encodingFormat]] property applied to the relevant [[MediaObject]]. For thecase of a single file published after Zip compression, the convention of appending '+zip' to the [[encodingFormat]] can be used. Geospatial, AR/VR, artistic/animation, gaming, engineering and scientific content can all be represented using [[3DModel]].
...
class _3DModelProperties(TypedDict):
    """A 3D model represents some kind of 3D content, which may have [[encoding]]s in one or more [[MediaObject]]s. Many 3D formats are available (e.g. see [Wikipedia](https://en.wikipedia.org/wiki/Category:3D_graphics_file_formats)); specific encoding formats can be represented using the [[encodingFormat]] property applied to the relevant [[MediaObject]]. For thecase of a single file published after Zip compression, the convention of appending '+zip' to the [[encodingFormat]] can be used. Geospatial, AR/VR, artistic/animation, gaming, engineering and scientific content can all be represented using [[3DModel]].
    ...
    Note:
        Model Depth 4
    Attributes:
        isResizable: (Optional[Union[List[Union[SchemaOrgObj, str, StrictBool]], SchemaOrgObj, str, StrictBool]]): Whether the 3DModel allows resizing. For example, room layout applications often do not allow 3DModel elements to be resized to reflect reality.
    """

    isResizable: NotRequired[
        Union[List[Union[SchemaOrgObj, str, StrictBool]], SchemaOrgObj, str, StrictBool]
    ]


class _3DModelAllProperties(_3DModelInheritedProperties, _3DModelProperties, TypedDict):
    pass


class _3DModelBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="_3DModel", alias="@id")
    ...
    class Config:
        ...
        fields = {"isResizable": {"exclude": True}}


def create_schema_org_model(
    type_: Union[
        _3DModelProperties, _3DModelInheritedProperties, _3DModelAllProperties
    ] = _3DModelAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "_3DModel"
    return model


_3DModel = create_schema_org_model()


def create__3dmodel_model(
    model: Union[_3DModelProperties, _3DModelInheritedProperties, _3DModelAllProperties]
):
    _type = deepcopy(_3DModelAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of _3DModelAllProperties")
    delete_keys = []
...
def schema_json(model: _3DModelAllProperties):
    pydantic_type = create__3dmodel_model(model=model)
