...
class ImageObjectInheritedProperties(TypedDict):
    """An image file.
...
class ImageObjectProperties(TypedDict):
    """An image file.
...
class ImageObjectAllProperties(
    ImageObjectInheritedProperties, ImageObjectProperties, TypedDict
):
    pass


class ImageObjectBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="ImageObject", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ImageObjectProperties, ImageObjectInheritedProperties, ImageObjectAllProperties
    ] = ImageObjectAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "ImageObject"
    return model


ImageObject = create_schema_org_model()


def create_imageobject_model(
    model: Union[
        ImageObjectProperties, ImageObjectInheritedProperties, ImageObjectAllProperties
    ]
):
    _type = deepcopy(ImageObjectAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of ImageObjectAllProperties")
    delete_keys = []
...
def schema_json(model: ImageObjectAllProperties):
    pydantic_type = create_imageobject_model(model=model)
