...
class GovernmentPermitInheritedProperties(TypedDict):
    ...
    Attributes:
        ...
        validUntil: (Optional[Union[List[Union[date, SchemaOrgObj, str]], date, SchemaOrgObj, str]]): The date when the item is no longer valid.
...
def create_governmentpermit_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of GovernmentPermitAllProperties")
    delete_keys = []
