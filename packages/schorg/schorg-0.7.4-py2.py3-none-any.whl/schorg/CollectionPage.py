...
class CollectionPageInheritedProperties(TypedDict):
    """Web page type: Collection page.
...
class CollectionPageProperties(TypedDict):
    """Web page type: Collection page.
...
class CollectionPageAllProperties(
    CollectionPageInheritedProperties, CollectionPageProperties, TypedDict
):
    pass


class CollectionPageBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="CollectionPage", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ...
        CollectionPageInheritedProperties,
        CollectionPageAllProperties,
    ] = CollectionPageAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "CollectionPage"
    return model


CollectionPage = create_schema_org_model()


def create_collectionpage_model(
    model: Union[
        CollectionPageProperties,
        CollectionPageInheritedProperties,
        CollectionPageAllProperties,
    ...
    _type = deepcopy(CollectionPageAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of CollectionPageAllProperties")
    delete_keys = []
...
def schema_json(model: CollectionPageAllProperties):
    pydantic_type = create_collectionpage_model(model=model)
