...
class MotorizedBicycleInheritedProperties(TypedDict):
    """A motorized bicycle is a bicycle with an attached motor used to power the vehicle, or to assist with pedaling.
...
class MotorizedBicycleProperties(TypedDict):
    """A motorized bicycle is a bicycle with an attached motor used to power the vehicle, or to assist with pedaling.
...
class AllProperties(MotorizedBicycleInheritedProperties , MotorizedBicycleProperties, TypedDict):
    pass


class MotorizedBicycleBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="MotorizedBicycle",alias='@id')
...
def create_schema_org_model(type_: Union[MotorizedBicycleProperties, MotorizedBicycleInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "MotorizedBicycle"
    return model
    

MotorizedBicycle = create_schema_org_model()


def create_motorizedbicycle_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_motorizedbicycle_model(model=model)
