...
class ProductModelInheritedProperties(TypedDict):
    """A datasheet or vendor specification of a product (in the sense of a prototypical description).
...
class ProductModelProperties(TypedDict):
    """A datasheet or vendor specification of a product (in the sense of a prototypical description).
    ...
    Attributes:
        isVariantOf: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): Indicates the kind of product that this is a variant of. In the case of [[ProductModel]], this is a pointer (from a ProductModel) to a base product from which this product is a variant. It is safe to infer that the variant inherits all product features from the base model, unless defined locally. This is not transitive. In the case of a [[ProductGroup]], the group description also serves as a template, representing a set of Products that vary on explicitly defined, specific dimensions only (so it defines both a set of variants, as well as which values distinguish amongst those variants). When used with [[ProductGroup]], this property can apply to any [[Product]] included in the group.
        successorOf: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): A pointer from a newer variant of a product  to its previous, often discontinued predecessor.
        predecessorOf: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): A pointer from a previous, often discontinued variant of the product to its newer variant.
    """

    isVariantOf: NotRequired[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]
    successorOf: NotRequired[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]
    predecessorOf: NotRequired[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]
    


class AllProperties(ProductModelInheritedProperties , ProductModelProperties, TypedDict):
    pass


class ProductModelBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="ProductModel",alias='@id')
    ...
    class Config:
        ...
        fields = {'isVariantOf': {'exclude': True}}
        fields = {'successorOf': {'exclude': True}}
        fields = {'predecessorOf': {'exclude': True}}
        


def create_schema_org_model(type_: Union[ProductModelProperties, ProductModelInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "ProductModel"
    return model
    

ProductModel = create_schema_org_model()


def create_productmodel_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_productmodel_model(model=model)
