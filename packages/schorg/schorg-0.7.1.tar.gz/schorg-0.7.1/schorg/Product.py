...
class ProductInheritedProperties(TypedDict):
    """Any offered product or service. For example: a pair of shoes; a concert ticket; the rental of a car; a haircut; or an episode of a TV show streamed online.
...
class AllProperties(ProductInheritedProperties , ProductProperties, TypedDict):
    pass


class ProductBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="Product",alias='@id')
...
def create_schema_org_model(type_: Union[ProductProperties, ProductInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Product"
    return model
    

Product = create_schema_org_model()


def create_product_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_product_model(model=model)
