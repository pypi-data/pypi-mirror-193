...
class TouristInformationCenterInheritedProperties(TypedDict):
    """A tourist information center.
...
class TouristInformationCenterProperties(TypedDict):
    """A tourist information center.
...
class AllProperties(TouristInformationCenterInheritedProperties , TouristInformationCenterProperties, TypedDict):
    pass


class TouristInformationCenterBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="TouristInformationCenter",alias='@id')
...
def create_schema_org_model(type_: Union[TouristInformationCenterProperties, TouristInformationCenterInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "TouristInformationCenter"
    return model
    

TouristInformationCenter = create_schema_org_model()


def create_touristinformationcenter_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_touristinformationcenter_model(model=model)
