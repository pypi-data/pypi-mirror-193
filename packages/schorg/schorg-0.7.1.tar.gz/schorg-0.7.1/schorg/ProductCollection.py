...
class ProductCollectionInheritedProperties(TypedDict):
    """A set of products (either [[ProductGroup]]s or specific variants) that are listed together e.g. in an [[Offer]].
...
class ProductCollectionProperties(TypedDict):
    """A set of products (either [[ProductGroup]]s or specific variants) that are listed together e.g. in an [[Offer]].
...
class AllProperties(ProductCollectionInheritedProperties , ProductCollectionProperties, TypedDict):
    pass


class ProductCollectionBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="ProductCollection",alias='@id')
...
def create_schema_org_model(type_: Union[ProductCollectionProperties, ProductCollectionInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "ProductCollection"
    return model
    

ProductCollection = create_schema_org_model()


def create_productcollection_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_productcollection_model(model=model)
