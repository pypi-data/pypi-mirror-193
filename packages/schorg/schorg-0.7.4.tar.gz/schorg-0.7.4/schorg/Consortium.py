...
class ConsortiumInheritedProperties(TypedDict):
    """A Consortium is a membership [[Organization]] whose members are typically Organizations.
...
class ConsortiumProperties(TypedDict):
    """A Consortium is a membership [[Organization]] whose members are typically Organizations.
...
class ConsortiumAllProperties(
    ConsortiumInheritedProperties, ConsortiumProperties, TypedDict
):
    pass


class ConsortiumBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Consortium", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ConsortiumProperties, ConsortiumInheritedProperties, ConsortiumAllProperties
    ] = ConsortiumAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Consortium"
    return model


Consortium = create_schema_org_model()


def create_consortium_model(
    model: Union[
        ConsortiumProperties, ConsortiumInheritedProperties, ConsortiumAllProperties
    ]
):
    _type = deepcopy(ConsortiumAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of ConsortiumAllProperties")
    delete_keys = []
...
def schema_json(model: ConsortiumAllProperties):
    pydantic_type = create_consortium_model(model=model)
