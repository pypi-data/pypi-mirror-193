...
class TaxiAllProperties(TaxiInheritedProperties, TaxiProperties, TypedDict):
    pass
...
def create_schema_org_model(
    type_: Union[
        TaxiProperties, TaxiInheritedProperties, TaxiAllProperties
    ] = TaxiAllProperties
) -> Type[SchemaOrgBase]:
...
def create_taxi_model(
    model: Union[TaxiProperties, TaxiInheritedProperties, TaxiAllProperties]
):
    _type = deepcopy(TaxiAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of TaxiAllProperties")
    delete_keys = []
...
def schema_json(model: TaxiAllProperties):
