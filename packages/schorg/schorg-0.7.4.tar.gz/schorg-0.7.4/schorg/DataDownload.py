...
class DataDownloadInheritedProperties(TypedDict):
    """All or part of a [[Dataset]] in downloadable form.
...
class DataDownloadProperties(TypedDict):
    """All or part of a [[Dataset]] in downloadable form.
...
class DataDownloadAllProperties(
    DataDownloadInheritedProperties, DataDownloadProperties, TypedDict
):
    pass


class DataDownloadBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="DataDownload", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        DataDownloadProperties,
        DataDownloadInheritedProperties,
        DataDownloadAllProperties,
    ] = DataDownloadAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "DataDownload"
    return model


DataDownload = create_schema_org_model()


def create_datadownload_model(
    model: Union[
        DataDownloadProperties,
        DataDownloadInheritedProperties,
        DataDownloadAllProperties,
    ...
    _type = deepcopy(DataDownloadAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of DataDownloadAllProperties")
    delete_keys = []
...
def schema_json(model: DataDownloadAllProperties):
    pydantic_type = create_datadownload_model(model=model)
