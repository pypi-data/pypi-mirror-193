...
class AirlineInheritedProperties(TypedDict):
    """An organization that provides flights for passengers.
...
class AirlineProperties(TypedDict):
    """An organization that provides flights for passengers.
    ...
    Attributes:
        ...
        iataCode: (Optional[Union[List[Union[SchemaOrgObj, str]], SchemaOrgObj, str]]): IATA identifier for an airline or airport.
    """

    boardingPolicy: NotRequired[
        Union[List[Union[SchemaOrgObj, str]], SchemaOrgObj, str]
    ]
    iataCode: NotRequired[Union[List[Union[SchemaOrgObj, str]], SchemaOrgObj, str]]


class AirlineAllProperties(AirlineInheritedProperties, AirlineProperties, TypedDict):
    pass


class AirlineBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Airline", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        AirlineProperties, AirlineInheritedProperties, AirlineAllProperties
    ] = AirlineAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Airline"
    return model


Airline = create_schema_org_model()


def create_airline_model(
    model: Union[AirlineProperties, AirlineInheritedProperties, AirlineAllProperties]
):
    _type = deepcopy(AirlineAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of AirlineAllProperties")
    delete_keys = []
...
def schema_json(model: AirlineAllProperties):
    pydantic_type = create_airline_model(model=model)
