...
class OnlineBusinessInheritedProperties(TypedDict):
    """A particular online business, either standalone or the online part of a broader organization. Examples include an eCommerce site, an online travel booking site, an online learning site, an online logistics and shipping provider, an online (virtual) doctor, etc.
...
class OnlineBusinessProperties(TypedDict):
    """A particular online business, either standalone or the online part of a broader organization. Examples include an eCommerce site, an online travel booking site, an online learning site, an online logistics and shipping provider, an online (virtual) doctor, etc.
...
class OnlineBusinessAllProperties(
    OnlineBusinessInheritedProperties, OnlineBusinessProperties, TypedDict
):
    pass


class OnlineBusinessBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="OnlineBusiness", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        OnlineBusinessProperties,
        OnlineBusinessInheritedProperties,
        OnlineBusinessAllProperties,
    ] = OnlineBusinessAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "OnlineBusiness"
    return model


OnlineBusiness = create_schema_org_model()


def create_onlinebusiness_model(
    model: Union[
        OnlineBusinessProperties,
        OnlineBusinessInheritedProperties,
        OnlineBusinessAllProperties,
    ...
    _type = deepcopy(OnlineBusinessAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of OnlineBusinessAllProperties")
    delete_keys = []
...
def schema_json(model: OnlineBusinessAllProperties):
    pydantic_type = create_onlinebusiness_model(model=model)
