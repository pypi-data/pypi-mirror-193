...
class CorporationInheritedProperties(TypedDict):
    """Organization: A business corporation.
...
class CorporationProperties(TypedDict):
    """Organization: A business corporation.
    ...
    Note:
        Model Depth 3
    Attributes:
        tickerSymbol: (Optional[Union[List[Union[SchemaOrgObj, str]], SchemaOrgObj, str]]): The exchange traded instrument associated with a Corporation object. The tickerSymbol is expressed as an exchange and an instrument name separated by a space character. For the exchange component of the tickerSymbol attribute, we recommend using the controlled vocabulary of Market Identifier Codes (MIC) specified in ISO 15022.
    """

    tickerSymbol: NotRequired[Union[List[Union[SchemaOrgObj, str]], SchemaOrgObj, str]]


class CorporationAllProperties(
    CorporationInheritedProperties, CorporationProperties, TypedDict
):
    pass


class CorporationBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Corporation", alias="@id")
    ...
    class Config:
        ...
        fields = {"tickerSymbol": {"exclude": True}}


def create_schema_org_model(
    type_: Union[
        CorporationProperties, CorporationInheritedProperties, CorporationAllProperties
    ] = CorporationAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Corporation"
    return model


Corporation = create_schema_org_model()


def create_corporation_model(
    model: Union[
        CorporationProperties, CorporationInheritedProperties, CorporationAllProperties
    ]
):
    _type = deepcopy(CorporationAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of CorporationAllProperties")
    delete_keys = []
...
def schema_json(model: CorporationAllProperties):
    pydantic_type = create_corporation_model(model=model)
