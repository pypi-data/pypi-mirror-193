...
class CategoryCodeInheritedProperties(TypedDict):
    ...
    Note:
        Model Depth 4
    Attributes:
        inDefinedTermSet: (Optional[Union[List[Union[AnyUrl, SchemaOrgObj, str]], AnyUrl, SchemaOrgObj, str]]): A [[DefinedTermSet]] that contains this term.
...
def create_categorycode_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of CategoryCodeAllProperties")
    delete_keys = []
