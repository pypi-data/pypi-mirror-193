...
class OrderItemProperties(TypedDict):
    ...
    Attributes:
        ...
        orderQuantity: (Optional[Union[List[Union[StrictInt, StrictFloat, SchemaOrgObj, str]], StrictInt, StrictFloat, SchemaOrgObj, str]]): The number of the item ordered. If the property is not set, assume the quantity is one.
...
def create_orderitem_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of OrderItemAllProperties")
    delete_keys = []
