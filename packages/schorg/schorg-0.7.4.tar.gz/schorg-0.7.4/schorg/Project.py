...
class ProjectInheritedProperties(TypedDict):
    """An enterprise (potentially individual but typically collaborative), planned to achieve a particular aim.Use properties from [[Organization]], [[subOrganization]]/[[parentOrganization]] to indicate project sub-structures.
...
class ProjectAllProperties(ProjectInheritedProperties, ProjectProperties, TypedDict):
    pass


class ProjectBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Project", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ProjectProperties, ProjectInheritedProperties, ProjectAllProperties
...
Project = create_schema_org_model()


def create_project_model(
    model: Union[ProjectProperties, ProjectInheritedProperties, ProjectAllProperties]
):
    _type = deepcopy(ProjectAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of ProjectAllProperties")
    delete_keys = []
...
def schema_json(model: ProjectAllProperties):
    pydantic_type = create_project_model(model=model)
