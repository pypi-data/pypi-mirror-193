...
class WebApplicationProperties(TypedDict):
    ...
    Note:
        Model Depth 4
    Attributes:
        browserRequirements: (Optional[Union[List[Union[SchemaOrgObj, str]], SchemaOrgObj, str]]): Specifies browser requirements in human-readable text. For example, 'requires HTML5 support'.
    """
...
def create_webapplication_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of WebApplicationAllProperties")
    delete_keys = []
