...
class MovieTheaterProperties(TypedDict):
    ...
    Note:
        Model Depth 4
    Attributes:
        screenCount: (Optional[Union[List[Union[StrictInt, StrictFloat, SchemaOrgObj, str]], StrictInt, StrictFloat, SchemaOrgObj, str]]): The number of screens in the movie theater.
    """
...
def create_movietheater_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of MovieTheaterAllProperties")
    delete_keys = []
