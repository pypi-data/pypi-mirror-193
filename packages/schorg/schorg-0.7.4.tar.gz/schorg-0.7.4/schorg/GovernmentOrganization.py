...
class GovernmentOrganizationInheritedProperties(TypedDict):
    """A governmental organization or agency.
...
class GovernmentOrganizationProperties(TypedDict):
    """A governmental organization or agency.
...
class GovernmentOrganizationAllProperties(
    GovernmentOrganizationInheritedProperties,
    GovernmentOrganizationProperties,
    TypedDict,
):
    pass


class GovernmentOrganizationBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="GovernmentOrganization", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        GovernmentOrganizationProperties,
        GovernmentOrganizationInheritedProperties,
        GovernmentOrganizationAllProperties,
    ] = GovernmentOrganizationAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "GovernmentOrganization"
    return model


GovernmentOrganization = create_schema_org_model()


def create_governmentorganization_model(
    model: Union[
        GovernmentOrganizationProperties,
        GovernmentOrganizationInheritedProperties,
        GovernmentOrganizationAllProperties,
    ...
    _type = deepcopy(GovernmentOrganizationAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of GovernmentOrganizationAllProperties")
    delete_keys = []
...
def schema_json(model: GovernmentOrganizationAllProperties):
    pydantic_type = create_governmentorganization_model(model=model)
