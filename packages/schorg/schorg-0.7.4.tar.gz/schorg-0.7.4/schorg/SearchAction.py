...
class SearchActionProperties(TypedDict):
    ...
    Note:
        Model Depth 3
    Attributes:
        query: (Optional[Union[List[Union[SchemaOrgObj, str]], SchemaOrgObj, str]]): A sub property of instrument. The query used on this action.
    """

    query: NotRequired[Union[List[Union[SchemaOrgObj, str]], SchemaOrgObj, str]]
...
def create_searchaction_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of SearchActionAllProperties")
    delete_keys = []
