...
class SportsOrganizationInheritedProperties(TypedDict):
    """Represents the collection of all sports organizations, including sports teams, governing bodies, and sports associations.
...
class SportsOrganizationProperties(TypedDict):
    """Represents the collection of all sports organizations, including sports teams, governing bodies, and sports associations.
...
class SportsOrganizationAllProperties(
    SportsOrganizationInheritedProperties, SportsOrganizationProperties, TypedDict
):
    pass


class SportsOrganizationBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="SportsOrganization", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        SportsOrganizationProperties,
        SportsOrganizationInheritedProperties,
        SportsOrganizationAllProperties,
    ] = SportsOrganizationAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "SportsOrganization"
    return model


SportsOrganization = create_schema_org_model()


def create_sportsorganization_model(
    model: Union[
        SportsOrganizationProperties,
        SportsOrganizationInheritedProperties,
        SportsOrganizationAllProperties,
    ...
    _type = deepcopy(SportsOrganizationAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of SportsOrganizationAllProperties")
    delete_keys = []
...
def schema_json(model: SportsOrganizationAllProperties):
    pydantic_type = create_sportsorganization_model(model=model)
