...
class AboutPageInheritedProperties(TypedDict):
    """Web page type: About page.
...
class AboutPageProperties(TypedDict):
    """Web page type: About page.
...
class AboutPageAllProperties(
    AboutPageInheritedProperties, AboutPageProperties, TypedDict
):
    pass


class AboutPageBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="AboutPage", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        AboutPageProperties, AboutPageInheritedProperties, AboutPageAllProperties
    ] = AboutPageAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "AboutPage"
    return model


AboutPage = create_schema_org_model()


def create_aboutpage_model(
    model: Union[
        AboutPageProperties, AboutPageInheritedProperties, AboutPageAllProperties
    ]
):
    _type = deepcopy(AboutPageAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of AboutPageAllProperties")
    delete_keys = []
...
def schema_json(model: AboutPageAllProperties):
    pydantic_type = create_aboutpage_model(model=model)
