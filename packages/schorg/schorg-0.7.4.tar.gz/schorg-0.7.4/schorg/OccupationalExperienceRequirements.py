...
class OccupationalExperienceRequirementsProperties(TypedDict):
    ...
    Note:
        Model Depth 3
    Attributes:
        monthsOfExperience: (Optional[Union[List[Union[StrictInt, StrictFloat, SchemaOrgObj, str]], StrictInt, StrictFloat, SchemaOrgObj, str]]): Indicates the minimal number of months of experience required for a position.
    """
...
def create_occupationalexperiencerequirements_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of OccupationalExperienceRequirementsAllProperties"
            )
    delete_keys = []
