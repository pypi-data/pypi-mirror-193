...
class FundingSchemeInheritedProperties(TypedDict):
    """A FundingScheme combines organizational, project and policy aspects of grant-based funding    that sets guidelines, principles and mechanisms to support other kinds of projects and activities.    Funding is typically organized via [[Grant]] funding. Examples of funding schemes: Swiss Priority Programmes (SPPs); EU Framework 7 (FP7); Horizon 2020; the NIH-R01 Grant Program; Wellcome institutional strategic support fund. For large scale public sector funding, the management and administration of grant awards is often handled by other, dedicated, organizations - [[FundingAgency]]s such as ERC, REA, ...
...
class FundingSchemeProperties(TypedDict):
    """A FundingScheme combines organizational, project and policy aspects of grant-based funding    that sets guidelines, principles and mechanisms to support other kinds of projects and activities.    Funding is typically organized via [[Grant]] funding. Examples of funding schemes: Swiss Priority Programmes (SPPs); EU Framework 7 (FP7); Horizon 2020; the NIH-R01 Grant Program; Wellcome institutional strategic support fund. For large scale public sector funding, the management and administration of grant awards is often handled by other, dedicated, organizations - [[FundingAgency]]s such as ERC, REA, ...
...
class FundingSchemeAllProperties(
    FundingSchemeInheritedProperties, FundingSchemeProperties, TypedDict
):
    pass


class FundingSchemeBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="FundingScheme", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        FundingSchemeProperties,
        FundingSchemeInheritedProperties,
        FundingSchemeAllProperties,
    ] = FundingSchemeAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "FundingScheme"
    return model


FundingScheme = create_schema_org_model()


def create_fundingscheme_model(
    model: Union[
        FundingSchemeProperties,
        FundingSchemeInheritedProperties,
        FundingSchemeAllProperties,
    ...
    _type = deepcopy(FundingSchemeAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of FundingSchemeAllProperties")
    delete_keys = []
...
def schema_json(model: FundingSchemeAllProperties):
    pydantic_type = create_fundingscheme_model(model=model)
