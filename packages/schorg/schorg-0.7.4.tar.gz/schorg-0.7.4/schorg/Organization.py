...
class OrganizationInheritedProperties(TypedDict):
    """An organization such as a school, NGO, corporation, club, etc.
...
class OrganizationProperties(TypedDict):
    """An organization such as a school, NGO, corporation, club, etc.
...
class OrganizationAllProperties(
    OrganizationInheritedProperties, OrganizationProperties, TypedDict
):
    pass


class OrganizationBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Organization", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ...
        OrganizationInheritedProperties,
        OrganizationAllProperties,
    ] = OrganizationAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Organization"
    return model


Organization = create_schema_org_model()
...
def create_organization_model(
    ...
    _type = deepcopy(OrganizationAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of OrganizationAllProperties")
    delete_keys = []
...
def schema_json(model: OrganizationAllProperties):
    pydantic_type = create_organization_model(model=model)
