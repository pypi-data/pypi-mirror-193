...
class ProfilePageInheritedProperties(TypedDict):
    """Web page type: Profile page.
...
class ProfilePageProperties(TypedDict):
    """Web page type: Profile page.
...
class ProfilePageAllProperties(
    ProfilePageInheritedProperties, ProfilePageProperties, TypedDict
):
    pass


class ProfilePageBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="ProfilePage", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ProfilePageProperties, ProfilePageInheritedProperties, ProfilePageAllProperties
    ] = ProfilePageAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "ProfilePage"
    return model


ProfilePage = create_schema_org_model()


def create_profilepage_model(
    model: Union[
        ProfilePageProperties, ProfilePageInheritedProperties, ProfilePageAllProperties
    ]
):
    _type = deepcopy(ProfilePageAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of ProfilePageAllProperties")
    delete_keys = []
...
def schema_json(model: ProfilePageAllProperties):
    pydantic_type = create_profilepage_model(model=model)
