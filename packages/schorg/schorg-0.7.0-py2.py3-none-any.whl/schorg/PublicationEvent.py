...
class PublicationEventInheritedProperties(TypedDict):
    """A PublicationEvent corresponds indifferently to the event of publication for a CreativeWork of any type, e.g. a broadcast event, an on-demand event, a book/journal publication via a variety of delivery media.
...
class PublicationEventProperties(TypedDict):
    """A PublicationEvent corresponds indifferently to the event of publication for a CreativeWork of any type, e.g. a broadcast event, an on-demand event, a book/journal publication via a variety of delivery media.
...
class AllProperties(PublicationEventInheritedProperties , PublicationEventProperties, TypedDict):
    pass


class PublicationEventBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="PublicationEvent",alias='@id')
...
def create_schema_org_model(type_: Union[PublicationEventProperties, PublicationEventInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "PublicationEvent"
    return model
    

PublicationEvent = create_schema_org_model()


def create_publicationevent_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_publicationevent_model(model=model)
