...
class PlaceInheritedProperties(TypedDict):
    """Entities that have a somewhat fixed, physical extension.
...
class PlaceProperties(TypedDict):
    """Entities that have a somewhat fixed, physical extension.
...
class AllProperties(PlaceInheritedProperties , PlaceProperties, TypedDict):
    pass


class PlaceBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="Place",alias='@id')
...
def create_schema_org_model(type_: Union[PlaceProperties, PlaceInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Place"
    return model
    

Place = create_schema_org_model()


def create_place_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_place_model(model=model)
