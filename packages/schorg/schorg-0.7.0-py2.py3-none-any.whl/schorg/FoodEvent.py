...
class FoodEventInheritedProperties(TypedDict):
    """Event type: Food event.
...
class FoodEventProperties(TypedDict):
    """Event type: Food event.
...
class AllProperties(FoodEventInheritedProperties , FoodEventProperties, TypedDict):
    pass


class FoodEventBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="FoodEvent",alias='@id')
...
def create_schema_org_model(type_: Union[FoodEventProperties, FoodEventInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "FoodEvent"
    return model
    

FoodEvent = create_schema_org_model()


def create_foodevent_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_foodevent_model(model=model)
