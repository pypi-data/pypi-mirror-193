...
class UserInteractionInheritedProperties(TypedDict):
    """UserInteraction and its subtypes is an old way of talking about users interacting with pages. It is generally better to use [[Action]]-based vocabulary, alongside types such as [[Comment]].
...
class UserInteractionProperties(TypedDict):
    """UserInteraction and its subtypes is an old way of talking about users interacting with pages. It is generally better to use [[Action]]-based vocabulary, alongside types such as [[Comment]].
...
class AllProperties(UserInteractionInheritedProperties , UserInteractionProperties, TypedDict):
    pass


class UserInteractionBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="UserInteraction",alias='@id')
...
def create_schema_org_model(type_: Union[UserInteractionProperties, UserInteractionInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "UserInteraction"
    return model
    

UserInteraction = create_schema_org_model()


def create_userinteraction_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_userinteraction_model(model=model)
