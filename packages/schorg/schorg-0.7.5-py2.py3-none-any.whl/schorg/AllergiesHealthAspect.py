"""
Content about the allergy-related aspects of a health topic.

https://schema.org/AllergiesHealthAspect
"""

from datetime import *
from copy import deepcopy
from typing import *
from time import *

from typing_extensions import TypedDict, NotRequired
from pydantic import *


from schorg.schema_org_obj import SchemaOrgObj, SchemaOrgBase


class AllergiesHealthAspectInheritedProperties(TypedDict):
    """Content about the allergy-related aspects of a health topic.

    References:
        https://schema.org/AllergiesHealthAspect
    Note:
        Model Depth 5
    Attributes:
    """


class AllergiesHealthAspectProperties(TypedDict):
    """Content about the allergy-related aspects of a health topic.

    References:
        https://schema.org/AllergiesHealthAspect
    Note:
        Model Depth 5
    Attributes:
    """


class AllergiesHealthAspectAllProperties(
    AllergiesHealthAspectInheritedProperties, AllergiesHealthAspectProperties, TypedDict
):
    pass


class AllergiesHealthAspectBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="AllergiesHealthAspect", alias="@id")
    context_: Optional[Any] = Field(default=None, alias="@context")
    graph_: Optional[Any] = Field(default=None, alias="@graph")

    class Config:
        ...


def create_schema_org_model(
    type_: Union[
        AllergiesHealthAspectProperties,
        AllergiesHealthAspectInheritedProperties,
        AllergiesHealthAspectAllProperties,
    ] = AllergiesHealthAspectAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "AllergiesHealthAspect"
    return model


AllergiesHealthAspect = create_schema_org_model()


def create_allergieshealthaspect_model(
    model: Union[
        AllergiesHealthAspectProperties,
        AllergiesHealthAspectInheritedProperties,
        AllergiesHealthAspectAllProperties,
    ]
):
    _type = deepcopy(AllergiesHealthAspectAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of AllergiesHealthAspect. Please see: https://schema.org/AllergiesHealthAspect"
            )
    # delete_keys = []
    # for k in _type.__annotations__.keys():
    #     if k not in model.__annotations__:
    #         delete_keys.append(k)
    # for k in delete_keys:
    #     del _type.__annotations__[k]
    return create_schema_org_model(type_=model)


def schema_json(model: AllergiesHealthAspectAllProperties):
    pydantic_type = create_allergieshealthaspect_model(model=model)
    return pydantic_type(model).schema_json()
