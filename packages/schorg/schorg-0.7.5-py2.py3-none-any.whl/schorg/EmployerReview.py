...
class EmployerReviewAllProperties(
    EmployerReviewInheritedProperties, EmployerReviewProperties, TypedDict
):
    pass


class EmployerReviewBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="EmployerReview", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        EmployerReviewProperties,
        EmployerReviewInheritedProperties,
        EmployerReviewAllProperties,
    ] = EmployerReviewAllProperties
) -> Type[SchemaOrgBase]:
...
def create_employerreview_model(
    model: Union[
        EmployerReviewProperties,
        EmployerReviewInheritedProperties,
        EmployerReviewAllProperties,
    ...
    _type = deepcopy(EmployerReviewAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of EmployerReview. Please see: https://schema.org/EmployerReview"
            )
    # delete_keys = []
...
def schema_json(model: EmployerReviewAllProperties):
    pydantic_type = create_employerreview_model(model=model)
