...
class ZooAllProperties(ZooInheritedProperties, ZooProperties, TypedDict):
    pass
...
def create_schema_org_model(
    type_: Union[
        ZooProperties, ZooInheritedProperties, ZooAllProperties
    ] = ZooAllProperties
) -> Type[SchemaOrgBase]:
...
def create_zoo_model(
    model: Union[ZooProperties, ZooInheritedProperties, ZooAllProperties]
):
    _type = deepcopy(ZooAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of Zoo. Please see: https://schema.org/Zoo")
    # delete_keys = []
...
def schema_json(model: ZooAllProperties):
