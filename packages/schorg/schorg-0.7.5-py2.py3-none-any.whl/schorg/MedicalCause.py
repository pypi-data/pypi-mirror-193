...
class MedicalCauseInheritedProperties(TypedDict):
    """The causative agent(s) that are responsible for the pathophysiologic process that eventually results in a medical condition, symptom or sign. In this schema, unless otherwise specified this is meant to be the proximate cause of the medical condition, symptom or sign. The proximate cause is defined as the causative agent that most directly results in the medical condition, symptom or sign. For example, the HIV virus could be considered a cause of AIDS. Or in a diagnostic context, if a patient fell and sustained a hip fracture and two days later sustained a pulmonary embolism which eventuated in a cardiac arrest, the cause of the cardiac arrest (the proximate cause) would be the pulmonary embolism and not the fall. Medical causes can include cardiovascular, chemical, dermatologic, endocrine, environmental, gastroenterologic, genetic, hematologic, gynecologic, iatrogenic, infectious, musculoskeletal, neurologic, nutritional, obstetric, oncologic, otolaryngologic, pharmacologic, psychiatric, pulmonary, renal, rheumatologic, toxic, traumatic, or urologic causes; medical conditions can be causes as well.
...
class MedicalCauseProperties(TypedDict):
    """The causative agent(s) that are responsible for the pathophysiologic process that eventually results in a medical condition, symptom or sign. In this schema, unless otherwise specified this is meant to be the proximate cause of the medical condition, symptom or sign. The proximate cause is defined as the causative agent that most directly results in the medical condition, symptom or sign. For example, the HIV virus could be considered a cause of AIDS. Or in a diagnostic context, if a patient fell and sustained a hip fracture and two days later sustained a pulmonary embolism which eventuated in a cardiac arrest, the cause of the cardiac arrest (the proximate cause) would be the pulmonary embolism and not the fall. Medical causes can include cardiovascular, chemical, dermatologic, endocrine, environmental, gastroenterologic, genetic, hematologic, gynecologic, iatrogenic, infectious, musculoskeletal, neurologic, nutritional, obstetric, oncologic, otolaryngologic, pharmacologic, psychiatric, pulmonary, renal, rheumatologic, toxic, traumatic, or urologic causes; medical conditions can be causes as well.
    ...
    Note:
        Model Depth 3
    Attributes:
        causeOf: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): The condition, complication, symptom, sign, etc. caused.
    """

    causeOf: NotRequired[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]


class MedicalCauseAllProperties(
    MedicalCauseInheritedProperties, MedicalCauseProperties, TypedDict
):
    pass


class MedicalCauseBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="MedicalCause", alias="@id")
    ...
    class Config:
        ...
        fields = {"causeOf": {"exclude": True}}


def create_schema_org_model(
    type_: Union[
        MedicalCauseProperties,
        MedicalCauseInheritedProperties,
        MedicalCauseAllProperties,
    ] = MedicalCauseAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "MedicalCause"
    return model


MedicalCause = create_schema_org_model()


def create_medicalcause_model(
    model: Union[
        MedicalCauseProperties,
        MedicalCauseInheritedProperties,
        MedicalCauseAllProperties,
    ...
    _type = deepcopy(MedicalCauseAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of MedicalCause. Please see: https://schema.org/MedicalCause"
            )
    # delete_keys = []
...
def schema_json(model: MedicalCauseAllProperties):
    pydantic_type = create_medicalcause_model(model=model)
