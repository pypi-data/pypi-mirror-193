...
class MedicalTestInheritedProperties(TypedDict):
    """Any medical test, typically performed for diagnostic purposes.
...
class MedicalTestProperties(TypedDict):
    """Any medical test, typically performed for diagnostic purposes.
...
class MedicalTestAllProperties(
    MedicalTestInheritedProperties, MedicalTestProperties, TypedDict
):
    pass


class MedicalTestBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="MedicalTest", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        MedicalTestProperties, MedicalTestInheritedProperties, MedicalTestAllProperties
    ] = MedicalTestAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "MedicalTest"
    return model


MedicalTest = create_schema_org_model()


def create_medicaltest_model(
    model: Union[
        MedicalTestProperties, MedicalTestInheritedProperties, MedicalTestAllProperties
    ]
):
    _type = deepcopy(MedicalTestAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of MedicalTest. Please see: https://schema.org/MedicalTest"
            )
    # delete_keys = []
...
def schema_json(model: MedicalTestAllProperties):
    pydantic_type = create_medicaltest_model(model=model)
