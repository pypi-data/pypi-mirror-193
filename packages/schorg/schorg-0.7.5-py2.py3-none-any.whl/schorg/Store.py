...
class StoreInheritedProperties(TypedDict):
    """A retail good store.
...
class StoreProperties(TypedDict):
    """A retail good store.
...
class StoreAllProperties(StoreInheritedProperties, StoreProperties, TypedDict):
    pass


class StoreBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Store", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        StoreProperties, StoreInheritedProperties, StoreAllProperties
    ] = StoreAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Store"
    return model


Store = create_schema_org_model()


def create_store_model(
    model: Union[StoreProperties, StoreInheritedProperties, StoreAllProperties]
):
    _type = deepcopy(StoreAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Store. Please see: https://schema.org/Store"
            )
    # delete_keys = []
...
def schema_json(model: StoreAllProperties):
    pydantic_type = create_store_model(model=model)
