...
class FollowActionProperties(TypedDict):
    ...
    Note:
        Model Depth 4
    Attributes:
        followee: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): A sub property of object. The person or organization being followed.
    """

    followee: NotRequired[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]
...
def create_followaction_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of FollowAction. Please see: https://schema.org/FollowAction"
            )
    # delete_keys = []
