...
class PlaceAllProperties(PlaceInheritedProperties, PlaceProperties, TypedDict):
    pass
...
def create_schema_org_model(
    type_: Union[
        PlaceProperties, PlaceInheritedProperties, PlaceAllProperties
    ] = PlaceAllProperties
) -> Type[SchemaOrgBase]:
...
def create_place_model(
    model: Union[PlaceProperties, PlaceInheritedProperties, PlaceAllProperties]
):
    _type = deepcopy(PlaceAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Place. Please see: https://schema.org/Place"
            )
    # delete_keys = []
...
def schema_json(model: PlaceAllProperties):
