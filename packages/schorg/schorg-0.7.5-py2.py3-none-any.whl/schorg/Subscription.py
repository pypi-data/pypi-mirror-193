...
class SubscriptionAllProperties(
    SubscriptionInheritedProperties, SubscriptionProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        ...
        SubscriptionInheritedProperties,
        SubscriptionAllProperties,
    ] = SubscriptionAllProperties
) -> Type[SchemaOrgBase]:
...
def create_subscription_model(
    ...
    _type = deepcopy(SubscriptionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Subscription. Please see: https://schema.org/Subscription"
            )
    # delete_keys = []
...
def schema_json(model: SubscriptionAllProperties):
