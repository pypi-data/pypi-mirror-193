...
class AggregateOfferProperties(TypedDict):
    ...
    Note:
        Model Depth 4
    Attributes:
        highPrice: (Optional[Union[List[Union[str, SchemaOrgObj, StrictInt, StrictFloat]], str, SchemaOrgObj, StrictInt, StrictFloat]]): The highest price of all offers available.Usage guidelines:* Use values from 0123456789 (Unicode 'DIGIT ZERO' (U+0030) to 'DIGIT NINE' (U+0039)) rather than superficially similar Unicode symbols.* Use '.' (Unicode 'FULL STOP' (U+002E)) rather than ',' to indicate a decimal point. Avoid using these symbols as a readability separator.
        offerCount: (Optional[Union[List[Union[str, SchemaOrgObj, int]], str, SchemaOrgObj, int]]): The number of offers for the product.
        lowPrice: (Optional[Union[List[Union[str, SchemaOrgObj, StrictInt, StrictFloat]], str, SchemaOrgObj, StrictInt, StrictFloat]]): The lowest price of all offers available.Usage guidelines:* Use values from 0123456789 (Unicode 'DIGIT ZERO' (U+0030) to 'DIGIT NINE' (U+0039)) rather than superficially similar Unicode symbols.* Use '.' (Unicode 'FULL STOP' (U+002E)) rather than ',' to indicate a decimal point. Avoid using these symbols as a readability separator.
...
class AggregateOfferAllProperties(
    AggregateOfferInheritedProperties, AggregateOfferProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        AggregateOfferProperties,
        AggregateOfferInheritedProperties,
        AggregateOfferAllProperties,
    ] = AggregateOfferAllProperties
) -> Type[SchemaOrgBase]:
...
def create_aggregateoffer_model(
    model: Union[
        AggregateOfferProperties,
        AggregateOfferInheritedProperties,
        AggregateOfferAllProperties,
    ...
    _type = deepcopy(AggregateOfferAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of AggregateOffer. Please see: https://schema.org/AggregateOffer"
            )
    # delete_keys = []
...
def schema_json(model: AggregateOfferAllProperties):
