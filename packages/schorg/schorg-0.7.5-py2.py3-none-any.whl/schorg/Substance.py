...
class SubstanceAllProperties(
    SubstanceInheritedProperties, SubstanceProperties, TypedDict
):
    pass


class SubstanceBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Substance", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        SubstanceProperties, SubstanceInheritedProperties, SubstanceAllProperties
    ] = SubstanceAllProperties
) -> Type[SchemaOrgBase]:
...
def create_substance_model(
    model: Union[
        SubstanceProperties, SubstanceInheritedProperties, SubstanceAllProperties
    ]
):
    _type = deepcopy(SubstanceAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Substance. Please see: https://schema.org/Substance"
            )
    # delete_keys = []
...
def schema_json(model: SubstanceAllProperties):
    pydantic_type = create_substance_model(model=model)
