...
class MoveActionInheritedProperties(TypedDict):
    """The act of an agent relocating to a place.Related actions:* [[TransferAction]]: Unlike TransferAction, the subject of the move is a living Person or Organization rather than an inanimate object.
...
class MoveActionProperties(TypedDict):
    """The act of an agent relocating to a place.Related actions:* [[TransferAction]]: Unlike TransferAction, the subject of the move is a living Person or Organization rather than an inanimate object.
...
class MoveActionAllProperties(
    MoveActionInheritedProperties, MoveActionProperties, TypedDict
):
    pass


class MoveActionBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="MoveAction", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        MoveActionProperties, MoveActionInheritedProperties, MoveActionAllProperties
    ] = MoveActionAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "MoveAction"
    return model


MoveAction = create_schema_org_model()


def create_moveaction_model(
    model: Union[
        MoveActionProperties, MoveActionInheritedProperties, MoveActionAllProperties
    ]
):
    _type = deepcopy(MoveActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of MoveAction. Please see: https://schema.org/MoveAction"
            )
    # delete_keys = []
...
def schema_json(model: MoveActionAllProperties):
    pydantic_type = create_moveaction_model(model=model)
