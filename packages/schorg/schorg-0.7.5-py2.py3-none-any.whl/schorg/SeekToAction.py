...
class SeekToActionInheritedProperties(TypedDict):
    """This is the [[Action]] of navigating to a specific [[startOffset]] timestamp within a [[VideoObject]], typically represented with a URL template structure.
...
class SeekToActionProperties(TypedDict):
    """This is the [[Action]] of navigating to a specific [[startOffset]] timestamp within a [[VideoObject]], typically represented with a URL template structure.
...
class SeekToActionAllProperties(
    SeekToActionInheritedProperties, SeekToActionProperties, TypedDict
):
    pass


class SeekToActionBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="SeekToAction", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        SeekToActionProperties,
        SeekToActionInheritedProperties,
        SeekToActionAllProperties,
    ] = SeekToActionAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "SeekToAction"
    return model


SeekToAction = create_schema_org_model()


def create_seektoaction_model(
    model: Union[
        SeekToActionProperties,
        SeekToActionInheritedProperties,
        SeekToActionAllProperties,
    ...
    _type = deepcopy(SeekToActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of SeekToAction. Please see: https://schema.org/SeekToAction"
            )
    # delete_keys = []
...
def schema_json(model: SeekToActionAllProperties):
    pydantic_type = create_seektoaction_model(model=model)
