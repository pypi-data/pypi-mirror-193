...
class BusStopAllProperties(BusStopInheritedProperties, BusStopProperties, TypedDict):
    pass
...
def create_schema_org_model(
    type_: Union[
        BusStopProperties, BusStopInheritedProperties, BusStopAllProperties
    ] = BusStopAllProperties
) -> Type[SchemaOrgBase]:
...
def create_busstop_model(
    model: Union[BusStopProperties, BusStopInheritedProperties, BusStopAllProperties]
):
    _type = deepcopy(BusStopAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of BusStop. Please see: https://schema.org/BusStop"
            )
    # delete_keys = []
...
def schema_json(model: BusStopAllProperties):
