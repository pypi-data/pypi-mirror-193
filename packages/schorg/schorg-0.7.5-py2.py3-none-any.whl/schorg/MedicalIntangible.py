...
class MedicalIntangibleAllProperties(
    MedicalIntangibleInheritedProperties, MedicalIntangibleProperties, TypedDict
):
    pass


class MedicalIntangibleBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="MedicalIntangible", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        MedicalIntangibleProperties,
        MedicalIntangibleInheritedProperties,
        MedicalIntangibleAllProperties,
    ] = MedicalIntangibleAllProperties
) -> Type[SchemaOrgBase]:
...
def create_medicalintangible_model(
    model: Union[
        MedicalIntangibleProperties,
        MedicalIntangibleInheritedProperties,
        MedicalIntangibleAllProperties,
    ...
    _type = deepcopy(MedicalIntangibleAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of MedicalIntangible. Please see: https://schema.org/MedicalIntangible"
            )
    # delete_keys = []
...
def schema_json(model: MedicalIntangibleAllProperties):
    pydantic_type = create_medicalintangible_model(model=model)
