...
class TouristDestinationProperties(TypedDict):
    ...
    Note:
        Model Depth 3
    Attributes:
        includesAttraction: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): Attraction located at destination.
...
def create_touristdestination_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of TouristDestination. Please see: https://schema.org/TouristDestination"
            )
    # delete_keys = []
