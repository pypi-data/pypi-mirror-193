...
class RentalCarReservationProperties(TypedDict):
    ...
    Attributes:
        ...
        dropoffTime: (Optional[Union[List[Union[str, SchemaOrgObj, datetime]], str, SchemaOrgObj, datetime]]): When a rental car can be dropped off.
    """
...
def create_rentalcarreservation_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of RentalCarReservation. Please see: https://schema.org/RentalCarReservation"
            )
    # delete_keys = []
