...
class InteractActionInheritedProperties(TypedDict):
    """The act of interacting with another person or organization.
...
class InteractActionProperties(TypedDict):
    """The act of interacting with another person or organization.
...
class InteractActionAllProperties(
    InteractActionInheritedProperties, InteractActionProperties, TypedDict
):
    pass


class InteractActionBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="InteractAction", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ...
        InteractActionInheritedProperties,
        InteractActionAllProperties,
    ] = InteractActionAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "InteractAction"
    return model


InteractAction = create_schema_org_model()


def create_interactaction_model(
    model: Union[
        InteractActionProperties,
        InteractActionInheritedProperties,
        InteractActionAllProperties,
    ...
    _type = deepcopy(InteractActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of InteractAction. Please see: https://schema.org/InteractAction"
            )
    # delete_keys = []
...
def schema_json(model: InteractActionAllProperties):
    pydantic_type = create_interactaction_model(model=model)
