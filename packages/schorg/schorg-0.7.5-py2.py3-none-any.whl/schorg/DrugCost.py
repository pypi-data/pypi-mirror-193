...
class DrugCostProperties(TypedDict):
    ...
    Note:
        Model Depth 3
    Attributes:
        costPerUnit: (Optional[Union[List[Union[str, SchemaOrgObj, StrictInt, StrictFloat]], str, SchemaOrgObj, StrictInt, StrictFloat]]): The cost per unit of the drug.
...
def create_drugcost_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of DrugCost. Please see: https://schema.org/DrugCost"
            )
    # delete_keys = []
