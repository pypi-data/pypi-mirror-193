...
class CreateActionAllProperties(
    CreateActionInheritedProperties, CreateActionProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        ...
        CreateActionInheritedProperties,
        CreateActionAllProperties,
    ] = CreateActionAllProperties
) -> Type[SchemaOrgBase]:
...
def create_createaction_model(
    ...
    _type = deepcopy(CreateActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of CreateAction. Please see: https://schema.org/CreateAction"
            )
    # delete_keys = []
...
def schema_json(model: CreateActionAllProperties):
