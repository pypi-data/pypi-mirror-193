...
class CarAllProperties(CarInheritedProperties, CarProperties, TypedDict):
    pass
...
def create_schema_org_model(
    type_: Union[
        CarProperties, CarInheritedProperties, CarAllProperties
    ] = CarAllProperties
) -> Type[SchemaOrgBase]:
...
def create_car_model(
    model: Union[CarProperties, CarInheritedProperties, CarAllProperties]
):
    _type = deepcopy(CarAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(f"{k} not part of Car. Please see: https://schema.org/Car")
    # delete_keys = []
...
def schema_json(model: CarAllProperties):
