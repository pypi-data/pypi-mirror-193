...
class CohortStudyAllProperties(
    CohortStudyInheritedProperties, CohortStudyProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        CohortStudyProperties, CohortStudyInheritedProperties, CohortStudyAllProperties
    ] = CohortStudyAllProperties
) -> Type[SchemaOrgBase]:
...
def create_cohortstudy_model(
    model: Union[
        CohortStudyProperties, CohortStudyInheritedProperties, CohortStudyAllProperties
    ]
):
    _type = deepcopy(CohortStudyAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of CohortStudy. Please see: https://schema.org/CohortStudy"
            )
    # delete_keys = []
...
def schema_json(model: CohortStudyAllProperties):
