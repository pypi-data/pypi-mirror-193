...
class CreditCardInheritedProperties(TypedDict):
    ...
    Attributes:
        ...
        cashBack: (Optional[Union[List[Union[StrictBool, SchemaOrgObj, StrictInt, StrictFloat, str]], StrictBool, SchemaOrgObj, StrictInt, StrictFloat, str]]): A cardholder benefit that pays the cardholder a small percentage of their net expenditures.
        contactlessPayment: (Optional[Union[List[Union[StrictBool, SchemaOrgObj, str]], StrictBool, SchemaOrgObj, str]]): A secure method for consumers to purchase products or services via debit, credit or smartcards by using RFID or NFC technology.
        monthlyMinimumRepaymentAmount: (Optional[Union[List[Union[str, SchemaOrgObj, StrictInt, StrictFloat]], str, SchemaOrgObj, StrictInt, StrictFloat]]): The minimum payment is the lowest amount of money that one is required to pay on a credit card statement each month.
...
def create_creditcard_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of CreditCard. Please see: https://schema.org/CreditCard"
            )
    # delete_keys = []
