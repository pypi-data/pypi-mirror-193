...
class WinActionProperties(TypedDict):
    ...
    Note:
        Model Depth 4
    Attributes:
        loser: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): A sub property of participant. The loser of the action.
    """

    loser: NotRequired[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]
...
def create_winaction_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of WinAction. Please see: https://schema.org/WinAction"
            )
    # delete_keys = []
