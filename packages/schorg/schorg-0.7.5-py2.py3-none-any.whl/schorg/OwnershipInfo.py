...
class OwnershipInfoProperties(TypedDict):
    ...
    Note:
        Model Depth 4
    Attributes:
        ownedThrough: (Optional[Union[List[Union[str, SchemaOrgObj, datetime]], str, SchemaOrgObj, datetime]]): The date and time of giving up ownership on the product.
        ownedFrom: (Optional[Union[List[Union[str, SchemaOrgObj, datetime]], str, SchemaOrgObj, datetime]]): The date and time of obtaining the product.
...
def create_ownershipinfo_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of OwnershipInfo. Please see: https://schema.org/OwnershipInfo"
            )
    # delete_keys = []
