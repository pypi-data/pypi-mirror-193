...
class LifestyleModificationAllProperties(
    LifestyleModificationInheritedProperties, LifestyleModificationProperties, TypedDict
):
    pass


class LifestyleModificationBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="LifestyleModification", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        LifestyleModificationProperties,
        LifestyleModificationInheritedProperties,
        LifestyleModificationAllProperties,
    ] = LifestyleModificationAllProperties
) -> Type[SchemaOrgBase]:
...
def create_lifestylemodification_model(
    model: Union[
        LifestyleModificationProperties,
        LifestyleModificationInheritedProperties,
        LifestyleModificationAllProperties,
    ...
    _type = deepcopy(LifestyleModificationAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of LifestyleModification. Please see: https://schema.org/LifestyleModification"
            )
    # delete_keys = []
...
def schema_json(model: LifestyleModificationAllProperties):
    pydantic_type = create_lifestylemodification_model(model=model)
