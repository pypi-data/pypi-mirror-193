...
class CommunityHealthAllProperties(
    CommunityHealthInheritedProperties, CommunityHealthProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        CommunityHealthProperties,
        CommunityHealthInheritedProperties,
        CommunityHealthAllProperties,
    ] = CommunityHealthAllProperties
) -> Type[SchemaOrgBase]:
...
def create_communityhealth_model(
    model: Union[
        CommunityHealthProperties,
        CommunityHealthInheritedProperties,
        CommunityHealthAllProperties,
    ...
    _type = deepcopy(CommunityHealthAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of CommunityHealth. Please see: https://schema.org/CommunityHealth"
            )
    # delete_keys = []
...
def schema_json(model: CommunityHealthAllProperties):
