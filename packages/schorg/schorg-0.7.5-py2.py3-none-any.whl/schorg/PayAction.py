...
class PayActionAllProperties(
    PayActionInheritedProperties, PayActionProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        PayActionProperties, PayActionInheritedProperties, PayActionAllProperties
    ] = PayActionAllProperties
) -> Type[SchemaOrgBase]:
...
def create_payaction_model(
    model: Union[
        PayActionProperties, PayActionInheritedProperties, PayActionAllProperties
    ]
):
    _type = deepcopy(PayActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of PayAction. Please see: https://schema.org/PayAction"
            )
    # delete_keys = []
...
def schema_json(model: PayActionAllProperties):
