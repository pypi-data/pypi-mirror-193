...
class MedicalConditionStageProperties(TypedDict):
    ...
    Attributes:
        ...
        stageAsNumber: (Optional[Union[List[Union[str, SchemaOrgObj, StrictInt, StrictFloat]], str, SchemaOrgObj, StrictInt, StrictFloat]]): The stage represented as a number, e.g. 3.
    """
...
def create_medicalconditionstage_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of MedicalConditionStage. Please see: https://schema.org/MedicalConditionStage"
            )
    # delete_keys = []
