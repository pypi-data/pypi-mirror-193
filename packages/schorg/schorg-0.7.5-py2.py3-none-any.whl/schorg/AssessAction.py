...
class AssessActionInheritedProperties(TypedDict):
    """The act of forming one's opinion, reaction or sentiment.
...
class AssessActionProperties(TypedDict):
    """The act of forming one's opinion, reaction or sentiment.
...
class AssessActionAllProperties(
    AssessActionInheritedProperties, AssessActionProperties, TypedDict
):
    pass


class AssessActionBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="AssessAction", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ...
        AssessActionInheritedProperties,
        AssessActionAllProperties,
    ] = AssessActionAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "AssessAction"
    return model


AssessAction = create_schema_org_model()


def create_assessaction_model(
    model: Union[
        AssessActionProperties,
        AssessActionInheritedProperties,
        AssessActionAllProperties,
    ...
    _type = deepcopy(AssessActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of AssessAction. Please see: https://schema.org/AssessAction"
            )
    # delete_keys = []
...
def schema_json(model: AssessActionAllProperties):
    pydantic_type = create_assessaction_model(model=model)
