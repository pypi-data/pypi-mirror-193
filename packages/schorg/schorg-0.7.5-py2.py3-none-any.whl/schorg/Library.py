...
class LibraryInheritedProperties(TypedDict):
    """A library.
...
class LibraryProperties(TypedDict):
    """A library.
...
class LibraryAllProperties(LibraryInheritedProperties, LibraryProperties, TypedDict):
    pass


class LibraryBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Library", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        LibraryProperties, LibraryInheritedProperties, LibraryAllProperties
    ] = LibraryAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Library"
    return model


Library = create_schema_org_model()


def create_library_model(
    model: Union[LibraryProperties, LibraryInheritedProperties, LibraryAllProperties]
):
    _type = deepcopy(LibraryAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Library. Please see: https://schema.org/Library"
            )
    # delete_keys = []
...
def schema_json(model: LibraryAllProperties):
    pydantic_type = create_library_model(model=model)
