...
class InternetCafeInheritedProperties(TypedDict):
    """An internet cafe.
...
class InternetCafeProperties(TypedDict):
    """An internet cafe.
...
class InternetCafeAllProperties(
    InternetCafeInheritedProperties, InternetCafeProperties, TypedDict
):
    pass


class InternetCafeBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="InternetCafe", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        InternetCafeProperties,
        InternetCafeInheritedProperties,
        InternetCafeAllProperties,
    ] = InternetCafeAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "InternetCafe"
    return model


InternetCafe = create_schema_org_model()


def create_internetcafe_model(
    model: Union[
        InternetCafeProperties,
        InternetCafeInheritedProperties,
        InternetCafeAllProperties,
    ...
    _type = deepcopy(InternetCafeAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of InternetCafe. Please see: https://schema.org/InternetCafe"
            )
    # delete_keys = []
...
def schema_json(model: InternetCafeAllProperties):
    pydantic_type = create_internetcafe_model(model=model)
