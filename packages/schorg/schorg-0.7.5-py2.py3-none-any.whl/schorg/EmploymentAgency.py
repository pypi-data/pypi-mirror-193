...
class EmploymentAgencyInheritedProperties(TypedDict):
    """An employment agency.
...
class EmploymentAgencyProperties(TypedDict):
    """An employment agency.
...
class EmploymentAgencyAllProperties(
    EmploymentAgencyInheritedProperties, EmploymentAgencyProperties, TypedDict
):
    pass


class EmploymentAgencyBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="EmploymentAgency", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        EmploymentAgencyProperties,
        EmploymentAgencyInheritedProperties,
        EmploymentAgencyAllProperties,
    ] = EmploymentAgencyAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "EmploymentAgency"
    return model


EmploymentAgency = create_schema_org_model()


def create_employmentagency_model(
    model: Union[
        EmploymentAgencyProperties,
        EmploymentAgencyInheritedProperties,
        EmploymentAgencyAllProperties,
    ...
    _type = deepcopy(EmploymentAgencyAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of EmploymentAgency. Please see: https://schema.org/EmploymentAgency"
            )
    # delete_keys = []
...
def schema_json(model: EmploymentAgencyAllProperties):
    pydantic_type = create_employmentagency_model(model=model)
