...
class OncologicAllProperties(
    OncologicInheritedProperties, OncologicProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        OncologicProperties, OncologicInheritedProperties, OncologicAllProperties
    ] = OncologicAllProperties
) -> Type[SchemaOrgBase]:
...
def create_oncologic_model(
    model: Union[
        OncologicProperties, OncologicInheritedProperties, OncologicAllProperties
    ]
):
    _type = deepcopy(OncologicAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Oncologic. Please see: https://schema.org/Oncologic"
            )
    # delete_keys = []
...
def schema_json(model: OncologicAllProperties):
