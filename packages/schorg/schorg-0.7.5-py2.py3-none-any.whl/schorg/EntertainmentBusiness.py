...
class EntertainmentBusinessInheritedProperties(TypedDict):
    """A business providing entertainment.
...
class EntertainmentBusinessProperties(TypedDict):
    """A business providing entertainment.
...
class EntertainmentBusinessAllProperties(
    EntertainmentBusinessInheritedProperties, EntertainmentBusinessProperties, TypedDict
):
    pass


class EntertainmentBusinessBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="EntertainmentBusiness", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        EntertainmentBusinessProperties,
        EntertainmentBusinessInheritedProperties,
        EntertainmentBusinessAllProperties,
    ] = EntertainmentBusinessAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "EntertainmentBusiness"
    return model


EntertainmentBusiness = create_schema_org_model()


def create_entertainmentbusiness_model(
    model: Union[
        EntertainmentBusinessProperties,
        EntertainmentBusinessInheritedProperties,
        EntertainmentBusinessAllProperties,
    ...
    _type = deepcopy(EntertainmentBusinessAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of EntertainmentBusiness. Please see: https://schema.org/EntertainmentBusiness"
            )
    # delete_keys = []
...
def schema_json(model: EntertainmentBusinessAllProperties):
    pydantic_type = create_entertainmentbusiness_model(model=model)
