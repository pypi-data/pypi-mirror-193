...
class MedicalRiskFactorInheritedProperties(TypedDict):
    """A risk factor is anything that increases a person's likelihood of developing or contracting a disease, medical condition, or complication.
...
class MedicalRiskFactorProperties(TypedDict):
    """A risk factor is anything that increases a person's likelihood of developing or contracting a disease, medical condition, or complication.
    ...
    Note:
        Model Depth 3
    Attributes:
        increasesRiskOf: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): The condition, complication, etc. influenced by this factor.
    """

    increasesRiskOf: NotRequired[
        Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]
    ]


class MedicalRiskFactorAllProperties(
    MedicalRiskFactorInheritedProperties, MedicalRiskFactorProperties, TypedDict
):
    pass


class MedicalRiskFactorBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="MedicalRiskFactor", alias="@id")
    ...
    class Config:
        ...
        fields = {"increasesRiskOf": {"exclude": True}}


def create_schema_org_model(
    type_: Union[
        MedicalRiskFactorProperties,
        MedicalRiskFactorInheritedProperties,
        MedicalRiskFactorAllProperties,
    ] = MedicalRiskFactorAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "MedicalRiskFactor"
    return model


MedicalRiskFactor = create_schema_org_model()


def create_medicalriskfactor_model(
    model: Union[
        MedicalRiskFactorProperties,
        MedicalRiskFactorInheritedProperties,
        MedicalRiskFactorAllProperties,
    ...
    _type = deepcopy(MedicalRiskFactorAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of MedicalRiskFactor. Please see: https://schema.org/MedicalRiskFactor"
            )
    # delete_keys = []
...
def schema_json(model: MedicalRiskFactorAllProperties):
    pydantic_type = create_medicalriskfactor_model(model=model)
