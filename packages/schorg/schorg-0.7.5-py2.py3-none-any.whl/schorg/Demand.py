...
class DemandAllProperties(DemandInheritedProperties, DemandProperties, TypedDict):
    pass
...
def create_schema_org_model(
    type_: Union[
        DemandProperties, DemandInheritedProperties, DemandAllProperties
    ] = DemandAllProperties
) -> Type[SchemaOrgBase]:
...
def create_demand_model(
    model: Union[DemandProperties, DemandInheritedProperties, DemandAllProperties]
):
    _type = deepcopy(DemandAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Demand. Please see: https://schema.org/Demand"
            )
    # delete_keys = []
...
def schema_json(model: DemandAllProperties):
