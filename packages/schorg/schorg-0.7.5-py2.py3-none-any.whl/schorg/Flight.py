...
class FlightProperties(TypedDict):
    ...
    Attributes:
        ...
        webCheckinTime: (Optional[Union[List[Union[str, SchemaOrgObj, datetime]], str, SchemaOrgObj, datetime]]): The time when a passenger can check into the flight online.
...
def create_flight_model(
    ...
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Flight. Please see: https://schema.org/Flight"
            )
    # delete_keys = []
