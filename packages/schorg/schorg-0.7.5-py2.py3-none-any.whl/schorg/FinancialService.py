...
class FinancialServiceInheritedProperties(TypedDict):
    """Financial services business.
...
class FinancialServiceProperties(TypedDict):
    """Financial services business.
...
class FinancialServiceAllProperties(
    FinancialServiceInheritedProperties, FinancialServiceProperties, TypedDict
):
    pass


class FinancialServiceBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="FinancialService", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        FinancialServiceProperties,
        FinancialServiceInheritedProperties,
        FinancialServiceAllProperties,
    ] = FinancialServiceAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "FinancialService"
    return model


FinancialService = create_schema_org_model()


def create_financialservice_model(
    model: Union[
        FinancialServiceProperties,
        FinancialServiceInheritedProperties,
        FinancialServiceAllProperties,
    ...
    _type = deepcopy(FinancialServiceAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of FinancialService. Please see: https://schema.org/FinancialService"
            )
    # delete_keys = []
...
def schema_json(model: FinancialServiceAllProperties):
    pydantic_type = create_financialservice_model(model=model)
