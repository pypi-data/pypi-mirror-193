...
class ThingAllProperties(ThingInheritedProperties, ThingProperties, TypedDict):
    pass


class ThingBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="Thing", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ThingProperties, ThingInheritedProperties, ThingAllProperties
    ] = ThingAllProperties
) -> Type[SchemaOrgBase]:
...
def create_thing_model(
    model: Union[ThingProperties, ThingInheritedProperties, ThingAllProperties]
):
    _type = deepcopy(ThingAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Thing. Please see: https://schema.org/Thing"
            )
    # delete_keys = []
...
def schema_json(model: ThingAllProperties):
    pydantic_type = create_thing_model(model=model)
