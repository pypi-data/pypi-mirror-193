...
class ReplyActionAllProperties(
    ReplyActionInheritedProperties, ReplyActionProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        ReplyActionProperties, ReplyActionInheritedProperties, ReplyActionAllProperties
    ] = ReplyActionAllProperties
) -> Type[SchemaOrgBase]:
...
def create_replyaction_model(
    model: Union[
        ReplyActionProperties, ReplyActionInheritedProperties, ReplyActionAllProperties
    ]
):
    _type = deepcopy(ReplyActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of ReplyAction. Please see: https://schema.org/ReplyAction"
            )
    # delete_keys = []
...
def schema_json(model: ReplyActionAllProperties):
