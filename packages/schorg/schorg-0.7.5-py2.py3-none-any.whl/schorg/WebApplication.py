...
class WebApplicationInheritedProperties(TypedDict):
    """Web applications.
...
class WebApplicationProperties(TypedDict):
    """Web applications.
    ...
    Note:
        Model Depth 4
    Attributes:
        browserRequirements: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): Specifies browser requirements in human-readable text. For example, 'requires HTML5 support'.
    """

    browserRequirements: NotRequired[
        Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]
    ]


class WebApplicationAllProperties(
    WebApplicationInheritedProperties, WebApplicationProperties, TypedDict
):
    pass


class WebApplicationBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="WebApplication", alias="@id")
    ...
    class Config:
        ...
        fields = {"browserRequirements": {"exclude": True}}


def create_schema_org_model(
    type_: Union[
        WebApplicationProperties,
        WebApplicationInheritedProperties,
        WebApplicationAllProperties,
    ] = WebApplicationAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "WebApplication"
    return model


WebApplication = create_schema_org_model()


def create_webapplication_model(
    model: Union[
        WebApplicationProperties,
        WebApplicationInheritedProperties,
        WebApplicationAllProperties,
    ...
    _type = deepcopy(WebApplicationAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of WebApplication. Please see: https://schema.org/WebApplication"
            )
    # delete_keys = []
...
def schema_json(model: WebApplicationAllProperties):
    pydantic_type = create_webapplication_model(model=model)
