...
class ResortAllProperties(ResortInheritedProperties, ResortProperties, TypedDict):
    pass
...
def create_schema_org_model(
    type_: Union[
        ResortProperties, ResortInheritedProperties, ResortAllProperties
    ] = ResortAllProperties
) -> Type[SchemaOrgBase]:
...
def create_resort_model(
    model: Union[ResortProperties, ResortInheritedProperties, ResortAllProperties]
):
    _type = deepcopy(ResortAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of Resort. Please see: https://schema.org/Resort"
            )
    # delete_keys = []
...
def schema_json(model: ResortAllProperties):
