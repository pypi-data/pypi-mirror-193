...
class BedTypeAllProperties(BedTypeInheritedProperties, BedTypeProperties, TypedDict):
    pass


class BedTypeBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="BedType", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        BedTypeProperties, BedTypeInheritedProperties, BedTypeAllProperties
    ] = BedTypeAllProperties
) -> Type[SchemaOrgBase]:
...
def create_bedtype_model(
    model: Union[BedTypeProperties, BedTypeInheritedProperties, BedTypeAllProperties]
):
    _type = deepcopy(BedTypeAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of BedType. Please see: https://schema.org/BedType"
            )
    # delete_keys = []
...
def schema_json(model: BedTypeAllProperties):
    pydantic_type = create_bedtype_model(model=model)
