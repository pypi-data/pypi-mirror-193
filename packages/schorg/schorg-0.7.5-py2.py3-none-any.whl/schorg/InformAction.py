...
class InformActionAllProperties(
    InformActionInheritedProperties, InformActionProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        InformActionProperties,
        InformActionInheritedProperties,
        InformActionAllProperties,
    ] = InformActionAllProperties
) -> Type[SchemaOrgBase]:
...
def create_informaction_model(
    ...
    _type = deepcopy(InformActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of InformAction. Please see: https://schema.org/InformAction"
            )
    # delete_keys = []
...
def schema_json(model: InformActionAllProperties):
