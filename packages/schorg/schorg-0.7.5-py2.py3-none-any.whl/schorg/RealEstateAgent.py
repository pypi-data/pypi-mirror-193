...
class RealEstateAgentInheritedProperties(TypedDict):
    """A real-estate agent.
...
class RealEstateAgentProperties(TypedDict):
    """A real-estate agent.
...
class RealEstateAgentAllProperties(
    RealEstateAgentInheritedProperties, RealEstateAgentProperties, TypedDict
):
    pass


class RealEstateAgentBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="RealEstateAgent", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        ...
        RealEstateAgentInheritedProperties,
        RealEstateAgentAllProperties,
    ] = RealEstateAgentAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "RealEstateAgent"
    return model


RealEstateAgent = create_schema_org_model()


def create_realestateagent_model(
    model: Union[
        RealEstateAgentProperties,
        RealEstateAgentInheritedProperties,
        RealEstateAgentAllProperties,
    ...
    _type = deepcopy(RealEstateAgentAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of RealEstateAgent. Please see: https://schema.org/RealEstateAgent"
            )
    # delete_keys = []
...
def schema_json(model: RealEstateAgentAllProperties):
    pydantic_type = create_realestateagent_model(model=model)
