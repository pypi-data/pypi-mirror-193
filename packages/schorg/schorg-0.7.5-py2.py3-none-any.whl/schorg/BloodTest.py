...
class BloodTestAllProperties(
    BloodTestInheritedProperties, BloodTestProperties, TypedDict
):
    pass
...
def create_schema_org_model(
    type_: Union[
        BloodTestProperties, BloodTestInheritedProperties, BloodTestAllProperties
    ] = BloodTestAllProperties
) -> Type[SchemaOrgBase]:
...
def create_bloodtest_model(
    model: Union[
        BloodTestProperties, BloodTestInheritedProperties, BloodTestAllProperties
    ]
):
    _type = deepcopy(BloodTestAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of BloodTest. Please see: https://schema.org/BloodTest"
            )
    # delete_keys = []
...
def schema_json(model: BloodTestAllProperties):
