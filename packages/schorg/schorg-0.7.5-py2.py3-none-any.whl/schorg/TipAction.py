...
class TipActionInheritedProperties(TypedDict):
    """The act of giving money voluntarily to a beneficiary in recognition of services rendered.
...
class TipActionProperties(TypedDict):
    """The act of giving money voluntarily to a beneficiary in recognition of services rendered.
...
class TipActionAllProperties(
    TipActionInheritedProperties, TipActionProperties, TypedDict
):
    pass


class TipActionBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="TipAction", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        TipActionProperties, TipActionInheritedProperties, TipActionAllProperties
    ] = TipActionAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "TipAction"
    return model


TipAction = create_schema_org_model()


def create_tipaction_model(
    model: Union[
        TipActionProperties, TipActionInheritedProperties, TipActionAllProperties
    ]
):
    _type = deepcopy(TipActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of TipAction. Please see: https://schema.org/TipAction"
            )
    # delete_keys = []
...
def schema_json(model: TipActionAllProperties):
    pydantic_type = create_tipaction_model(model=model)
