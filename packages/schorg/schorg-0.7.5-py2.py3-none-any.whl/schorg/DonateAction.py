...
class DonateActionInheritedProperties(TypedDict):
    """The act of providing goods, services, or money without compensation, often for philanthropic reasons.
...
class DonateActionProperties(TypedDict):
    """The act of providing goods, services, or money without compensation, often for philanthropic reasons.
...
class DonateActionAllProperties(
    DonateActionInheritedProperties, DonateActionProperties, TypedDict
):
    pass


class DonateActionBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="DonateAction", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        DonateActionProperties,
        DonateActionInheritedProperties,
        DonateActionAllProperties,
    ] = DonateActionAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "DonateAction"
    return model


DonateAction = create_schema_org_model()


def create_donateaction_model(
    model: Union[
        DonateActionProperties,
        DonateActionInheritedProperties,
        DonateActionAllProperties,
    ...
    _type = deepcopy(DonateActionAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of DonateAction. Please see: https://schema.org/DonateAction"
            )
    # delete_keys = []
...
def schema_json(model: DonateActionAllProperties):
    pydantic_type = create_donateaction_model(model=model)
