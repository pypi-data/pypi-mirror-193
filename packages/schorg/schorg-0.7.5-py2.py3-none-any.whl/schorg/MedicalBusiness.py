...
class MedicalBusinessInheritedProperties(TypedDict):
    """A particular physical or virtual business of an organization for medical purposes. Examples of MedicalBusiness include different businesses run by health professionals.
...
class MedicalBusinessProperties(TypedDict):
    """A particular physical or virtual business of an organization for medical purposes. Examples of MedicalBusiness include different businesses run by health professionals.
...
class MedicalBusinessAllProperties(
    MedicalBusinessInheritedProperties, MedicalBusinessProperties, TypedDict
):
    pass


class MedicalBusinessBaseModel(SchemaOrgBase):
    id_: Optional[Any] = Field(default="MedicalBusiness", alias="@id")
...
def create_schema_org_model(
    type_: Union[
        MedicalBusinessProperties,
        MedicalBusinessInheritedProperties,
        MedicalBusinessAllProperties,
    ] = MedicalBusinessAllProperties
) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "MedicalBusiness"
    return model


MedicalBusiness = create_schema_org_model()


def create_medicalbusiness_model(
    model: Union[
        MedicalBusinessProperties,
        MedicalBusinessInheritedProperties,
        MedicalBusinessAllProperties,
    ...
    _type = deepcopy(MedicalBusinessAllProperties)
    for k in model.__annotations__.keys():
        if k not in _type.__annotations__:
            raise TypeError(
                f"{k} not part of MedicalBusiness. Please see: https://schema.org/MedicalBusiness"
            )
    # delete_keys = []
...
def schema_json(model: MedicalBusinessAllProperties):
    pydantic_type = create_medicalbusiness_model(model=model)
