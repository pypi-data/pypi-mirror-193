...
class IndividualProductInheritedProperties(TypedDict):
    """A single, identifiable product instance (e.g. a laptop with a particular serial number).
...
class IndividualProductProperties(TypedDict):
    """A single, identifiable product instance (e.g. a laptop with a particular serial number).
...
class AllProperties(IndividualProductInheritedProperties , IndividualProductProperties, TypedDict):
    pass


class IndividualProductBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="IndividualProduct",alias='@id')
...
def create_schema_org_model(type_: Union[IndividualProductProperties, IndividualProductInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "IndividualProduct"
    return model
    

IndividualProduct = create_schema_org_model()


def create_individualproduct_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_individualproduct_model(model=model)
