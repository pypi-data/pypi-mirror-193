...
class HomeAndConstructionBusinessInheritedProperties(TypedDict):
    """A construction business.A HomeAndConstructionBusiness is a [[LocalBusiness]] that provides services around homes and buildings.As a [[LocalBusiness]] it can be described as a [[provider]] of one or more [[Service]]\(s).
...
class HomeAndConstructionBusinessProperties(TypedDict):
    """A construction business.A HomeAndConstructionBusiness is a [[LocalBusiness]] that provides services around homes and buildings.As a [[LocalBusiness]] it can be described as a [[provider]] of one or more [[Service]]\(s).
...
class AllProperties(HomeAndConstructionBusinessInheritedProperties , HomeAndConstructionBusinessProperties, TypedDict):
    pass


class HomeAndConstructionBusinessBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="HomeAndConstructionBusiness",alias='@id')
...
def create_schema_org_model(type_: Union[HomeAndConstructionBusinessProperties, HomeAndConstructionBusinessInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "HomeAndConstructionBusiness"
    return model
    

HomeAndConstructionBusiness = create_schema_org_model()


def create_homeandconstructionbusiness_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_homeandconstructionbusiness_model(model=model)
