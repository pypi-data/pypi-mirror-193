...
class CarInheritedProperties(TypedDict):
    """A car is a wheeled, self-powered motor vehicle used for transportation.
...
class CarProperties(TypedDict):
    ...
    Note:
        Model Depth 4
    Attributes:
        roofLoad: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): The permitted total weight of cargo and installations (e.g. a roof rack) on top of the vehicle.Typical unit code(s): KGM for kilogram, LBR for pound* Note 1: You can indicate additional information in the [[name]] of the [[QuantitativeValue]] node.* Note 2: You may also link to a [[QualitativeValue]] node that provides additional information using [[valueReference]]* Note 3: Note that you can use [[minValue]] and [[maxValue]] to indicate ranges.
        acrissCode: (Optional[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]): The ACRISS Car Classification Code is a code used by many car rental companies, for classifying vehicles. ACRISS stands for Association of Car Rental Industry Systems and Standards.
    """

    roofLoad: NotRequired[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]
    acrissCode: NotRequired[Union[List[Union[str, SchemaOrgObj]], str, SchemaOrgObj]]
    


class AllProperties(CarInheritedProperties , CarProperties, TypedDict):
    pass


class CarBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="Car",alias='@id')
...
def create_schema_org_model(type_: Union[CarProperties, CarInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "Car"
    return model
    

Car = create_schema_org_model()


def create_car_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_car_model(model=model)
