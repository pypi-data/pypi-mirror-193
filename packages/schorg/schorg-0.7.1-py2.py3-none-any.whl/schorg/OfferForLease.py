...
class OfferForLeaseInheritedProperties(TypedDict):
    """An [[OfferForLease]] in Schema.org represents an [[Offer]] to lease out something, i.e. an [[Offer]] whose  [[businessFunction]] is [lease out](http://purl.org/goodrelations/v1#LeaseOut.). See [Good Relations](https://en.wikipedia.org/wiki/GoodRelations) for  background on the underlying concepts.  
...
class OfferForLeaseProperties(TypedDict):
    """An [[OfferForLease]] in Schema.org represents an [[Offer]] to lease out something, i.e. an [[Offer]] whose  [[businessFunction]] is [lease out](http://purl.org/goodrelations/v1#LeaseOut.). See [Good Relations](https://en.wikipedia.org/wiki/GoodRelations) for  background on the underlying concepts.  
...
class AllProperties(OfferForLeaseInheritedProperties , OfferForLeaseProperties, TypedDict):
    pass


class OfferForLeaseBaseModel(SchemaOrgBase):
    id_ : Optional[Any] = Field(default="OfferForLease",alias='@id')
...
def create_schema_org_model(type_: Union[OfferForLeaseProperties, OfferForLeaseInheritedProperties, AllProperties] = AllProperties) -> Type[SchemaOrgBase]:
    model = create_model_from_typeddict(type_, __base__=SchemaOrgBase)
    model.__name__ = "OfferForLease"
    return model
    

OfferForLease = create_schema_org_model()


def create_offerforlease_model(model: AllProperties):
...
def schema_json(model: AllProperties):
    pydantic_type =  create_offerforlease_model(model=model)
