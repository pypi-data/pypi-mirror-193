...
def shuffle_tag_handler(d, k, v):
    d[k] = Shuffle(int(v))


def repeat_tag_handler(d, k, v):
    d[k] = Repeat(int(v))


player_status_tag_handlers = {
        'playlist shuffle': (
            lambda d, k, v: shuffle_tag_handler(d, 'shuffle', v)),
        'playlist repeat': (
            lambda d, k, v: repeat_tag_handler(d, 'repeat', v)),
...
def playlist_query_shuffle(tn, player_id):
    match send_receive(tn, [player_id, 'playlist', 'shuffle', '?']):
        case [player_id, 'playlist', 'shuffle', state]:
            return Shuffle(int(state))


def playlist_set_shuffle(tn, player_id, shuffle_type):
    send_receive(tn, [player_id,
                      'playlist',
                      'shuffle',
                      str(shuffle_type.value)])


def playlist_toggle_shuffle(tn, player_id):
    send_receive(tn, [player_id, 'playlist', 'shuffle'])


def playlist_query_repeat(tn, player_id):
    match send_receive(tn, [player_id, 'playlist', 'repeat', '?']):
        case [player_id, 'playlist', 'repeat', state]:
            return Repeat(int(state))


def playlist_set_repeat(tn, player_id, repeat_type):
    send_receive(tn, [player_id, 'playlist', 'repeat', str(repeat_type.value)])


def playlist_toggle_repeat(tn, player_id):
    send_receive(tn, [player_id, 'playlist', 'repeat'])
