...
class ActorAddFieldValue(Resource):
    def delete(self):
        print(self.url)
        print(f"{self.url[:-1]}ActorAddField={self.ActorAddField}, Actor={self.Actor})")
        return self.session.delete(f"{self.url[:-1]}Actor={self.Actor}L,ActorAddField={self.ActorAddField}L,BasicList=0L,TextSelection=0L)")

    def updateText(self, text):
        return self.session.patch(f"{self.url[:-1]}Actor={self.Actor}L,ActorAddField={self.ActorAddField}L,BasicList=0L,TextSelection=0L)",
                                  json={'Text': text, 'FormattedTextOutput': text})

    def post(self):
        print(self.url)
        print(self.data)
        temp = self.session.post(self.url, json=self.data)
...
class Student(Resource):
    ...
    def additionalField(self, addFieldId=None):
        if addFieldId:
            url = self.base + f"ActorAddFieldValue()?$filter=Actor eq {self.PersonId} and ActorAddField eq {addFieldId}"
        else:
            url = self.base + f"ActorAddFieldValue()?$filter=Actor eq {self.PersonId}"
        data = self.session.get(url).json()["value"]
        if not data:
            return None
        return ActorAddFieldValue(self.base, self.session, data[0], db=self.db)
