...
from cbra.ext.oauth2 import AuthorizationCodeCallbackEndpoint
...
class CallbackEndpoint(AuthorizationCodeCallbackEndpoint):
    ...
    async def on_success(
        ...
        subject = await self.onboard.oidc(id_token)
        await self.session
        self.session.update({
            **id_token.dict(exclude_none=True),
            'iss': self.get_issuer(),
            'sub': str(subject.uid),
        })
...
app.add(CallbackEndpoint, path='/oauth2/callback/{client_id}')
