...
class DesiredResponsesData:
    ...
    test_volume_create = (
        "[32mVolume test1 of size 1.0 GB on SSD created. Volume is ReadWriteMany.[0m"
    )
    test_volume_delete = "[32mVolume test1 deleted.[0m"
    test_volume_unmount = (
        "[32mVolume test1 successfully unmounted from selected apps.[0m"
    )
    test_volume_mount = (
        "[32mVolume test1 successfully mounted to test, mount path: /tf/test1.[0m"
    )
    test_compute_get_start_path = (
        "[32mStart path for datascience-jupyter: /home/jovyan.[0m"
    )
    test_compute_list_empty = "[33mNo apps to list.[0m"
    test_compute_list_no_labels = get_desired_response_data(
        "test_compute_list_no_labels.txt"
    )
    test_compute_create_filebrowser = (
        "[32mFilebrowser creation started!\n"
        "Will be accessible at: https://filebrowser.pytest.dev.quantdevlabs.com\n"
        "Username: pytest\n"
        "Password: 9bb9c88f8fc14eadba5c5fbdafc06275[0m"
    )
    test_compute_create = (
        "[32mnvidia-pytorch app test1 creation started!\n"
        "Will be accessible at: https://test1.pytest.dev.quantdevlabs.com\n"
        "App token: 1ce03c31ab9c40df8594d97b301eaaea\n"
        "To monitor the startup status use command: cgc compute list[0m"
    )
    test_compute_create_no_volume_found = "[33mVolume name not found in namespace.[0m"
    test_compute_delete = "[32mApp test1 and its service successfully deleted.[0m"
    test_compute_restart = "[32mApp test1 has been successfully restarted.[0m"
    test_compute_list = get_desired_response_data("test_compute_list.txt")
    test_compute_template_list = "[32mAvailable templates: tensorflow-jupyter, datascience-jupyter, nvidia-rapids, nvidia-pytorch, nvidia-tensorflow[0m"
    test_tabulate_a_response = get_desired_response_data("test_tabulate_response.txt")
    test_tabulate_a_response_empty = "[33m[0m"
    test_tabulate_a_response_empty_and_full_in_2_lists = []
    test_fill_missing_values_in_response = [
        [
            ("name", "filebrowser"),
    ...
    test_get_app_list_missing_labels = [{"missing_labels": "roberta-5476f5bcb7-cqnq8"}]
    test_get_app_list_missing_and_correct_lables_in_2_pods = [
        {
            "name": "test1",
            "type": "tensorflow-jupyter",
