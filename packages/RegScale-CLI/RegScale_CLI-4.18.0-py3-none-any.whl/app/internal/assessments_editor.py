...
from app.utils.app_utils import check_file_path, error_and_exit, str_to_date
...
def all_assessments(regscale_id: str, regscale_module: str, path: Path):
    ...
    if (
        ...
        for a in raw_data:
            ...
            PlannedStart = str_to_date(a["plannedStart"])
            PlannedFinish = str_to_date(a["plannedFinish"])
            Status = a["status"]
            ActualFinish = (
                str_to_date(a["actualFinish"]) if a["actualFinish"] else "None"
            )
            AssessmentResult = (
