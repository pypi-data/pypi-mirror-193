...
class SysfsBase(object):
    ...
    @classmethod
    def _prop_string(cls, attr, read, write):
        def R(self, attr=attr): return self._read(attr)
        def W(self, val, attr=attr): return self._write(attr, val)
        return property(read and R or None, write and W or None, cls._fdel)

    @classmethod
    def _prop_number(cls, attr, read, write):
        def R(self, attr=attr): return self._read(attr, 10)
        def W(self, val, attr=attr): return self._write(attr, val)
        return property(read and R or None, write and W or None, cls._fdel)

    @classmethod
    def _prop_boolean(cls, attr, read, write):
    ...
    def setup(self, reopen=True):                   # open or reopen attr files
        ...
        for attr in set(self._files).difference(self._attrs):
            ...
            if getattr(prop, 'fdel', None) is self._fdel:  # created by _prop_
        ...
        for attr in self._attrs:
            ...
            try:
                ...
                rdwr = ('r' in mode or '+' in mode, 'w' in mode)
            except Exception:
            ...
            name = '_prop_%s' % self._attrs[attr]
            if hasattr(self, name) and not hasattr(self.__class__, attr):
                setattr(self.__class__, attr, getattr(self, name)(attr, *rdwr))
...
class SysfsLED(SysfsBase):
    ...
    def trigger(self, trig):
        ...
        if trig not in self.triggers or not self._write('trigger', trig):
            return
        time.sleep(1e-3)
