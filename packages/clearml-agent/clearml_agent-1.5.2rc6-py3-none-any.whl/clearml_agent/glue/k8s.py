...
class K8sIntegration(Worker):
    ...
    def _get_used_pods(self):
        ...
        """
        For Jobs:
            - Get Jobs, not pods (in the same way)
        """
    ...
    def get_jobs_info(self, info_path: str, condition: str = None, namespace=None, debug_msg: str = None):
        ...
        output = f"jsonpath='{{.items[?(@.{cond})].{info_path}}}'"
        ...
        try:
            data_items = {l.strip() for l in output.splitlines()}
            return data_items
        except Exception as ex:
    ...
    def _kubectl_apply(
        ...
        if labels:
            ...
            metadata.setdefault('labels', {}).update(labels_dict)

        spec = template.setdefault('spec', {})
        if self.using_jobs:
            spec.setdefault('backoffLimit', 0)
            spec = spec.setdefault('template', {}).setdefault('spec', {})
    ...
    def _delete_jobs_by_names(self, names: Iterable[str], namespace: str, msg: str = None) -> List[str]:
        if not names:
            return []
        kubectl_cmd = \
            "kubectl delete job --namespace={namespace} --output=name {names}".format(
                namespace=namespace,
                names=" ".join(names)
            )
        self.log.debug("Deleting jobs {} for ns {}: {}".format(
            msg or "", namespace, kubectl_cmd
        ))
        lines = self._process_bash_lines_response(kubectl_cmd)
    ...
    def _delete_completed_or_failed_pods(self, namespace, msg: str = None):
        ...
        if failed_pods:
            job_names_to_delete = list(
                filter(None, (get_path(pod, "metadata", "labels", "job-name") for pod in failed_pods))
            )
        ...
        if completed_job_names:
            ...
            job_names_to_delete.extend(completed_job_names)

        return self._delete_jobs_by_names(names=job_names_to_delete, namespace=namespace, msg=msg)
    ...
    def run_tasks_loop(self, queues: List[Text], worker_params, **kwargs):
        ...
        For Jobs:
            - Rename output to "jobs" instead of pods
