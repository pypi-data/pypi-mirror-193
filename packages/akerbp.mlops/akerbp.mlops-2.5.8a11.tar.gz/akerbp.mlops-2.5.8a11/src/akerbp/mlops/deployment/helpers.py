...
def get_deployment_folder_content(deployment_folder: Path) -> Dict:
    ...
    for tup in dirwalk:
        if "__pycache__" in tup[0].split("/"):
            continue
        elif ".pytest_cache" in tup[0].split("/"):
            continue
        if len(tup[1]) > 0 and (
            "__pycache__" not in tup[1] or ".pytest_cache" not in tup[1]
        ):  # includes subdirectories, skip pycache dirs
    ...
    if c.test_file:
        ...
        for log_line in output.splitlines():  # type: ignore
            if log_line == "":
                ...
                continue
            elif log_line.startswith('{"input": [') and deploy:
