...
from akerbp.mlops.core.exceptions import MissingClientError, MissingDeploymentFolder
...
def find_model_version(model_name: str, env: str, metadata: Dict) -> str:
    ...
    if (n_models := file_list.shape[0]) == 0:
        ...
        latest_artifact_version = file_list.loc[
            file_list.uploadedTime.argmax(), "externalId"
        ].split("/")[-1]
...
def get_latest_model_in_env(model_name: str, env: str, deployed: bool = False) -> int:
    ...
    if deployed:
        ...
        functions_client = cdf.get_client(
            api_key=os.environ.get("COGNITE_API_KEY_FUNCTIONS")
        )
