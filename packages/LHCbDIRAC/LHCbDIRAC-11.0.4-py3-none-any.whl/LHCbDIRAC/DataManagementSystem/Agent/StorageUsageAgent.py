...
class StorageUsageAgent(AgentModule):
    ...
    def initialize(self):
        ...
        if self.enableStartupSleep:
            ...
            self.log.info(f"Sleeping for {rndSleep} seconds")
            time.sleep(rndSleep)
    ...
    def __writeReplicasListFiles(self, dirPathList):
        ...
        try:
            self.log.info(f"Dumping replicas for {len(dirPathList)} dirs")
            result = self.catalog.getDirectoryReplicas(dirPathList)
            if not result["OK"]:
                self.log.error("Could not get directory replicas", f"{dirPathList} -> {result['Message']}")
                return result
            resData = result["Value"]
            filesOpened = {}
            for dirPath in dirPathList:
                if dirPath in result["Value"]["Failed"]:
                    self.log.error("Could not get directory replicas", f"{dirPath} -> {resData['Failed'][dirPath]}")
                    continue
                    ...
                    for seName in dirData[lfn]:
                        if seName not in filesOpened:
                            filePath = os.path.join(
                                self.__replicaListFilesDir, f"replicas.{seName}.{self.__baseDirLabel}.filling"
                            )
                        ...
                        filesOpened[seName].write(f"{lfn} -> {dirData[lfn][seName]}\n")
            # Close the files
    ...
    def __resetReplicaListFiles(self):
        ...
        self.log.info(f"Replica Lists directory is {self.__replicaListFilesDir}")
    ...
    def __replicaListFilesDone(self):
        ...
        try:
            ...
            for fileName in os.listdir(self.__replicaListFilesDir):
                ...
                if match:
                    newFileName = f"replicas.{match.group(1)}.{self.__baseDirLabel}.old"
                    self.log.info(
                        "Moving \n %s\n to \n %s"
                        % (
            ...
            for fileName in os.listdir(self.__replicaListFilesDir):
                ...
                if match:
                    newFileName = f"replicas.{match.group(1)}.{self.__baseDirLabel}"
                    self.log.info(
                        "Moving \n %s\n to \n %s"
                        % (
                            os.path.join(self.__replicaListFilesDir, fileName),
    ...
    def execute(self):
        ...
        self.log.notice(f"Initiating with {self.__baseDir} as base directory.")
        ...
        while self.__dirExplorer.isActive():
            ...
            self.log.verbose(f"Query took {iterTime:.2f} seconds for {len(d2E)} dirs")
    ...
    def __exploreDirList(self, dirList):
        ...
        self.log.notice(f"Retrieving info for {len(dirList)} dirs")
        ...
        for error in errorReason:
            self.log.error(
                "Failed to get directory info", "- {} for:\n\t{}".format(error, "\n\t".join(errorReason[error]))
            )
        self.log.info(
            f"Time to retrieve size of {len(dirListSize)} directories: {time.time() - startTime1:.1f} seconds"
        )
    ...
    def __processDirDFC(self, dirPath, metadata, subDirectories):
        ...
        for se, usage in list(dirUsage.items()):
            ...
            if not usage["Files"] and not usage["Size"]:
                ...
                self.log.error("Directory inconsistent", f"{dirPath} @ {se}: {str(usage)}")
    ...
    def __processDir(self, dirPath, dirMetadata):
        ...
        ##############################
        closedDirs = []
        prStr = f"{dirPath}: found {len(subDirs) if subDirs else 'no'} sub-directories"
        if closedDirs:
            prStr += f", {len(closedDirs)} are closed (ignored)"
        ...
        if numberOfFiles:
            prStr += f" and {numberOfFiles} files ({totalSize} bytes)"
        else:
            prStr += " and no files"
    ...
    def __getOwnerProxy(self, dirPath):
        ...
        for ownerGroup in ownerGroups:
            ...
            if not result["OK"]:
                downErrors.append(f"{cacheKey} : {result['Message']}")
                continue
            ...
            self.log.info(f"Got proxy for {ownerName} {ownerDN}@{ownerGroup} [{ownerRole}]")
            return S_OK(upFile)
        self.__noProxy.add(cacheKey)
        return S_ERROR(
            "Could not download proxy for user ({}, {}):\n{} ".format(ownerDN, ownerRole, "\n ".join(downErrors))
        )

    def removeEmptyDir(self, dirPath):
        self.log.notice(f"Deleting empty directory {dirPath}")
    ...
    def __executePublishData(self):
        ...
        try:
            ...
            self.log.info(f"Publishing usage for {len(toPublish)} directories")
