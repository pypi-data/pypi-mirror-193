...
class InjectPodVisitor(StateVisitor):
	...
	def _(self,state_container):
		...
		C=A.path.removeprefix(config.dirs.data).strip('/')+'/';B=[A for A in self.zip.namelist()if A.startswith(C)]
...
class CloudPodArchive:
	...
	def stores_index(self):
		...
		for C in D:
			A=C.lstrip('/')
			if not A.startswith(_A):continue
			if not A.endswith('store.state'):continue
	...
	def moto_backend_index(self):
		...
		for C in D:
			A=C.lstrip('/')
			if not A.startswith(_A):continue
			if not A.endswith('backend.state'):continue
			E,F,G=A.split('/')[1:4];B[F].append((E,G,C))
		return B