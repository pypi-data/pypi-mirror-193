...
class LoadSnapshotVisitor(StateVisitor):
	...
	def __init__(A,service,data_dir=_A,decoder=_A):A.service=service;A.data_dir=data_dir or os.path.join(config.dirs.data,'api_states');A.decoder=decoder or pickle.PickleDecoder()
	...
	def _(self,state_container):
		A=state_container;C=os.path.join(self.data_dir,A.service_name,'store.state');B=self._deserialize_file(C)
		if B is _A:return
		A.update(dict(B));A.__dict__.update(B.__dict__)
	@visit.register(BackendDict)
	def _(self,state_container):
		A=state_container;C=os.path.join(self.data_dir,A.service_name,'backend.state');B=self._deserialize_file(C)
		if B is _A:return
	...
	def _deserialize_file(A,fpath):
		with open(fpath,'rb')as B:return A.decoder.decode(B)