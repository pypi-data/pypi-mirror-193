...
def pickle_openssl_backend(pickler,obj):
	A=obj;from cryptography.hazmat.backends import default_backend as B
	if not A==B():raise TypeError('cannot pickle backend %s, differs from global backend %s'%(A,B()))
