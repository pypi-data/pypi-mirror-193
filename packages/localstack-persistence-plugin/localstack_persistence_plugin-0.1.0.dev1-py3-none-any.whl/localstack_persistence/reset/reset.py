...
def reset_state(service):
	A=service;B=ResetStateVisitor();call_safe(A.lifecycle_hook.on_before_state_reset);LOG.debug('Resetting state of service %s',A.name())
...
class ResetStateVisitor(StateVisitor):
	...
	def _reset_moto_backend_state(J,state_container,region_key):
		D=state_container;B=region_key;from moto.applicationautoscaling.models import ApplicationAutoscalingBackend as G;from moto.autoscaling.models import AutoScalingBackend as H;from moto.redshift.models import RedshiftBackend as I;A=D.get(B);E=getattr(A,'reset',None)
