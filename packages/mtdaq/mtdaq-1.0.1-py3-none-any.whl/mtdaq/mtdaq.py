...
if system == 'Windows': #Win
    ...
    if bit[0] == '64bits':
        dllpath = r'/Library/Application Support/MangoTree/DLL/MTDAQ_x64.so'
    else:
        raise Exception("")
elif system == "Linux": #Linux
    if bit[0] == '64bit':
        dllpath = r'/usr/local/lib/MTDAQ_x64.so'
...
def getConfig(mode=Mode.General, ip=None, device_name=None, device_id=None, slot=None, custom_rom_path=None):
    dll.MTDAQ_Config_Module.argtypes = [c_char_p, c_char_p, c_char_p, c_char_p, c_uint16, c_char_p, c_char_p, c_int32]
    c_ip = c_char_p(ip.encode() if ip is not None else b"")
    c_device = c_char_p(device_name.encode() if device_name is not None else b"")
    c_id = c_char_p(device_id.encode() if device_id is not None else b"*")
    c_slot = c_char_p(slot.encode() if slot is not None else b"*")
    c_mode = c_uint16(mode)    
    c_path = c_char_p(custom_rom_path.encode() if custom_rom_path is not None else b"")
    c_config = create_string_buffer(20000)
    dll.MTDAQ_Config_Module(c_ip, c_device, c_id, c_slot, c_mode, c_path, c_config, 20000)
