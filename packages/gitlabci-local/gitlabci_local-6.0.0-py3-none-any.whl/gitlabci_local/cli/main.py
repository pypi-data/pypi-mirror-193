...
def main() -> None:
    ...
    group.add_argument('--set', dest='set', action='store',
                       ...
                       help='Set settings specific \'VAL\' value to [GROUP] > VAL')
    ...
    if not options.no_console:
        if settings.has('runner', 'no_console'):
            options.no_console = int(settings.get('runner', 'no_console')) == 1
        else:
            options.no_console = False
            settings.set('runner', 'no_console', 1 if options.no_script_fail else 0)

    # Prepare no_git_safeties
    if not options.no_git_safeties:
        if settings.has('runner', 'no_git_safeties'):
            options.no_git_safeties = int(settings.get('runner', 'no_git_safeties')) == 1
        else:
            options.no_git_safeties = False
            settings.set('runner', 'no_git_safeties', 1 if options.no_script_fail else 0)

    # Prepare no_script_fail
    if not options.no_script_fail:
        if settings.has('parsers', 'no_script_fail'):
            options.no_script_fail = int(settings.get('parsers', 'no_script_fail')) == 1
        else:
            options.no_script_fail = False
            settings.set('parsers', 'no_script_fail', 1 if options.no_script_fail else 0)
