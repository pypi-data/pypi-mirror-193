...
class SWSReceiver(FLReceiver):
    ...
    async def establish_websocket_connection(self, loop, stop):
        ...
        while not self.stopped:
            try:
                ...
                async with websockets.connect(uri, close_timeout=100, max_size=2 ** 29, read_limit=2 ** 29, write_limit=2**29, loop=loop, ssl=self.ssl_context, extra_headers=headers) as websocket:
    ...
    async def receive_message_handler(self, websocket):
        try:
            ...
            logger.info("receive_message_handler - ConnectionClosedError encountered.")
    ...
    async def send_message_handler(self, websocket):
        while self.stopped is not True:
            ...
            try:
                ...
                logger.info("send_message_handler ConnectionClosedError")
            except websockets.exceptions.ConnectionClosedOK as ex:
                logger.info("send_message_handler ConnectionClosedOK")
            except websockets.exceptions.WebSocketException as ex:
                logger.info("send_message_handler WebSocketException")
            except Exception as ex:
                logger.info("send_message_handler Exception")
                logger.info(str(ex))
    ...
    def handle_response_message(self):
        logger.info("Received response from aggregator with status payload")
