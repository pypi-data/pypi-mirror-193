...
class RConn(object):
    ...
    def __init__(
        ...
        if type(conn) == Engine:
            ...
            conn = conn.conn(close_with_result=once)
        self.user = user
    ...
    def execute(
        ...
        **kw_params: Any
        ...
        if report:
            ...
            if params == {}:
                rprint(report_info, sql, title="SQL", frame=roption.print_default_frame_half)
            else:
                rprint(report_info, sql, params, title="SQL", frame=roption.print_default_frame_half)
        else:
    ...
    def execute_update(
        ...
        table : Table name.
        database : Database name.
        where_field : 'WHERE' clause content.
        ...
        for index, row in enumerate(data):
            ...
            sql = (
                ...
                f"SET {set_content}\n"
                f"WHERE `{where_field}` = :{index}_{where_field}"
            )
            sqls.append(sql)
        sqls = ";\n".join(sqls)
