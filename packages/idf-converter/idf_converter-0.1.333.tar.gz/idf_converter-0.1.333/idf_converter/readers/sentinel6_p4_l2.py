...
def mask_rain(rain: numpy.typing.NDArray
    ...
    rain_mask = numpy.ushort(0)
    for frain in ('rain', 'high_rain_probability from_altimeter'):
        b_rain = rain_mask + (1 << RAIN_BITS[frain])
...
def read_data(input_opts: InputOptions,
    ...
    full_granule.vars['lat'] = {'array': None,
                                'units': 'degrees north',
                                'datatype': lat.dtype,
                                'options': {}}

    full_granule.vars['lon'] = {'array': None,
                                'units': 'degrees east',
                                'datatype': lon.dtype,
                                'options': {}}
    ...
    full_granule.vars['time'] = {'array': None,
                                 'units': time_units,
    ...
    full_granule.meta['idf_subsampling_factor'] = 0
    full_granule.meta['idf_spatial_resolution'] = spatial_res_meters
    full_granule.meta['idf_spatial_resolution_units'] = 'm'

    # Split long trajectories into chunks to avoid issues in SEAScope
    slices = [slice(i, i + maxlength)
              for i in range(0, len(time), maxlength)]

    for chunk, _slice in enumerate(slices):
        granule = idf_converter.lib.create_granule(idf_version, DATA_MODEL)
        granule.vars = copy.deepcopy(full_granule.vars)
        for var_id, full_band in values.items():
            band = full_band[_slice]
            granule.vars[var_id]['array'] = band
            granule.vars[var_id]['datatype'] = band.dtype

        granule.vars['lat']['array'] = lat[_slice]
        granule.vars['lon']['array'] = lon[_slice],
        granule.vars['time']['array'] = shifted_time[_slice]
        ...
        granule.meta = copy.deepcopy(full_granule.meta)
        granule.meta['idf_granule_id'] = f'{granule_name}_chunk{1 + chunk:02d}'
        granule.meta['time_coverage_start'] = start_dt
        granule.meta['time_coverage_end'] = stop_dt

        transforms = []

        surface_mask = (surface[_slice] > 0)
        transforms.append(('static_common_mask', {'targets': channels,
                                                  'mask': surface_mask}))
        if freq_hz == 1:
            rms_mask = (rms[_slice] > 1)
            transforms.append(('static_common_mask', {'targets': channels,
                                                      'mask': rms_mask}))

            rain_mask = full_rain_mask[_slice]
