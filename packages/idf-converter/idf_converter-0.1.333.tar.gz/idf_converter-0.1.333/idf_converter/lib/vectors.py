...
def from_dir(input_opts: InputOptions,
    ...
    for target in targets:
        ...
        if is_meteo_convention:
            ...
            dir_data = numpy.mod(numpy.pi + dir_data, 2 * numpy.pi)
        ...
        try:
            ...
            dir_data = numpy.mod(1000.0 * _dir_data, 1000.0 * 2.0 * numpy.pi,
                                 where=numpy.isfinite(_dir_data))
            epsilon = numpy.finfo(dir_data.dtype).eps
            underflow_mask = ((dir_data > 0.0) & (dir_data < 1000.0 * epsilon))
            if underflow_mask.any():
                logger.warning(f'Some direction angles lower than {epsilon}'
                               ' radians have been truncated to 0 to avoid'
                               ' underflow errors')
                dir_data[numpy.where(underflow_mask)] = 0.0
            dir_data = dir_data / 1000.0
