...
from idf_converter.lib.types import Granule, ReaderResult, IdxSlices
...
DimInfo = typing.Tuple[str, bool, typing.Optional[int]]
ShiftInfo = typing.Tuple[int, typing.Optional[int], int, typing.Optional[int]]
...
def build_slices(ndims: int, time_axis: int, depth_axis: int, dim1_axis: int,
                 dim2_axis: int, time_slice: slice, depth_slice: slice,
                 dim1_reversed: bool, dim2_reversed: bool) -> IdxSlices:
    """"""
...
def get_slices_from_values(var_id: str,
                           ...
                           ) -> typing.Tuple[IdxSlices, ShiftInfo]:
    """"""
    depth_dim_id, depth_slice = depth_info
    time_dim_id, time_slice = time_info
    dim1_id, dim1_reversed, dim1_shift = dim1_info
    dim2_id, dim2_reversed, dim2_shift = dim2_info
    ...
    idx = build_slices(ndims, time_axis, depth_axis, dim1_axis, dim2_axis,
                       time_slice, depth_slice, dim1_reversed, dim2_reversed)
    shift_info = (dim1_axis, dim1_shift, dim2_axis, dim2_shift)
...
def extract_values(granule: Granule, f_handler: netCDF4.Dataset,
                   ...
                   dim1_info: typing.Tuple[str, bool, typing.Optional[int]],
                   dim2_info: typing.Tuple[str, bool, typing.Optional[int]],
                   var_id: str) -> None:
    """"""
    depth_dim_id, depth_slice = depth_info
    time_dim_id, time_slice = time_info
    ...
    # If dimension variable required reordering, then all variables must be
    # reordered along the dimension axis too
    dim1_axis, dim1_shift, dim2_axis, dim2_shift = shift_info
    if dim1_shift is not None:
        band = numpy.roll(band, dim1_shift, axis=dim1_axis)
    if dim2_shift is not None:
        band = numpy.roll(band, dim2_shift, axis=dim2_axis)
