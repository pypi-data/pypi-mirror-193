...
# Original path: Anaconda3\Lib\site-packages\cvxpy\reductions
"""
...
# CHANGE -----------------------
# added this method
def stack_vals(variables): 
	"""Stacks the values of the given variables
	"""
    value = []
    for variable in variables:
        if variable.value is not None:
            value.append(np.ravel(variable.value, 'F'))
        else:
            value.append(np.full(variable.size, np.nan))
