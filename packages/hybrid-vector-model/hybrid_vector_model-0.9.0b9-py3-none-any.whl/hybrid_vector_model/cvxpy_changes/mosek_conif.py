...
This file has been changed to implement initial conditions for the mosek
solver. The changes are marked with the keyword `CHANGE`.
# Original path: <python>\Lib\site-packages\cvxpy\reductions\solvers\conic_solvers
"""
...
class MOSEK(ConicSolver):
    ...
    def apply(self, problem):
		...
		# CHANGE --------------------------
		# handing over the variable value as potential initial guess
        if var.value is not None:
            data[s.VALUE] = var.value
		# ---------------------------------
    ...
    def solve_via_data(self, data, warm_start, verbose, solver_opts, solver_cache=None):
		...
		# CHANGE --------------------------
		# Using the provided variable value as initial guess.
		# If interior point methods are used and no variable is
		# integer, these information will not be used. This could
		# be checked for if desired
        if warm_start and s.VALUE in data:
            ...
            value = data[s.VALUE]
            value_given = np.isfinite(value)
            slice_change = value_given^np.roll(value_given,1)
            slice_change[0] = value_given[0]
            if value_given[-1]:
                slice_change[-1] = ~value_given[-2]
                slice_change = np.append(slice_change, True)
            change_indices = np.nonzero(slice_change)[0]
            assert not (change_indices.size % 2)
            for start, end in zip(change_indices[::2], change_indices[1::2]):
                task.putxxslice(sol, start, end, list(value[start:end]))
		# -----------------------------------
