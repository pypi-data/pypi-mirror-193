...
def isPointInsideMesh(vtkData, point):
    vtkPoint = createPointsVTKData(point[np.newaxis,:])
    res = arePointsPenetrating(vtkData, vtkPoint)
    return res


def findClosestPointToSurface(p, vtkData):
    locator = vtk.vtkCellLocator()
    locator.SetDataSet(vtkData)
    locator.BuildLocator()
    cellId = vtk.reference(0)
    c = [0.0, 0.0, 0.0]
    subId = vtk.reference(0)
    d = vtk.reference(0.0)
    locator.FindClosestPoint(p, c, cellId, subId, d)
    return c



def externalizePointFromMeshes(point, meshes, factor=0.2):
    p1 = point[:]
    p3 = p1[:]
    vtkPoint = createPointsVTKData(point[np.newaxis,:])
    for m in range(len(meshes)):
        res = arePointsPenetrating(meshes[m], vtkPoint)
        if res:
            print('point inside mesh %d, externalizing it ...' % m)
            p2 = findClosestPointToSurface(p1, meshes[m])
            v = p2 - p1
            p3 = p1 + (1.+factor) * v
    return p3
