...
class PackGreedilyOp(_ods_ir.OpView):
  ...
  def __init__(self, packed_op, target, gemm_packed_sizes, *, static_gemm_packed_sizes=None, gemm_inner_dims_order=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    operands.append(_get_op_result_or_value(target))
    operands.extend(_get_op_results_or_values(gemm_packed_sizes))
    _ods_context = _ods_get_default_loc_context(loc)
    if static_gemm_packed_sizes is not None: attributes["static_gemm_packed_sizes"] = (static_gemm_packed_sizes if (
        issubclass(type(static_gemm_packed_sizes), _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('DenseI64ArrayAttr')) else
          _ods_ir.AttrBuilder.get('DenseI64ArrayAttr')(static_gemm_packed_sizes, context=_ods_context))
    if gemm_inner_dims_order is not None: attributes["gemm_inner_dims_order"] = (gemm_inner_dims_order if (
        issubclass(type(gemm_inner_dims_order), _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('DenseI64ArrayAttr')) else
          _ods_ir.AttrBuilder.get('DenseI64ArrayAttr')(gemm_inner_dims_order, context=_ods_context))
...
class PadOp(_ods_ir.OpView):
  ...
  def __init__(self, transformed, target, *, padding_values=None, padding_dimensions=None, pack_paddings=None, hoist_paddings=None, transpose_paddings=None, loc=None, ip=None):
    ...
    if transpose_paddings is not None: attributes["transpose_paddings"] = (transpose_paddings if (
        ...
        not _ods_ir.AttrBuilder.contains('anonymous_409')) else
          _ods_ir.AttrBuilder.get('anonymous_409')(transpose_paddings, context=_ods_context))
    results.append(transformed)
    _ods_successors = None
    super().__init__(self.build_generic(
      attributes=attributes, results=results, operands=operands,
