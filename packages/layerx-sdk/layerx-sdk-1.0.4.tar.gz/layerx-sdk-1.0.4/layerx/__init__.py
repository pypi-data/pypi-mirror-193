...
class LayerxClient:
    ...
    def __create_dataset(
        ...
        if("isSuccess" in response):
            if(response["isSuccess"] == False):
                get_selection_id_success = False
        if get_selection_id_success == True:
            project_response = _dataset_client.create_dataset(dataset_name, response['selectionTag'], split_nfo, labels, export_types)
            print('dataset_response: ',project_response)
            return project_response
        else:
            print('project_response: ',{
                "isSuccess": False
    ...
    def __update_dataset_version(
        ...
        if("isSuccess" in response):
            if(response["isSuccess"] == False):
                get_selection_id_success = False
        if get_selection_id_success == True:
            project_response = _dataset_client.update_dataset_version(version_id, response['selectionTag'], split_info, labels, export_types, is_new_version_requried)
            print('dataset_response: ',project_response)
            return project_response
        else:
            print('project_response: ',{
    ...
    def download_project_annotations(self, project_id: str, task_status_list: list = [], is_annotated_only: bool = False, custom_download_path: str = None):
        # init dataset client
        _dataset_client = dataset.DatasetClient(self.encoded_key_secret, self.layerx_url, custom_download_path)

        # start download
        _dataset_client.download_annotations_for_project_v2(project_id, task_status_list, is_annotated_only)
    ...
    # def download_project_annotations_v2(self, project_id: str, status_list, is_annotated_only, download_path):
    #     # init studio client
    #     _studio_client = studio.StudioClient(self.encoded_key_secret, self.layerx_url)

    #     # start download
    #     _studio_client.download_annotations_for_project(project_id, status_list, is_annotated_only, download_path)
