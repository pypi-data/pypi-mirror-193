...
def get_propellant_name(Name=None, PcentL=None):
    ...
    if type(Name) == list:
        ...
        raise Exception(
            f"Blend Components NOT Recognized:  Name={str(Name)}, PcentL={str(PcentL)}"
        )
    ...
    print(f'WARNING... Blend Name NOT Recognized "{str(Name)}"')
    return Name
...
def addHYD_AmmoniaBlend(name, cea_deck):
    ...
    try:
        ...
        raise Exception(f"N2H4 un-dissociated Ammonia Blend NOT Recognized:  {name}")

    if (pcent >= 0.0) and pcent <= 100.0:
        cardL = [
            f" name = diss UA {xUA:g} H {xH:g} N {xN:g} wt%= 99.5  ",
            " h,cal=12094. t(k)=298.15 rho,g/cc = 1.004     ",
            "  ",
            " name = water H 2.0 O 1.0  wt%=0.5   ",
            " h,cal=-68308.  t(k)=298.15 rho,g/cc = 0.9998   ",
            f" !\tTHIS IS {100.0 * x:g}% AMMONIA DISSOCIATION ",
            " omit NH3 ",
        ]
...
def addPeroxideBlend(oxName, cea_deck):
    ...
    try:
        ...
        raise Exception(f"Peroxide/Water Ox Blend NOT Recognized:  {oxName}")
...
def addMMH_N2H4_Blend(fuelName, cea_deck):
    ...
    if mmhPcent > 0.0 and mmhPcent < 100.0:
        blendName = newFuelBlend(
            fuelL=["MMH", "N2H4"], fuelPcentL=[mmhPcent, 100.0 - mmhPcent]
        )
    else:
        raise Exception(f"MMH + N2H4 Blend NOT Recognized:  {fuelName}")
...
def addMON_Blend(oxName, cea_deck):
    ...
    try:
        ...
        raise Exception(f"MON Ox Blend NOT Recognized:  {oxName}")
...
def addFLOX_Blend(oxName, cea_deck):
    ...
    try:
        ...
        raise Exception(f"FLOX Ox Blend NOT Recognized:  {oxName}")
...
def giveCardNewHfAndTref(card, newHfCalPerMole, newTrefDegR):
    ...
    for sp in spL:
        ...
        if len(speL) == 2:
            if speL[0] == "h,cal":
                speL[1] = f"{newHfCalPerMole:.1f}"
            if speL[0] == "t(k)":
                speL[1] = f"{TrefDegK:.2f}"
            s = "=".join(speL)
        else:
            s = sp
        cNew += " " + s
...
def newOxBlend(oxL=None, oxPcentL=None):
    ...
    if 1:  # try:
        ...
        for i, name in enumerate(oxL):
            newName += name + f"_{oxPcentL[i]:g}"
            if i < len(oxL) - 1:
                newName += "_"

            if len(oxCards[name]) > 2:
                raise Exception(
                    "ERROR... can NOT specify ox Blend for multi propellant card= "
                    + name
                )
...
def newFuelBlend(fuelL=None, fuelPcentL=None):
    ...
    if 1:  # try:
        ...
        for i, name in enumerate(fuelL):
            newName += name + f"_{fuelPcentL[i]:g}"
            if i < len(fuelL) - 1:
                newName += "_"

            if len(fuelCards[name]) > 2:
                raise Exception(
                    "ERROR... can NOT specify Fuel Blend for multi propellant card= "
                    + name
                )
...
def newPropWithNewState(cardDict, name, newHfCalPerMole, newTrefDegR):
    ...
    """
    try:
        suffix = f"_{newHfCalPerMole:g}_{newTrefDegR:g}"
        ...
        if newName not in cardDict:
            ...
            if len(cardL) > 2:
                raise Exception(
                    "ERROR... can NOT specify new Hf, Tref for multi propellant card: "
                    + name
                )
                # return name
            newCardL = []
            for card in cardL:
                newCardL.append(
                    giveCardNewHfAndTref(card, newHfCalPerMole, newTrefDegR)
                )
...
def getOxHfCalPerMole(name):
    ...
    try:
        ...
        raise Exception(f'Could NOT find "h,cal" in getOxHfCalPerMole for {name}')
...
def makeCardForNewTemperature(ceaName="CH4", newTdegR=536.0, CpAve=0.791, MolWt=16.04):
    ...
    if ceaName in oxCards:
        ...
        raise Exception(
            f"Could NOT find ceaName={ceaName} in makeCardForNewTemperature"
        )
    ...
    if Hf == None or TrefR == None:
        raise Exception(
            "Did NOT find Heat of Formation and/or Reference Temperature in makeCardForNewTemperature"
        )
    ...
    if prop_type == "fuel":
        ...
        raise Exception(
            "Could NOT find prop_type of ceaName=%s in makeCardForNewTemperature"
            % ceaName
        )
...
def renamePropIfNewHfOrTrefInName(cardDict, name):
    ...
    if len(spL) > 1:
        ...
        if dictName in cardDict:
            ...
            if len(nL) != 3:
                s = (
                    f'ERROR in adjusted propellant format:\n    "{name}"\n'
            ...
            try:
                ...
                newName = newPropWithNewState(
                    cardDict, dictName, newHfCalPerMole, newTrefDegR
                )
                return newName
            except:
                s = (
