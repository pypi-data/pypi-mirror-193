...
class ModelWindow(Model):
    ...
    def logprob_mode(self, params: jnp.ndarray,
        ...
        elif self.dist == 'NewDist':
            lp = dist.LeftTruncatedNewDist.logprob
            lp = lp(data, r, p, self.left)
...
class ModelWindowRec(ModelWindow):
    ...
    def logprob_mode(self, params: jnp.ndarray,
        ...
        if self.dist == 'NB':
            ...
            lp = dist.LeftTruncatedNewDist.logprob_recurrent
            lp = lp(ref, r, p, self.left, self.cur_max_count)
        elif self.dist == 'BetaNB':
            b = self.get_param('b_k', params)
            mu = self.get_param('mu_k', params)
            k = self.calc_k(alt, b, mu)
            lp = dist.LeftTruncatedBetaNB.logprob_recurrent
            lp = lp(data, p, k, r, self.left)
        return lp
