...
@partial(jit, static_argnames=['n', 'eps'])
def _logbetainc(p, q, x, n=100, eps=1e-12):
    ...
    def lentz_cond(prev):
        return (jnp.abs(1 - prev[-2] * prev[-3]) > eps) & (prev[-1] <= n)
    f = x / (1.0 - x)
    b = betaln(p, q)
...
@partial(jit, inline=True, static_argnames=['n'])
def _betainc(p, q, x, n=20):
    return jnp.exp(_logbetainc(p, q, x, n=n))


@partial(jit, inline=True, static_argnames=['n'])
def _betaincc(p, q, x, n=20):
    return -jnp.expm1(_logbetainc(q, p, 1.0 - x, n=n))


@partial(jit, inline=True, static_argnames=['n'])
def _logbetaincc(p, q, x, n=20):
    return jnp.log(_betaincc(p, q, x, n=n))


@partial(jit, static_argnames=['n'])
@jnp.vectorize
def logbetainc(p, q, x, n=20):
    c = q < (1 - x) / x * p
    return cond(c, partial(_logbetainc, n=n), partial(_logbetaincc, n=n), p, q, x)


@partial(jit, static_argnames=['n'])
@jnp.vectorize
def betainc(p, q, x, n=2000):
    c = q < (1 - x) / x * p # 4 * (0.5 - x)
    return cond(c, partial(_betainc, n=n), partial(_betaincc, n=n), p, q, x)
