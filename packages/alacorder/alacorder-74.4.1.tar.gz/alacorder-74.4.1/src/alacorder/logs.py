...
def complete(conf, start_time, output=None):
	...
	if print_log:
		click.secho(f'''TASK COMPLETED: Successfully processed {max_cases} cases. Last batch completed in {elapsed:.2f} seconds ({cases_per_sec:.2f} cases/sec)''',bold=True,fg='green')
def debug(conf, *msg):
	if conf.DEBUG == True:
		click.echo(msg)
		return msg
