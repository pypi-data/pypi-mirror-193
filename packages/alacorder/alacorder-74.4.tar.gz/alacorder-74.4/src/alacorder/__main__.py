...
def cli(input_path, output_path, count, table, append,overwrite, launch, dedupe, log, no_write, no_prompt, debug, no_batch):
	...
	cf = config.set(cin, cout, count=count, table=table, overwrite=overwrite, append=append, launch=launch, log=log, dedupe=dedupe, warn=warn, no_write=no_write, no_prompt=no_prompt, no_batch=no_batch, debug=debug)
	...
	if show_options:
		if not click.confirm("Continue without changing settings? "):
			...
			cf = config.set(cin, cout, count=count, table=table, overwrite=overwrite, append=append, launch=launch, log=log, dedupe=dedupe, warn=warn, no_write=no_write, no_prompt=no_prompt, debug=debug, no_batch=no_batch, skip_echo=True)
	...
	if not overwrite and os.path.isfile(output_path) and not cout.IS_APPENDABLE:
		...
		overwrite = True
		cf = config.set(cin, cout, count=count, table=table, overwrite=overwrite, append=append, launch=launch, log=log, dedupe=dedupe, warn=warn, no_write=no_write, no_prompt=no_prompt, no_batch=no_batch, debug=debug, skip_echo=True)
