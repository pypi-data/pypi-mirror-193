...
def cases(conf):
    ...
    with click.progressbar(batches) as bar:
        for i, c in enumerate(bar):
            ...
            # write     
            if appendtable and temp_no_write_tab == False and (i % 5 == 0 or i == len(batches) - 1):
                if type(old_table) == list:
                    appcase = [cases, old_table[0]]
                    appcharge = [charges, old_table[1]]
                    appfees = [fees, old_table[2]]
                    try:
                        cases = pd.concat(appcase,axis=0)
                        fees = pd.concat(appfees,axis=0)
                        charges = pd.concat(appcharge,axis=0)
                    except:
                        pass
                else:
