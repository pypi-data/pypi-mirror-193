...
TEXT_MODELS = ["text-ada-001", "text-babbage-001", "text-curie-001", "text-davinci-003"]
DEFAULT_SETTING = {"memory_length": 3, "max_token": 200, "ai_temperature": 75, "bot_active": 1, "level": 2,
                   "model": TEXT_MODELS[2], "daily_limit": 50, "admin_limit": 50, "password": ""}


class Engine(object):

    def __init__(self, telegram_token: str = None, bot_name: str = "Bot-GPT"):
        # internal variables
        self.name = bot_name
        self.ver = pkg_resources.get_distribution("telegram-bot-gpt").version
        self.db = "bot_gpt.db"

        db, cr = self.get_data("chats")
        self.num_chats = len(cr.fetchall())
        ...
        if self.telegram_token is not None:
            try:
                ...
                print("ERROR: Telegram token cannot be used, bot initialization failed.")
                exit()
    ...
    def set_level(self, level: int = 2):
        level = min(max(level, 0), 3)
        self.set(["level", "model"], [level, TEXT_MODELS[level]])
        return level
    ...
    def get_data(self, table: str, col: str = "id", param: str = ""):
        db = sqlite3.connect(self.db, check_same_thread=False)
        cr = db.cursor()

        if len(col) > 0:
            try:
                cr.execute(f"SELECT {col} FROM {table}{' WHERE ' if len(param) > 0 else ''}{param}")
            except:
                cr.execute("CREATE TABLE IF NOT EXISTS users "
                           "(id INTEGER, reg TEXT, uid TEXT, unick TEXT, daily_allowed TINYINT, adm TINYINT)")
                cr.execute("CREATE TABLE IF NOT EXISTS chats (id INTEGER, reg TEXT, uid TEXT, inp TEXT, res TEXT)")
                cr.execute("CREATE TABLE IF NOT EXISTS setting (var TEXT primary key, val TEXT)")
    ...
    def start(self, uid, unick: str = None):
        ...
        db, cr = self.get_data("users", "uid")
        uids = [c[0] for c in cr.fetchall()]
        ...
        if str(uid) not in uids:
            ...
            query = f"INSERT INTO users(id, reg, uid, unick, daily_allowed, adm) " \
                    f"VALUES({len(uids) + 1}, '{server_time}', '{uid}', '{unick}', {limit}, {i_am_admin})"
            cr.execute(query)
            db.commit()
    ...
    def command(self, message):
        ...
        if uid in self.admin:
            if inp == "today":
                ...
                db, cr = self.get_data("chats", col="uid", param=f"reg LIKE '{time[:10]}%'")
                uids = [c[0] for c in cr.fetchall()]
                db.close()
                ...
                if len(uids) == 0:
                    ...
                    uids_set.sort()
                    text = "\n".join(f"`{u}: {uids.count(u)}`" for u in uids_set)
                ...
                if re.match(r"^(get_)?password$", inp):
                    ...
                    text = "To create a new one, send /new\_password\n\n" \
                           "Use /reset\_password to change it and to force all users re-entering the new one."
    ...
    def respond(self, message):
        ...
        if uid not in self.admin:
            ...
            if is_allowed < 1:
                ...
                if inp == self.setting["password"]:
                    ...
                    db.commit()
                    text = f"🙂 _Welcome to {self.name}, what do you have in mind?_"
                    ret = "ok"
        ...
        db, cr = self.get_data("chats", col="inp, res", param=f"uid='{uid}' AND reg LIKE '{time[:10]}%'")
        ...
        words = re.findall("[A-Za-z0-9-']+", prompt)
        mem = self.setting["memory_length"] if len(words) <= 3 else 1 if len(words) <= 9 else 0
        if inp.count(" ") < 3 and len(rows) > 0:
            memory = ["\n\n\n".join(r) for r in rows[-mem:]]
            memory = "\n\n\n".join(memory)
            prompt = f"{memory}\n\n\n{inp}"
        ...
        if len(answer) > 2:
            split = [ans for ans in answer.split("\n\n") if ans != inp]
            if split[0] == split[0].lower():
                if len(split) > 1:
                    self.bot.send_message(uid, f"_... {split[0]}{'?' if split[0][-1] not in '.?!' else ''}_",
                                          parse_mode="Markdown")
                    split = split[1:]
                else:
                    split[0] = "... " + split[0]
        ...
        if len(response.replace("\n", "")) < 2:
            response = random.choices(["OK", "Cool", "I see", "Nice", "Hmmm"])[0] + "."

        self.bot.send_message(uid, response, parse_mode="Markdown")

        esc_inp = inp.replace("'", "''")
        esc_res = response.replace("'", "''")
        self.num_chats += 1
        query = f"INSERT INTO chats(id, reg, uid, inp, res) " \
                f"VALUES({self.num_chats}, '{time}', '{uid}', '{esc_inp}', '{esc_res}')"
        cr.execute(query)
        db.commit()
    ...
    def send_error_report(self, uid, report, send=True):
        ...
        with open(pkg_resources.resource_filename(__name__, "alerts.json"), "r", encoding="utf-8") as f:
            errors = json.loads(f.read())["errors"]
            for key in errors:
...
def get_text_file(txt_file: str):
    with open(pkg_resources.resource_filename(__name__, txt_file), "r", encoding="utf-8") as f:
        return f.read()


def help_dev():
    print(get_text_file("help_dev.txt"))
