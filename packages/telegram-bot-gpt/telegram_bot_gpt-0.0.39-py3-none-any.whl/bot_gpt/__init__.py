...
with open(pkg_resources.resource_filename(__name__, "setting.json"), "r", encoding="utf-8") as f:
    DEFAULT_SETTING = json.loads(f.read())
...
class Engine(object):
    ...
    def respond(self, message):
        ...
        if len(check) < 2:
            empty = random.choices(self.setting["empty_response"].split(";"))[0]
            response = empty.strip() + "."
        elif check == "N/A":
            no_idea = random.choices(self.setting["no_idea_response"].split(";"))[0]
            response = no_idea.strip() + "."
...
def help_dev(what="full"):
    text = get_text_file("help_dev.txt")
    if what == "Telegram":
        text = text.split("\n\n")[-3]
    elif what == "OpenAI":
        text = text.split("\n\n")[-2]
