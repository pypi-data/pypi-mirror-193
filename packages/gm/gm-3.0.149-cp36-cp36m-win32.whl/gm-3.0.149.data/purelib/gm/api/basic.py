...
from gm.api.errors import error_dict
...
def run(strategy_id='', filename='', mode=MODE_UNKNOWN, token='',
    ...
    backtest_initial_cash = convert(options.backtest_initial_cash, float, backtest_initial_cash)
    backtest_transaction_ratio = convert(options.backtest_transaction_ratio, float, backtest_transaction_ratio)
    backtest_commission_ratio = convert(options.backtest_commission_ratio, float, backtest_commission_ratio)
    backtest_commission_unit = convert(options.backtest_commission_unit, float, backtest_commission_unit)
    backtest_slippage_ratio = convert(options.backtest_slippage_ratio, float, backtest_slippage_ratio)
    backtest_marginfloat_ratio1 = convert(options.backtest_marginfloat_ratio1, float, backtest_marginfloat_ratio1)
    backtest_marginfloat_ratio2 = convert(options.backtest_marginfloat_ratio2, float, backtest_marginfloat_ratio2)
    ...
    backtest_check_cache = convert(options.backtest_check_cache, int, backtest_check_cache)
    serv_addr = options.serv_addr
    backtest_match_mode = convert(options.backtest_match_mode, int, backtest_match_mode)
    ...
    init_code = gmi_init()
    if init_code in error_dict:
        gmsdklogger.warn("error code={}, error info={}".format(init_code, error_dict[init_code]))
        return
    elif init_code != 0:
        gmsdklogger.warn("error code={}, error info={}".format(init_code, "未知错误码"))
        return
