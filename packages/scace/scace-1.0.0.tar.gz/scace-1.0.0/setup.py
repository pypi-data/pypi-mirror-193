import os
from setuptools import setup, find_packages

path = os.path.abspath(os.path.dirname(__file__))

try:
    with open(os.path.join(path, 'README.md'), "r", encoding='utf-8') as f:
        long_description = f.read()
except Exception as e:
    long_description = "single-cell RNA-sequencing automatic clustering based on cluster merging"

setup(
    name="scace",
    version="1.0.0",
    keywords=["single-cell RNA-sequencing", "clustering", "cluster merging"],
    description="single-cell RNA-sequencing automatic clustering based on cluster merging",
    long_description=long_description,
    long_description_content_type='text/markdown',
    license="MIT Licence",

    url="https://github.com/sldyns",
    author="Kun Qian, Xinwei He",
    author_email="kun_qian@foxmail.com",
    maintainer="Kun Qian",
    maintainer_email="kun_qian@foxmail.com",

    packages=find_packages(),
    include_package_data=True,
    install_requires = [
        "numpy",
        "scanpy",
        "torch",
        "pandas",
        "tqdm",
        "scipy",
        "sklearn"
        ],
    platforms='any'
)
