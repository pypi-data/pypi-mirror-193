...
LastEditTime: 2022-12-27 16:54:20
Description: NULL
...
def model_para(model: nn.Module, datasize: List, depth=3):
    ...
    print(
        f'Output size of the model: {out.shape}(First dimension is batch_size)')
...
# model_para(unet.UNet(spatial_dims=3, in_channels=3, out_channels=1, channels=(
#     6, 12, 24), strides=(1, 1, 1), num_res_units=3), datasize=[2, 3, 10, 10, 40], depth=5)
# model_para(VST_Direc(), datasize=[2, 3, 40, 10, 10], depth=10)
# model_para(MLP3(),datasize=[3,1,17,17])
# model_optim()
# model=MLP3()
# print(model._get_name())
# print(f'Total Time used: {time.time()-start}s')
# model_para(ResMax2(), datasize=(2, 3, 10, 10, 40))
# model_para(TRIP(), datasize=[2, 3, 10, 10, 40])
# del_pycache()
summary(MLP3(),input_data=torch.rand(2,1,17,17))
