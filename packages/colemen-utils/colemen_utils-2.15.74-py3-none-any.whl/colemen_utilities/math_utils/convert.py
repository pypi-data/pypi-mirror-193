...
def pretty_bytes(value,decimals:int=None):
    results = []
    print(f"value: {value}")
    if value < 1000:
        return f"{value}{_digital_storage['kilobytes']}"
    if 1000 < value < 1000000:
        return bytes_to_kilobytes(value,pretty=True,decimal=decimals)
        # return f"{round(value,decimals)}{_digital_storage['kilobytes']}"
    if 1000000 < value < 1000000000:
        return bytes_to_megabytes(value,pretty=True,decimal=decimals)
        # return f"{round(value,decimals)}{_digital_storage['megabytes']}"
    if 1000000000 < value < 1000000000000:
        return bytes_to_gigabytes(value,pretty=True,decimal=decimals)
        # return f"{round(value,decimals)}{_digital_storage['gigabytes']}"
    if 1000000000000 < value < 1000000000000:
        return bytes_to_terabytes(value,pretty=True,decimal=decimals)
