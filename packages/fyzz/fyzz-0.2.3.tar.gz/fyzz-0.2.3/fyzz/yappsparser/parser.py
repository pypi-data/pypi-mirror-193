...
class FyzzParserPrefixError(FyzzParserError):
    ...
    def __str__(self):
        return "Invalid prefix {!r} should be in {}".format(
...
class FyzzScanner(runtime.Scanner):
    patterns = [
        ...
        (
            "BLANK_NODE_LABEL",
            re.compile("_:[A-Za-z_0-9]([A-Za-z0-9_.-]*[A-Za-z0-9_-])?"),
        ),
...
class Fyzz(runtime.Parser):
    ...
    def Prologue(self, R, _parent=None):
        ...
        while (
            self._peek(
                "PREFIX", "SELECT", "CONSTRUCT", "DESCRIBE", "ASK", context=_context
            )
            == "PREFIX"
        ):
    ...
    def SelectQuery(self, R, _parent=None):
        ...
        R.type = "select"
        if self._peek(
            "DISTINCT", "REDUCED", "STAR", "VAR1", "VAR2", context=_context
        ) in ["DISTINCT", "REDUCED"]:
    ...
    def DescribeQuery(self, R, _parent=None):
        ...
        _token = self._peek(
            "STAR", "VAR1", "VAR2", "IRI_REF", "PNAME_NS", context=_context
        )
        if _token != "STAR":
            while 1:
                ...
                if self._peek(
                    ...
                    "RPAREN",
                    context=_context,
                ) not in ["VAR1", "VAR2", "IRI_REF", "PNAME_NS"]:
                    break
        else:  # == 'STAR'
            STAR = self._scan("STAR", context=_context)
            R.selected.append("*")
    ...
    def GroupGraphPattern(self, R, _parent=None):
        ...
        if self._peek(
            ...
            "DOUBLE_NEGATIVE",
            context=_context,
        ) not in ["RBRACK", "FILTER", "OPTIONAL", "LBRACK", "GRAPH"]:
            TriplesBlock = self.TriplesBlock(R, _context)
        triples.extend(locals().get("TriplesBlock", []))
        while (
            self._peek(
                "RBRACK", "FILTER", "OPTIONAL", "LBRACK", "GRAPH", context=_context
            )
            != "RBRACK"
        ):
            _token = self._peek(
                "FILTER", "OPTIONAL", "LBRACK", "GRAPH", context=_context
            )
            if _token != "FILTER":
    ...
    def PropertyListNotEmpty(self, R, _parent=None):
        ...
        while (
            ...
            if self._peek(
                ...
                "GRAPH",
                context=_context,
            ) in ["A", "VAR1", "VAR2", "IRI_REF", "PNAME_NS"]:
                Verb = self.Verb(R, _context)
                ObjectList = self.ObjectList(R, _context)
                propertylist.extend((Verb, obj) for obj in ObjectList)
        return propertylist
    ...
    def Verb(self, R, _parent=None):
        ...
        _token = self._peek(
            "A", "VAR1", "VAR2", "IRI_REF", "PNAME_NS", context=_context
        )
        if _token != "A":
            VarOrIRIref = self.VarOrIRIref(R, _context)
            return VarOrIRIref
    ...
    def NumericLiteralPositive(self, _parent=None):
        ...
        _token = self._peek(
            "INTEGER_POSITIVE", "DECIMAL_POSITIVE", "DOUBLE_POSITIVE", context=_context
        )
    ...
    def NumericLiteralNegative(self, _parent=None):
        ...
        _token = self._peek(
            "INTEGER_NEGATIVE", "DECIMAL_NEGATIVE", "DOUBLE_NEGATIVE", context=_context
        )
