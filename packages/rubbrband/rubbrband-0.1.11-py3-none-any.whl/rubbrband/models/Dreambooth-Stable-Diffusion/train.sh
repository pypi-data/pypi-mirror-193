...
docker run --name rb-dreambooth-stable-diffusion --gpus all -it -v $(pwd)/sd-v1-4-full-ema.ckpt:/home/engineering/sd-v1-4-full-ema.ckpt -v ${dataset_dir}:${dataset_dir} -d rubbrband/dreambooth-stable-diffusion:latest
...
--actual_resume /home/engineering/sd-v1-4-full-ema.ckpt -n Experiment --gpus 1 \
 --data_root ${dataset_dir} --reg_data_root outputs/txt2img-samples/samples  --class_word ${class_name} --no-test -l ${logdir};"
