...
db = {}
temp = csv.DictReader(open("db.csv", "r"))
...
def launch(model):
    ...
    with yaspin() as sp:
        ...
        image_name = f"rubbrband/{item['model'].lower()}"
...
def train(model, dataset_dir):
    ...
    container = client.containers.get(f"rb-{model.lower()}")

    if container.status != "running":
        container.start()

    with yaspin():
        subprocess.run(["chmod", "a+x", f"models/{item['model']}/train.sh"])
        subprocess.run(["/bin/bash", "-c", f"models/{item['model']}/train.sh"])
