...
def get_price_data(pd_file):
    if pd_file is None:
        soup = BeautifulSoup(requests.get('http://107.175.127.29/history/').text, 'html.parser')
        return requests.get(sorted(['http://107.175.127.29/history/' + node.get('href')
                                    for node in soup.find_all('a') if node.get('href').endswith('json')])[-1]).json()
    else:
        return requests.get(f"http://107.175.127.29/history/{pd_file}").json()
