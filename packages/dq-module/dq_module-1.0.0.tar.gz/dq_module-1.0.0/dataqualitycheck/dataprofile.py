...
class DataProfile():
    def __init__(self, tables_list, interaction_between_tables, data_read_ob, data_write_ob, data_right_structure, job_id, time_zone=None, no_of_partition=4, output_db_name="data_quality_output", run_engine='pyspark'):
        ...
        if self.run_engine.lower() == 'polars':
            self.data_profiling_schema = [("job_id", pl.Utf8), ("source_type", pl.Utf8), ("layer", pl.Utf8), ("source_name", pl.Utf8), ("filename", pl.Utf8), ("column_name", pl.Utf8), ("column_type", pl.Utf8), ("total_column_count", pl.Utf8), ("total_row_count", pl.Utf8), ("min", pl.Utf8), (
                "max", pl.Utf8), ("avg", pl.Utf8), ("sum", pl.Utf8), ("stddev", pl.Utf8), ("25th_per", pl.Utf8), ("50th_per", pl.Utf8), ("75th_per", pl.Utf8), ("missing_count", pl.Utf8), ("unique_count", pl.Utf8), ("mode", pl.Utf8), ("list_of_uniques", pl.Utf8), ("run_date", pl.Utf8)]
        else:
    ...
    def recommanded_data_quality_rules(self, data_profiling_report, columns_to_be_excluded):    
        try:
            ...
            for key, value in itertools.groupby(data_profiling_rows, key= operator.itemgetter("source_type","layer", "source_name", "filename")):
                ...
                "schema_check",column_names.strip(','), column_types.strip(','), date_format_config, date_formate_dict, "Mandatory", "1",
            ...
            if self.data_right_structure == 'file':
                report_folder_path = f"{data_profiling_rows[0]['output_folder_structure']}data_profiling/recommanded_data_quality_rules/"
    ...
    def dq_rules_df(self, data_quality_rules):
        if self.run_engine.lower() == 'polars':
            schema = [("container_name", pl.Utf8), ("source_type", pl.Utf8), ("layer", pl.Utf8), ("source_name", pl.Utf8), ("filename", pl.Utf8), 
                    ("read_connector_method", pl.Utf8), ("rule_name", pl.Utf8), ("column_to_be_checked", pl.Utf8), ("value", pl.Utf8), ("date_column_config", pl.Utf8),
                    ("date_format_dictionary", pl.Utf8), ("ruletype", pl.Utf8), ("active", pl.Utf8), ("latest_file_path", pl.Utf8), ("output_folder_structure", pl.Utf8)
                    ]
            data_quality_rules_df = pl.DataFrame(data=data_quality_rules, schema=schema)
        else:
            schema = StructType([StructField("container_name", StringType(), True),
                            ...
                            StructField("column_to_be_checked",StringType(), True),
                            StructField("value",StringType(), True),
                            StructField("date_column_config",StringType(), True),
                            StructField("date_format_dictionary",StringType(), True),
    ...
    def data_qulity_rule_on_column(self, data_profiling_row, column_type):
        ...
        if column_type == 'categorical':
            ...
            new_rules.append((data_profiling_row["container_name"], data_profiling_row["source_type"], data_profiling_row["layer"], data_profiling_row["source_name"],
            data_profiling_row["filename"], data_profiling_row["read_connector_method"], 
            "allowed_values_check",data_profiling_row["column_name"], data_profiling_row["mode"], "", "", "Mandatory", "1",
            data_profiling_row["latest_file_path"], data_profiling_row["output_folder_structure"]))
        if column_type == 'Date':
            # new_rules.append((data_profiling_row["container_name"], data_profiling_row["source_type"], data_profiling_row["layer"], data_profiling_row["source_name"],
            # data_profiling_row["filename"], data_profiling_row["read_connector_method"], 
        ...
        new_rules.append((data_profiling_row["container_name"], data_profiling_row["source_type"], data_profiling_row["layer"], data_profiling_row["source_name"],
            data_profiling_row["filename"], data_profiling_row["read_connector_method"], 
            "unique_record_check","", "", "", "", "Mandatory", "1",
            data_profiling_row["latest_file_path"], data_profiling_row["output_folder_structure"]))
        return new_rules
