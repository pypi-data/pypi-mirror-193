...
async def get_awsaccount(request: web.Request) -> web.Response:
    ...
    """
    account_dict, volume_id = await _get_account_by_id(request)
...
async def get_awsaccount_by_name(request: web.Request) -> web.Response:
    ...
    responses:
      ...
      '404':
        $ref: '#/components/responses/404'
    """
    account_dict, volume_id = await _get_account_by_name(request)
    request.match_info['volume_id'] = volume_id
    return await response.get(request, account_dict)
...
async def get_awsaccounts(request: web.Request) -> web.Response:
    ...
    responses:
      '200':
        $ref: '#/components/responses/200'
    """
    aws_account_dicts = await _get_all_accounts(request)
...
async def get_new_bucket_form(request: web.Request) -> web.Response:
    ...
    responses:
      ...
      '404':
        $ref: '#/components/responses/404'
    """
    account_dict, _ = await _get_account_by_id(request)
    return await response.get(request, account_dict)
...
async def post_new_bucket_by_volume_id(request: web.Request) -> web.Response:
    ...
    """
    logger = logging.getLogger(__name__)
    sub = request.match_info.get('SUB', None)
    volume_id = request.match_info['volume_id']
...
async def _account_opener(request: Request, volume_id: str) -> Response:
    ...
    """
    result = await awsservicelib.get_account(request, volume_id)
    if result is None:
...
async def _get_account(request: Request, volume_id: str) -> Response:
    ...
    """
    aws_object_dict = await awsservicelib.get_account(request, volume_id)
    return await response.get(request, aws_object_dict)


async def _get_account_by_id(request: web.Request) -> tuple[DesktopObjectDict | None, str | None]:
    """
    ...
    account, volume_id = next((a for a in await gather(
        *[get_account_and_volume_id(awsservicelib.get_account(request, v.id), v.id) async for v in
          request.app[HEA_DB].get_volumes(request, AWSFileSystem)])
                               if a[0]['id'] == request.match_info['id']), (None, None))
    return account, volume_id


async def _get_account_by_name(request: web.Request) -> tuple[DesktopObjectDict | None, str | None]:
    """
    Gets an account by its id and the account's volume id. The id is expected to be the request object's match_info
    mapping, with key 'id'.
...
async def _get_all_accounts(request: web.Request) -> list[DesktopObjectDict]:
    """
    ...
    return [a for a in await gather(
        *[awsservicelib.get_account(request, v.id) async for v in request.app[HEA_DB].get_volumes(request, AWSFileSystem)])]
