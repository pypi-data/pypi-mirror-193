...
from clusterondemand.wait_helpers import wait_for_cmdaemon_to_start
...
CMDAEMON_WAIT_TIMEOUT = 2 * 60  # 2 minutes
...
def run_command():
    ...
    for head_vm in inactive_head_vms:
        external_ip = cluster_ips[head_vm.name]
        clientutils.unmount_ovf_environment_image(head_vm, external_ip)
        _ = wait_for_cmdaemon_to_start(external_ip, CMDAEMON_WAIT_TIMEOUT)
