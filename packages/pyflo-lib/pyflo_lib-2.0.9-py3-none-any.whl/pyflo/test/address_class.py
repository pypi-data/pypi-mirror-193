...
# from pyflo.transaction import *
from pyflo import OPCODE
...
class AddressClassTests(unittest.TestCase):
    ...
    def test_is_WIF_valid(self):
        # mainnet
        self.assertEqual(pyflo.PrivateKey("7B56E2B7BD189F4491D43A1D209E6268046DF1741F61B6397349D7AA54978E76",
        ...
        self.assertEqual(pyflo.PrivateKey("L1MU1jUjUwZ6Fd1L2HDZ8qH4oSWxct5boCQ4C87YvoSZbTW41hg4",
                                            compressed=False, testnet=True).wif,
                         'L1MU1jUjUwZ6Fd1L2HDZ8qH4oSWxct5boCQ4C87YvoSZbTW41hg4')

        p = pyflo.PrivateKey("L1MU1jUjUwZ6Fd1L2HDZ8qH4oSWxct5boCQ4C87YvoSZbTW41hg4")
        pub = pyflo.PublicKey(p)
        a = pyflo.Address(p)
        self.assertEqual(a.address, 'bc1qxsms4rt5axt9674du2az7vq3pvephu3k5jyky8')
        a = pyflo.Address(p, address_type = "P2PKH")
        self.assertEqual(a.address, '15m65JmFohJiioQbzMWhqFeCS3ZL1KVaNh')
        a = pyflo.Address(p, address_type = "PUBKEY")
        self.assertEqual(a.address, '15m65JmFohJiioQbzMWhqFeCS3ZL1KVaNh')
        a = pyflo.Address(p, address_type = "P2SH_P2WPKH")
        ...
        self.assertEqual(pyflo.PublicKey("7b56e2b7bd189f4491d43a1d209e6268046df1741f61b6397349d7aa54978e76",
        ...
        self.assertEqual(pyflo.PublicKey("L1MU1jUjUwZ6Fd1L2HDZ8qH4oSWxct5boCQ4C87YvoSZbTW41hg4",
                                           compressed=False).hex, cpk)
        # public key compressed from  PrivateKey instance (flags have no effect)
        p = pyflo.PrivateKey("L1MU1jUjUwZ6Fd1L2HDZ8qH4oSWxct5boCQ4C87YvoSZbTW41hg4")
        self.assertEqual(pyflo.PublicKey(p, compressed=False).hex, cpk)

        # public key compressed from  public
        self.assertEqual(pyflo.PublicKey(cpk, compressed=False).hex, cpk)

        # public key compressed from  public
        self.assertEqual(pyflo.PublicKey(unhexlify(cpk), compressed=False).hex, cpk)

        # compressed public key
        # private key hex string to compressed pub key
        p = pyflo.PrivateKey("7b56e2b7bd189f4491d43a1d209e6268046df1741f61b6397349d7aa54978e76", compressed=False)
        pub = pyflo.PublicKey(p)
        a = pyflo.Address(p, address_type="P2PKH")
        ...
        # from pubkey
        p = pyflo.PublicKey('02a8fb85e98c99b79150df12fde488639d8445c57babef83d53c66c1e5c818eeb4')
        a = pyflo.Address(p)
        self.assertEqual(a.address, 'bc1qxsms4rt5axt9674du2az7vq3pvephu3k5jyky8')
        a = pyflo.Address(p, address_type="P2PKH")
        self.assertEqual(a.address, '15m65JmFohJiioQbzMWhqFeCS3ZL1KVaNh')
        ...
        p = pyflo.PublicKey('04a8fb85e98c99b79150df12fde488639d8445c57babef83d53c66c1e5c818eeb43bbd96a641808'
        ...
        a = pyflo.ScriptAddress(redeem, witness_version=None)
