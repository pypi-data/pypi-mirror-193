...
def LegacyRWKV(ops: module, *args):
    class myRWKV(ops.module):
        ...
        #     def body(pqo, t):
        #         # wc, uc, kc, vc = w[c], u[c], k[:,c], v[:, c]
        #         kt, vt = k[t, :], v[t, :]
        #         p, q, o = pqo

        #         no = ops.maximum(o, ops.add(u, kt))
        #         A = ops.exp(ops.subtract(o, no))
        #         B = ops.exp(ops.subtract(ops.add(u, kt), no))
        #         y = ops.divide(ops.add(ops.multiply(A, p), ops.multiply(
        #             B, vt)), ops.add(ops.multiply(A, q), B))

        #         no = ops.maximum(ops.add(w, o), kt)
        #         A = ops.exp(ops.subtract(ops.add(w, o) - no))
        #         B = ops.exp(ops.subtract(kt, no))
        #         p = ops.add(ops.multiply(A * p), ops.multiply(B * vt))
        #         q = ops.add(ops.multiply(A * q), B)
        #         o = no
        #         return ops.stack([p, q, o]), y
        #     p = q = ops.initTensor([0]*C)
        #     o = ops.initTensor([-1e30]*C)
        #     return jax.lax.scan(body, ops.stack([p, q, o]), ops.initTensor(list(range(T))))
