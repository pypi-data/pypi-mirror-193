...
class StringValidator(Validator):
    ...
    __empty = ForceType(bool)
    __black = ForceType(bool)

    def __init__(self, minsize: Optional[int] = None, maxsize: Optional[int] = None, prefix: Optional[str] = None,
                 suffix: Optional[str] = None, predicate: Optional[Callable] = None, empty: bool = False,
                 black: bool = False):
        self.__min = minsize
        self.__max = maxsize
    ...
    def validate(self, value):
        ...
        if self.__black and StringUtils.is_not_black(value):
            raise ValidatorException(f"Excepted black, but got a '{value}'")
        if self.__empty and StringUtils.is_not_empty(value):
            raise ValidatorException(f"Excepted empty, but got a '{value}'")
    ...
    def __init__(self, *types: type):
        ...
        for t in types:
            if not isinstance(t, type):
                raise ValidatorException(f"Excepted 'type' object, got a '{t}' from {types}")
            self.__type_names.append(t.__name__)

    def validate(self, value):
        value_type = type(value)
        if not issubclass(value_type, tuple(self.__types)):
            raise ValidatorException(f"Expected \"{self.__type_names}\", got a '{value_type.__name__}'")
...
class BoolValidator(Validator):
    ...
    def validate(self, value):
        if self.__true:
            if not value:
                raise ValidatorException(f"Excepted true, but false.")
        else:
            if value:
                raise ValidatorException(f"Excepted false, but true.")
...
class DatetimeValidator(Validator):
    ...
    datetime validator
    """
    __format = ForceType(str)
    __future = ForceType(bool)
    __past = ForceType(bool)
    ...
    def validate(self, value):
        ...
        if isinstance(value, datetime):
            ...
            if not self.__format:
                raise ValidatorException(f"datetime format is must required.")
        ...
        if self.__future and difference < 0:
            raise ValidatorException(f"Excepted datetime is after, got a before datetime: {value}")
        if self.__past and difference > 0:
            raise ValidatorException(f"Excepted datetime is before, got a after datetime: {value}")
