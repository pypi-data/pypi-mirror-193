...
from typing import List, Any, Callable, Tuple, Dict, Union, Type, Optional

from ._process import _do
from .._internal._method_helper import func_full_args
from ..classes import ForceType, StringValidator, CompareValidator, MemberValidator, Validator, ValidatorExecutor
...
class Valid(object):
    ...
    def __init__(self, name: str, compare: Optional[CompareValidator] = None, string: Optional[StringValidator] = None,
                 ...
                 types: Tuple[Type] = None, validators: Tuple[Validator] or ValidatorExecutor = None):
        """
...
def validate(conditions: List[Valid]):
    """
    ...
    Example:
        ...
        class Person:
            ...
            @validate(conditions=[Valid(name="name", contained=MemberValidator(in_=["Jerry", "Tom"]))])
        ...
        class Person:
            ...
            @validate(conditions=[Valid(name="name", compare=CompareValidator(le=10, ge=1))])
            def __init__(self, age, name):
                self.age = age
                self.name = name
