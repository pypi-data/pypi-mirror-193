...
def around(before: Tuple[Callable] or Callable = None, after: Tuple[Callable] or Callable = None,
    ...
    all callback functions can and only support communication via the chain keyword parameter.
    example:
        callback() is ok,
        callback(chain=None) is ok,
        callback(chain=None, other=None) is ok(other arg will not be assigned),
    ...
    For example:
        ...
        class AroundTest(object):
            ...
            @around(before=[setup_module, hook.setup_static, hook.setup_class, hook.setup_instance, setup_static,
                    ...
                    after=[teardown_instance, teardown_class, teardown_static, hook.teardown_instance,
                           hook.teardown_class,
            ...
            @staticmethod
            @around(before=[setup_module, hook.setup_static, hook.setup_class, hook.setup_instance, setup_static],
                    after=[teardown_static, hook.teardown_instance, hook.teardown_class,hook.teardown_static,
                    teardown_module])
            ...
            @around(before=[setup_module, hook.setup_static, hook.setup_class, hook.setup_instance, setup_static,
                    setup_class],
                    after=[teardown_class, teardown_static, hook.teardown_instance, hook.teardown_class,
                    hook.teardown_static, teardown_module])
