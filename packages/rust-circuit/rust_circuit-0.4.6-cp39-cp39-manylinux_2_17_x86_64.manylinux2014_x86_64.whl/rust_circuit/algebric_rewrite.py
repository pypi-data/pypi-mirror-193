...
def push_down_indexes_through_modules(
    ...
    def is_sym(y):
        return y.is_symbol() or (isinstance(y, SetSymbolicShape) and is_sym(y.children[0]))

    def rename_idx_set_shape(y) -> Circuit:
        if isinstance(y, SetSymbolicShape) or y.is_index():
            y = y.map_child(0, rename_idx_set_shape)
            return y.rename(y.get_autoname())
        return y

    def update_bind(y):
        y = rename_idx_set_shape(y)
        if y.is_index():
            return push_down(y.cast_index())
        return y

    m = Matcher(Index) & (lambda y: is_sym(y.children[0])) & no_parent_fvs
