...
fn _rust(py: Python<'_>, m: &PyModule) -> PyResult<()> {
    ...
    use circuit_base::{
        circuit_utils::{count_nodes, toposort_circuit, total_arrayconstant_size, total_flops},
        flat_concat,
    ...
    use circuit_utils::cast_circuit;
    ...
    m.add_class::<nb_operations::generalfunction::GeneralFunctionSpecTester>()?;
    ...
    m.add_class::<circuit_utils::SetOfCircuitIdentities>()?;
    m.add_function(wrap_pyfunction!(
    ...
    m.add_function(wrap_pyfunction!(
        rr_util::tensor_util::upcast_tensor_device_dtypes_py,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        rr_util::tensor_util::upcast_tensor_devices_py,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
    ...
    m.add_function(wrap_pyfunction!(
        circuit_utils::replace_all_randn_seeded,
        m
    )?)?;
    ...
    m.add_function(wrap_pyfunction!(circuit_utils::replace_nodes_py, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(
        nb_operations::modules::pull_out_of_modules,
        m
    )?)?;

    m.add_function(wrap_pyfunction!(
        nb_operations::modules::pull_out_of_modules_get,
        m
    )?)?;
