...
use crate::{
    ...
    tensor_util::{broadcast_shapes, Shape},
};
...
pub fn conv_shape(
    ...
    let input_batch_shape: Shape =
        if let Some(input_batch_rank) = input_shape.len().checked_sub(dims + 1) {
            input_shape[..input_batch_rank].to_smallvec()
        } else {
    ...
    let filter_batch_shape: Shape =
        if let Some(filter_batch_rank) = filter_shape.len().checked_sub(dims + 2) {
            input_shape[..filter_batch_rank].to_smallvec()
        } else {
    ...
    if in_channels != input_shape[input_shape.len() - 1] {
