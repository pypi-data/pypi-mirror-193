...
pub fn eq_or_symbolic(x: &[usize], y: &[usize]) -> bool {
    x.len() == y.len()
        && x.iter().zip(y).all(|(a, b)| {
            SymbolicSizeProduct::has_symbolic(*a) || SymbolicSizeProduct::has_symbolic(*b) || a == b
        })
}
...
impl TensorIndex {
    ...
    pub fn is_identity(&self, shape: &[usize]) -> bool {
