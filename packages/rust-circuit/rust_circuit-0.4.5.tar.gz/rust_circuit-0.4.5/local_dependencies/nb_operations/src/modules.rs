...
use circuit_base::{
    expand_node::{expand_node, replace_expand_map, MapReplaceExpander, ReplaceMapRc},
...
pub fn pull_out_of_modules_get(
    ...
    fn recurse(
        ...
        if found {
            ...
            let vars: IndexSet<Symbol> =
                m.spec.arg_specs.iter().map(|s| s.symbol.clone()).collect();
            ...
            let m_new = if rs_with_fvs.len() > 0 {
            ...
            let (_, _, new_matcher2_again) = matcher_new_binds
                .clone()
                .match_iterate_continue(m_new.clone())?;

            let mut out = rs_no_fvs;
            let circ_out = m_new.map_children_enumerate(|i, c| {
                Ok(if i == 0 {
                    if rs_with_fvs.len() == 0 {
                        spec_circ_2.clone()
                    } else {
                        c
                    }
            ...
            let circ_out = if rs_with_fvs.len() > 0 && let Some(cb) = &module_construct_callback {
