...
class CustomUserManager(BaseUserManager):
    ...
    def create_user(
        ...
        **kwargs,
        ...
        """
        user: AbstractZonesmartUser = self.model(
            email=email, is_staff=is_staff, is_superuser=is_superuser, **kwargs
        )
...
class AbstractZonesmartUser(AbstractBaseUser, PermissionsMixin):
    ...
    password_last_modified = MonitorField(
        monitor="password", verbose_name="Дата последнего изменения пароля"
    )
