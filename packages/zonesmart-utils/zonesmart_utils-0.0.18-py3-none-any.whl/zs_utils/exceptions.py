...
class CustomException(Exception):
        ...
        **kwargs,
        ...
        if instance:
            if not isinstance(instance, self.__class__):
                raise ValueError(
                    f"Параметр 'instance' должен быть экземпляром класса '{self.__class__.__name__}'."
                )
    ...
    def add_key_prefix(
        self, data: dict, prefix: str, connector: str = ".", safe: bool = True
    ):
        updated_data = {}
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                updated_data[updated_key] = self.add_key_prefix(
                    data=value, prefix=prefix, connector=connector
                )
            else:
    ...
    def message_dict_to_str(self) -> str:
        if self.message_dict:
            return "\n".join(
                f"{key}: {value}" for key, value in self.message_dict.items()
            )
        else:
            return ""
    ...
    def messages_to_str(self) -> str:
        if self.messages:
            if len(self.messages) == 1:
                ...
                return "\n".join(
                    f"{num}) {msg}" for num, msg in enumerate(self.messages, start=1)
                )
        else:
            return ""
        ...
        for key, value in data.items():
            if isinstance(value, dict) or isinstance(value, list):
                result.update(
                    flatten_errors_dict(
                        data=value,
                        parent_string=parent_string + "." + str(key)
                        if parent_string
                        else str(key),
                    )
                )
        ...
        for item in data:
            if isinstance(item, dict):
                for num, item in enumerate(data):
                    result.update(
                        flatten_errors_dict(
                            data=item, parent_string=f"{parent_string}.{num}"
                        )
                    )
                break
        else:
