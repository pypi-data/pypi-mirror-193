...
def get_class_method_path(method) -> str:
    ...
    return ".".join(
        [method.__module__, get_class_by_method(method).__name__, method.__name__]
    )
...
def import_object(module_name: str, object_name: str):
    ...
    if not func:
        raise AttributeError(
            f"Функция '{object_name}' не найдена в модуле '{module_name}'."
        )
...
def import_object_by_path(path: str):
    ...
    return import_object(
        module_name=".".join(path_parts[:-1]), object_name=path_parts[-1]
    )
