...
class BaseChannelsConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if self.user:
            ...
            # Анонимные пользователи объединяются в одну группу
            await self.add_to_group(self.get_anon_group_name())
    ...
    async def ping_action_processor(self, content: dict, **kwargs):
        # if self.user:
        #     from zonesmart.users.services import UserSessionLogService

        #     UserSessionLogService.save_visit_log(user=self.user)
