...
from morphonet.tools import load_credentials,add_slashes,try_parse_int,strblue,strred,strgreen,nodata,ss,_get_objects,_get_object,_get_type,_get_last_curation,_get_string,_check_version,error_request, rm
...
class Net:
    ...
    def _connect(self,timeout=None):
        ...
        conn = HTTPSConnection(self.url,timeout=timeout)
    ...
    def upload_info(self,name,field):
        ...
        if self._is_dataset():
            ...
            if nodata(data2):
                if not data2 == None:
                    print(strred(ss+'ERROR : info not uploaded '+str(data2['error_message'])))
                else:
                    print(strred(ss+'ERROR : info not uploaded : request data is null '))
            else :
                ids = data2['result']
        ...
        if self._own_dataset():
            ...
            if nodata(data):
                if not data == None:
                    ...
                    print(strred(ss+'ERROR : Unable to share that information : request data is null '))
            else :
        ...
        if self._own_dataset():
            ...
            if nodata(data):
                if not data == None:
                    ...
                    print(strred(ss+'ERROR : Unable to unshare that information : request data is null '))
            else :
