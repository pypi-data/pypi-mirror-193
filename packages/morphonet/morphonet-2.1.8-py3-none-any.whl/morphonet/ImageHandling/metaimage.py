...
def read_metaimage(filename):
    ...
    # return
    f.close()
    return img,tuple_res


def write_metaimage_to_stream(stream, img,voxel_size=(1,1,1)):
    ...
    # image resolutions
    #
    print("in meta seg vs = ")
    #res = getattr(img, "resolution", voxel_size)
    #if vo
    info["ElementSize"] = str(voxel_size[0]) + ' ' + str(voxel_size[1]) + ' ' + str(voxel_size[2])
    info["ElementSpacing"] = str(voxel_size[0]) + ' ' + str(voxel_size[1]) + ' ' + str(voxel_size[2])
    print(str(info["ElementSize"]))
    #
    # data type
    #
...
def write_metaimage(filename, img,voxel_size=(1,1,1)):
   ...
   :Parameters:
    ...
    try:
        print("in wmi vs = "+str(voxel_size))
        write_metaimage_to_stream(f, img,voxel_size=voxel_size)
    except:
