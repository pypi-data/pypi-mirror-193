...
class HttpRequest:
    ...
    def get_response_stream(self, url: Url, pipes: List[Observable] = [], headers: Dict[str, str] = None, allow_redirects: bool = True, timeout: int = 5, verify: bool = False, max_retries: int = 3, proxy_builder: Callable[[], Dict[str, str]]=None) -> Any:
        ...
        while retries < max_retries:
            try:
                response = self.session_builder().request(
                    url.method, url.url, headers=headers, json=url.json, params=url.params, allow_redirects=allow_redirects, timeout=timeout, verify=verify, proxies=proxy_builder() if proxy_builder is not None else None, stream=True)
