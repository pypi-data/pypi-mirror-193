...
def create_role(engine, role):
    with engine.connect() as conn:
        user_exists = conn.execute("SELECT 1 FROM pg_roles WHERE rolname='%s'" % (role))
...
def wrds_to_pg(table_name, schema, engine, wrds_id=None,
    ...
    with engine.connect() as conn:
        res = conn.execute("DROP TABLE IF EXISTS " + schema + "." + alt_table_name + " CASCADE")
  
    # Create schema (and associated role) if necessary
    insp = inspect(engine)
    if not schema in insp.get_schema_names():
        with engine.connect() as conn:
            res = conn.execute("CREATE SCHEMA " + schema)
        
        if create_roles:
            if not role_exists(engine, schema):
                create_role(engine, schema)
            with engine.connect() as conn:
                res = conn.execute("ALTER SCHEMA " + schema + " OWNER TO " + schema)
            if not role_exists(engine, "%s_access" % schema):
                create_role(engine, "%s_access" % schema)
            with engine.connect() as conn:
                res = conn.execute("GRANT USAGE ON SCHEMA " + schema + " TO " + schema + "_access")
    
    with engine.connect() as conn:
        res = conn.execute(make_table_data["sql"])
...
def role_exists(engine, role):
    with engine.connect() as conn:
        res = conn.execute("SELECT COUNT(*) FROM pg_roles WHERE rolname='%s'" % role)
        rs = [r[0] for r in res]
...
def create_role(engine, role):
    with engine.connect() as conn:
        res = conn.execute("CREATE ROLE %s" % role)
    return True
