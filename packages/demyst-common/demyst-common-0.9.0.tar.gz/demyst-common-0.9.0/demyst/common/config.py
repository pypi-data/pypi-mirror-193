...
class Config(object):
    ...
    def provider_cost(self, provider_name):
        self.init_provider_cache()
        return self.provider_cache[provider_name]["credit_cost"]

    def provider_price(self, provider_name):
        self.new_provider_cache()
        return self.provider_cache[provider_name]["credit_price"]
    ...
    # Instead of changing init_provider_cache
    def new_provider_cache(self):
        if self.provider_cache == None:
            url = self.auth_get(self.get("MANTA_URL") + "catalog/providers?pages").json()
            pagination_info = url.get('pagination')
            tot_entries = pagination_info.get('total_entries')
            to_get_providers = self.auth_get(self.get("MANTA_URL") + "catalog/providers?per_page=" + str(tot_entries)).json()
            providers = to_get_providers['providers']
            results = []
            for p in providers:
                copy = p.copy()
                copy["latest_version"] = self.provider_latest_version_catalog(copy["id"])
                results.append(copy)
            self.new_provider_cache_from_json(results)

    def new_provider_cache_from_json(self, providers):
        self.provider_cache = {}
        for p in providers:
            self.provider_cache[p["name"]] = p        

    def provider_latest_version_catalog(self, provider_id):
        provider_page = self.auth_get(self.get("MANTA_URL") + "catalog/providers/" + str(provider_id)).json()
        if (provider_page['latest_version']):
            latest_version = provider_page['latest_version']
            return latest_version

    def new_lookup_provider(self, provider_name):
        self.new_provider_cache()
        return self.provider_cache.get(provider_name)

    def new_all_providers(self):
        self.new_provider_cache()
        return list(self.provider_cache.values())
