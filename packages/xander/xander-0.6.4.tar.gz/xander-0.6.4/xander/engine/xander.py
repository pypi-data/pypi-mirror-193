...
class XanderEngine:
    ...
    def run(self):
        ...
        t0 = get_time()
        self.logger.warning("[XANDER] Xander Engine is running.")

        for i, name in enumerate(self.pipelines):
            t1 = get_time()
            self.logger.info("[XANDER] Running pipeline '{}' ({}/{})".format(name, i + 1, len(self.pipelines)))
            ...
            self.logger.info(f"[XANDER] Completed pipeline '{pipeline.pipeline_slug}' ({i+1}/{len(self.pipelines)}) in {elapsed(t1)}")
    ...
    def add_pipeline(self, name):
        ...
        self.logger.info("[XANDER] Pipeline '{}' added to the engine".format(self.pipelines[name].pipeline_name))
