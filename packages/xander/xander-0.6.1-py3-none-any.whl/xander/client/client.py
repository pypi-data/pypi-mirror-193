...
class XanderClient:
    ...
    def __init__(self, server_address='35.192.211.33', server_port=5000, local_mode=False):
        ...
        if not Path("credentials.json").exists():
            logging.critical(
                f'Credentials file not found in the specified path: {os.path.join(os.getcwd(), credentials_file_path)}')
        ...
        if not local_mode:
            ...
            try:
                ...
                if success:
                    ...
                    logging.debug('[XANDER]You are logged as {} ({})'.format(self.api['user_name'], self.api['user_mail']))
                else:
                    raise Exception

            except FileNotFoundError as e:
                logging.debug('[XANDER]Local mode is active, API auth file not found!', exc_info=e)

            except Exception as e:
                logging.debug('[XANDER]Local mode is active, connection to server failed!', exc_info=e)
