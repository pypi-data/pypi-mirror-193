...
from staircase.config import VAR_FOLDER, UserConfig
...
@click.option("-c", "--component", flag_value=True)
@click.option("-a", "--all", flag=True, default=False)
@staircase_provider()
@postman_provider()
@async_cmd
async def command(
    ...
    if not all and not component:
        components_names = prompt(components_names, header="Select your component name")
