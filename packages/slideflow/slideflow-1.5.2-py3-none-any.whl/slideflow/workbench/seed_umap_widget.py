...
class SeedUMAPWidget:
    ...
    def refresh_model_path(self):
        if hasattr(self.viz, 'pkl') and self._pkl != self.viz.pkl:
            ...
            if exists(seed_maps_dir):
                seed_umaps = [u[:-8] for u in os.listdir(seed_maps_dir)
                              if u.endswith('.parquet')]
            else:
                seed_umaps = []
            if self._pkl is not None and len(seed_umaps):
                print("Loading seed UMAPs...")
                for layer in sorted(seed_umaps):
    ...
    def render(self):
        ...
        if self.show and self._umap_dfs:
            ...
            _, self.show = imgui.begin("##seed_umap", closable=True, flags=(imgui.WINDOW_NO_COLLAPSE | imgui.WINDOW_NO_RESIZE))
