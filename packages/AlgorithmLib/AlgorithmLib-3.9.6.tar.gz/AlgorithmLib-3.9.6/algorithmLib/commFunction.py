...
def get_max_cc_by_dll(refData, testData,mydll,step):
    ...
    refstruct = np.ascontiguousarray(refData, dtype=c_double).ctypes.data_as(POINTER(c_double))
    teststruct = np.ascontiguousarray(testData, dtype=c_double).ctypes.data_as(POINTER(c_double))
