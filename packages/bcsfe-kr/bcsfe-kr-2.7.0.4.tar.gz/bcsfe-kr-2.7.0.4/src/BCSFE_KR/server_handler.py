...
def prepare_upload(
    ...
    if print_text:
        helper.colored_text("기타 데이터 추가 중...", helper.GREEN)
...
def upload_handler(
    ...
    helper.colored_text("세이브 데이터 업로드 중...", helper.GREEN)
...
def meta_data_upload_handler(
    ...
    helper.colored_text("기타 데이터 업로드 중...", helper.GREEN)
...
def download_handler() -> Optional[str]:
    ...
    transfer_code = helper.check_hex(
        input("기종변경 코드를 입력하세요:").lower().replace("o", "0")
    )
    if transfer_code is None:
        helper.colored_text(
            "기종변경 코드는 숫자와 알파벳으로만 이루어져야 합니다.",
            helper.RED,
        )
        return None
    confirmation_code = helper.check_dec(
        input("기종변경 인증번호를 입력하세요:").lower().replace("o", "0")
    )
    if confirmation_code is None:
        helper.colored_text(
            "인증번호는 숫자로만 이루어져야 합니다.",
            helper.RED,
        )
        return None
    game_version = input("게임 버전을 입력하세요 (예시: 11.3, 9.6, 10.4.0):")
    ...
    if test_is_save_data(save_data):
        helper.colored_text("성공적으로 세이브 데이터를 다운로드 하였습니다.\n", base=helper.GREEN)
    else:
        helper.colored_text(
            "잘못된 코드 또는 번호",
            base=helper.RED,
