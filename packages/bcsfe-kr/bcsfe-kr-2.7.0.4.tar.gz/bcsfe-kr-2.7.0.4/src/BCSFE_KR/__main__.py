...
def print_start_up():
    ...
    helper.colored_text(
        ...
        + "Made by &fieryhenry&\n"
        + "한국어 번역: &CintagramABP&\n"
        + "Korean Translation by &CintagramABP&\n\n"
        + "Translator's Discord: &https://discord.gg/r8aTf9W8K2&\n"
...
def check_update() -> None:
    ...
    if version_info is None:
        helper.colored_text("업데이트 확인 실패.", base=helper.RED)
        return
    ...
    helper.colored_text(
        f"로컬 버전: &{local_version} | &최신 버전: &{stable_ver}",
        base=helper.CYAN,
        new=helper.WHITE,
        end="",
    )
