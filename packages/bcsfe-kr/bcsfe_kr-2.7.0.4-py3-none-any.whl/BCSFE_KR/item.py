...
class Item:
    ...
    def ban_warning(self) -> bool:
        ...
        helper.colored_text(
            text=f"경고: {self.name} 를 에딧하는 것은 밴 가능성이 있습니다!",
            base=helper.RED,
        )
        ...
        leave = (
            user_input_handler.colored_input("계속하시겠습니까? (&y&/&n&):")
            == "n"
        )
        return leave
    ...
    def edit_int(self) -> None:
        ...
        if self.value is not None:
            helper.colored_text(
                f"현재 {self.edit_name}의 {self.name} 보유량: &{int(self.value)+self.offset}&"
            )
        if self.max_value is None:
            max_str = ""
        else:
            max_str = f" (최대 &{self.max_value+self.offset}&)"

        text = f"원하는 {self.name}의 값 입력 {self.set_name}{max_str}:"
