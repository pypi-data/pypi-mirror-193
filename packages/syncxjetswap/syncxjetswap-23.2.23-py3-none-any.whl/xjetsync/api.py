...
class Cheques:
    ...
    * `cheque_activate` - Activate cheque
    ...
    def cheque_status(self, cheque_id: str):
        """ Get cheque status 
        
        :param `cheque_id` [str]: Cheque id
        """
        return self.request(method = 'cheque.status', params = {'cheque_id': cheque_id})

    def cheque_list(self):
        """ Get list of cheques """
        return self.request(method = 'cheque.list')

    def cheque_cancel(self, cheque_id: str):
        """ Cancel cheque 
        
        :param `cheque_id` [str]: Cheque id
        """
        return self.request(method = 'cheque.cancel', json = {'cheque_id': cheque_id})
...
class Invoices:
    ...
    * invoice_list()
    ...
    def invoice_create(self, currency: str, amount: int, description: str = None, max_payments: int = 1):
        """ Create invoice 
    ...
    def invoice_status(self, invoice_id: str):
        """ Get invoice status
        
        :param `invoice_id` [str]: Invoice id
        """
        return self.request(method = 'invoice.status', params = {'invoice_id': invoice_id})

    def invoice_list(self):
        """ Get invoice list """
        return self.request(method = 'invoice.list')
