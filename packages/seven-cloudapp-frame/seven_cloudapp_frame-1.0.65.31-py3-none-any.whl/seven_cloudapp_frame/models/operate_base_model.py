...
@LastEditTime: 2022-12-01 14:56:54
            ...
            if operation_log["update_detail"] and isinstance(operation_log["update_detail"], dict):
                ...
                if operation_config:
                    for key in operation_log["update_detail"]:
                        ...
                        config_field =  query(config_list).where(lambda x: x["field"] == key).first_or_default(None)
                        if not config_field or config_field.get("is_show",1) == 0:
                            continue
