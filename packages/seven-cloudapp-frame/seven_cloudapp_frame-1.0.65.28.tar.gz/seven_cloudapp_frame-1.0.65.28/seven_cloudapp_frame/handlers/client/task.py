...
@LastEditTime: 2023-02-20 15:12:40
...
class JoinMemberHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        app_info_dict = app_base_model.get_app_info_dict(app_id)
        access_token = app_info_dict["access_token"] if app_info_dict else ""
        invoke_result_data = task_base_model.process_join_member(app_id, act_id, module_id, user_id, login_token, self.__class__.__name__, access_token, app_key, app_secret, self.request_code, check_user_nick, continue_request_expire, is_stat, info_json, mix_nick)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        ref_params = {}
        invoke_result_data = self.business_process_executed(invoke_result_data, ref_params)
