...
def conditional_hmc(embedding, target_words, data, model="sgns", ws=5, ns=5, batch_size=25000, iter=1000, chains=4):
    if not isinstance(data, tf.Tensor):
        data = tf.constant(data)

    i,j,x  = generate_cbow_batch(data, ws=ws, ns=ns, batch=batch_size, start_ix=0)
