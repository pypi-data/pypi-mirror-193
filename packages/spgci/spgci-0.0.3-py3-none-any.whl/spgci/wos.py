...
class WorldOilSupply:
    ...
    def get_cost_of_supplies(
        ...
        **Get Production Costs**
        >>> WorldOilSupply().get_cost_of_supplies(cost_of_supplies_type="Production")

        **Get Crude supplies costs in Albania**
        >>> WorldOilSupply().get_cost_of_supplies(supply_type="Crude", country="Albania")

        **Compose Ref Data with Costs**
        >>> sup_sub_types = WorldOilSupply().get_reference_data(type=WorldOilSupply.RefType.SupplySubTypes)
        >>> WorldOilSupply().get_cost_of_supplies(supply_subtype=sup_sub_types['supplySubTypeName'][:2])
        """
    ...
    def get_ownership(
        ...
        **Get Ownership by Company**
        >>> WorldOilSupply().get_ownership(company="ADNOC")

        **Get Ownership by state companies in the Middle East, Africa, in 2040**
        >>> WorldOilSupply().get_ownership(company_type="State", year=2040, region=["Middle East", "Africa"])

        **Compose Ref Data with Ownership**
        >>> res_locations = WorldOilSupply.get_reference_data(type=WorldOilSupply.RefType.ReserveLocations)
        >>> WorldOilSupply().get_ownership(reserve_location=res_locations['reserveLocationName'][:2], country='Norway', year=2031)
        """
    ...
    def get_production(
        ...
        **Get Production by Country**
        >>> WorldOilSupply().get_production(scenario_term_id=1, country="Qatar", year=2050)

        **Get Production of Crude in Middle East, Africa, in 2024**
        >>> WorldOilSupply().get_production(year=2024, region=["Middle East", "Africa"], production_element="Crude")

        **Compose Ref Data with Production**
        >>> opec = WorldOilSupply().get_reference_data(type=WorldOilSupply().RefType.OPECCountries)
        >>> filt = opec['effectiveLeaveDate'].isna()
        >>> opec = opec[filt]
        >>> WorldOilSupply().get_production(country=opec['countryName'][3:], production_type="Production", year=2024)
        """
    ...
    def get_production_archive(
        ...
        **Get Production Archive by Country**
        >>> WorldOilSupply().get_production_archive(scenario_id=2243, scenario_term_id=2, country="Qatar", year=2021)

        **Get List of Scenario IDs**
        >>> WorldOilSupply().get_reference_data(type=WorldOilSupply().RefType.ScenarioList)

        **Compose Ref Data with Production Archive**
        >>> scenarios = WorldOilSupply.get_reference_data(type=WorldOilSupply().RefType.ScenarioList)
        >>> s_id = scenarios['scenarioId'][1]
        >>> WorldOilSupply().get_production_archive(scenario_id=s_id, production_type="Capacity", country="Albania")
        """
    ...
    def get_reference_data(
        ...
        --------
        **Simple**
        >>> WorldOilSupply().get_reference_data(type=WorldOilSupply.RefTypes.Grades)
        """
