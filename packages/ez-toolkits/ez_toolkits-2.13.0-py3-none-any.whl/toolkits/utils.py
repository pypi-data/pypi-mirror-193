...
def divisor_1000(dividend: int | float) -> float:
    try:
...
def divisor_1024(dividend: int | float) -> float:
    try:
...
def divisor_square_1000(dividend: int | float) -> float:
    try:
...
def divisor_square_1024(dividend: int | float) -> float:
    try:
...
def stat(target: str = None, type: any = None) -> bool:
    ''' 检查目标类型 '''
    try:
        _stat = Path(target)
        match True:
...
def range_zfill(start: int, stop: int, step: int, width: int) -> list:
...
def real_path(path: str = '', **kwargs) -> str | None:
    '''获取真实路径'''
    try:
        return realpath(path, **kwargs) if vTrue(path, str) else None
    except Exception as e:
        logger.exception(e)
        return None
...
def retry(times: int = None, func: function = None, *args, **kwargs) -> any:
    '''
    重试 
...
def datetime_to_date(datetime_object: datetime) -> datetime | None:
...
def pool_process(process_func: function, process_data: any, process_num: int = None, thread: bool = None, *args, **kwargs) -> list | None:
    '''
    Pool 进程池
    ThreadPool 线程池
