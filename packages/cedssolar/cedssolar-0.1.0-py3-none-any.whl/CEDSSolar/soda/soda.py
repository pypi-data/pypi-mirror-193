...
class SolarSite(object):
    ...
    def get_nsrdb_data(self, year, leap_year, interval, utc):
        ...
        idx = pd.date_range(
            start="1/1/{yr}".format(yr=year),
            freq=interval + "Min",
            end="12/31/{yr} 23:59:00".format(yr=year),
        )
        if leap_year == False:
    ...
    def generate_solar_power_from_nsrdb(
        self,
        clearsky,
        capacity,
        DC_AC_ratio,
        ...
        ssc.data_set_number(
            wfd, "tz".encode("utf-8"), self.meta_resource_data["Time Zone"]
        )
        ssc.data_set_number(
            wfd, "elev".encode("utf-8"), self.meta_resource_data["Elevation"]
        )
        ...
        ssc.data_set_array(
            wfd, "dn".encode("utf-8"), self.resource_data["{}DNI".format(clearsky_str)]
        )
        ssc.data_set_array(
            wfd, "df".encode("utf-8"), self.resource_data["{}DHI".format(clearsky_str)]
        )
        ssc.data_set_array(
            wfd, "wspd".encode("utf-8"), self.resource_data["Wind Speed"]
        )
        ...
        ssc.data_set_number(
            dat, "gcr".encode("utf-8"), 0.4
        )  # Set ground coverage ratio
        ssc.data_set_number(
            dat, "adjust:constant".encode("utf-8"), 0
    ...
    def generate_high_resolution_power_data(self, resolution, date):
        ...
        date_index2 = pd.date_range(
            start="2015-01-10", end="2015-01-10" + " 23:59:59", freq="1S"
        )
        ts = (
            self.solar_power_from_nsrdb[date]
            .resample("1S")
        ...
        # N = 86400
        hm = np.array(
            [exp(-(t**2) / 2) * cos(5 * t) for t in np.linspace(-4, 4, M_hat)]
        )
        hw = np.array(
            [0.54 - 0.46 * cos(2 * pi * t / (M_hat - 1)) for t in range(0, M_hat)]
        )
        ...
        η[boolean] = self.trunc_gauss(
            0, df.generation[boolean], df.generation[boolean], σ, sum(boolean)
        )

        generated_ts = (
            df.generation.values.reshape(-1, 1)
            + matmul_toeplitz(
                (abs(first_col1), abs(first_row1)), (bm.reshape(-1, 1) * zm)
            )
            - matmul_toeplitz((first_col2, first_row2), ((bw.reshape(-1, 1) * zw)))
            + η.reshape(-1, 1)
        )
        ts["HighRes"] = 0.0
