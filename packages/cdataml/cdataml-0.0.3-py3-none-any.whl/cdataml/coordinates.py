...
# Default arguments
input_loc, output_loc, images, scoring = None, None, None, False
...
for opt, arg in opts:
    if opt in "-h":
        ...
        -f <Folder path containing TIFF images>
...
def SkipSpots(row, colname, varcurrent):
    tempoffsets = [] # If next image is part of the same replicate, the offsets should be continuous between the two images
    skip = 0

    while True:
        if(currentrow+skip == len(cdata)): # If the end of cdata is reached, save and quit
            print("End of Dataframe!")
            SaveAndQuit()
            break_out_flag = True
            break
        else:
            testid = int(cdata.loc[currentrow+skip, colname]) # Find value of variable in potential next row
            if testid == varcurrent: # Compare it with the current variable value
                Temp_Offsets.append(cdata.loc[currentrow+skip, "Offset"]) # If no change, store offset to be shunted along later
                # Fill the coordinates and scores with "Skip" and offset values with 0, since they must be integers.
                cdata.loc[currentrow+skip, "y1"] = "Skip"
                cdata.loc[currentrow+skip, "y2"] = "Skip"
                cdata.loc[currentrow+skip, "x1"] = "Skip"
                cdata.loc[currentrow+skip, "x2"] = "Skip"
                cdata.loc[currentrow+skip, "Score"] = "Skip"
                cdata.loc[currentrow+skip, "Offset"] = 0
                skip = skip + 1
                ...
                if colname == "Img-ID": # If the chosen skip was image, then the offset values may need to be shunted along.
                    if cdata.loc[currentrow, "Offset"] != cdata["Offset"].max(): # If the current offset isn't the maximum value, there are still leaves left in this replicate
                        cdata.iloc[currentrow:currentrow+skip, cdata.columns.get_loc("Offset")] = 0 # Replace previous offset values with 0s
                        cdata.iloc[currentrow+skip:currentrow+(2*skip), cdata.columns.get_loc("Offset")] = tempoffsets # Update the next offset values to continue from point of skip
                        cdata.iloc[currentrow+(2*skip):currentrow+(3*skip), cdata.columns.get_loc("Offset")] = 0 # Update any offsets attributed to the image after the maximum to 0
...
while True:
    ...
    while True:
        ...
        if keypress == 27: # Escape
            ...
            currentrow = SkipSpots(currentrow, "Leaf-ID", leafid)

        elif keypress == ord('n'): # Next image
            print("Skipping to next image")
            currentrow = SkipSpots(currentrow, "Img-ID", imgid)

        else:
            if scoring == True:
                if keypress in [ord('0'), ord('1'), ord('2'), ord('3'), ord('4'), ord('5'), ord('6')]: # if a score is pressed
                    print("Score given: " + str(chr(keypress)))
                    cdata.loc[currentrow, "Josh_Score"] = chr(keypress) # save the score (chr is reverse of ord)
                else:
