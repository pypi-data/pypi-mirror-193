...
def fitscores(input_loc, output_loc, rawfile):
    ...
    datasubset = cdata.loc[cdata["Score"] != "Skip"].reset_index()

    # Reset USI Unique_Spot_ID column
    for row in range(1, length(datasubset)+1):
        datasubset.loc[row, "Unique_Spot_ID"] = 'S' + str(row)
    ...
    datasubset.to_csv(output_loc + "scored_cdata.csv", index=False)
