...
class Geometry(DrqObjectBase):
    ...
    def __deserialize__(self, data):
        ...
        for sld in slds_list:
            self._slds += [Solid(self._server, obj_index=sld)]
        ...
        for srf in srfs_list:
            self._srfs += [Surface(self._server, obj_index=srf)]
        ...
        for crv in crvs_list:
            self._crvs += [Curve(self._server, obj_index=crv)]
        ...
        for crnr in crnr_list:
            self._cnrs += [Corner(self._server, obj_index=crnr)]
        ...
        for file_obj_index in files_list:
            self._files += [GeometryFile(self._server, obj_index=file_obj_index)]
...
class GeometryFile(DrqObjectBase):
    """Holds information on a loaded file and applied transformations"""
    # pylint: disable=R0902

    def __init__(self, server, name='', file_path='', translation=np.zeros(3), rotations=None, obj_index=None):
    ...
    def __deserialize__(self, data):
        ...
        name_len = int.from_bytes(data[seeker:seeker + 8], 'little')
        seeker += 8
        self._name = data[seeker:seeker + name_len].decode('UTF-8')
        seeker += name_len
        ...
        for obj_index in rot_ids:
            self.rotations += [Rotation(self._server, obj_index=obj_index)]
    ...
    def name(self):
        """
        Unique name for the file object
        """
        return self._name
...
class Rotation(DrqObjectBase):
    ...
    def __eq__(self, other):
        if self.refresh_stamp == -1 or other.refresh_stamp == -1:
            return self.obj_index == other.obj_index
        return (self.cnt == other.cnt).all() and (self.axis == other.axis).all() and self.angle == other.angle
    ...
    def __serialize__(self):
        ...
        res = bytes()
        res += self.obj_index.to_bytes(8, 'little')
        res += utils.serialize_list([self.cnt, self.axis, np.array(self.angle)])
        return res
...
class GeometrySelectionType(DrqEnum):
    """
...
class GeometrySelection(DrqObjectBase):
    """
...
class Solid(DrqObjectBase):
    ...
    def __deserialize__(self, data):
        ...
        for obj_index in lst:
            self._srfs += [Surface(self._server, obj_index=obj_index)]
        seeker += data_len
        ...
        lst = utils.deserialize_list(data[seeker:seeker + data_len])
        for obj_index in lst:
            self._crvs += [Curve(self._server, obj_index=obj_index)]
        seeker += data_len
        ...
        lst = utils.deserialize_list(data[seeker:seeker + data_len])
        for obj_index in lst:
            self._cnrs += [Corner(self._server, obj_index=obj_index)]
        seeker += data_len
...
class Surface(DrqObjectBase):
    ...
    def __deserialize__(self, data):
        ...
        lst = utils.deserialize_list(data[seeker:seeker + data_len])
        for obj_index in lst:
            self._slds += [Solid(self._server, obj_index=obj_index)]
        seeker += data_len
...
class Tessellation(DrqObjectBase):
    """
    ...
    def __init__(self, server, obj_index=None):
        ...
        # All surface triangles described by node indicis
        self._ed = np.zeros([0, 2], dtype=int)
        # All edges described as a tuple of two nodes
        self._srfs = np.zeros(self.inz.shape[0], dtype=int)
        # 0 based NURBS surface obj_index of each triangle
        self._slds = np.zeros(self.inz.shape[0], dtype=int)
        # 0 based solid obj_index of each triangle
        self._crvs = np.zeros(self.ed.shape[0], dtype=int)
        # 0 based curve obj_index for all edges; -1 means not assigned
        self._cnrs = np.zeros(self.nc.shape[0], dtype=int)
        # 0 based corner obj_index for all nodes; -1 means not assigned
        super().__init__(server, obj_index)
