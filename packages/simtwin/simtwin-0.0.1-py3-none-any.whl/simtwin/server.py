...
class Server:
    ...
    def __init__(self, config_path='', server='', port=-1, api_key='', timeout=5.):
        if exists(config_path):
            self.__init_with_config__(api_key, config_path, port, server)
        ...
        self._socket.connect((self.address, self._port))
    ...
    def push(self, obj_index):
        ...
        if obj.refresh_stamp <= self.get_refresh_stamp(obj_index):
            return
    ...
    def pull(self, obj_index):
        ...
        if stamp_at_server <= self._objs[obj_index].refresh_stamp:
            return True
    ...
    def call_method(self, name, obj_index, args):
        """
        ...
        if not reply.strip() == 'OK':
            raise exceptions.ServerException(reply.strip())
        reply = utils.rcv_bytes(self._socket).decode('UTF-8')
        return_len = int(reply.split('|')[1])
        return_data = utils.rcv_bytes(self._socket, return_len).decode('UTF-8')
        return utils.obj_to_bytes(return_data)
    ...
    def __init_with_config__(self, api_key, config_path, port=-1, server=''):
        """
        ...
        if server == '':
            self.address = config['DEFAULT']['server']
    ...
    def __eq__(self, other):
        return self.address == other.address and self._port == other._port
