...
class ValiotWorker():
    ...
    def unlockQueue(self, queue_name, lock):
        ...
        current_lock, errors = self.gql.query(queries.get_queueLock, variables={"lockId": lock['id']})
        if not current_lock:
        ...
        if job_success and queue_success:
            def safe_fn(**kwargs):
                ...
                try:
                    ...
                    self.log(
                        LogLevel.ERROR, f'Error starting job from queue {kwargs["queue"]["name"]}:\n{str(e)}')
                    self.log(
                    ...
                    errors.append(error)
                    self.log(LogLevel.ERROR, f"job {job['id']} crashed unexpectedly.\n{error_info}", extra={
                        "error": error
                    })
                finally:
    ...
    def setQueueLock(self, queue, lock, action):
        ...
        """
        queue_index = __.find_index(self.currentQueues, lambda q: q['name'] == queue['name'])
        current_queue = __.get(self.currentQueues, queue_index)
        if queue_index == -1:
            return # no op, queue not found
        if action == 'created':
            self.log(LogLevel.INFO, f'Queue {queue["name"]} locked')
            self.currentQueues[queue_index]['locks'] = [lock]
        elif action == 'updated':
            self.log(LogLevel.INFO, f'Queue {queue["name"]} lock updated')
            lock_index = __.find_index(current_queue['locks'], lambda l: l['id'] == lock['id'])
            if lock_index == -1:
                self.currentQueues[queue_index]['locks'].append(lock)
                self.currentQueues[queue_index]['locks'] = __.sort_by(self.currentQueues[queue_index]['locks'], 'updatedAt', reverse=True)
            else:
                self.currentQueues[queue_index]['locks'][lock_index] = lock
        elif action == 'deleted':
            self.log(LogLevel.INFO, f'Queue {queue["name"]} lock released')
            lock_index = __.find_index(current_queue['locks'], lambda l: l['id'] == lock['id'])
            if lock_index != -1:
                self.currentQueues[queue_index]['locks'].pop(lock_index)
        else:
            self.log(LogLevel.INFO, f'Queue {queue["name"]} unknown lock action: {action}')
        ...
        for i, job_lock in enumerate(self.lockedJobs):
            if not job_lock['process'].is_alive() or force:
                if self.unlockQueue(job_lock['job']['queue']['name'], job_lock['lock']):
                    ...
                    unlocked.append(i) # remove from lockedJobs anyways to avoid infinite prints
        if unlocked:
            unlocked.reverse()
            for u in unlocked:
    ...
    def startHealthCheck(self):
        ...
        self.healthCheckThread = Thread(target=health_check_probe, daemon=True, name='health_check_probe', args=(self.log, self.loggingStyle))
    ...
    def waitUntilWorkerIsReady(self):
        ...
        current_env = os.environ.get('ENV')
        if current_env == 'dev':
            self.log(LogLevel.WARNING, 'Skipping worker readiness check in dev environment')
            return
        initial_delay = os.environ.get('INITIAL_DELAY', 30)
        self.log(LogLevel.INFO, f'Waiting {initial_delay} seconds before starting worker...')
