...
"""
This module checks that the Python environment is ready for the OTC 2023
...
def check_install_directory() -> typing.Tuple[bool, str]:
    """Check that the virtual environment is within the SEAScope workspace.

    Installing the virtual environment in the SEAScope workspace is not
    mandatory, but it will be much easier to provide instructions during the
    training if all students create the virtual environment there."""
    import os
    ...
    workspace_path = os.path.dirname(venv_path)
    config_path = os.path.join(workspace_path, 'config.ini')
    colormaps_path = os.path.join(workspace_path, 'colormaps')
    state_path = os.path.join(workspace_path, 'state.db')

    if not os.path.isdir(colormaps_path):
        return False, 'Directory "{}" not found'.format(colormaps_path)

    if not os.path.isfile(config_path):
        return False, 'File "{}" not found'.format(config_path)

    if not os.path.isfile(state_path):
        return False, 'File "{}" not found'.format(state_path)

    msg = 'Environment "{}" is in the SEAScope workspace'.format(venv_path)
    return True, msg
...
def read_report() -> None:
    ...
    with open('SEAScope-env-OTC2023_report.json', 'rt') as f:
        report = json.load(f)
    ...
    if ok is True:
        ...
        print('\nSome things are not working as expected, please send the '
              'following file to <support-otc2023@oceandatalab.com>:\n')
        print(os.path.abspath('SEAScope-env-OTC2023_report.json'))
...
def start_notebook() -> None:
    ...
    notebook_path = resource_filename('SEAScope_env_OTC2023',
                                      'SEAScope-env-OTC2023_report.ipynb')
    shutil.copy(notebook_path, 'SEAScope-env-OTC2023_report.ipynb')
    try:
...
def check_all(cli_args: typing.Optional[typing.Sequence[str]] = None
    ...
    filename = 'SEAScope-env-OTC2023_report.json'
