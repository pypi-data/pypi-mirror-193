...
def node_handle(node, item, call):
    ...
    # 详细内容
    if node.sections:
        if '<!DOCTYPE html>' in node.sections[0][-1]:
            d["print"] = node.sections[0][-1].replace('<', '&lt;').replace('>', '&gt;')
        else:
            d["print"] = node.sections[0][-1]
    if call.excinfo:
        try:
            ...
            try:
                ...
                if excinfo_str.endswith('>'):
                    excinfo_str = excinfo_str[:-1]
            try:
                _traceback = f'\n_traceback: {call.excinfo._traceback}' if d['status'] != 'skipped' else ''
            except Exception:
                pass
            excinfo = {"error": excinfo_str + _traceback}
        d.update(excinfo)
    return d
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if ddreport:
        report_dir = ddreport.replace('\\', '/')
        report_name = time.strftime("%Y-%m-%d_%H%M%S") + ' ' + 'Report.html'
...
def ddreport(request):
    ...
    return MyFixture(host, db, db_ssh, receive_dict)


class MyFixture:
    def __init__(self, host, _db, ssh_db, receive_dict):
