...
class QueryBuilder():
    ...
    def get(self):
        ...
        if self._paginate:
            ...
            # and end cursor
            has_next_page = eval(data['pageInfo']['hasNextPage']) \
                if isinstance(data['pageInfo']['hasNextPage'], str) \
                else data['pageInfo']['hasNextPage']
            end_cursor = int(data['pageInfo']['end_cursor'])

            # Since paginated queries work differently, all outputs are
            # stored in 'edge'
            instances = [self._kind(**datum['edge']) for datum in data]

            # Currently pagination is implemented to get all available results
            while has_next_page:
                _query.compile(cursor=end_cursor)
                data, errors = self._gql.query(_query)
                has_next_page = eval(data['pageInfo']['hasNextPage'])
                end_cursor = int(data['pageInfo']['endCursor'])
                instances.extend([self._kind(**datum['edge'])
                                 for datum in data])
    ...
    def compile_plural_paginated(self,
        ...
        _query = "query { $Q_NAME ( cursor: $CURSOR $Q_PARAMS){ edge { $Q_FIELDS } pageInfo { endCursor hasNextPage } } }"\
