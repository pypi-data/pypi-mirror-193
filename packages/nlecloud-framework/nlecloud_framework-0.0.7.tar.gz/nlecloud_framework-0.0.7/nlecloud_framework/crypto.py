...
def plain_to_ciphert(key:str,plaintext:str)->str:
    """
    @description: 对数据进行加密
    @param key: 秘钥
...
def cipher_to_plain(key:str,ciphertext:str)->str:
    """
    @description:
    @param key: 秘钥
    ...
    # 打印解密后的数据
    print("打印解密后的数据:",plaintext.decode("utf8"))
    return plaintext.decode("utf8")


if __name__ == '__main__':
    key = genRandomKey()
    ciphertext = plain_to_ciphert(key,"你好")
    plaintext = cipher_to_plain(key,ciphertext)
    print(ciphertext)
    print(plaintext)
