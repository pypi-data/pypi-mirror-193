...
def create_validata_ui_report(rows_count: int, validata_core_report, schema_dict, header_case: bool):
    ...
    if not header_case:
        ...
        lower_fields_dict = {field.lower(): fields_dict[field] for field in fields_dict.keys()}
        ui_report["table"]["headers_title"] = [
            lower_fields_dict[h][0] if h in lower_fields_dict else "Colonne inconnue" for h in lower_headers
        ]
...
def validate(schema_instance: SchemaInstance, validata_resource: ValidataResource, header_case: bool):
    ...
    if general_errors and 'Duplicate labels in header' in validata_core_report['tasks'][0]['errors'][0]['note']:
        flash_error(f"Le fichier '{Path(validata_resource.get_source()).name}' comporte des colonnes avec le même nom. "
    ...
    if request.method == "GET":
        ...
        if schema_validation_report['errors']:
            if 'schema_url' in request.args:
                flash_error(f"Le schéma choisi '{schema_instance.schema['title']}', "
                ...
                return redirect(compute_validation_form_url(
                    {
                        'schema_name': schema_instance.request_parameters()['schema_name'],
                        'schema_ref': 'master'
                    }
                ))
            else:
        ...
        # Process URL
            ...
            try:
                ...
                return validate(schema_instance, URLValidataResource(url_param), header_case=False)
        ...
        if input_param == "file":
            ...
            # Validating data ignoring header case by default
            return validate(
                schema_instance, FileContentValidataResource(f.filename, bytes_data(f)), header_case=False
            )
